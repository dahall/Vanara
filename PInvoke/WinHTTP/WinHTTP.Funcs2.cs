...
namespace Vanara.PInvoke
{
	...
	public static partial class WinHTTP
	{
		...
		/// depending on what option is being queried; see the Option Flags topic to determine which handle is appropriate to use in querying
		/// a particular option.
		/// </param>
		/// <param name="dwOption">
		...
		/// allocated, so the calling application must globally free the string when it finishes using it. Setting this parameter to
		/// <c>NULL</c> causes this function to return <c>FALSE</c>. Calling GetLastError then returns ERROR_INSUFFICIENT_BUFFER and
		/// <c>lpdwBufferLength</c> contains the number of bytes required to hold the requested information.
		/// </param>
		/// <param name="lpdwBufferLength">
		/// A pointer to an unsigned long integer variable that contains the length of <c>lpBuffer</c>, in bytes. When the function returns,
		/// the variable receives the length of the data placed into <c>lpBuffer</c>. If GetLastError returns ERROR_INSUFFICIENT_BUFFER, this
		...
		{
			var rType = CorrespondingTypeAttribute.GetCorrespondingTypes(dwOption, CorrespondingAction.Get).FirstOrDefault() ?? typeof(T);
		...
		/// <param name="hRequest">
		/// Valid HINTERNET handle returned from a previous call to WinHttpOpenRequest. WinHttpReceiveResponse or WinHttpQueryDataAvailable
		/// must have been called for this handle and must have completed before <c>WinHttpReadData</c> is called. Although calling
		/// <c>WinHttpReadData</c> immediately after completion of <c>WinHttpReceiveResponse</c> avoids the expense of a buffer copy, doing
		/// so requires that the application use a fixed-length buffer for reading.
		/// </param>
		/// <param name="lpBuffer">
		...
		/// Pointer to an unsigned long integer variable that receives the number of bytes read. <c>WinHttpReadData</c> sets this value to
		/// zero before doing any work or error checking. When using WinHTTP asynchronously, always set this parameter to <c>NULL</c> and
		/// retrieve the information in the callback function; not doing so can cause a memory fault.
		/// </param>
		/// <returns>
		/// <para>
		/// Returns <c>TRUE</c> if successful, or <c>FALSE</c> otherwise. For extended error information, call GetLastError. The following
		/// table identifies the error codes that are returned.
		/// </para>
		/// <list type="table">
		/// <listheader>
		/// <term>Error Code</term>
		...
		/// WinHTTP 5.1 does not support SSL2 unless the client specifically enables it.
		/// </term>
		/// </item>
		/// <item>
		/// <term><c>ERROR_WINHTTP_INCORRECT_HANDLE_STATE</c></term>
		...
		/// <remarks>
		/// <para>
		/// Starting in Windows Vista and Windows Server 2008, WinHttp enables applications to perform chunked transfer encoding on data sent
		/// to the server. When the Transfer-Encoding header is present on the WinHttp response, <c>WinHttpReadData</c> strips the chunking
		/// information before giving the data to the application.
		/// </para>
		/// <para>
		/// Even when WinHTTP is used in asynchronous mode (that is, when <c>WINHTTP_FLAG_ASYNC</c> has been set in WinHttpOpen), this
		/// function can operate either synchronously or asynchronously. If this function returns <c>FALSE</c>, this function failed and you
		/// can call GetLastError to get extended error information. If this function returns <c>TRUE</c>, use the
		/// WINHTTP_CALLBACK_STATUS_READ_COMPLETE completion to determine whether this function was successful and the value of the
		/// parameters. The WINHTTP_CALLBACK_STATUS_REQUEST_ERROR completion indicates that the operation completed asynchronously, but failed.
		/// </para>
		/// <para>
		/// <c>Warning</c> When WinHTTP is used in asynchronous mode, always set the <c>lpdwNumberOfBytesRead</c> parameter to <c>NULL</c>
		/// and retrieve the bytes read in the callback function; otherwise, a memory fault can occur.
		/// </para>
		/// <para>
		/// When the read buffer is very small, <c>WinHttpReadData</c> might complete synchronously. If the
		/// WINHTTP_CALLBACK_STATUS_READ_COMPLETE completion triggers another call to <c>WinHttpReadData</c>, the situation can result in a
		/// stack overflow. In general, it is best to use a read buffer that is comparable in size, or larger than the internal read buffer
		/// used by WinHTTP, which is 8 KB.
		/// </para>
		/// <para>
		/// If you are using <c>WinHttpReadData</c> synchronously, and the return value is <c>TRUE</c> and the number of bytes read is zero,
		/// the transfer has been completed and there are no more bytes to read on the handle. This is analogous to reaching end-of-file in a
		/// local file. If you are using the function asynchronously, the WINHTTP_CALLBACK_STATUS_READ_COMPLETE callback is called with the
		...
		/// <c>WINHTTP_CALLBACK_STATUS_READ_COMPLETE</c> completion then triggers another call to <c>WinHttpReadData</c>, a stack overflow
		/// can result. It is best to use a read buffer that is 8 Kilobytes or larger in size.
		/// </para>
		/// <para>
		/// If sufficient data has not arrived from the server, <c>WinHttpReadData</c> does not entirely fill the buffer pointed to by
		/// <c>lpBuffer</c>. The buffer must be large enough at least to hold the HTTP headers on the first read, and when reading HTML
		/// encoded directory entries, it must be large enough to hold at least one complete entry.
		/// </para>
		/// <para>
		/// If a status callback function has been installed by using WinHttpSetStatusCallback, then those of the following notifications
		/// that have been set in the <c>dwNotificationFlags</c> parameter of <c>WinHttpSetStatusCallback</c> indicate progress in checking
		/// for available data:
		/// </para>
		/// <list type="bullet">
		/// <item>
		...
		/// <para>Examples</para>
		/// <para>
		/// The following example shows how to use secure transaction semantics to download a resource from an Secure Hypertext Transfer
		/// Protocol (HTTPS) server. The sample code initializes the WinHTTP application programming interface (API), selects a target HTTPS
		/// server, then opens and sends a request for this secure resource. WinHttpQueryDataAvailable is used with the request handle to
		/// determine how much data is available for download, then <c>WinHttpReadData</c> is used to read that data. This process repeats
		/// until the entire document has been retrieved and displayed.
		/// </para>
		/// <para>
		...
		/// </para>
		/// </remarks>
		// https://docs.microsoft.com/en-us/windows/win32/api/winhttp/nf-winhttp-winhttpreaddata BOOL WinHttpReadData( [in] HINTERNET
		// hRequest, [out] LPVOID lpBuffer, [in] DWORD dwNumberOfBytesToRead, [out] LPDWORD lpdwNumberOfBytesRead );
		[DllImport(Lib_Winhttp, SetLastError = true, ExactSpelling = true)]
		...
		/// <c>WinHttpReceiveResponse</c> avoids the expense of a buffer copy, doing so requires that your application use a fixed-length
		/// buffer for reading.
		/// </para>
		/// </param>
		/// <param name="lpBuffer">
		...
		/// <para>
		/// Pointer to an unsigned long integer variable that receives the number of bytes read. <c>WinHttpReadDataEx</c> sets this value to
		/// zero before doing any work or error checking. When using WinHTTP asynchronously, always set this parameter to <c>NULL</c> and
		/// retrieve the information in the callback function; not doing so can cause a memory fault.
		/// </para>
		/// </param>
		...
		/// <para>
		/// If you pass <c>WINHTTP_READ_DATA_EX_FLAG_FILL_BUFFER</c>, then WinHttp won't complete the call to <c>WinHttpReadDataEx</c> until
		/// the provided data buffer has been filled, or the response is complete. Passing this flag makes the behavior of this API
		/// equivalent to that of WinHttpReadData.
		/// </para>
		/// </param>
		...
		/// </returns>
		/// <remarks>
		/// By default, <c>WinHttpReadDataEx</c> returns after any amount of data has been written to the buffer that you provide (the
		/// function won't always completely fill the buffer that you provide).
		/// </remarks>
		// https://docs.microsoft.com/en-us/windows/win32/api/winhttp/nf-winhttp-winhttpreaddataex WINHTTPAPI DWORD WinHttpReadDataEx(
		// HINTERNET hRequest, LPVOID lpBuffer, DWORD dwNumberOfBytesToRead, LPDWORD lpdwNumberOfBytesRead, ULONGLONG ullFlags, DWORD
		// cbProperty, PVOID pvProperty );
		...
		/// <c>WinHttpReceiveResponse</c> completes successfully, the status code and response headers have been received and are available
		/// for the application to inspect using WinHttpQueryHeaders. An application must call <c>WinHttpReceiveResponse</c> before it can
		/// use WinHttpQueryDataAvailable and WinHttpReadData to access the response entity body (if any).
		/// </summary>
		/// <param name="hRequest">
		/// HINTERNET handle returned by WinHttpOpenRequest and sent by WinHttpSendRequest. Wait until <c>WinHttpSendRequest</c> has
		/// completed for this handle before calling <c>WinHttpReceiveResponse</c>.
		/// </param>
		...
		/// <term>
		/// The login attempt failed. When this error is encountered, the request handle should be closed with WinHttpCloseHandle. A new
		/// request handle must be created before retrying the function that originally produced this error.
		/// </term>
		/// </item>
		/// <item>
		...
		/// <term>
		/// One or more errors were found in the Secure Sockets Layer (SSL) certificate sent by the server. To determine what type of error
		/// was encountered, check for a WINHTTP_CALLBACK_STATUS_SECURE_FAILURE notification in a status callback function. For more
		...
		/// Even when WinHTTP is used in asynchronous mode (that is, when <c>WINHTTP_FLAG_ASYNC</c> has been set in WinHttpOpen), this
		/// function can operate either synchronously or asynchronously. If this function returns <c>FALSE</c>, this function failed and you
		/// can call GetLastError to get extended error information. If this function returns <c>TRUE</c>, the application should expect
		/// either the <c>WINHTTP_CALLBACK_STATUS_HEADERS_AVAILABLE</c> completion callback, indicating success, or the
		...
		/// have been set in the <c>dwNotificationFlags</c> parameter of <c>WinHttpSetStatusCallback</c> indicate progress in receiving the response:
		/// </para>
		/// <list type="bullet">
		/// <item>
		/// <term>WINHTTP_CALLBACK_STATUS_RECEIVING_RESPONSE</term>
		...
		/// between calls to reset the state of the auto-proxy. If there is less than 30 seconds, the <c>WinHttpResetAutoProxy</c> function
		/// call may return <c>ERROR_SUCCESS</c> but the reset won't happen.
		/// </para>
		/// </remarks>
		// https://docs.microsoft.com/en-us/windows/win32/api/winhttp/nf-winhttp-winhttpresetautoproxy WINHTTPAPI DWORD
		// WinHttpResetAutoProxy( [in] HINTERNET hSession, [in] DWORD dwFlags );
		[DllImport(Lib_Winhttp, SetLastError = false, ExactSpelling = true)]
		[PInvokeData("winhttp.h", MSDNShortId = "NF:winhttp.WinHttpResetAutoProxy")]
		public static extern uint WinHttpResetAutoProxy(HINTERNET hSession, WINHTTP_RESET dwFlags);
		...
		/// An unsigned long integer value that contains the length, in characters, of the additional headers. If this parameter is
		/// <c>-1L</c> and <c>pwszHeaders</c> is not <c>NULL</c>, this function assumes that <c>pwszHeaders</c> is <c>null</c>-terminated,
		/// and the length is calculated.
		/// </param>
		/// <param name="lpOptional">
		/// <para>
		/// A pointer to a buffer that contains any optional data to send immediately after the request headers. This parameter is generally
		/// used for POST and PUT operations. The optional data can be the resource or data posted to the server. This parameter can be
		...
		/// <para>
		/// An unsigned long integer value that contains the length, in bytes, of the optional data. This parameter can be zero if there is
		/// no optional data to send.
		/// </para>
		/// <para>
		/// This parameter must contain a valid length when the <c>lpOptional</c> parameter is not <c>NULL</c>. Otherwise, <c>lpOptional</c>
		/// is ignored and set to <c>NULL</c>.
		/// </para>
		/// </param>
		...
		/// Content-Length header of the request. If the value of this parameter is greater than the length specified by
		/// <c>dwOptionalLength</c>, then WinHttpWriteData can be used to send additional data.
		/// </para>
		/// <para>
		/// <c>dwTotalLength</c> must not change between calls to <c>WinHttpSendRequest</c> for the same request. If <c>dwTotalLength</c>
		...
		/// WinHttpQueryOption with the <c>WINHTTP_OPTION_CLIENT_CERT_ISSUER_LIST</c> option. If the server requests the client certificate,
		/// but does not require it, the application can alternately call WinHttpSetOption with the <c>WINHTTP_OPTION_CLIENT_CERT_CONTEXT</c>
		/// option. In this case, the application specifies the WINHTTP_NO_CLIENT_CERT_CONTEXT macro in the <c>lpBuffer</c> parameter of
		/// <c>WinHttpSetOption</c>. For more information, see the <c>WINHTTP_OPTION_CLIENT_CERT_CONTEXT</c> option. <c>Windows Server 2003
		/// with SP1, Windows XP with SP2 and Windows 2000:</c> This error is not supported.
		/// </term>
		/// </item>
		/// <item>
		/// <term><c>ERROR_WINHTTP_CONNECTION_ERROR</c></term>
		...
		/// <term>
		/// One or more errors were found in the Secure Sockets Layer (SSL) certificate sent by the server. To determine what type of error
		/// was encountered, verify through a WINHTTP_CALLBACK_STATUS_SECURE_FAILURE notification in a status callback function. For more
		/// information, see WINHTTP_STATUS_CALLBACK.
		/// </term>
		/// </item>
		/// <item>
		...
		/// Windows 2000:</c> The TCP reservation range set with the <c>WINHTTP_OPTION_PORT_RESERVATION</c> option is not large enough to
		/// send this request.
		/// </term>
		/// </item>
		/// <item>
		...
		/// <term>
		/// The content length specified in the <c>dwTotalLength</c> parameter does not match the length specified in the Content-Length
		/// header. The <c>lpOptional</c> parameter must be <c>NULL</c> and the <c>dwOptionalLength</c> parameter must be zero when the
		/// Transfer-Encoding header is present. The Content-Length header cannot be present when the Transfer-Encoding header is present.
		/// </term>
		/// </item>
		/// <item>
		/// <term><c>ERROR_WINHTTP_RESEND_REQUEST</c></term>
		/// <term>
		/// The application must call WinHttpSendRequest again due to a redirect or authentication challenge. <c>Windows Server 2003 with
		...
		/// <para>
		/// Even when WinHTTP is used in asynchronous mode, that is, when <c>WINHTTP_FLAG_ASYNC</c> has been set in WinHttpOpen, this
		/// function can operate either synchronously or asynchronously. In either case, if the request is sent successfully, the application
		/// is called back with the completion status set to <c>WINHTTP_CALLBACK_STATUS_SENDREQUEST_COMPLETE</c>. The
		...
		/// receiving the <c>WINHTTP_CALLBACK_STATUS_SENDREQUEST_COMPLETE</c> status callback, the application can start to receive a
		/// response from the server with WinHttpReceiveResponse. Before then, no other asynchronous functions can be called, otherwise,
		/// <c>ERROR_WINHTTP_INCORRECT_HANDLE_STATE</c> is returned.
		/// </para>
		/// <para>
		/// An application must not delete or alter the buffer pointed to by <c>lpOptional</c> until the request handle is closed or the call
		/// to WinHttpReceiveResponse has completed, because an authentication challenge or redirect that required the optional data could be
		/// encountered in the course of receiving the response. If the operation must be aborted with WinHttpCloseHandle, the application
		/// must keep the buffer valid until it receives the callback <c>WINHTTP_CALLBACK_STATUS_REQUEST_ERROR</c> with an
		...
		/// </para>
		/// <para>
		/// This function also lets the client specify optional data to send to the HTTP server immediately following the request headers.
		/// This feature is generally used for write operations such as PUT and POST.
		/// </para>
		/// <para>
		/// An application can use the same HTTP request handle in multiple calls to <c>WinHttpSendRequest</c> to re-send the same request,
		/// but the application must read all data returned from the previous call before calling this function again.
		/// </para>
		/// <para>
		...
		/// </para>
		/// <para>
		/// <c>Windows 2000:</c> When sending requests from multiple threads, there may be a significant decrease in network and CPU performance.
		/// </para>
		/// <para><c>Windows XP and Windows 2000:</c> See Run-Time Requirements.</para>
		/// <para>WinHttpSetStatusCallback</para>
		/// <para>
		/// If a status callback function has been installed with WinHttpSetStatusCallback, then those of the following notifications that
		/// have been set in the <c>dwNotificationFlags</c> parameter of <c>WinHttpSetStatusCallback</c> indicate the progress in sending the request:
		/// </para>
		/// <list type="bullet">
		/// <item>
		...
		/// bytes) using the Content-Length header. Payload lengths specified in the call to <c>WinHttpSendRequest</c> are limited to the
		/// size of a <c>DWORD</c> (2^32 bytes). To upload data to a URL larger than a <c>DWORD</c>, the application must provide the length
		/// in the Content-Length header of the request. In this case, the WinHttp client application calls <c>WinHttpSendRequest</c> with
		/// the <c>dwTotalLength</c> parameter set to <c>WINHTTP_IGNORE_REQUEST_TOTAL_LENGTH</c>.
		/// </para>
		/// <para>
		/// If the Content-Length header specifies a length less than a 2^32, the application must also specify the content length in the
		/// call to <c>WinHttpSendRequest</c>. If the <c>dwTotalLength</c> parameter does not match the length specified in the
		/// Content-Length header, the call fails and returns <c>ERROR_INVALID_PARAMETER</c>.
		/// </para>
		/// <para>
		...
		/// <para>
		/// Starting in Windows Vista and Windows Server 2008, WinHttp enables applications to perform chunked transfer encoding on data sent
		/// to the server. When the Transfer-Encoding header is present on the WinHttp request, the <c>dwTotalLength</c> parameter in the
		/// call to <c>WinHttpSendRequest</c> is set to <c>WINHTTP_IGNORE_REQUEST_TOTAL_LENGTH</c> and the application sends the entity body
		/// in one or more calls to WinHttpWriteData. The <c>lpOptional</c> parameter of <c>WinHttpSendRequest</c> must be <c>NULL</c> and
		/// the <c>dwOptionLength</c> parameter must be zero, otherwise an <c>ERROR_WINHTTP_INVALID_PARAMETER</c> error is returned. To
		/// terminate the chunked data transfer, the application generates a zero length chunk and sends it in the last call to <c>WinHttpWriteData</c>.
		/// </para>
		/// <para>Examples</para>
		/// <para>
		...
		/// </para>
		/// <para>
		/// The credentials set by <c>WinHttpSetCredentials</c> are only used for a single request; WinHTTP does not cache these credentials
		/// for use in subsequent requests. As a result, applications must be written so that they can respond to multiple challenges. If an
		/// authenticated connection is re-used, subsequent requests cannot be challenged, but your code should be able to respond to a
		...
		/// </remarks>
		// https://docs.microsoft.com/en-us/windows/win32/api/winhttp/nf-winhttp-winhttpsetcredentials BOOL WinHttpSetCredentials( [in]
		// HINTERNET hRequest, [in] DWORD AuthTargets, [in] DWORD AuthScheme, [in] LPCWSTR pwszUserName, [in] LPCWSTR pwszPassword, [in]
		// LPVOID pAuthParams );
		[DllImport(Lib_Winhttp, SetLastError = true, ExactSpelling = true)]
		...
		/// <remarks>
		/// <para>
		/// The default proxy configuration set by <c>WinHttpSetDefaultProxyConfiguration</c> can be overridden for an existing WinHTTP
		/// session by calling WinHttpSetOption and specifying the WINHTTP_OPTION_PROXY flag. The default proxy configuration can be
		/// overridden for a new session by specifying the configuration with the WinHttpOpen function.
		/// </para>
		/// <para>
		...
		/// <param name="hInternet">
		/// The HINTERNET handle on which to set data. Be aware that this can be either a Session handle or a Request handle, depending on
		/// what option is being set. For more information about how to determine which handle is appropriate to use in setting a particular
		/// option, see the Option Flags.
		/// </param>
		/// <param name="dwOption">
		...
		/// <term><c>ERROR_INVALID_PARAMETER</c></term>
		/// <term>
		/// A parameter is not valid. This value will be returned if <c>WINHTTP_OPTION_WEB_SOCKET_KEEPALIVE_INTERVAL</c> is set to a value
		/// lower than 15000.
		/// </term>
		/// </item>
		/// <item>
		...
		/// Pointer to the callback function to call when progress is made. Set this to <c>NULL</c> to remove the existing callback function.
		/// For more information about the callback function, see WINHTTP_STATUS_CALLBACK.
		/// </param>
		...
		/// <returns>
		/// <para>
		/// If successful, returns a pointer to the previously defined status callback function or <c>NULL</c> if there was no previously
		/// defined status callback function. Returns <c>WINHTTP_INVALID_STATUS_CALLBACK</c> if the callback function could not be installed.
		/// For extended error information, call GetLastError. Among the error codes returned are the following.
		/// </para>
		/// <list type="table">
		/// <listheader>
		/// <term>Error Code</term>
		...
		/// <remarks>
		/// <para>
		/// If you set the callback on the session handle before creating the request handle, the request handle inherits the callback
		/// function pointer from its parent session.
		/// </para>
		/// <para>
		/// Even when WinHTTP is used in asynchronous mode (that is, when <c>WINHTTP_FLAG_ASYNC</c> has been set in WinHttpOpen), this
		/// function operates synchronously. The return value indicates success or failure. To get extended error information, call GetLastError.
		...
		/// a name, connecting to a server, and so on. The callback function is required for an asynchronous operation.
		/// </para>
		/// <para>
		...
		/// changing the callback function on a handle does not change the callbacks on derived handles, such as that returned by
		/// WinHttpConnect. You must change the callback function at each level.
		/// </para>
		/// <para>
		...
		/// <para>
		/// Note, however, that WinHTTP does not synchronize <c>WinHttpSetStatusCallback</c> with worker threads. If a callback originating
		/// in another thread is in progress when an application calls <c>WinHttpSetStatusCallback</c>, the application still receives a
		/// callback notification even after <c>WinHttpSetStatusCallback</c> successfully sets the callback function to <c>NULL</c> and returns.
		/// </para>
		/// <para><note type="note">For Windows XP and Windows 2000, see the Run-Time Requirements section of the WinHttp start page.</note></para>
		/// <para>Examples</para>
		/// <para>
		...
		// WinHttpSetStatusCallback( [in] HINTERNET hInternet, [in] WINHTTP_STATUS_CALLBACK lpfnInternetCallback, [in] DWORD
		// dwNotificationFlags, [in] DWORD_PTR dwReserved );
		[DllImport(Lib_Winhttp, SetLastError = true, ExactSpelling = true)]
		...
		/// <para>
		/// A value of type integer that specifies the time-out value, in milliseconds, to use for name resolution. If resolution takes
		/// longer than this time-out value, the action is canceled. The initial value is zero, meaning no time-out (infinite).
		/// </para>
		/// <para>
		/// <c>Windows Vista and Windows XP:</c> If DNS timeout is specified using NAME_RESOLUTION_TIMEOUT, there is an overhead of one
		/// thread per request.
		/// </para>
		/// </param>
		/// <param name="nConnectTimeout">
		/// <para>
		/// A value of type integer that specifies the time-out value, in milliseconds, to use for server connection requests. If a
		/// connection request takes longer than this time-out value, the request is canceled. The initial value is 60,000 (60 seconds).
		/// </para>
		/// <para>
		...
		/// A value of type integer that specifies the time-out value, in milliseconds, to use for sending requests. If sending a request
		/// takes longer than this time-out value, the send is canceled. The initial value is 30,000 (30 seconds).
		/// </param>
		/// <param name="nReceiveTimeout">
		/// A value of type integer that specifies the time-out value, in milliseconds, to receive a response to a request. If a response
		/// takes longer than this time-out value, the request is canceled. The initial value is 30,000 (30 seconds).
		/// </param>
		/// <returns>
		/// <para>
		/// Returns <c>TRUE</c> if successful, or <c>FALSE</c> otherwise. For extended error information, call GetLastError. Among the error
		...
		/// </para>
		/// <para>
		/// A value of 0 or -1 sets a time-out to wait infinitely. A value greater than 0 sets the time-out value in milliseconds. For
		/// example, 30,000 would set the time-out to 30 seconds. All negative values other than -1 cause the function to fail with ERROR_INVALID_PARAMETER.
		/// </para>
		/// <para>
		/// <c>Important</c> If a small timeout is set using WinHttpSetOption and WINHTTP_OPTION_RECEIVE_TIMEOUT, it can override the value
		/// set with the <c>dwReceiveTimeout</c> parameter, causing a response to terminate earlier than expected. To avoid this, do not set
		/// a timeout with the <c>WINHTTP_OPTION_RECEIVE_TIMEOUT</c> option that is smaller than the value set using <c>dwReceiveTimeout</c>.
		/// </para>
		/// <para><c>Note</c> For Windows XP and Windows 2000, see the Run-Time Requirements section of the WinHTTP start page.</para>
		/// <para>Examples</para>
		...
		/// </para>
		/// <para>
		/// It is possible to receive a close frame during regular receive operations. In this case, <c>WinHttpWebSocketClose</c> will also
		/// send a close frame.
		/// </para>
		/// <para>The close timer can be set by the property WINHTTP_OPTION_WEB_SOCKET_CLOSE_TIMEOUT. The default is 10 seconds.</para>
		/// </remarks>
		// https://docs.microsoft.com/en-us/windows/win32/api/winhttp/nf-winhttp-winhttpwebsocketclose WINHTTPAPI DWORD
		// WinHttpWebSocketClose( [in] HINTERNET hWebSocket, [in] USHORT usStatus, [in, optional] PVOID pvReason, [in] DWORD dwReasonLength );
		[DllImport(Lib_Winhttp, SetLastError = false, ExactSpelling = true)]
		...
		/// <remarks>
		/// <para>
		/// <c>WinHttpWebSocketCompleteUpgrade</c> can be called on an open HTTP request to get a WebSocket handle for performing other
		/// WebSocket operations.
		/// </para>
		/// <para>
		/// The request handle must be marked as a WebSocket upgrade by calling WinHttpSetOption with
		/// <c>WINHTTP_OPTION_UPGRADE_TO_WEB_SOCKET</c> before sending the request.
		/// </para>
		/// <para>
		/// The caller should check the HTTP status code returned by the server and call this function only if the status code was 101.
		/// Calling it with any other status code will result in a failure.
		/// </para>
		/// </remarks>
		...
		/// <para>Type: <c>DWORD*</c></para>
		/// <para>
		/// The number of bytes consumed. If <c>pvReason</c> is <c>NULL</c> and <c>dwReasonLength</c> is 0, <c>pdwReasonLengthConsumed</c>
		/// will contain the size of the buffer that needs to be allocated by the calling application.
		/// </para>
		/// </param>
		/// <returns>
		/// <para>Type: <c>DWORD</c></para>
		...
		// WinHttpWebSocketQueryCloseStatus( [in] HINTERNET hWebSocket, [out] USHORT *pusStatus, [out] PVOID pvReason, [in] DWORD
		// dwReasonLength, [out] DWORD *pdwReasonLengthConsumed );
		[DllImport(Lib_Winhttp, SetLastError = false, ExactSpelling = true)]
		...
		/// <para>Type: <c>DWORD*</c></para>
		/// <para>
		/// Pointer to a <c>DWORD</c> that receives the number of bytes read from the connection at the end of the operation. This is set
		/// only if <c>WinHttpWebSocketReceive</c> returns <c>NO_ERROR</c> and the handle was opened in synchronous mode.
		/// </para>
		/// </param>
		/// <param name="peBufferType">
		/// <para>Type: <c>WINHTTP_WEB_SOCKET_BUFFER_TYPE*</c></para>
		/// <para>
		/// The type of a returned buffer. This is only set if <c>WinHttpWebSocketReceive</c> returns <c>NO_ERROR</c> and the handle was
		...
		// WinHttpWebSocketReceive( [in] HINTERNET hWebSocket, [out] PVOID pvBuffer, [in] DWORD dwBufferLength, [out] DWORD *pdwBytesRead,
		...
		/// </list>
		/// </returns>
		// https://docs.microsoft.com/en-us/windows/win32/api/winhttp/nf-winhttp-winhttpwebsocketsend WINHTTPAPI DWORD WinHttpWebSocketSend(
		// [in] HINTERNET hWebSocket, [in] WINHTTP_WEB_SOCKET_BUFFER_TYPE eBufferType, [in] PVOID pvBuffer, [in] DWORD dwBufferLength );
		[DllImport(Lib_Winhttp, SetLastError = false, ExactSpelling = true)]
		...
		/// <remarks>
		/// <para>
		/// <c>WinHttpWebSocketShutdown</c> sends a close frame and prevents additional data from being sent over the WebSocket connection.
		/// It does not close the receive channel. Use WinHttpWebSocketClose when you want to completely close the connection and prevent any
		...
		// WinHttpWebSocketShutdown( [in] HINTERNET hWebSocket, [in] USHORT usStatus, [in, optional] PVOID pvReason, [in] DWORD
		// dwReasonLength );
		[DllImport(Lib_Winhttp, SetLastError = false, ExactSpelling = true)]
		...
		/// function sets this value to zero before doing any work or error checking. When using WinHTTP asynchronously, this parameter must
		/// be set to <c>NULL</c> and retrieve the information in the callback function. Not doing so can cause a memory fault.
		/// </param>
		/// <returns>
		/// <para>
		/// Returns <c>TRUE</c> if successful, or <c>FALSE</c> otherwise. For extended error information, call GetLastError. Among the error
		...
		/// <para>
		/// Even when WinHTTP is used in asynchronous mode (that is, when <c>WINHTTP_FLAG_ASYNC</c> has been set in WinHttpOpen), this
		/// function can operate either synchronously or asynchronously. If this function returns <c>FALSE</c>, you can call GetLastError to
		/// get extended error information. If this function returns <c>TRUE</c>, use the WINHTTP_CALLBACK_STATUS_WRITE_COMPLETE completion
		/// to determine whether this function was successful and the value of the parameters. The WINHTTP_CALLBACK_STATUS_REQUEST_ERROR
		...
		/// </para>
		/// <para>
		/// When the application is sending data, it can call WinHttpReceiveResponse to end the data transfer. If WinHttpCloseHandle is
		/// called, then the data transfer is aborted.
		/// </para>
		/// <para>
		/// If a status callback function has been installed with WinHttpSetStatusCallback, then those of the following notifications that
		/// have been set in the <c>dwNotificationFlags</c> parameter of <c>WinHttpSetStatusCallback</c> indicate progress in sending data to
		/// the server:
		/// </para>
		/// <list type="bullet">
		/// <item>
		...
		/// POST'ed, in which case not only does <c>WinHttpWriteData</c> fail, but also WinHTTP cannot handle the authentication challenges.
		/// NTLM and Negotiate require that all authentication handshakes be exchanged on the same socket connection, so authentication fails
		...
		/// Since an authenticated socket can only be reused for subsequent requests within the same session, the POST should go out in the
		/// same socket as long as the socket is not pooled with concurrent requests competing for it.
		/// </para>
		/// <para><c>Note</c> For Windows XP and Windows 2000, see the Run-Time Requirements section of the WinHTTP start page.</para>
		/// <para>Examples</para>
		/// <para>
		...
		/// </para>
		/// </remarks>
		// https://docs.microsoft.com/en-us/windows/win32/api/winhttp/nf-winhttp-winhttpwritedata BOOL WinHttpWriteData( [in] HINTERNET
		// hRequest, [in] LPCVOID lpBuffer, [in] DWORD dwNumberOfBytesToWrite, [out] LPDWORD lpdwNumberOfBytesWritten );
		[DllImport(Lib_Winhttp, SetLastError = true, ExactSpelling = true)]
