...
public static partial class SetupAPI
{
	...
	/// <summary>Provides a handle to a log file.</summary>
	[StructLayout(LayoutKind.Sequential)]
	public struct HSPFILELOG : IHandle
	{
		private readonly IntPtr handle;

		/// <summary>Initializes a new instance of the <see cref="HSPFILELOG"/> struct.</summary>
		...
		/// <summary>Performs an explicit conversion from <see cref="HSPFILELOG"/> to <see cref="IntPtr"/>.</summary>
		/// <param name="h">The handle.</param>
		/// <returns>The result of the conversion.</returns>
		public static explicit operator IntPtr(HSPFILELOG h) => h.handle;
		...
		public static bool operator !=(HSPFILELOG h1, HSPFILELOG h2) => !(h1 == h2);
	...
	/// <summary>Provides a <see cref="SafeHandle"/> for <see cref="HSPFILELOG"/> that is disposed using <see cref="SetupTerminateFileLog"/>.</summary>
	public class SafeHSPFILELOG : SafeHANDLE
	{
		/// <summary>Initializes a new instance of the <see cref="SafeHSPFILELOG"/> class and assigns an existing handle.</summary>
		/// <param name="preexistingHandle">An <see cref="IntPtr"/> object that represents the pre-existing handle to use.</param>
		/// <param name="ownsHandle">
		/// <see langword="true"/> to reliably release the handle during the finalization phase; otherwise, <see langword="false"/> (not recommended).
		/// </param>
		public SafeHSPFILELOG(IntPtr preexistingHandle, bool ownsHandle = true) : base(preexistingHandle, ownsHandle) { }

		/// <summary>Initializes a new instance of the <see cref="SafeHSPFILELOG"/> class.</summary>
		private SafeHSPFILELOG() : base() { }

		/// <summary>Performs an implicit conversion from <see cref="SafeHSPFILELOG"/> to <see cref="HSPFILELOG"/>.</summary>
		/// <param name="h">The safe handle instance.</param>
		/// <returns>The result of the conversion.</returns>
		public static implicit operator HSPFILELOG(SafeHSPFILELOG h) => h.handle;

		/// <inheritdoc/>
		protected override bool InternalReleaseHandle() => SetupTerminateFileLog(handle);
	}

	/// <summary>Provides a <see cref="SafeHandle"/> for <see cref="HSPFILEQ"/> that is disposed using <see cref="SetupCloseFileQueue"/>.</summary>
	public class SafeHSPFILEQ : SafeHANDLE
	{
		/// <summary>Initializes a new instance of the <see cref="SafeHSPFILEQ"/> class and assigns an existing handle.</summary>
		/// <param name="preexistingHandle">An <see cref="IntPtr"/> object that represents the pre-existing handle to use.</param>
		/// <param name="ownsHandle">
		/// <see langword="true"/> to reliably release the handle during the finalization phase; otherwise, <see langword="false"/> (not recommended).
		/// </param>
		public SafeHSPFILEQ(IntPtr preexistingHandle, bool ownsHandle = true) : base(preexistingHandle, ownsHandle) { }

		/// <summary>Initializes a new instance of the <see cref="SafeHSPFILEQ"/> class.</summary>
		private SafeHSPFILEQ() : base() { }

		/// <summary>Performs an implicit conversion from <see cref="SafeHSPFILEQ"/> to <see cref="HSPFILEQ"/>.</summary>
		/// <param name="h">The safe handle instance.</param>
		/// <returns>The result of the conversion.</returns>
		public static implicit operator HSPFILEQ(SafeHSPFILEQ h) => h.handle;

		/// <inheritdoc/>
		protected override bool InternalReleaseHandle() => SetupCloseFileQueue(handle);
	}
}