...
public static partial class D3D12
{
	...
	public struct D3D12_AUTO_BREADCRUMB_NODE
	{
		...
		/// <summary>A pointer to a constant <c>UINT32</c> containing the number of pCommandHistory breadcrumbs ops completed. As such, the last successfully completed breadcrumb op is at index in pCommandHistory.</summary>
		public StructPointer<uint> pLastBreadcrumbValue;
		/// <summary>A pointer to a constant array of D3D12_AUTO_BREADCRUMB_OP values representing all of the render/compute operations recorded into the associated command list.</summary>
		public ArrayPointer<D3D12_AUTO_BREADCRUMB_OP> pCommandHistory;
		/// <summary>A pointer to a constant <c>D3D12_AUTO_BREADCRUMB_NODE</c> representing the next auto-breadcrumb node in the list, or if this is the last node.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_barrier_group
	// typedef struct D3D12_BARRIER_GROUP { D3D12_BARRIER_TYPE Type; UINT32 NumBarriers; union { const D3D12_GLOBAL_BARRIER *pGlobalBarriers; const D3D12_TEXTURE_BARRIER *pTextureBarriers; const D3D12_BUFFER_BARRIER *pBufferBarriers; }; } D3D12_BARRIER_GROUP;
	...
	/// <summary>Describes a buffer memory access barrier. Used by buffer barriers to indicate when resource memory must be made visible for a specific access type.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_buffer_barrier
	// typedef struct D3D12_BUFFER_BARRIER { D3D12_BARRIER_SYNC SyncBefore; D3D12_BARRIER_SYNC SyncAfter; D3D12_BARRIER_ACCESS AccessBefore; D3D12_BARRIER_ACCESS AccessAfter; ID3D12Resource *pResource; UINT64 Offset; UINT64 Size; } D3D12_BUFFER_BARRIER;
	...
	public struct D3D12_BUFFER_BARRIER
	{
		...
		/// <summary>Access bits corresponding with resource usage since the preceding barrier, or the start of <b>ExecuteCommandLists</b> scope.</summary>
	...
	/// <summary>Describes a resource memory access barrier. Used by global, texture, and buffer barriers to indicate when resource memory must be made visible for a specific access type.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_global_barrier
	// typedef struct D3D12_GLOBAL_BARRIER { D3D12_BARRIER_SYNC SyncBefore; D3D12_BARRIER_SYNC SyncAfter; D3D12_BARRIER_ACCESS AccessBefore; D3D12_BARRIER_ACCESS AccessAfter; } D3D12_GLOBAL_BARRIER;
	[PInvokeData("d3d12.h", MSDNShortId = "NS:d3d12.D3D12_GLOBAL_BARRIER")]
	[StructLayout(LayoutKind.Sequential)]
	public struct D3D12_GLOBAL_BARRIER
	{
		/// <summary>Synchronization scope of all preceding GPU work that must be completed before executing the barrier.</summary>
		public D3D12_BARRIER_SYNC SyncBefore;
		/// <summary>Synchronization scope of all subsequent GPU work that must wait until the barrier execution is finished.</summary>
		public D3D12_BARRIER_SYNC SyncAfter;
		/// <summary>Access bits corresponding with any relevant resource usage since the preceding barrier or the start of <b>ExecuteCommandLists</b> scope.</summary>
		public D3D12_BARRIER_ACCESS AccessBefore;
		/// <summary>Access bits corresponding with any relevant resource usage after the barrier completes.</summary>
		public D3D12_BARRIER_ACCESS AccessAfter;
	}

	/// <summary>Describes a texture memory access barrier. Used by texture barriers to indicate when resource memory must be made visible for a specific access type. Layout transitions are needed only for textures.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_texture_barrier
	// typedef struct D3D12_TEXTURE_BARRIER { D3D12_BARRIER_SYNC SyncBefore; D3D12_BARRIER_SYNC SyncAfter; D3D12_BARRIER_ACCESS AccessBefore; D3D12_BARRIER_ACCESS AccessAfter; D3D12_BARRIER_LAYOUT LayoutBefore; D3D12_BARRIER_LAYOUT LayoutAfter; ID3D12Resource *pResource; D3D12_BARRIER_SUBRESOURCE_RANGE Subresources; D3D12_TEXTURE_BARRIER_FLAGS Flags; } D3D12_TEXTURE_BARRIER;
	...
	// typedef struct D3D12_BARRIER_SUBRESOURCE_RANGE { UINT IndexOrFirstMipLevel; UINT NumMipLevels; UINT FirstArraySlice; UINT NumArraySlices; UINT FirstPlane; UINT NumPlanes; } D3D12_BARRIER_SUBRESOURCE_RANGE;
	[PInvokeData("d3d12.h", MSDNShortId = "NS:d3d12.D3D12_BARRIER_SUBRESOURCE_RANGE")]
	[StructLayout(LayoutKind.Sequential)]
	public struct D3D12_BARRIER_SUBRESOURCE_RANGE
	{
		/// <summary>The index of the first mip level in the range; or a subresource index, if NumMipLevels is zero. If a subresource index, then you can use the value <c>0xffffffff</c> to specify all subresources.</summary>
	...
	/// <remarks>
	/// <para>A <c>D3D12_GRAPHICS_PIPELINE_STATE_DESC</c> object contains a blend-state structure that controls blending by the output-merger stage.</para>
	...
	{
		/// <summary>Specifies whether to use alpha-to-coverage as a multisampling technique when setting a pixel to a render target. For more info about using alpha-to-coverage, see <c>Alpha-To-Coverage</c>.</summary>
		[MarshalAs(UnmanagedType.Bool)]
		...
		/// <summary>An array of <c>D3D12_RENDER_TARGET_BLEND_DESC</c> structures that describe the blend states for render targets; these correspond to the eight render targets that can be bound to the <c>output-merger stage</c> at one time.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_buffer_rtv
	...
	/// <para>If the value of StructureByteStride is not 0, then a view of a structured buffer is created, and then the D3D12_SHADER_RESOURCE_VIEW_DESC::Format field must be <b>DXGI_FORMAT_UNKNOWN</b>. If StructureByteStride is 0, then a typed view of a buffer is created, and then a format must be supplied. The specified format for the typed view must be supported by the hardware.</para>
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_buffer_srv
	// typedef struct D3D12_BUFFER_SRV { UINT64 FirstElement; UINT NumElements; UINT StructureByteStride; D3D12_BUFFER_SRV_FLAGS Flags; } D3D12_BUFFER_SRV;
	...
	public struct D3D12_BUFFER_SRV
	{
		...
		public uint NumElements;
		/// <summary>The size of each element in the buffer structure (in bytes) when the buffer represents a structured buffer. The size must match the struct size declared in shaders that access the view.</summary>
		public uint StructureByteStride;
		/// <summary>A <c>D3D12_BUFFER_SRV_FLAGS</c>-typed value that identifies view options for the buffer. Currently, the only option is to identify a raw view of the buffer. For more info about raw viewing of buffers, see <c>Raw Views of Buffers</c>.</summary>
		public D3D12_BUFFER_SRV_FLAGS Flags;
	}

	/// <summary>Describes the blend state for a render target.</summary>
	/// <remarks>
	/// <para><para>Note</para> <para>It's not valid for LogicOpEnable and BlendEnable to both be <b>TRUE</b>.</para></para>
	/// <para>You specify an array of <b>D3D12_RENDER_TARGET_BLEND_DESC</b> structures in the <b>RenderTarget</b> member of the <c>D3D12_BLEND_DESC</c> structure to describe the blend states for render targets; you can bind up to eight render targets to the <c>output-merger stage</c> at one time.</para>
	...
	public struct D3D12_RENDER_TARGET_BLEND_DESC
	{
		...
		///   <para>
		///     <para>Note</para> <para>It's not valid for LogicOpEnable and BlendEnable to both be <b>TRUE</b>.</para></para>
		/// </summary>
		[MarshalAs(UnmanagedType.Bool)]
		...
		/// <summary>A <c>D3D12_BLEND</c>-typed value that specifies the operation to perform on the RGB value that the pixel shader outputs. The <b>BlendOp</b> member defines how to combine the <b>SrcBlend</b> and <b>DestBlend</b> operations.</summary>
		public D3D12_BLEND SrcBlend;
		/// <summary>A <c>D3D12_BLEND</c>-typed value that specifies the operation to perform on the current RGB value in the render target. The <b>BlendOp</b> member defines how to combine the <b>SrcBlend</b> and <b>DestBlend</b> operations.</summary>
		...
		/// <summary>A <c>D3D12_BLEND</c>-typed value that specifies the operation to perform on the alpha value that the pixel shader outputs. Blend options that end in _COLOR are not allowed. The <b>BlendOpAlpha</b> member defines how to combine the <b>SrcBlendAlpha</b> and <b>DestBlendAlpha</b> operations.</summary>
		public D3D12_BLEND SrcBlendAlpha;
		/// <summary>A <c>D3D12_BLEND</c>-typed value that specifies the operation to perform on the current alpha value in the render target. Blend options that end in _COLOR are not allowed. The <b>BlendOpAlpha</b> member defines how to combine the <b>SrcBlendAlpha</b> and <b>DestBlendAlpha</b> operations.</summary>
		public D3D12_BLEND DestBlendAlpha;
		/// <summary>A <c>D3D12_BLEND_OP</c>-typed value that defines how to combine the <b>SrcBlendAlpha</b> and <b>DestBlendAlpha</b> operations.</summary>
		...
		/// <summary>A combination of <see cref="D3D12_COLOR_WRITE_ENABLE"/>-typed values that are combined by using a bitwise OR operation. The resulting value specifies a write mask.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_dred_breadcrumb_context
	...
	{
		/// <summary>Index of the command list operation in the command history of the associated command list.  The command history is the array pointed to by the pCommandHistory member of the D3D12_AUTO_BREADCRUMB_NODE1 structure.</summary>
	...
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_command_queue_desc
	...
	public struct D3D12_COMMAND_QUEUE_DESC
	{
		...
		/// <summary>The priority for the command queue, as a <see cref="D3D12_COMMAND_QUEUE_PRIORITY"/> enumeration constant to select normal or high priority.</summary>
	...
	/// <summary>Contains a pointer to the head of a linked list of <c>D3D12_AUTO_BREADCRUMB_NODE</c> objects. The list represents the auto-breadcrumb state prior to device removal.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_dred_auto_breadcrumbs_output
	// typedef struct D3D12_DRED_AUTO_BREADCRUMBS_OUTPUT { const D3D12_AUTO_BREADCRUMB_NODE *pHeadAutoBreadcrumbNode; } D3D12_DRED_AUTO_BREADCRUMBS_OUTPUT;
	...
	{
		/// <summary>A pointer to a constant <c>D3D12_AUTO_BREADCRUMB_NODE</c> object representing the head of a linked list of auto-breadcrumb nodes, or <c>nullptr</c> if the list is empty.</summary>
		public IntPtr pHeadAutoBreadcrumbNode;
	}
	
	/// <summary>
	/// Contains a pointer to the head of a linked list of D3D12_AUTO_BREADCRUMB_NODE1 objects. The list represents the auto-breadcrumb state prior to device removal.
	/// </summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_dred_auto_breadcrumbs_output1
	...
	/// <para>If <i>StructureByteStride</i> value is not 0, a view of a structured buffer is created and the D3D12_UNORDERED_ACCESS_VIEW_DESC::Format field must be DXGI_FORMAT_UNKNOWN. If <i>StructureByteStride</i> is 0, a typed view of a buffer is created and a format must be supplied. The specified format for the typed view must be supported by the hardware. More information on this topic can be found in the <c>Typed unordered access view (UAV) loads</c> page.</para>
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_buffer_uav
	// typedef struct D3D12_BUFFER_UAV { UINT64 FirstElement; UINT NumElements; UINT StructureByteStride; UINT64 CounterOffsetInBytes; D3D12_BUFFER_UAV_FLAGS Flags; } D3D12_BUFFER_UAV;
	...
	/// <summary>Describes a raytracing acceleration structure. Pass this structure into <c>ID3D12GraphicsCommandList4::BuildRaytracingAccelerationStructure</c> to describe the acceleration structure to be built.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_build_raytracing_acceleration_structure_desc
	// typedef struct D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC { D3D12_GPU_VIRTUAL_ADDRESS DestAccelerationStructureData; D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS Inputs; D3D12_GPU_VIRTUAL_ADDRESS SourceAccelerationStructureData; D3D12_GPU_VIRTUAL_ADDRESS ScratchAccelerationStructureData; } D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC;
	...
	{
		/// <summary>
		///   <para>Location to store resulting acceleration structure. <c>ID3D12Device5::GetRaytracingAccelerationStructurePrebuildInfo</c> reports the amount of memory required for the result here given a set of acceleration structure build parameters.</para>
		///   <para>The address must be aligned to 256 bytes, defined as <c>D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BYTE_ALIGNMENT</c>.</para>
		///   <para>
		///     <para>Important</para> <para>The memory must be in state <c><b>D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE</b></c>.</para></para>
		/// </summary>
		...
		/// <summary>
		///   <para>Address of an existing acceleration structure if an acceleration structure update (an incremental build) is being requested, by setting <c>D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_PERFORM_UPDATE</c> in the Flags parameter. Otherwise this address must be NULL.</para>
		...
		///   <para>The address must be aligned to 256 bytes, defined as <c>D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BYTE_ALIGNMENT</c>, which should automatically be the case because any existing acceleration structure passed in here would have already been required to be placed with such alignment.</para>
		///   <para>
		///     <para>Important</para> <para>The memory must be in state <c><b>D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE</b></c>.</para></para>
		...
		///   <para>
		///     <para>Important</para> <para>The memory must be in state <c><b>D3D12_RESOURCE_STATE_UNORDERED_ACCESS</b></c>.</para></para>
		/// </summary>
	...
	/// <summary>Defines the inputs for a raytracing acceleration structure build operation. This structure is used by <c>ID3D12GraphicsCommandList4::BuildRaytracingAccelerationStructure</c> and <c>ID3D12Device5::GetRaytracingAccelerationStructurePrebuildInfo</c>.</summary>
	/// <remarks>When used with <c>GetRaytracingAccelerationStructurePrebuildInfo</c>, which actually perform a build, any parameter that is referenced via <c>D3D12_GPU_VIRTUAL_ADDRESS</c> (an address in GPU memory), like <i>InstanceDescs</i>, will not be accessed by the operation. So this memory does not need to be initialized yet or be in a particular resource state. Whether GPU addresses are null or not can be inspected by the operation, even though the pointers are not dereferenced.</remarks>
	...
	public struct D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS
	{
		...
		/// <summary>
		///   <para>If <i>Type</i> is <b>D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TOP_LEVEL</b>, this refers to <i>NumDescs</i><c>D3D12_RAYTRACING_INSTANCE_DESC</c> structures in GPU memory describing instances. Each instance must be aligned to 16 bytes, defined as <c>D3D12_RAYTRACING_INSTANCE_DESC_BYTE_ALIGNMENT</c>.</para>
		...
		///   <para>If <i>DescLayout</i> is <b>D3D12_ELEMENTS_LAYOUT_ARRAY_OF_POINTERS</b>, <i>InstanceDescs</i> points to an array in GPU memory of <c>D3D12_GPU_VIRTUAL_ADDRESS</c> pointers to instance descriptions.</para>
	...
	/// <summary>Describes a set of geometry that is used in the <c>D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS</c> structure to provide input data to a raytracing acceleration structure build operation.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_raytracing_geometry_desc
	// typedef struct D3D12_RAYTRACING_GEOMETRY_DESC { D3D12_RAYTRACING_GEOMETRY_TYPE Type; D3D12_RAYTRACING_GEOMETRY_FLAGS Flags; union { D3D12_RAYTRACING_GEOMETRY_TRIANGLES_DESC Triangles; D3D12_RAYTRACING_GEOMETRY_AABBS_DESC AABBs; }; } D3D12_RAYTRACING_GEOMETRY_DESC;
	...
	public struct D3D12_RAYTRACING_GEOMETRY_DESC
	{
		...
		/// <summary>A <c>D3D12_RAYTRACING_GEOMETRY_TRIANGLES_DESC</c> describing triangle geometry, if <i>Type</i> is <c>D3D12_RAYTRACING_GEOMETRY_TYPE_TRIANGLES</c>. Otherwise this parameter is unused.</summary>
		[FieldOffset(8)]
		public D3D12_RAYTRACING_GEOMETRY_TRIANGLES_DESC Triangles;
		/// <summary>A <c>D3D12_RAYTRACING_GEOMETRY_AABBS_DESC</c> describing triangle geometry, if <i>Type</i> is <c>D3D12_RAYTRACING_GEOMETRY_TYPE_PROCEDURAL_PRIMITIVE_AABBS</c>. Otherwise this parameter is unused.</summary>
		[FieldOffset(8)]
		public D3D12_RAYTRACING_GEOMETRY_AABBS_DESC AABBs;
	}

	/// <summary>Describes a set of Axis-aligned bounding boxes that are used in the <c>D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS</c> structure to provide input data to a raytracing acceleration structure build operation.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_raytracing_geometry_aabbs_desc
	// typedef struct D3D12_RAYTRACING_GEOMETRY_AABBS_DESC { UINT64 AABBCount; D3D12_GPU_VIRTUAL_ADDRESS_AND_STRIDE AABBs; } D3D12_RAYTRACING_GEOMETRY_AABBS_DESC;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_raytracing_geometry_triangles_desc
	...
	public struct D3D12_RAYTRACING_GEOMETRY_TRIANGLES_DESC
	{
		...
		///   <para>The memory pointed to must be in state <c>D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE</c>. The address must be aligned to 16 bytes, defined as <c>D3D12_RAYTRACING_TRANSFORM3X4_BYTE_ALIGNMENT</c>.</para>
		/// </summary>
		...
		///     <item>
		///       <description>
		///         <b>DXGI_FORMAT_UNKNOWN</b> - when IndexBuffer is NULL</description>
		///     </item>
		///     <item>
		///       <description>
		///         <b>DXGI_FORMAT_R32_UINT</b>
		///       </description>
		///     </item>
		///     <item>
		///       <description>
		///         <b>DXGI_FORMAT_R16_UINT</b>
		///       </description>
		///     </item>
		///   </list>
		/// </summary>
		...
		///     <item>
		///       <description>
		///         <b>DXGI_FORMAT_R32G32_FLOAT</b> - third component is assumed 0</description>
		///     </item>
		///     <item>
		///       <description>
		///         <b>DXGI_FORMAT_R32G32B32_FLOAT</b>
		///       </description>
		///     </item>
		///     <item>
		///       <description>
		///         <b>DXGI_FORMAT_R16G16_FLOAT</b> - third component is assumed 0</description>
		///     </item>
		///     <item>
		///       <description>
		///         <b>DXGI_FORMAT_R16G16B16A16_FLOAT</b> - A16 component is ignored, other data can be packed there, such as setting vertex stride to 6 bytes.</description>
		///     </item>
		///     <item>
		///       <description>
		///         <b>DXGI_FORMAT_R16G16_SNORM</b> - third component is assumed 0</description>
		///     </item>
		///     <item>
		///       <description>
		///         <b>DXGI_FORMAT_R16G16B16A16_SNORM</b> - A16 component is ignored, other data can be packed there, such as setting vertex stride to 6 bytes.</description>
		///     </item>
		///   </list>
		...
		///     <item>
		///       <description>
		///         <b>DXGI_FORMAT_R16G16B16A16_UNORM</b> - A16 component is ignored, other data can be packed there, such as setting vertex stride to 6 bytes</description>
		///     </item>
		///     <item>
		///       <description>
		///         <b>DXGI_FORMAT_R16G16_UNORM</b> - third component assumed 0</description>
		///     </item>
		///     <item>
		///       <description>
		///         <b>DXGI_FORMAT_R10G10B10A2_UNORM</b> - A2 component is ignored, stride must be 4 bytes</description>
		///     </item>
		///     <item>
		///       <description>
		///         <b>DXGI_FORMAT_R8G8B8A8_UNORM</b> - A8 component is ignored, other data can be packed there, such as setting vertex stride to 3 bytes</description>
		///     </item>
		///     <item>
		///       <description>
		///         <b>DXGI_FORMAT_R8G8_UNORM</b> - third component assumed 0</description>
		///     </item>
		///     <item>
		///       <description>
		///         <b>DXGI_FORMAT_R8G8B8A8_SNORM</b> - A8 component is ignored, other data can be packed there, such as setting vertex stride to 3 bytes</description>
		///     </item>
		///     <item>
		///       <description>
		///         <b>DXGI_FORMAT_R8G8_SNORM</b> - third component assumed 0</description>
		///     </item>
		///   </list>
		/// </summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_gpu_virtual_address_and_stride
	// typedef struct D3D12_GPU_VIRTUAL_ADDRESS_AND_STRIDE { D3D12_GPU_VIRTUAL_ADDRESS StartAddress; UINT64 StrideInBytes; } D3D12_GPU_VIRTUAL_ADDRESS_AND_STRIDE;
	...
	/// <remarks>This structure functions like the inverse of the inputs to an acceleration structure build, focused on the instance or geometry details, depending on the acceleration structure type.</remarks>
	...
	// typedef struct D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_TOOLS_VISUALIZATION_HEADER { D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE Type; UINT NumDescs; } D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_TOOLS_VISUALIZATION_HEADER;
	...
	/// <remarks>
	/// <para>This structure is used by the <c>D3D12_GRAPHICS_PIPELINE_STATE_DESC</c> structure, and the <c>D3D12_COMPUTE_PIPELINE_STATE_DESC</c> structure.</para>
	/// <para>This structure is intended to be filled with the data retrieved from <c>ID3D12PipelineState::GetCachedBlob</c>. This cached PSO contains data specific to the hardware, driver, and machine that it was retrieved from. Compilation using this data should be faster than compilation without. The rest of the data in the PSO needs to still be valid, and needs to match the cached PSO, otherwise E_INVALIDARG might be returned.</para>
	...
	// typedef struct D3D12_CACHED_PIPELINE_STATE { const void *pCachedBlob; SIZE_T CachedBlobSizeInBytes; } D3D12_CACHED_PIPELINE_STATE;
	...
	// typedef struct D3D12_CLEAR_VALUE { DXGI_FORMAT Format; union { FLOAT Color[4]; D3D12_DEPTH_STENCIL_VALUE DepthStencil; }; } D3D12_CLEAR_VALUE;
	...
	public struct D3D12_CLEAR_VALUE
	{
		...
		/// <summary>Specifies a 4-entry array of float values, determining the RGBA value. The order of RGBA matches the order used with <c>ClearRenderTargetView</c>.</summary>
		public float[] Color { get => [u.Color0, u.Color1, u.Color2, u.Color3]; set { u.Color0 = value[0]; u.Color1 = value[1]; u.Color2 = value[2]; u.Color3 = value[3]; } }

		/// <summary>Specifies one member of <c>D3D12_DEPTH_STENCIL_VALUE</c>. These values match the semantics of <i>Depth</i> and <i>Stencil</i> in <c>ClearDepthStencilView</c>.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_depth_stencil_value
	// typedef struct D3D12_DEPTH_STENCIL_VALUE { FLOAT Depth; UINT8 Stencil; } D3D12_DEPTH_STENCIL_VALUE;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_command_signature_desc
	// typedef struct D3D12_COMMAND_SIGNATURE_DESC { UINT ByteStride; UINT NumArgumentDescs; const D3D12_INDIRECT_ARGUMENT_DESC *pArgumentDescs; UINT NodeMask; } D3D12_COMMAND_SIGNATURE_DESC;
	...
	public struct D3D12_COMMAND_SIGNATURE_DESC
	{
		...
		/// <summary>An array of <c>D3D12_INDIRECT_ARGUMENT_DESC</c> structures, containing details of the arguments, including whether the argument is a vertex buffer, constant, constant buffer view, shader resource view, or unordered access view.</summary>
		public ArrayPointer<D3D12_INDIRECT_ARGUMENT_DESC> pArgumentDescs;
		/// <summary>For single GPU operation, set this to zero. If there are multiple GPU nodes, set bits to identify the nodes (the device's physical adapters) for which the command signature is to apply. Each bit in the mask corresponds to a single node. Refer to <c>Multi-adapter systems</c>.</summary>
		public uint NodeMask;
	}
	...
	// typedef struct D3D12_COMPUTE_PIPELINE_STATE_DESC { ID3D12RootSignature *pRootSignature; D3D12_SHADER_BYTECODE CS; UINT NodeMask; D3D12_CACHED_PIPELINE_STATE CachedPSO; D3D12_PIPELINE_STATE_FLAGS Flags; } D3D12_COMPUTE_PIPELINE_STATE_DESC;
	...
	public struct D3D12_COMPUTE_PIPELINE_STATE_DESC
	{
		...
		/// <summary>For single GPU operation, set this to zero. If there are multiple GPU nodes, set bits to identify the nodes (the device's physical adapters) for which the compute pipeline state is to apply. Each bit in the mask corresponds to a single node. Refer to <c>Multi-adapter systems</c>.</summary>
		public uint NodeMask;
		/// <summary>A cached pipeline state object, as a <c>D3D12_CACHED_PIPELINE_STATE</c> structure. pCachedBlob and CachedBlobSizeInBytes may be set to NULL and 0 respectively.</summary>
	...
	// typedef struct D3D12_CONSTANT_BUFFER_VIEW_DESC { D3D12_GPU_VIRTUAL_ADDRESS BufferLocation; UINT SizeInBytes; } D3D12_CONSTANT_BUFFER_VIEW_DESC;
	...
	/// <remarks>
	/// <para>A <c>D3D12_GRAPHICS_PIPELINE_STATE_DESC</c> object contains a depth-stencil-state structure that controls how depth-stencil testing is performed by the output-merger stage.</para>
	...
	// typedef struct D3D12_DEPTH_STENCIL_DESC { BOOL DepthEnable; D3D12_DEPTH_WRITE_MASK DepthWriteMask; D3D12_COMPARISON_FUNC DepthFunc; BOOL StencilEnable; UINT8 StencilReadMask; UINT8 StencilWriteMask; D3D12_DEPTH_STENCILOP_DESC FrontFace; D3D12_DEPTH_STENCILOP_DESC BackFace; } D3D12_DEPTH_STENCIL_DESC;
	...
	public struct D3D12_DEPTH_STENCIL_DESC
	{
		...
		/// <summary>A <c>D3D12_DEPTH_WRITE_MASK</c>-typed value that identifies a portion of the depth-stencil buffer that can be modified by depth data.</summary>
		public D3D12_DEPTH_WRITE_MASK DepthWriteMask;
		/// <summary>A <c>D3D12_COMPARISON_FUNC</c>-typed value that identifies a function that compares depth data against existing depth data.</summary>
		...
		/// <summary>A <c>D3D12_DEPTH_STENCILOP_DESC</c> structure that describes how to use the results of the depth test and the stencil test for pixels whose surface normal is facing towards the camera.</summary>
		public D3D12_DEPTH_STENCILOP_DESC FrontFace;
		/// <summary>A <c>D3D12_DEPTH_STENCILOP_DESC</c> structure that describes how to use the results of the depth test and the stencil test for pixels whose surface normal is facing away from the camera.</summary>
	...
	/// <para>The formats that support stenciling are DXGI_FORMAT_D24_UNORM_S8_UINT and DXGI_FORMAT_D32_FLOAT_S8X24_UINT.</para>
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_depth_stencil_desc1
	...
	public struct D3D12_DEPTH_STENCIL_VIEW_DESC
	{
		...
		[FieldOffset(0)]
		public DXGI_FORMAT Format;
		/// <summary>A <c>D3D12_DSV_DIMENSION</c>-typed value that specifies how the depth-stencil resource will be accessed. This member also determines which _DSV to use in the following union.</summary>
		[FieldOffset(4)]
		public D3D12_DSV_DIMENSION ViewDimension;
		/// <summary>A combination of <c>D3D12_DSV_FLAGS</c> enumeration constants that are combined by using a bitwise OR operation. The resulting value specifies whether the texture is read only. Pass 0 to specify that it isn't read only; otherwise, pass one or more of the members of the <b>D3D12_DSV_FLAGS</b> enumerated type.</summary>
		[FieldOffset(8)]
	...
	/// <remarks>
	/// <para>All stencil operations are specified as a <c>D3D12_STENCIL_OP</c>-typed value. Each stencil operation can be set differently based on the outcome of the stencil test, which is referred to as <b>StencilFunc</b>, in the stencil test portion of depth-stencil testing.</para>
	/// <para>Members of <c>D3D12_DEPTH_STENCIL_DESC</c> have this structure for their data type.</para>
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_depth_stencilop_desc
	...
	public struct D3D12_DEPTH_STENCILOP_DESC
	{
		...
		/// <summary>A <c>D3D12_STENCIL_OP</c>-typed value that identifies the stencil operation to perform when stencil testing passes and depth testing fails.</summary>
		public D3D12_STENCIL_OP StencilDepthFailOp;
		/// <summary>A <c>D3D12_STENCIL_OP</c>-typed value that identifies the stencil operation to perform when stencil testing and depth testing both pass.</summary>
		public D3D12_STENCIL_OP StencilPassOp;
		/// <summary>A <c>D3D12_COMPARISON_FUNC</c>-typed value that identifies the function that compares stencil data against existing stencil data.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_descriptor_range
	...
	// typedef struct D3D12_DESCRIPTOR_RANGE1 { D3D12_DESCRIPTOR_RANGE_TYPE RangeType; UINT NumDescriptors; UINT BaseShaderRegister; UINT RegisterSpace; D3D12_DESCRIPTOR_RANGE_FLAGS Flags; UINT OffsetInDescriptorsFromTableStart; } D3D12_DESCRIPTOR_RANGE1;
	...
	/// <summary>
	/// <para>Note</para>
	/// <para>As of Windows 10, version 1903, <b>D3D12_DEVICE_REMOVED_EXTENDED_DATA</b> is deprecated, and it may not be available in future versions of Windows. Use <c><b>D3D12_DEVICE_REMOVED_EXTENDED_DATA1</b></c>, instead.</para>
	/// </summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_device_removed_extended_data
	// typedef struct D3D12_DEVICE_REMOVED_EXTENDED_DATA { D3D12_DRED_FLAGS Flags; D3D12_AUTO_BREADCRUMB_NODE *pHeadAutoBreadcrumbNode; } D3D12_DEVICE_REMOVED_EXTENDED_DATA;
	...
	public struct D3D12_DEVICE_REMOVED_EXTENDED_DATA
	{
		...
		/// <summary>An output parameter of type pointer to <c>D3D12_AUTO_BREADCRUMB_NODE</c> representing the returned auto-breadcrumb object(s). This is a pointer to the head of a linked list of auto-breadcrumb objects. All of the nodes in the linked list represent potentially incomplete command list execution on the GPU at the time of the device-removal event.</summary>
		public ManagedStructPointer<D3D12_AUTO_BREADCRUMB_NODE> pHeadAutoBreadcrumbNode;
	}

	/// <summary>
	/// <para>Represents Device Removed Extended Data (DRED) version 1.1 device removal data, so that debuggers and debugger extensions can access DRED data. Also see <c>D3D12_VERSIONED_DEVICE_REMOVED_EXTENDED_DATA</c>.</para>
	/// <para>This structure is not used by any interface methods, and it provides no runtime API access.</para>
	/// </summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_device_removed_extended_data1
	// typedef struct D3D12_DEVICE_REMOVED_EXTENDED_DATA1 { HRESULT DeviceRemovedReason; D3D12_DRED_AUTO_BREADCRUMBS_OUTPUT AutoBreadcrumbsOutput; D3D12_DRED_PAGE_FAULT_OUTPUT PageFaultOutput; } D3D12_DEVICE_REMOVED_EXTENDED_DATA1;
	...
	{
		/// <summary>An <c>HRESULT</c> containing the reason the device was removed (matches the return value of <c>GetDeviceRemovedReason</c>). Also see <c>COM Error Codes (UI, Audio, DirectX, Codec)</c>.</summary>
		...
		/// <summary>A <c>D3D12_DRED_PAGE_FAULT_OUTPUT</c> value that contains page fault data if device removal was the result of a GPU page fault.</summary>
	...
	/// </summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_device_removed_extended_data2
	// typedef struct D3D12_DEVICE_REMOVED_EXTENDED_DATA2 { HRESULT DeviceRemovedReason; D3D12_DRED_AUTO_BREADCRUMBS_OUTPUT1 AutoBreadcrumbsOutput; D3D12_DRED_PAGE_FAULT_OUTPUT1 PageFaultOutput; } D3D12_DEVICE_REMOVED_EXTENDED_DATA2;
	...
	/// </summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_device_removed_extended_data3
	// typedef struct D3D12_DEVICE_REMOVED_EXTENDED_DATA3 { HRESULT DeviceRemovedReason; D3D12_DRED_AUTO_BREADCRUMBS_OUTPUT1 AutoBreadcrumbsOutput; D3D12_DRED_PAGE_FAULT_OUTPUT2 PageFaultOutput; D3D12_DRED_DEVICE_STATE DeviceState; } D3D12_DEVICE_REMOVED_EXTENDED_DATA3;
	...
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_discard_region
	// typedef struct D3D12_DISCARD_REGION { UINT NumRects; const D3D12_RECT *pRects; UINT FirstSubresource; UINT NumSubresources; } D3D12_DISCARD_REGION;
	...
	public struct D3D12_DISCARD_REGION
	{
		...
		/// <summary>An array of <b>D3D12_RECT</b> structures for the rectangles in the resource to discard. If <b>NULL</b>, <c>DiscardResource</c> discards the entire resource.</summary>
	...
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_dispatch_arguments
	// typedef struct D3D12_DISPATCH_ARGUMENTS { UINT ThreadGroupCountX; UINT ThreadGroupCountY; UINT ThreadGroupCountZ; } D3D12_DISPATCH_ARGUMENTS;
	...
	/// </summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_dispatch_mesh_arguments
	// typedef struct D3D12_DISPATCH_MESH_ARGUMENTS { UINT ThreadGroupCountX; UINT ThreadGroupCountY; UINT ThreadGroupCountZ; } D3D12_DISPATCH_MESH_ARGUMENTS;
	...
	public struct D3D12_DISPATCH_RAYS_DESC
	{
		...
		///   <para>The memory pointed to must be in state <c>D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE</c>.</para>
		///   <para>The address must be aligned to 64 bytes, defined as <c>D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT</c>, and in the range [0...4096] bytes.</para>
		/// </summary>
		public D3D12_GPU_VIRTUAL_ADDRESS_RANGE RayGenerationShaderRecord;
		/// <summary>
		///   <para>The shader table for miss shaders.</para>
		///   <para>The stride is record stride, and must be aligned to 32 bytes, defined as <c>D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT</c>, and in the range [0...4096] bytes. 0 is allowed.</para>
		///   <para>The memory pointed to must be in state <c>D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE</c>.</para>
		...
		///   <para>The stride is record stride, and must be aligned to 32 bytes, defined as <c>D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT</c>. 0 is allowed.</para>
		///   <para>The memory pointed to must be in state <c>D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE</c>.</para>
		///   <para>The address must be aligned to 64 bytes, defined as <c>D3D12_RAYTRACING_SHADER_TABLE_BYTE_ALIGNMENT</c>.</para>
		/// </summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_draw_arguments
	// typedef struct D3D12_DRAW_ARGUMENTS { UINT VertexCountPerInstance; UINT InstanceCount; UINT StartVertexLocation; UINT StartInstanceLocation; } D3D12_DRAW_ARGUMENTS;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_draw_indexed_arguments
	// typedef struct D3D12_DRAW_INDEXED_ARGUMENTS { UINT IndexCountPerInstance; UINT InstanceCount; UINT StartIndexLocation; INT BaseVertexLocation; UINT StartInstanceLocation; } D3D12_DRAW_INDEXED_ARGUMENTS;
	...
	/// <summary>
	/// <para>Describes, as a node in a linked list, data about an allocation tracked by Device Removed Extended Data (DRED). This data includes the GPU VA allocation ranges, and an associated runtime object debug name and type. Each <b>D3D12_DRED_ALLOCATION_NODE</b> object is singly linked to the next via its <c>pNext</c> member; except for the last node in the list, which has its <c>pNext</c> set to <c>nullptr</c>. A linked list structure is necessary because a runtime object can share allocation ranges with other objects.</para>
	...
	/// </summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_dred_allocation_node1
	// typedef struct D3D12_DRED_ALLOCATION_NODE1 { const char *ObjectNameA; const wchar_t *ObjectNameW; D3D12_DRED_ALLOCATION_TYPE AllocationType; const D3D12_DRED_ALLOCATION_NODE1 *pNext; struct D3D12_DRED_ALLOCATION_NODE1; const IUnknown *pObject; } D3D12_DRED_ALLOCATION_NODE1;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_dred_page_fault_output
	// typedef struct D3D12_DRED_PAGE_FAULT_OUTPUT { D3D12_GPU_VIRTUAL_ADDRESS PageFaultVA; const D3D12_DRED_ALLOCATION_NODE *pHeadExistingAllocationNode; const D3D12_DRED_ALLOCATION_NODE *pHeadRecentFreedAllocationNode; } D3D12_DRED_PAGE_FAULT_OUTPUT;
	[PInvokeData("d3d12.h", MSDNShortId = "NS:d3d12.D3D12_DRED_PAGE_FAULT_OUTPUT")]
	[StructLayout(LayoutKind.Sequential)]
	public struct D3D12_DRED_PAGE_FAULT_OUTPUT
	{
		/// <summary>A <c>D3D12_GPU_VIRTUAL_ADDRESS</c> containing the GPU virtual address (VA) of the faulting operation if device removal was due to a GPU page fault.</summary>
		public D3D12_GPU_VIRTUAL_ADDRESS PageFaultVA;
		/// <summary>A pointer to a constant <c>D3D12_DRED_ALLOCATION_NODE</c> object representing the head of a linked list of allocation nodes for active allocated runtime objects with virtual address (VA) ranges that match the faulting VA (<c>PageFaultVA</c>). Has a value of <c>nullptr</c> if the list is empty.</summary>
		public ManagedStructPointer<D3D12_DRED_ALLOCATION_NODE> pHeadExistingAllocationNode;
		/// <summary>A pointer to a constant <c>D3D12_DRED_ALLOCATION_NODE</c> object representing the head of a linked list of allocation nodes for recently freed runtime objects with virtual address (VA) ranges that match the faulting VA (<c>PageFaultVA</c>). Has a value of <c>nullptr</c> if the list is empty.</summary>
		public ManagedStructPointer<D3D12_DRED_ALLOCATION_NODE> pHeadRecentFreedAllocationNode;
	}

	/// <summary>Describes allocation data related to a GPU page fault on a given virtual address (VA). Contains the VA of a GPU page fault, together with a list of matching allocation nodes for active objects, and a list of allocation nodes for recently deleted objects.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_dred_page_fault_output1
	// typedef struct D3D12_DRED_PAGE_FAULT_OUTPUT1 { D3D12_GPU_VIRTUAL_ADDRESS PageFaultVA; const D3D12_DRED_ALLOCATION_NODE1 *pHeadExistingAllocationNode; const D3D12_DRED_ALLOCATION_NODE1 *pHeadRecentFreedAllocationNode; } D3D12_DRED_PAGE_FAULT_OUTPUT1;
	...
	public struct D3D12_DRED_PAGE_FAULT_OUTPUT1
	{
		/// <summary>A <c>D3D12_GPU_VIRTUAL_ADDRESS</c> containing the GPU virtual address (VA) of the faulting operation if device removal was due to a GPU page fault.</summary>
		public D3D12_GPU_VIRTUAL_ADDRESS PageFaultVA;
		/// <summary>A pointer to a constant <c>D3D12_DRED_ALLOCATION_NODE1</c> object representing the head of a linked list of allocation nodes for active allocated runtime objects with virtual address (VA) ranges that match the faulting VA (<c>PageFaultVA</c>). Has a value of <c>nullptr</c> if the list is empty.</summary>
		public ManagedStructPointer<D3D12_DRED_ALLOCATION_NODE1> pHeadExistingAllocationNode;
		/// <summary>A pointer to a constant <c>D3D12_DRED_ALLOCATION_NODE1</c> object representing the head of a linked list of allocation nodes for recently freed runtime objects with virtual address (VA) ranges that match the faulting VA (<c>PageFaultVA</c>). Has a value of <c>nullptr</c> if the list is empty.</summary>
	...
	/// </summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_dred_page_fault_output2
	// typedef struct D3D12_DRED_PAGE_FAULT_OUTPUT2 { D3D12_GPU_VIRTUAL_ADDRESS PageFaultVA; const D3D12_DRED_ALLOCATION_NODE1 *pHeadExistingAllocationNode; const D3D12_DRED_ALLOCATION_NODE1 *pHeadRecentFreedAllocationNode; D3D12_DRED_PAGE_FAULT_FLAGS PageFaultFlags; } D3D12_DRED_PAGE_FAULT_OUTPUT2;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_dxil_library_desc
	// typedef struct D3D12_DXIL_LIBRARY_DESC { D3D12_SHADER_BYTECODE DXILLibrary; UINT NumExports; const D3D12_EXPORT_DESC *pExports; } D3D12_DXIL_LIBRARY_DESC;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_dxil_subobject_to_exports_association
	// typedef struct D3D12_DXIL_SUBOBJECT_TO_EXPORTS_ASSOCIATION { LPCWSTR SubobjectToAssociate; UINT NumExports; LPCWSTR *pExports; } D3D12_DXIL_SUBOBJECT_TO_EXPORTS_ASSOCIATION;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_existing_collection_desc
	// typedef struct D3D12_EXISTING_COLLECTION_DESC { ID3D12StateObject *pExistingCollection; UINT NumExports; const D3D12_EXPORT_DESC *pExports; } D3D12_EXISTING_COLLECTION_DESC;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_export_desc
	// typedef struct D3D12_EXPORT_DESC { LPCWSTR Name; LPCWSTR ExportToRename; D3D12_EXPORT_FLAGS Flags; } D3D12_EXPORT_DESC;
	...
	/// <para><b>Note</b>  This structure has been superseded by the <c>D3D12_FEATURE_DATA_ARCHITECTURE1</c> structure. If your application targets Windows 10, version 1703 (Creators' Update) or higher, then use <b>D3D12_FEATURE_DATA_ARCHITECTURE1</b> (and <c>D3D12_FEATURE_ARCHITECTURE1</c>) instead.</para>
	/// <para> </para>
	/// </summary>
	/// <remarks>
	/// <para>How to use UMA and CacheCoherentUMA</para>
	/// <para>D3D12 apps should be concerned about managing memory residency and providing the optimal heap properties. D3D12 apps can stay simplified and run reasonably well across many GPU architectures by only managing the residency for resources in <c>D3D12_HEAP_TYPE</c>_DEFAULT heaps. Those apps only need to call <c>IDXGIAdapter3::QueryVideoMemoryInfo</c> for DXGI_MEMORY_SEGMENT_GROUP_LOCAL, and they must be tolerant that D3D12_HEAP_TYPE_UPLOAD and D3D12_HEAP_TYPE_READBACK come from that same memory segment group.</para>
	...
	// typedef struct D3D12_FEATURE_DATA_ARCHITECTURE { UINT NodeIndex; BOOL TileBasedRenderer; BOOL UMA; BOOL CacheCoherentUMA; } D3D12_FEATURE_DATA_ARCHITECTURE;
	[PInvokeData("d3d12.h", MSDNShortId = "NS:d3d12.D3D12_FEATURE_DATA_ARCHITECTURE")]
	[StructLayout(LayoutKind.Sequential)]
	public struct D3D12_FEATURE_DATA_ARCHITECTURE
	{
		/// <summary>In multi-adapter operation, this indicates which physical adapter of the device is relevant. See <c>Multi-adapter systems</c>. <b>NodeIndex</b> is filled out by the application before calling <c>CheckFeatureSupport</c>, as the application can retrieve details about the architecture of each adapter.</summary>
	...
	/// <para><b>Note</b>  This structure, introduced in Windows 10, version 1703 (Creators' Update), supersedes the <c>D3D12_FEATURE_DATA_ARCHITECTURE</c> structure. If your application targets Windows 10, version 1703 (Creators' Update) or higher, then use <b>D3D12_FEATURE_DATA_ARCHITECTURE1</b> (and <c>D3D12_FEATURE_ARCHITECTURE1</c>).</para>
	/// <para> </para>
	/// </summary>
	/// <remarks>
	/// <para>How to use UMA and CacheCoherentUMA</para>
	...
	/// <para>Some applications may want to better optimize for integrated/ UMA adapters, especially those that are interested in extending battery life on mobile device. Simple D3D12 applications are forced into copying data between heaps with different attributions, when it isn't always necessary on UMA. However, the UMA property, by itself, encompasses a reasonably vague grey area of GPU designs. Do not assume UMA means all GPU-accessible memory can be freely made CPU-accessible, because it doesn't. There's a property that more closely aligns to that type of thinking: <b>CacheCoherentUMA</b>.</para>
	/// <para>When <b>CacheCoherentUMA</b> is <b>false</b>, a single residency budget is available but the UMA design commonly benefits from the three heap attributions. Opportunities do exist to remove resource copying through wise usage of upload and readback resources and heaps, that provide CPU-access to the memory. Such opportunities are not clear-cut, though. So, applications should be cautious; and experimentation across a variety of "UMA" systems is advisable, as resorting to enabling or precluding certain device IDs may be warranted. An understanding of the GPU memory architecture and how heap types translate to cache properties is recommended. The feasibility of success is likely dependent on how often each processor either reads or writes the data, the size and locality of data accesses, etc. For advanced developers: when <b>UMA</b> is true and <b>CacheCoherentUMA</b> is <b>false</b>, the most unique characteristic for these adapters is that upload heaps are still write-combined. However, some UMA adapters benefit from both the no-CPU-access and write-combine properties of default and upload heaps. See <c>GetCustomHeapProperties</c> for more details.</para>
	/// <para>When <b>CacheCoherentUMA</b> is true, applications can more strongly entertain abandoning the attribution of heaps and using the custom heap equivalent of upload heaps everywhere. Zero-copy UMA optimizations such those offered by <c>WriteToSubresource</c> are more generally encouraged as more scenarios will just benefit from shared usage. The memory model is very conducive to more scenarios and wider adoption. Some corner cases may still exist where benefits are not easily obtained, but they should be much rarer and less detrimental than other options. For advanced developers: <b>CacheCoherentUMA</b> means that a significant amount of the caches in the memory hierarchy are also unified or integrated between the CPU and GPU. The most unique observable characteristic is that upload heaps are actually write-back on <b>CacheCoherentUMA</b>. For these architecture, the usage of write-combine on upload heaps is commonly a detriment.</para>
	/// <para>The low-level details should be ignored by a vast majority of single-adapter applications. As usual, single-adapter applications can simplify the landscape and ensure that the CPU writes to upload heaps use patterns that are write-combine-friendly. The lower-level details help reinforce the concepts for multi-adapter applications. Multi-adapter applications likely need to understand adapter architecture properties well enough to choose the optimal custom heap properties to efficiently move data between adapters.</para>
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_architecture1
	// typedef struct D3D12_FEATURE_DATA_ARCHITECTURE1 { UINT NodeIndex; BOOL TileBasedRenderer; BOOL UMA; BOOL CacheCoherentUMA; BOOL IsolatedMMU; } D3D12_FEATURE_DATA_ARCHITECTURE1;
	...
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_command_queue_priority
	// typedef struct D3D12_FEATURE_DATA_COMMAND_QUEUE_PRIORITY { D3D12_COMMAND_LIST_TYPE CommandListType; UINT Priority; BOOL PriorityForTypeIsSupported; } D3D12_FEATURE_DATA_COMMAND_QUEUE_PRIORITY;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_cross_node
	// typedef struct D3D12_FEATURE_DATA_CROSS_NODE { D3D12_CROSS_NODE_SHARING_TIER SharingTier; BOOL AtomicShaderInstructions; } D3D12_FEATURE_DATA_CROSS_NODE;
	...
	public struct D3D12_FEATURE_DATA_D3D12_OPTIONS
	{
		...
		/// <summary>A combination of <c>D3D12_SHADER_MIN_PRECISION_SUPPORT</c>-typed values that are combined by using a bitwise OR operation. The resulting value specifies minimum precision levels that the driver supports for shader stages. A value of zero indicates that the driver supports only full 32-bit precision for all shader stages.</summary>
		public D3D12_SHADER_MIN_PRECISION_SUPPORT MinPrecisionSupport;
		/// <summary>Specifies whether the hardware and driver support tiled resources. The runtime sets this member to a <c>D3D12_TILED_RESOURCES_TIER</c>-typed value that indicates if the hardware and driver support tiled resources and at what tier level.</summary>
		public D3D12_TILED_RESOURCES_TIER TiledResourcesTier;
		/// <summary>Specifies the level at which the hardware and driver support resource binding. The runtime sets this member to a <c>D3D12_RESOURCE_BINDING_TIER</c>-typed value that indicates the tier level.</summary>
		...
		/// <summary>Specifies whether <c>Rasterizer Order Views</c> (ROVs) are supported. If <b>TRUE</b>, they're supported; otherwise <b>FALSE</b>.</summary>
		[MarshalAs(UnmanagedType.Bool)]
		public bool ROVsSupported;
		/// <summary>Specifies the level at which the hardware and driver support conservative rasterization. The runtime sets this member to a <c>D3D12_CONSERVATIVE_RASTERIZATION_TIER</c>-typed value that indicates the tier level.</summary>
		public D3D12_CONSERVATIVE_RASTERIZATION_TIER ConservativeRasterizationTier;
		/// <summary>Don't use this field; instead, use the <c>D3D12_FEATURE_DATA_GPU_VIRTUAL_ADDRESS_SUPPORT</c> query (a structure with a <b>MaxGPUVirtualAddressBitsPerResource</b> member), which is more accurate.</summary>
		public uint MaxGPUVirtualAddressBitsPerResource;
		/// <summary>TRUE if the hardware supports textures with the 64KB standard swizzle pattern. Support for this pattern enables zero-copy texture optimizations while providing near-equilateral locality for each dimension within the texture. For texture swizzle options and restrictions, see <c>D3D12_TEXTURE_LAYOUT</c>.</summary>
		[MarshalAs(UnmanagedType.Bool)]
		public bool StandardSwizzle64KBSupported;
		/// <summary>A <c>D3D12_CROSS_NODE_SHARING_TIER</c> enumeration constant that specifies the level of sharing across nodes of an adapter that has multiple nodes, such as Tier 1 Emulated, Tier 1, or Tier 2.</summary>
		...
		/// <summary>Whether the viewport (VP) and Render Target (RT) array index from any shader feeding the rasterizer are supported without geometry shader emulation. Compare the <b>VPAndRTArrayIndexFromAnyShaderFeedingRasterizer</b> member of the <c>D3D11_FEATURE_DATA_D3D11_OPTIONS3</c> structure. In <c>ID3D12ShaderReflection::GetRequiresFlags</c>, see the #define D3D_SHADER_REQUIRES_VIEWPORT_AND_RT_ARRAY_INDEX_FROM_ANY_SHADER_FEEDING_RASTERIZER.</summary>
		[MarshalAs(UnmanagedType.Bool)]
		public bool VPAndRTArrayIndexFromAnyShaderFeedingRasterizerSupportedWithoutGSEmulation;
		/// <summary>Specifies the level at which the hardware and driver require heap attribution related to resource type. The runtime sets this member to a <c>D3D12_RESOURCE_HEAP_TIER</c> enumeration constant.</summary>
	...
	public struct D3D12_FEATURE_DATA_D3D12_OPTIONS1
	{
		...
		/// <summary>Indicates transitions are possible in and out of the CBV, and indirect argument states, on compute command lists. If <c>CheckFeatureSupport</c> succeeds this value will always be true.</summary>
		[MarshalAs(UnmanagedType.Bool)]
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_d3d12_options10
	// typedef struct D3D12_FEATURE_DATA_D3D12_OPTIONS10 { BOOL VariableRateShadingSumCombinerSupported; BOOL MeshShaderPerPrimitiveShadingRateSupported; } D3D12_FEATURE_DATA_D3D12_OPTIONS10;
	...
	public struct D3D12_FEATURE_DATA_D3D12_OPTIONS10
	{
		...
		///   <para>Indicates whether or not the SUM combiner can be used (this relates to <c>variable-rate shading</c> Tier 2). <c>true</c> if it can, otherwise <c>false</c>.</para>
		/// </summary>
		[MarshalAs(UnmanagedType.Bool)]
		public bool VariableRateShadingSumCombinerSupported;
		/// <summary>
		///   <para>Type: _Out_ <b><c>BOOL</c></b></para>
		///   <para>Indicates whether or not SV_ShadingRate can be set from a mesh shader (this relates to <c>variable-rate shading</c> Tier 2). <c>true</c> if it can, otherwise <c>false</c>.</para>
		/// </summary>
		[MarshalAs(UnmanagedType.Bool)]
	...
	// typedef struct D3D12_FEATURE_DATA_D3D12_OPTIONS11 { BOOL AtomicInt64OnDescriptorHeapResourceSupported; } D3D12_FEATURE_DATA_D3D12_OPTIONS11;
	...
	public struct D3D12_FEATURE_DATA_D3D12_OPTIONS11
	{
		/// <summary>
		///   <para>Type: _Out_ <b><c>BOOL</c></b></para>
		///   <para>Indicates whether or not 64-bit integer atomics on resources in descriptor heaps are supported. <c>true</c> if supported, otherwise <c>false</c>.</para>
		/// </summary>
		[MarshalAs(UnmanagedType.Bool)]
	...
	// typedef struct D3D12_FEATURE_DATA_D3D12_OPTIONS12 { D3D12_TRI_STATE MSPrimitivesPipelineStatisticIncludesCulledPrimitives; BOOL EnhancedBarriersSupported; BOOL RelaxedFormatCastingSupported; } D3D12_FEATURE_DATA_D3D12_OPTIONS12;
	...
	public struct D3D12_FEATURE_DATA_D3D12_OPTIONS12
	{
		...
		///   <para>Enhanced Barriers is not currently a hardware or driver requirement. So before using command list Barrier APIs, or resource creation APIs using the InitialLayout parameter, you must check for optional driver support via EnhancedBarriersSupported.</para>
	...
	/// <remarks>
	/// <para>Use this structure with <c>CheckFeatureSupport</c> to determine the level of support offered for the optional Depth-bounds test and programmable sample positions features.</para>
	/// <para>See the enumeration constant D3D12_FEATURE_D3D12_OPTIONS2 in the <c>D3D12_FEATURE</c> enumeration.</para>
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_d3d12_options2
	// typedef struct D3D12_FEATURE_DATA_D3D12_OPTIONS2 { BOOL DepthBoundsTestSupported; D3D12_PROGRAMMABLE_SAMPLE_POSITIONS_TIER ProgrammableSamplePositionsTier; } D3D12_FEATURE_DATA_D3D12_OPTIONS2;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_d3d12_options3
	// typedef struct D3D12_FEATURE_DATA_D3D12_OPTIONS3 { BOOL CopyQueueTimestampQueriesSupported; BOOL CastingFullyTypedFormatSupported; D3D12_COMMAND_LIST_SUPPORT_FLAGS WriteBufferImmediateSupportFlags; D3D12_VIEW_INSTANCING_TIER ViewInstancingTier; BOOL BarycentricsSupported; } D3D12_FEATURE_DATA_D3D12_OPTIONS3;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_d3d12_options4
	...
	public struct D3D12_FEATURE_DATA_D3D12_OPTIONS4
	{
		...
		/// <summary>
		///   <para>Type: <b><c>BOOL</c></b></para>
		///   <para>Indicates native 16-bit shader operations are supported. These operations require shader model 6_2. For more information, see the <c>16-Bit Scalar Types</c> HLSL reference.</para>
		/// </summary>
		[MarshalAs(UnmanagedType.Bool)]
	...
	/// <remarks>Pass <c>D3D12_FEATURE_D3D12_OPTIONS5</c> to <c>ID3D12Device::CheckFeatureSupport</c> to retrieve a <b>D3D12_FEATURE_DATA_D3D12_OPTIONS5</b> structure.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_d3d12_options5
	// typedef struct D3D12_FEATURE_DATA_D3D12_OPTIONS5 { BOOL SRVOnlyTiledResourceTier3; D3D12_RENDER_PASS_TIER RenderPassesTier; D3D12_RAYTRACING_TIER RaytracingTier; } D3D12_FEATURE_DATA_D3D12_OPTIONS5;
	...
	{
		/// <summary>A boolean value indicating whether the options require shader-resource view tier 3 tiled resource support. For more information, see <c>D3D12_TILED_RESOURCES_TIER</c>.</summary>
		[MarshalAs(UnmanagedType.Bool)]
	...
	/// <summary>Indicates the level of support that the adapter provides for variable-rate shading (VRS), and indicates whether or not background processing is supported. For more info, see <c>Variable-rate shading (VRS)</c>, and the <c>Direct3D 12 background processing spec</c>.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_d3d12_options6
	// typedef struct D3D12_FEATURE_DATA_D3D12_OPTIONS6 { BOOL AdditionalShadingRatesSupported; BOOL PerPrimitiveShadingRateSupportedWithViewportIndexing; D3D12_VARIABLE_SHADING_RATE_TIER VariableShadingRateTier; UINT ShadingRateImageTileSize; BOOL BackgroundProcessingSupported; } D3D12_FEATURE_DATA_D3D12_OPTIONS6;
	...
	public struct D3D12_FEATURE_DATA_D3D12_OPTIONS6
	{
		/// <summary>
		///   <para>Type: <b><c>BOOL</c></b></para>
		///   <para>Indicates whether 2x4, 4x2, and 4x4 coarse pixel sizes are supported for single-sampled rendering; and whether coarse pixel size 2x4 is supported for 2x MSAA. <c>true</c> if those sizes are supported, otherwise <c>false</c>.</para>
		/// </summary>
		[MarshalAs(UnmanagedType.Bool)]
		public bool AdditionalShadingRatesSupported;
		/// <summary>
		///   <para>Type: <b><c>BOOL</c></b></para>
		///   <para>Indicates whether the per-provoking-vertex (also known as per-primitive) rate can be used with more than one viewport. If so, then, in that case, that rate can be used when <c>SV_ViewportIndex</c> is written to. <c>true</c> if that rate can be used with more than one viewport, otherwise <c>false</c>.</para>
		/// </summary>
		[MarshalAs(UnmanagedType.Bool)]
		...
		/// <summary>
		///   <para>Type: <b><c>BOOL</c></b></para>
		///   <para>Indicates whether or not background processing is supported. <c>true</c> if background processing is supported, otherwise <c>false</c>. For more info, see the <c>Direct3D 12 background processing spec</c>.</para>
		/// </summary>
		[MarshalAs(UnmanagedType.Bool)]
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_d3d12_options7
	// typedef struct D3D12_FEATURE_DATA_D3D12_OPTIONS7 { D3D12_MESH_SHADER_TIER MeshShaderTier; D3D12_SAMPLER_FEEDBACK_TIER SamplerFeedbackTier; } D3D12_FEATURE_DATA_D3D12_OPTIONS7;
	...
	// typedef struct D3D12_FEATURE_DATA_D3D12_OPTIONS8 { BOOL UnalignedBlockTexturesSupported; } D3D12_FEATURE_DATA_D3D12_OPTIONS8;
	...
	public struct D3D12_FEATURE_DATA_D3D12_OPTIONS8
	{
		...
		///   <para>If <c>false</c>, then Direct3D 12 requires that the dimensions of the top-level mip of a block-compressed texture are aligned to multiples of 4 (such alignment requirements do not apply to less-detailed mips). If <c>true</c>, then no such alignment requirement applies to any mip level of a block-compressed texture.</para>
		/// </summary>
		[MarshalAs(UnmanagedType.Bool)]
		public bool UnalignedBlockTexturesSupported;
	}

	/// <summary>Indicates whether or not support exists for mesh shaders, values of SV_RenderTargetArrayIndex that are 8 or greater, typed resource 64-bit integer atomics, derivative and derivative-dependent texture sample operations, and the level of support for WaveMMA (wave_matrix) operations.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_d3d12_options9
	// typedef struct D3D12_FEATURE_DATA_D3D12_OPTIONS9 { BOOL MeshShaderPipelineStatsSupported; BOOL MeshShaderSupportsFullRangeRenderTargetArrayIndex; BOOL AtomicInt64OnTypedResourceSupported; BOOL AtomicInt64OnGroupSharedSupported; BOOL DerivativesInMeshAndAmplificationShadersSupported; D3D12_WAVE_MMA_TIER WaveMMATier; } D3D12_FEATURE_DATA_D3D12_OPTIONS9;
	...
	public struct D3D12_FEATURE_DATA_D3D12_OPTIONS9
	{
		...
		/// <summary>
		///   <para>Type: _Out_ <b><c>BOOL</c></b></para>
		///   <para>Indicates whether or not values of SV_RenderTargetArrayIndex that are 8 or greater are supported. <c>true</c> if supported, otherwise <c>false</c>.</para>
		/// </summary>
		[MarshalAs(UnmanagedType.Bool)]
		...
		/// <summary>
		///   <para>Type: _Out_ <b><c>BOOL</c></b></para>
		///   <para>Indicates whether or not 64-bit integer atomics are supported on <c>groupshared</c> variables. <c>true</c> if supported, otherwise <c>false</c>.</para>
		/// </summary>
		[MarshalAs(UnmanagedType.Bool)]
		public bool AtomicInt64OnGroupSharedSupported;
		/// <summary>
		///   <para>Type: _Out_ <b><c>BOOL</c></b></para>
		///   <para>Indicates whether or not derivative and derivative-dependent texture sample operations are supported. <c>true</c> if supported, otherwise <c>false</c>.</para>
		/// </summary>
		[MarshalAs(UnmanagedType.Bool)]
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_displayable
	// typedef struct D3D12_FEATURE_DATA_DISPLAYABLE { BOOL DisplayableTexture; D3D12_SHARED_RESOURCE_COMPATIBILITY_TIER SharedResourceCompatibilityTier; } D3D12_FEATURE_DATA_DISPLAYABLE;
	...
	/// <summary>Provides detail about whether the adapter supports creating heaps from existing system memory. Such heaps are not intended for general use, but are exceptionally useful for diagnostic purposes, because they are guaranteed to persist even after the adapter faults or experiences a device-removal event. Persistence is not guaranteed for heaps returned by <c>ID3D12Device::CreateHeap</c> or <c>ID3D12Device::CreateCommittedResource</c>, even when the heap resides in system memory.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_existing_heaps
	// typedef struct D3D12_FEATURE_DATA_EXISTING_HEAPS { BOOL Supported; } D3D12_FEATURE_DATA_EXISTING_HEAPS;
	...
	{
		/// <summary>
		///   <b>TRUE</b> if the adapter can create a heap from existing system memory. Otherwise, <b>FALSE</b>.</summary>
		[MarshalAs(UnmanagedType.Bool)]
	...
	/// <remarks>See <c>D3D12_FEATURE</c>.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_feature_levels
	...
	public struct D3D12_FEATURE_DATA_FEATURE_LEVELS
	{
		...
		/// <summary>A pointer to an array of <c>D3D_FEATURE_LEVEL</c>s that the application is requesting for the driver and hardware to evaluate.</summary>
	...
	/// <para>Examples</para>
	/// <para><c>inline UINT8 D3D12GetFormatPlaneCount( _In_ ID3D12Device* pDevice, DXGI_FORMAT Format ) { D3D12_FEATURE_DATA_FORMAT_INFO formatInfo{ Format }; if (FAILED(pDevice-&gt;CheckFeatureSupport(D3D12_FEATURE_FORMAT_INFO, &amp;formatInfo, sizeof(formatInfo)))) { return 0; } return formatInfo.PlaneCount; } </c></para>
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_format_info
	// typedef struct D3D12_FEATURE_DATA_FORMAT_INFO { DXGI_FORMAT Format; UINT8 PlaneCount; } D3D12_FEATURE_DATA_FORMAT_INFO;
	...
	// typedef struct D3D12_FEATURE_DATA_FORMAT_SUPPORT { DXGI_FORMAT Format; D3D12_FORMAT_SUPPORT1 Support1; D3D12_FORMAT_SUPPORT2 Support2; } D3D12_FEATURE_DATA_FORMAT_SUPPORT;
	...
	public struct D3D12_FEATURE_DATA_FORMAT_SUPPORT
	{
		/// <summary>A <c>DXGI_FORMAT</c>-typed value for the format to return info about.</summary>
		public DXGI_FORMAT Format;
		/// <summary>A combination of <c>D3D12_FORMAT_SUPPORT1</c>-typed values that are combined by using a bitwise OR operation. The resulting value specifies which resources are supported.</summary>
		public D3D12_FORMAT_SUPPORT1 Support1;
		/// <summary>A combination of <c>D3D12_FORMAT_SUPPORT2</c>-typed values that are combined by using a bitwise OR operation. The resulting value specifies which unordered resource options are supported.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_gpu_virtual_address_support
	// typedef struct D3D12_FEATURE_DATA_GPU_VIRTUAL_ADDRESS_SUPPORT { UINT MaxGPUVirtualAddressBitsPerResource; UINT MaxGPUVirtualAddressBitsPerProcess; } D3D12_FEATURE_DATA_GPU_VIRTUAL_ADDRESS_SUPPORT;
	...
	public struct D3D12_FEATURE_DATA_GPU_VIRTUAL_ADDRESS_SUPPORT
	{
		...
		///   <para>When this value is nearly equal to the available residency budget, <c>Evict</c> will not be a feasible option to manage residency. See <c>MakeResident</c> for more details.</para>
		/// </summary>
	...
	// typedef struct D3D12_FEATURE_DATA_MULTISAMPLE_QUALITY_LEVELS { DXGI_FORMAT Format; UINT SampleCount; D3D12_MULTISAMPLE_QUALITY_LEVEL_FLAGS Flags; UINT NumQualityLevels; } D3D12_FEATURE_DATA_MULTISAMPLE_QUALITY_LEVELS;
	...
	public struct D3D12_FEATURE_DATA_MULTISAMPLE_QUALITY_LEVELS
	{
		...
		/// <summary>Flags to control quality levels, as a bitwise-OR'd combination of <c>D3D12_MULTISAMPLE_QUALITY_LEVEL_FLAGS</c> enumeration constants. The resulting value specifies options for determining quality levels.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_protected_resource_session_support
	// typedef struct D3D12_FEATURE_DATA_PROTECTED_RESOURCE_SESSION_SUPPORT { UINT NodeIndex; D3D12_PROTECTED_RESOURCE_SESSION_SUPPORT_FLAGS Support; } D3D12_FEATURE_DATA_PROTECTED_RESOURCE_SESSION_SUPPORT;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_protected_resource_session_type_count
	// typedef struct D3D12_FEATURE_DATA_PROTECTED_RESOURCE_SESSION_TYPE_COUNT { UINT NodeIndex; UINT Count; } D3D12_FEATURE_DATA_PROTECTED_RESOURCE_SESSION_TYPE_COUNT;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_protected_resource_session_types
	// typedef struct D3D12_FEATURE_DATA_PROTECTED_RESOURCE_SESSION_TYPES { UINT NodeIndex; UINT Count; GUID *pTypes; } D3D12_FEATURE_DATA_PROTECTED_RESOURCE_SESSION_TYPES;
	...
	public struct D3D12_FEATURE_DATA_PROTECTED_RESOURCE_SESSION_TYPES
	{
		...
		/// <summary>
		///   <para>Type: <b><c>UINT</c></b></para>
		///   <para>An input parameter indicating the size of the pTypes array. This must match the count returned through the <c>D3D12_FEATURE_PROTECTED_RESOURCE_SESSION_TYPE_COUNT</c> query.</para>
		/// </summary>
		public uint Count;
		/// <summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_query_meta_command
	// typedef struct D3D12_FEATURE_DATA_QUERY_META_COMMAND { GUID CommandId; UINT NodeMask; const void *pQueryInputData; SIZE_T QueryInputDataSizeInBytes; void *pQueryOutputData; SIZE_T QueryOutputDataSizeInBytes; } D3D12_FEATURE_DATA_QUERY_META_COMMAND;
	...
	public struct D3D12_FEATURE_DATA_QUERY_META_COMMAND
	{
		...
		/// <summary>
		///   <para>Type: <b><c>UINT</c></b></para>
		///   <para>For single GPU operation, this is zero. If there are multiple GPU nodes, a bit is set to identify a node (the device's physical adapter). Each bit in the mask corresponds to a single node. Only 1 bit must be set. Refer to <c>Multi-adapter systems</c>.</para>
		/// </summary>
		public uint NodeMask;
		/// <summary>
	...
	// typedef struct D3D12_FEATURE_DATA_ROOT_SIGNATURE { D3D_ROOT_SIGNATURE_VERSION HighestVersion; } D3D12_FEATURE_DATA_ROOT_SIGNATURE;
	...
	{
		/// <summary>On input, specifies the highest version <c>D3D_ROOT_SIGNATURE_VERSION</c> to check for. On output specifies the highest version, up to the input version specified, actually available.</summary>
		public D3D_ROOT_SIGNATURE_VERSION HighestVersion;
	}

	/// <summary>Indicates the level of support for heap serialization.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_serialization
	// typedef struct D3D12_FEATURE_DATA_SERIALIZATION { UINT NodeIndex; D3D12_HEAP_SERIALIZATION_TIER HeapSerializationTier; } D3D12_FEATURE_DATA_SERIALIZATION;
	...
	/// <remarks>
	/// <para>Use this structure with <c>CheckFeatureSupport</c> to determine the level of support offered for the optional shader-caching features.</para>
	/// <para>See the enumeration constant D3D12_FEATURE_SHADER_CACHE in the <c>D3D12_FEATURE</c> enumeration.</para>
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_shader_cache
	// typedef struct D3D12_FEATURE_DATA_SHADER_CACHE { D3D12_SHADER_CACHE_SUPPORT_FLAGS SupportFlags; } D3D12_FEATURE_DATA_SHADER_CACHE;
	...
	/// <para>When used with the <c>ID3D12Device::CheckFeatureSupport</c> function, before calling the function initialize the HighestShaderModel field to the highest shader model that your application understands. After the function completes successfully, the HighestShaderModel field contains the highest shader model that is both supported by the device and no higher than the shader model passed in.</para>
	/// <para><para>Note</para> <para><b>ID3D12Device::CheckFeatureSupport</b> returns <b>E_INVALIDARG</b> if HighestShaderModel isn't known by the current runtime. For that reason, we recommend that you call this in a loop with decreasing shader models to determine the highest supported shader model. Alternatively, use the caps checking helper to simplify this; see the blog post <c>Introducing a New API for Checking Feature Support in Direct3D 12</c>.</para></para>
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_feature_data_shader_model
	// typedef struct D3D12_FEATURE_DATA_SHADER_MODEL { D3D_SHADER_MODEL HighestShaderModel; } D3D12_FEATURE_DATA_SHADER_MODEL;
	...
	/// <para>To get the handle increment size use <c>ID3D12Device.GetDescriptorHandleIncrementSize</c></para>
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_gpu_descriptor_handle
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_gpu_virtual_address_range
	// typedef struct D3D12_GPU_VIRTUAL_ADDRESS_RANGE { D3D12_GPU_VIRTUAL_ADDRESS StartAddress; UINT64 SizeInBytes; } D3D12_GPU_VIRTUAL_ADDRESS_RANGE;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_gpu_virtual_address_range_and_stride
	// typedef struct D3D12_GPU_VIRTUAL_ADDRESS_RANGE_AND_STRIDE { D3D12_GPU_VIRTUAL_ADDRESS StartAddress; UINT64 SizeInBytes; UINT64 StrideInBytes; } D3D12_GPU_VIRTUAL_ADDRESS_RANGE_AND_STRIDE;
	...
	/// </item>
	/// <item>
	/// <description>If the <b>HS</b> and <b>DS</b> members are specified, the <b>PrimitiveTopologyType</b> member for topology type must be set to <c>D3D12_PRIMITIVE_TOPOLOGY_TYPE_PATCH</c>.</description>
	/// </item>
	/// <item>
	...
	/// </item>
	/// <item>
	/// <description>If the <b>ForcedSampleCount</b> member of <c>D3D12_RASTERIZER_DESC</c> that <b>RasterizerState</b> specifies isn't zero:</description>
	/// </item>
	/// <item>
	...
	public struct D3D12_GRAPHICS_PIPELINE_STATE_DESC
	{
		...
		/// <summary>For single GPU operation, set this to zero. If there are multiple GPU nodes, set bits to identify the nodes (the device's physical adapters) for which the graphics pipeline state is to apply. Each bit in the mask corresponds to a single node. Refer to <c>Multi-adapter systems</c>.</summary>
		public uint NodeMask;
		/// <summary>A cached pipeline state object, as a <c>D3D12_CACHED_PIPELINE_STATE</c> structure. pCachedBlob and CachedBlobSizeInBytes may be set to NULL and 0 respectively.</summary>
		public D3D12_CACHED_PIPELINE_STATE CachedPSO;
		/// <summary>A <c>D3D12_PIPELINE_STATE_FLAGS</c> enumeration constant such as for "tool debug".</summary>
	...
	public struct D3D12_HEAP_DESC
	{
		...
		/// <summary>A combination of <c>D3D12_HEAP_FLAGS</c>-typed values that are combined by using a bitwise-OR operation. The resulting value identifies heap options. When creating heaps to support adapters with resource heap tier 1, an application must choose some flags.</summary>
	...
	/// <list type="bullet">
	/// <item>
	/// <description>When <b>Type</b> is <c>D3D12_HEAP_TYPE</c>_CUSTOM, <b>CPUPageProperty</b> and <b>MemoryPoolPreference</b> must not be ..._UNKNOWN.</description>
	/// </item>
	/// <item>
	/// <description>When <b>Type</b> is not D3D12_HEAP_TYPE_CUSTOM, <b>CPUPageProperty</b> and <b>MemoryPoolPreference</b> must be ..._UNKNOWN.</description>
	/// </item>
	/// <item>
	/// <description>When using D3D12_HEAP_TYPE_CUSTOM and <c>D3D12_MEMORY_POOL</c>_L1, on NUMA adapters, <b>CPUPageProperty</b> must be <c>D3D12_CPU_PAGE_PROPERTY</c>_NOT_AVAILABLE. To differentiate NUMA from UMA adapters, see <c>D3D12_FEATURE</c>_ARCHITECTURE and <c>D3D12_FEATURE_DATA_ARCHITECTURE</c>.</description>
	/// </item>
	/// </list>
	/// </remarks>
	...
	public struct D3D12_HEAP_PROPERTIES
	{
		...
		///   <para>
		///     <i>VisibleNodeMask</i> must have the same bit set that is set in <i>CreationNodeMask</i>. <i>VisibleNodeMask</i> can also have additional bits set for cross-node resources, but doing so can potentially reduce performance for resource accesses, so you should do so only when needed.</para>
		///   <para>Passing zero is equivalent to passing one, in order to simplify the usage of single-GPU adapters.</para>
		/// </summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_hit_group_desc
	// typedef struct D3D12_HIT_GROUP_DESC { LPCWSTR HitGroupExport; D3D12_HIT_GROUP_TYPE Type; LPCWSTR AnyHitShaderImport; LPCWSTR ClosestHitShaderImport; LPCWSTR IntersectionShaderImport; } D3D12_HIT_GROUP_DESC;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_index_buffer_view
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_indirect_argument_desc
	// typedef struct D3D12_INDIRECT_ARGUMENT_DESC { D3D12_INDIRECT_ARGUMENT_TYPE Type; union { struct { UINT Slot; } VertexBuffer; struct { UINT RootParameterIndex; UINT DestOffsetIn32BitValues; UINT Num32BitValuesToSet; } Constant; struct { UINT RootParameterIndex; } ConstantBufferView; struct { UINT RootParameterIndex; } ShaderResourceView; struct { UINT RootParameterIndex; } UnorderedAccessView; struct { UINT RootParameterIndex; UINT DestOffsetIn32BitValues; } IncrementingConstant; }; } D3D12_INDIRECT_ARGUMENT_DESC;
	...
	public struct D3D12_INDIRECT_ARGUMENT_DESC
	{
		...
		public struct CONSTANT
		{
			...
			/// <summary>The offset, in 32-bit values, to set the first constant of the group. Supports multi-value constants at a given root index. Root constant entries must be sorted from smallest to largest DestOffsetIn32BitValues.</summary>
			public uint DestOffsetIn32BitValues;
			/// <summary>The number of 32-bit constants that are set at the given root index. Supports multi-value constants at a given root index.</summary>
	...
	/// <remarks>This structure is a member of the <c>D3D12_INPUT_LAYOUT_DESC</c> structure. A pipeline state object contains a input-layout structure that defines one element being read from an input slot.</remarks>
	...
	public struct D3D12_INPUT_ELEMENT_DESC
	{
		...
		public DXGI_FORMAT Format;
		/// <summary>An integer value that identifies the input-assembler. For more info, see <c>Input Slots</c>. Valid values are between 0 and 15.</summary>
		...
		/// <summary>The number of instances to draw using the same per-instance data before advancing in the buffer by one element. This value must be 0 for an element that contains per-vertex data (the slot class is set to the D3D12_INPUT_PER_VERTEX_DATA member of <c>D3D12_INPUT_CLASSIFICATION</c>).</summary>
	...
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_local_root_signature
	// typedef struct D3D12_LOCAL_ROOT_SIGNATURE { ID3D12RootSignature *pLocalRootSignature; } D3D12_LOCAL_ROOT_SIGNATURE;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_memcpy_dest
	// typedef struct D3D12_MEMCPY_DEST { void *pData; SIZE_T RowPitch; SIZE_T SlicePitch; } D3D12_MEMCPY_DEST;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_meta_command_desc
	// typedef struct D3D12_META_COMMAND_DESC { GUID Id; LPCWSTR Name; D3D12_GRAPHICS_STATES InitializationDirtyState; D3D12_GRAPHICS_STATES ExecutionDirtyState; } D3D12_META_COMMAND_DESC;
	...
	public struct D3D12_META_COMMAND_DESC
	{
		...
		///   <para>Declares the command list states that are modified by the call to initialize the meta command. If all state bits are set, then that's equivalent to calling <c>ID3D12GraphicsCommandList::ClearState</c>.</para>
		/// </summary>
		public D3D12_GRAPHICS_STATES InitializationDirtyState;
		/// <summary>
		///   <para>Type: <b><c>D3D12_GRAPHICS_STATES</c></b></para>
		///   <para>Declares the command list states that are modified by the call to execute the meta command. If all state bits are set, then that's equivalent to calling <c>ID3D12GraphicsCommandList::ClearState</c>.</para>
		/// </summary>
		public D3D12_GRAPHICS_STATES ExecutionDirtyState;
	}

	/// <summary>Describes a parameter to a meta command.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_meta_command_parameter_desc
	// typedef struct D3D12_META_COMMAND_PARAMETER_DESC { LPCWSTR Name; D3D12_META_COMMAND_PARAMETER_TYPE Type; D3D12_META_COMMAND_PARAMETER_FLAGS Flags; D3D12_RESOURCE_STATES RequiredResourceState; UINT StructureOffset; } D3D12_META_COMMAND_PARAMETER_DESC;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_mip_region
	// typedef struct D3D12_MIP_REGION { UINT Width; UINT Height; UINT Depth; } D3D12_MIP_REGION;
	...
	/// <para><para>Important</para> <para>On some versions of the DirectX Runtime, specifying a node via <b>D3D12_NODE_MASK</b> in a <c><b>D3D12_STATE_SUBOBJECT</b></c> with type <c><b>D3D12_STATE_SUBOBJECT_TYPE_NODE_MASK</b></c>, the runtime will incorrectly handle a node mask value of <c>0</c>, which should use node #1, which will lead to errors when attempting to use the state object later. Specify an explicit node value of 1, or omit the <b>D3D12_NODE_MASK</b> subobject to avoid this issue.</para></para>
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_node_mask
	// typedef struct D3D12_NODE_MASK { UINT NodeMask; } D3D12_NODE_MASK;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_packed_mip_info
	// typedef struct D3D12_PACKED_MIP_INFO { UINT8 NumStandardMips; UINT8 NumPackedMips; UINT NumTilesForPackedMips; UINT StartTileIndexInOverallResource; } D3D12_PACKED_MIP_INFO;
	...
	// typedef struct D3D12_PIPELINE_STATE_STREAM_DESC { SIZE_T SizeInBytes; void *pPipelineStateSubobjectStream; } D3D12_PIPELINE_STATE_STREAM_DESC;
	...
	public struct D3D12_PIPELINE_STATE_STREAM_DESC
	{
		...
		/// <summary>
		///   <para>
		///     <c>SAL</c>: <c>In_reads(Inexpressible("Dependentonsizeofsubobjects"))</c></para>
	...
	/// <para>All the data referenced by the footprint structure must fit within the bounds of the parent resource. If you use <c>GetCopyableFootprints</c> to fill out the structure, the <i>pTotalBytes</i> output field indicates the required size of the resource.</para>
	...
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_placed_subresource_footprint
	// typedef struct D3D12_PLACED_SUBRESOURCE_FOOTPRINT { UINT64 Offset; D3D12_SUBRESOURCE_FOOTPRINT Footprint; } D3D12_PLACED_SUBRESOURCE_FOOTPRINT;
	...
	{
		/// <summary>The offset of the subresource within the parent resource, in bytes. The offset between the start of the parent resource and this subresource.</summary>
		public ulong Offset;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_protected_resource_session_desc
	// typedef struct D3D12_PROTECTED_RESOURCE_SESSION_DESC { UINT NodeMask; D3D12_PROTECTED_RESOURCE_SESSION_FLAGS Flags; } D3D12_PROTECTED_RESOURCE_SESSION_DESC;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_protected_resource_session_desc1
	// typedef struct D3D12_PROTECTED_RESOURCE_SESSION_DESC1 { UINT NodeMask; D3D12_PROTECTED_RESOURCE_SESSION_FLAGS Flags; GUID ProtectionType; } D3D12_PROTECTED_RESOURCE_SESSION_DESC1;
	...
	// typedef struct D3D12_QUERY_DATA_PIPELINE_STATISTICS { UINT64 IAVertices; UINT64 IAPrimitives; UINT64 VSInvocations; UINT64 GSInvocations; UINT64 GSPrimitives; UINT64 CInvocations; UINT64 CPrimitives; UINT64 PSInvocations; UINT64 HSInvocations; UINT64 DSInvocations; UINT64 CSInvocations; } D3D12_QUERY_DATA_PIPELINE_STATISTICS;
	...
	/// </summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_query_data_pipeline_statistics1
	// typedef struct D3D12_QUERY_DATA_PIPELINE_STATISTICS1 { UINT64 IAVertices; UINT64 IAPrimitives; UINT64 VSInvocations; UINT64 GSInvocations; UINT64 GSPrimitives; UINT64 CInvocations; UINT64 CPrimitives; UINT64 PSInvocations; UINT64 HSInvocations; UINT64 DSInvocations; UINT64 CSInvocations; UINT64 ASInvocations; UINT64 MSInvocations; UINT64 MSPrimitives; } D3D12_QUERY_DATA_PIPELINE_STATISTICS1;
	...
	/// <remarks>Use this structure with <c>D3D12_QUERY_HEAP_TYPE</c> and <c>CreateQueryHeap</c>.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_query_data_so_statistics
	// typedef struct D3D12_QUERY_DATA_SO_STATISTICS { UINT64 NumPrimitivesWritten; UINT64 PrimitivesStorageNeeded; } D3D12_QUERY_DATA_SO_STATISTICS;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_query_heap_desc
	// typedef struct D3D12_QUERY_HEAP_DESC { D3D12_QUERY_HEAP_TYPE Type; UINT Count; UINT NodeMask; } D3D12_QUERY_HEAP_DESC;
	...
	public struct D3D12_QUERY_HEAP_DESC
	{
		...
		public uint Count;
		/// <summary>For single GPU operation, set this to zero. If there are multiple GPU nodes, set a bit to identify the node (the device's physical adapter) to which the query heap applies. Each bit in the mask corresponds to a single node. Only 1 bit must be set. Refer to <c>Multi-adapter systems</c>.</summary>
		public uint NodeMask;
	}
	...
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_range
	...
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_range_uint64
	// typedef struct D3D12_RANGE_UINT64 { UINT64 Begin; UINT64 End; } D3D12_RANGE_UINT64;
	...
	public struct D3D12_RASTERIZER_DESC
	{
		...
		///   <note>If you want to render with <b>ForcedSampleCount</b> set to 1 or greater, you must follow these guidelines: Otherwise, rendering behavior is undefined.</note>
		/// </summary>
		public uint ForcedSampleCount;
		/// <summary>A <c>D3D12_CONSERVATIVE_RASTERIZATION_MODE</c>-typed value that identifies whether conservative rasterization is on or off.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_raytracing_aabb
	// typedef struct D3D12_RAYTRACING_AABB { FLOAT MinX; FLOAT MinY; FLOAT MinZ; FLOAT MaxX; FLOAT MaxY; FLOAT MaxZ; } D3D12_RAYTRACING_AABB;
	...
	{
		/// <summary>Space currently used by an acceleration structure. If the acceleration structure hasn’t had a compaction operation performed on it, this size is the same one reported by <c>GetRaytracingAccelerationStructurePrebuildInfo</c>, and if it has been compacted this size is the same reported for post-build info with <c>D3D12_RAYTRACING_ACCELERATION_STRUCTURE_POSTBUILD_INFO_COMPACTED_SIZE</c>.</summary>
		public ulong CurrentSizeInBytes;
	}

	/// <summary>Description of the post-build information to generate from an acceleration structure. Use this structure in calls to <c>EmitRaytracingAccelerationStructurePostbuildInfo</c> and <c>BuildRaytracingAccelerationStructure</c>.</summary>
	...
	// typedef struct D3D12_RAYTRACING_ACCELERATION_STRUCTURE_POSTBUILD_INFO_DESC { D3D12_GPU_VIRTUAL_ADDRESS DestBuffer; D3D12_RAYTRACING_ACCELERATION_STRUCTURE_POSTBUILD_INFO_TYPE InfoType; } D3D12_RAYTRACING_ACCELERATION_STRUCTURE_POSTBUILD_INFO_DESC;
	...
	public struct D3D12_RAYTRACING_ACCELERATION_STRUCTURE_POSTBUILD_INFO_DESC
	{
		...
		///   <para>The memory pointed to must be in state <c>D3D12_RESOURCE_STATE_UNORDERED_ACCESS</c>. The memory must be aligned to the natural alignment for the members of the particular output structure being generated (e.g. 8 bytes for a struct with the largest members being UINT64).</para>
		/// </summary>
		public D3D12_GPU_VIRTUAL_ADDRESS DestBuffer;
		/// <summary>A <c>D3D12_RAYTRACING_ACCELERATION_STRUCTURE_POSTBUILD_INFO_TYPE</c> value specifying the type of post-build information to retrieve.</summary>
	...
	// typedef struct D3D12_RAYTRACING_ACCELERATION_STRUCTURE_POSTBUILD_INFO_SERIALIZATION_DESC { UINT64 SerializedSizeInBytes; UINT64 NumBottomLevelAccelerationStructurePointers; } D3D12_RAYTRACING_ACCELERATION_STRUCTURE_POSTBUILD_INFO_SERIALIZATION_DESC;
	...
	{
		/// <summary>The size of the serialized acceleration structure, including a header. The header is <c>D3D12_SERIALIZED_RAYTRACING_ACCELERATION_STRUCTURE_HEADER</c> followed by followed by a list of pointers to bottom-level acceleration structures.</summary>
		public ulong SerializedSizeInBytes;
		/// <summary>
		///   <para>The number of 64-bit GPU virtual addresses that will be at the start of the serialized acceleration structure, after the <c>D3D12_SERIALIZED_RAYTRACING_ACCELERATION_STRUCTURE_HEADER</c>. For a bottom-level acceleration structure this will be 0. For a top-level acceleration structure, the pointers indicate the acceleration structures being referred to.</para>
	...
	/// <summary>Represents prebuild information about a raytracing acceleration structure. Get an instance of this structure by calling <c>GetRaytracingAccelerationStructurePrebuildInfo</c>.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_raytracing_acceleration_structure_prebuild_info
	// typedef struct D3D12_RAYTRACING_ACCELERATION_STRUCTURE_PREBUILD_INFO { UINT64 ResultDataMaxSizeInBytes; UINT64 ScratchDataSizeInBytes; UINT64 UpdateScratchDataSizeInBytes; } D3D12_RAYTRACING_ACCELERATION_STRUCTURE_PREBUILD_INFO;
	...
	public struct D3D12_RAYTRACING_ACCELERATION_STRUCTURE_PREBUILD_INFO
	{
		...
		///   <para>Scratch storage on GPU required during an acceleration structure update based on the specified inputs. This only needs to be called for the original acceleration structure build, and defines the scratch storage requirement for every acceleration structure update, other than the initial build.</para>
		///   <para>If the <c>D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_ALLOW_UPDATE</c> flag is not specified when calling <c>GetRaytracingAccelerationStructurePrebuildInfo</c>, the returned value of this field is 0.</para>
		/// </summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_raytracing_acceleration_structure_srv
	// typedef struct D3D12_RAYTRACING_ACCELERATION_STRUCTURE_SRV { D3D12_GPU_VIRTUAL_ADDRESS Location; } D3D12_RAYTRACING_ACCELERATION_STRUCTURE_SRV;
	...
	/// <remarks>This C++ struct definition is useful if you're generating instance data on the CPU first, then uploading to the GPU. But your application is also free to generate instance descriptions directly into GPU memory (from compute shaders, for instance) following the same layout.</remarks>
	...
	public struct D3D12_RAYTRACING_INSTANCE_DESC
	{
		...
		///   <note>The layout of <c>Transform</c> is a transpose of how affine matrices are typically stored in memory. Instead of four 3-vectors, <c>Transform</c> is laid out as three 4-vectors.</note>
		/// </summary>
		...
		///   <para>An 8-bit mask assigned to the instance, which can be used to include/reject groups of instances on a per-ray basis. If the value is zero, then the instance will never be included, so typically this should be set to some non-zero value. For more information see, the <c>InstanceInclusionMask</c> parameter to the <c>TraceRay</c> function.</para>
		/// </summary>
		...
		///   <para>Address of the bottom-level acceleration structure that is being instanced. The address must be aligned to 256 bytes, defined as <c>D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BYTE_ALIGNMENT</c>. Any existing acceleration structure passed in here would already have been required to be placed with such alignment.</para>
	...
	/// <remarks>A raytracing pipeline needs one raytracing pipeline configuration. If multiple pipeline configurations are present, then they must all match in content. But there's no benefit to such duplication. For example, defining it once per collection doesn't help drivers do early shader compilation before a raytracing pipeline is created. This is unlike <c>D3D12_RAYTRACING_SHADER_CONFIG</c>, which does benefit from duplication per collection.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_raytracing_pipeline_config
	// typedef struct D3D12_RAYTRACING_PIPELINE_CONFIG { UINT MaxTraceRecursionDepth; } D3D12_RAYTRACING_PIPELINE_CONFIG;
	...
	/// </summary>
	/// <remarks>A raytracing pipeline needs one raytracing pipeline configuration. If multiple pipeline configurations are present, then they must all match in content. But there's no benefit to such duplication. For example, defining it once per collection doesn't help drivers do early shader compilation before a raytracing pipeline is created. This is unlike <c>D3D12_RAYTRACING_SHADER_CONFIG</c>, which does benefit from duplication per collection.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_raytracing_pipeline_config1
	// typedef struct D3D12_RAYTRACING_PIPELINE_CONFIG1 { UINT MaxTraceRecursionDepth; D3D12_RAYTRACING_PIPELINE_FLAGS Flags; } D3D12_RAYTRACING_PIPELINE_CONFIG1;
	...
	/// <remarks>A raytracing pipeline needs one raytracing shader configuration. If multiple shader configurations are present, such as one in each collection to enable independent driver compilation for each one, they must all match when combined into a raytracing pipeline.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_raytracing_shader_config
	// typedef struct D3D12_RAYTRACING_SHADER_CONFIG { UINT MaxPayloadSizeInBytes; UINT MaxAttributeSizeInBytes; } D3D12_RAYTRACING_SHADER_CONFIG;
	...
	public struct D3D12_RAYTRACING_SHADER_CONFIG
	{
		...
		/// <summary>The maximum number of scalars (counted as 4 bytes each) that can be used for attributes in pipelines that contain this shader. The value cannot exceed <c>D3D12_RAYTRACING_MAX_ATTRIBUTE_SIZE_IN_BYTES</c>.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_render_pass_beginning_access
	// typedef struct D3D12_RENDER_PASS_BEGINNING_ACCESS { D3D12_RENDER_PASS_BEGINNING_ACCESS_TYPE Type; union { D3D12_RENDER_PASS_BEGINNING_ACCESS_CLEAR_PARAMETERS Clear; D3D12_RENDER_PASS_BEGINNING_ACCESS_PRESERVE_LOCAL_PARAMETERS PreserveLocal; }; } D3D12_RENDER_PASS_BEGINNING_ACCESS;
	...
	public struct D3D12_RENDER_PASS_BEGINNING_ACCESS
	{
		...
		/// <summary>A <c>D3D12_RENDER_PASS_BEGINNING_ACCESS_CLEAR_PARAMETERS</c>. Appropriate when <b>Type</b> is <c>D3D12_RENDER_PASS_BEGINNING_ACCESS_TYPE_CLEAR</c>. The clear value to which resource(s) should be cleared.</summary>
		[FieldOffset(4)]
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_render_pass_beginning_access_clear_parameters
	// typedef struct D3D12_RENDER_PASS_BEGINNING_ACCESS_CLEAR_PARAMETERS { D3D12_CLEAR_VALUE ClearValue; } D3D12_RENDER_PASS_BEGINNING_ACCESS_CLEAR_PARAMETERS;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_render_pass_depth_stencil_desc
	...
	public struct D3D12_RENDER_PASS_DEPTH_STENCIL_DESC
	{
		...
		/// <summary>A <c>D3D12_RENDER_PASS_BEGINNING_ACCESS</c>. The access to the depth buffer requested at the transition into a render pass.</summary>
		public D3D12_RENDER_PASS_BEGINNING_ACCESS DepthBeginningAccess;
		/// <summary>A <c>D3D12_RENDER_PASS_BEGINNING_ACCESS</c>. The access to the stencil buffer requested at the transition into a render pass.</summary>
		public D3D12_RENDER_PASS_BEGINNING_ACCESS StencilBeginningAccess;
		/// <summary>A <c>D3D12_RENDER_PASS_ENDING_ACCESS</c>. The access to the depth buffer requested at the transition out of a render pass.</summary>
		public D3D12_RENDER_PASS_ENDING_ACCESS DepthEndingAccess;
		/// <summary>A <c>D3D12_RENDER_PASS_ENDING_ACCESS</c>. The access to the stencil buffer requested at the transition out of a render pass.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_render_pass_ending_access
	// typedef struct D3D12_RENDER_PASS_ENDING_ACCESS { D3D12_RENDER_PASS_ENDING_ACCESS_TYPE Type; union { D3D12_RENDER_PASS_ENDING_ACCESS_RESOLVE_PARAMETERS Resolve; D3D12_RENDER_PASS_ENDING_ACCESS_PRESERVE_LOCAL_PARAMETERS PreserveLocal; }; } D3D12_RENDER_PASS_ENDING_ACCESS;
	...
	public struct D3D12_RENDER_PASS_ENDING_ACCESS
	{
		...
		/// <summary>A <c>D3D12_RENDER_PASS_ENDING_ACCESS_RESOLVE_PARAMETERS</c>. Appropriate when <b>Type</b> is <c>D3D12_RENDER_PASS_ENDING_ACCESS_TYPE_RESOLVE</c>. Description of the resource to resolve to.</summary>
		[FieldOffset(4)]
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_render_pass_ending_access_resolve_parameters
	// typedef struct D3D12_RENDER_PASS_ENDING_ACCESS_RESOLVE_PARAMETERS { ID3D12Resource *pSrcResource; ID3D12Resource *pDstResource; UINT SubresourceCount; const D3D12_RENDER_PASS_ENDING_ACCESS_RESOLVE_SUBRESOURCE_PARAMETERS *pSubresourceParameters; DXGI_FORMAT Format; D3D12_RESOLVE_MODE ResolveMode; BOOL PreserveResolveSource; } D3D12_RENDER_PASS_ENDING_ACCESS_RESOLVE_PARAMETERS;
	...
	public struct D3D12_RENDER_PASS_ENDING_ACCESS_RESOLVE_PARAMETERS
	{
		...
		/// <summary>
		///   <para>A pointer to a constant array of <c>D3D12_RENDER_PASS_ENDING_ACCESS_RESOLVE_SUBRESOURCE_PARAMETERS</c>. These subresources can be a subset of the render target's array slices, but you can't target subresources that aren't part of the render target view (RTV) or the depth/stencil view (DSV).</para>
		///   <para>
		///     <para>Note</para> <para>This pointer is directly referenced by the command list, and the memory for this array must remain alive and intact until <c>EndRenderPass</c> is called.</para></para>
		/// </summary>
	...
	// typedef struct D3D12_RENDER_PASS_ENDING_ACCESS_RESOLVE_SUBRESOURCE_PARAMETERS { UINT SrcSubresource; UINT DstSubresource; UINT DstX; UINT DstY; D3D12_RECT SrcRect; } D3D12_RENDER_PASS_ENDING_ACCESS_RESOLVE_SUBRESOURCE_PARAMETERS;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_render_pass_render_target_desc
	// typedef struct D3D12_RENDER_PASS_RENDER_TARGET_DESC { D3D12_CPU_DESCRIPTOR_HANDLE cpuDescriptor; D3D12_RENDER_PASS_BEGINNING_ACCESS BeginningAccess; D3D12_RENDER_PASS_ENDING_ACCESS EndingAccess; } D3D12_RENDER_PASS_RENDER_TARGET_DESC;
	...
	public struct D3D12_RENDER_PASS_RENDER_TARGET_DESC
	{
		...
		public D3D12_CPU_DESCRIPTOR_HANDLE cpuDescriptor;
		/// <summary>A <c>D3D12_RENDER_PASS_BEGINNING_ACCESS</c>. The access to the RTV(s) requested at the transition into a render pass.</summary>
	...
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_render_target_view_desc
	...
	public struct D3D12_RENDER_TARGET_VIEW_DESC
	{
		...
		[FieldOffset(0)]
		public DXGI_FORMAT Format;
		/// <summary>A <c>D3D12_RTV_DIMENSION</c>-typed value that specifies how the render-target resource will be accessed. This type specifies how the resource will be accessed. This member also determines which _RTV to use in the following union.</summary>
		[FieldOffset(4)]
		...
		/// <summary>A <c>D3D12_TEX2DMS_RTV</c> structure that specifies a single subresource because a multisampled 2D texture only contains one subresource.</summary>
		[FieldOffset(8)]
		public D3D12_TEX2DMS_RTV Texture2DMS;
		/// <summary>A <c>D3D12_TEX2DMS_ARRAY_RTV</c> structure that specifies the subresources in a multisampled 2D texture array that can be accessed.</summary>
		[FieldOffset(8)]
	...
	/// <remarks>This structure is used by the <c>ID3D12Device::GetResourceAllocationInfo</c> and <c>ID3D12Device::GetResourceAllocationInfo1</c> methods.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_resource_allocation_info
	// typedef struct D3D12_RESOURCE_ALLOCATION_INFO { UINT64 SizeInBytes; UINT64 Alignment; } D3D12_RESOURCE_ALLOCATION_INFO;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_resource_allocation_info1
	// typedef struct D3D12_RESOURCE_ALLOCATION_INFO1 { UINT64 Offset; UINT64 Alignment; UINT64 SizeInBytes; } D3D12_RESOURCE_ALLOCATION_INFO1;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_resource_barrier
	...
	{
		/// <summary>A <c>D3D12_RESOURCE_BARRIER_TYPE</c>-typed value that specifies the type of resource barrier. This member determines which type to use in the union below.</summary>
		[FieldOffset(0)]
		...
		/// <summary>A <c>D3D12_RESOURCE_TRANSITION_BARRIER</c> structure that describes the transition of subresources between different usages. Members specify the before and after usages of the subresources.</summary>
		[FieldOffset(8)]
		public D3D12_RESOURCE_TRANSITION_BARRIER Transition;
		/// <summary>A <c>D3D12_RESOURCE_ALIASING_BARRIER</c> structure that describes the transition between usages of two different resources that have mappings into the same heap.</summary>
		[FieldOffset(8)]
		public D3D12_RESOURCE_ALIASING_BARRIER Aliasing;
		/// <summary>A <c>D3D12_RESOURCE_UAV_BARRIER</c> structure that describes a resource in which all UAV accesses (reads or writes) must complete before any future UAV accesses (read or write) can begin.</summary>
		[FieldOffset(8)]
	...
	/// <para>Buffers are a contiguous memory region. <i>Width</i> may be between 1 and either the <i>MaxGPUVirtualAddressBitsPerResource</i> field of <c>D3D12_FEATURE_DATA_GPU_VIRTUAL_ADDRESS_SUPPORT</c> for reserved resources or the <i>MaxGPUVirtualAddressBitsPerProcess</i> field for committed resources. However, exhaustion of GPU virtual address space, memory residency budget (see <c>IDXGIAdapter3::QueryVideoMemoryInfo</c>), and or system memory may easily occur first.</para>
	...
	/// <para><i>Layout</i> must be D3D12_TEXTURE_LAYOUT_ROW_MAJOR, as buffer memory layouts are understood by applications and row-major texture data is commonly marshaled through buffers.</para>
	/// <para><i>Flags</i> must still be accurately filled out by applications for buffers, with minor exceptions. However, applications can use the most amount of capability support without concern about the efficiency impact on buffers. The flags field is meant to control properties related to textures.</para>
	/// <para><c></c><c></c><c></c>Textures</para>
	/// <para>Textures are a multi-dimensional arrangement of texels in a contiguous region of memory, heavily optimized to maximize bandwidth for rendering and sampling. Texture sizes are hard to predict and vary from adapter to adapter. Applications must use <c>ID3D12Device::GetResourceAllocationInfo</c> to accurately understand their size.</para>
	/// <para>TEXTURE1D, TEXTURE2D, and TEXTURE3D are not supported orthogonally on every format. See the use of D3D12_FORMAT_SUPPORT1_TEXTURE1D, D3D12_FORMAT_SUPPORT1_TEXTURE2D, and D3D12_FORMAT_SUPPORT1_TEXTURE3D in <c>D3D12_FORMAT_SUPPORT1</c>.</para>
	/// <para><i>Width</i>, <i>Height</i>, and <i>DepthOrArraySize</i> must be between 1 and the maximum dimension supported for the particular feature level and texture dimension. However, exhaustion of GPU virtual address space, memory residency budget (see <c>IDXGIAdapter3::QueryVideoMemoryInfo</c>), and or system memory may easily occur first. For compressed formats, these dimensions are logical. For example:</para>
	/// <list type="bullet">
	/// <item>
	...
	/// <para>If <i>Alignment</i> is set to 0, the runtime will use 4MB for MSAA textures and 64KB for everything else. The application may choose smaller alignments than these defaults for a couple of texture types when the texture is small. Textures with UNKNOWN layout and MSAA may be created with 64KB alignment (if they pass the small size restriction detailed below).</para>
	/// <para>Textures with UNKNOWN layout without MSAA and without render-target nor depth-stencil flags may be created with 4KB Alignment (again, passing the small size restriction).</para>
	/// <para>Applications can create smaller aligned resources when the estimated size of the most-detailed mip level is a total of the larger alignment restriction or less. The runtime will use an architecture-independent mechanism of size-estimation, that mimics the way standard swizzle and D3D12 tiled resources are sized. However, the tile sizes will be of the smaller alignment restriction for such calculations. Using the non-render-target and non-depth-stencil texture as an example, the runtime will assume near-equilateral tile shapes of 4KB, and calculate the number of tiles needed for the most-detailed mip level. If the number of tiles is equal to or less than 16, then the application can create a 4KB aligned resource. So, a mipped tex2d array of any array size and any number of mip levels can be 4KB, as long as the width and height are small enough for the particular format and MSAA.</para>
	/// <para><c></c><c></c><c></c>MipLevels</para>
	/// <para><i>MipLevels</i> may be 0, or 1 to the maximum mip levels supported by the <i>Width</i>, <i>Height</i> , and <i>DepthOrArraySize</i> dimensions. When 0 is used, the API will automatically calculate the maximum mip levels supported and use that. But, some resource and heap properties preclude mip levels, so the app must specify the value as 1.</para>
	/// <para>Refer to the D3D12_FORMAT_SUPPORT1_MIP field of <c>D3D12_FORMAT_SUPPORT1</c> for per-format restrictions. MSAA resources, textures with D3D12_RESOURCE_FLAG_ALLOW_CROSS_ADAPTER, and heaps with D3D12_HEAP_FLAG_ALLOW_DISPLAY all preclude mip levels.</para>
	...
	/// <para>A <i>SampleDesc.Count</i> greater than 1 and/ or non-zero <i>Quality</i> are only supported for TEXTURE2D and when either D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET or D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL are set.</para>
	...
	{
		/// <summary>One member of <c>D3D12_RESOURCE_DIMENSION</c>, specifying the dimensions of the resource (for example, D3D12_RESOURCE_DIMENSION_TEXTURE1D), or whether it is a buffer ((D3D12_RESOURCE_DIMENSION_BUFFER).</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_resource_desc1
	// typedef struct D3D12_RESOURCE_DESC1 { D3D12_RESOURCE_DIMENSION Dimension; UINT64 Alignment; UINT64 Width; UINT Height; UINT16 DepthOrArraySize; UINT16 MipLevels; DXGI_FORMAT Format; DXGI_SAMPLE_DESC SampleDesc; D3D12_TEXTURE_LAYOUT Layout; D3D12_RESOURCE_FLAGS Flags; D3D12_MIP_REGION SamplerFeedbackMipRegion; } D3D12_RESOURCE_DESC1;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_resource_transition_barrier
	...
	public struct D3D12_RESOURCE_TRANSITION_BARRIER
	{
		...
		public uint Subresource;
		/// <summary>The "before" usages of the subresources, as a bitwise-OR'd combination of <c>D3D12_RESOURCE_STATES</c> enumeration constants.</summary>
		public D3D12_RESOURCE_STATES StateBefore;
		/// <summary>The "after" usages of the subresources, as a bitwise-OR'd combination of <c>D3D12_RESOURCE_STATES</c> enumeration constants.</summary>
	...
	/// <para><b>D3D12_ROOT_CONSTANTS</b> is the data type of the <b>Constants</b> member of <c>D3D12_ROOT_PARAMETER</c>. Use a <b>D3D12_ROOT_CONSTANTS</b> when you set <b>D3D12_ROOT_PARAMETER</b>'s <b>SlotType</b> field to the D3D12_ROOT_PARAMETER_TYPE_32BIT_CONSTANTS member of <c>D3D12_ROOT_PARAMETER_TYPE</c>.</para>
	/// </remarks>
	...
	public struct D3D12_ROOT_CONSTANTS
	{
		...
		public uint RegisterSpace;
		/// <summary>The number of constants that occupy a single shader slot (these constants appear like a single constant buffer). All constants occupy a single root signature bind slot.</summary>
	...
	/// <remarks><b>D3D12_ROOT_DESCRIPTOR</b> is the data type of the <b>Descriptor</b> member of <c>D3D12_ROOT_PARAMETER</c>. Use a <b>D3D12_ROOT_DESCRIPTOR</b> when you set <b>D3D12_ROOT_PARAMETER</b>'s <b>ParameterType</b> field to the D3D12_ROOT_PARAMETER_TYPE_CBV, D3D12_ROOT_PARAMETER_TYPE_SRV, or D3D12_ROOT_PARAMETER_TYPE_UAV members of <c>D3D12_ROOT_PARAMETER_TYPE</c>.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_root_descriptor
	...
	/// <para><b>D3D12_ROOT_DESCRIPTOR_TABLE</b> is the data type of the <b>DescriptorTable</b> member of <c>D3D12_ROOT_PARAMETER</c>. Use a <b>D3D12_ROOT_DESCRIPTOR_TABLE</b> when you set <b>D3D12_ROOT_PARAMETER</b>'s <b>ParameterType</b> member to <c>D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE</c>.</para>
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_root_descriptor_table
	...
	/// <remarks>
	/// <para>Samplers are not allowed in the same descriptor table as constant-buffer views (CBVs), unordered-access views (UAVs), and shader-resource views (SRVs).</para>
	/// <para><b>D3D12_ROOT_DESCRIPTOR_TABLE1</b> is the data type of the <b>DescriptorTable</b> member of <c>D3D12_ROOT_PARAMETER1</c>. Use a <b>D3D12_ROOT_DESCRIPTOR_TABLE1</b> when you set <b>D3D12_ROOT_PARAMETER1</b>'s <b>SlotType</b> member to <c>D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE</c>.</para>
	/// <para>Refer to the helper structure <c>CD3DX12_ROOT_DESCRIPTOR_TABLE1</c>.</para>
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_root_descriptor_table1
	// typedef struct D3D12_ROOT_DESCRIPTOR_TABLE1 { UINT NumDescriptorRanges; const D3D12_DESCRIPTOR_RANGE1 *pDescriptorRanges; } D3D12_ROOT_DESCRIPTOR_TABLE1;
	...
	/// <remarks>
	/// <para><b>D3D12_ROOT_DESCRIPTOR1</b> is the data type of the <b>Descriptor</b> member of <c>D3D12_ROOT_PARAMETER1</c>. Use a <b>D3D12_ROOT_DESCRIPTOR1</b> when you set <b>D3D12_ROOT_PARAMETER1</b>'s <b>ParameterType</b> field to the D3D12_ROOT_PARAMETER_TYPE_CBV, D3D12_ROOT_PARAMETER_TYPE_SRV, or D3D12_ROOT_PARAMETER_TYPE_UAV members of <c>D3D12_ROOT_PARAMETER_TYPE</c>.</para>
	/// <para>Refer to the helper structure <c>CD3DX12_ROOT_DESCRIPTOR1</c>.</para>
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_root_descriptor1
	// typedef struct D3D12_ROOT_DESCRIPTOR1 { UINT ShaderRegister; UINT RegisterSpace; D3D12_ROOT_DESCRIPTOR_FLAGS Flags; } D3D12_ROOT_DESCRIPTOR1;
	...
	public struct D3D12_ROOT_DESCRIPTOR1
	{
		...
		/// <summary>The register space.</summary>
		public uint RegisterSpace;
		/// <summary>Specifies the <c>D3D12_ROOT_DESCRIPTOR_FLAGS</c> that determine the volatility of descriptors and the data they reference.</summary>
		public D3D12_ROOT_DESCRIPTOR_FLAGS Flags;
	}

	/// <summary>Describes the slot of a root signature version 1.0.</summary>
	/// <remarks>A <c>D3D12_ROOT_SIGNATURE_DESC</c> can contain descriptor tables and inline constants. More capable hardware could support inline descriptors in the root signature as well. The number of bind slots in the root signature are most efficient if kept below a certain size, and can have an upper bound as well.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_root_parameter
	...
	{
		/// <summary>A <c>D3D12_ROOT_PARAMETER_TYPE</c>-typed value that specifies the type of root signature slot. This member determines which type to use in the union below.</summary>
		[FieldOffset(0)]
		public D3D12_ROOT_PARAMETER_TYPE ParameterType;
		/// <summary>A <c>D3D12_ROOT_DESCRIPTOR_TABLE</c> structure that describes the layout of a descriptor table as a collection of descriptor ranges that appear one after the other in a descriptor heap.</summary>
		[FieldOffset(4)]
		public D3D12_ROOT_DESCRIPTOR_TABLE DescriptorTable;
		/// <summary>A <c>D3D12_ROOT_CONSTANTS</c> structure that describes constants inline in the root signature that appear in shaders as one constant buffer.</summary>
		[FieldOffset(4)]
		public D3D12_ROOT_CONSTANTS Constants;
		/// <summary>A <c>D3D12_ROOT_DESCRIPTOR</c> structure that describes descriptors inline in the root signature that appear in shaders.</summary>
		[FieldOffset(4)]
		public D3D12_ROOT_DESCRIPTOR Descriptor;
		/// <summary>A <c>D3D12_SHADER_VISIBILITY</c>-typed value that specifies the shaders that can access the contents of the root signature slot.</summary>
		[FieldOffset(4)]
	...
	// typedef struct D3D12_ROOT_PARAMETER1 { D3D12_ROOT_PARAMETER_TYPE ParameterType; union { D3D12_ROOT_DESCRIPTOR_TABLE1 DescriptorTable; D3D12_ROOT_CONSTANTS Constants; D3D12_ROOT_DESCRIPTOR1 Descriptor; }; D3D12_SHADER_VISIBILITY ShaderVisibility; } D3D12_ROOT_PARAMETER1;
	...
	public struct D3D12_ROOT_PARAMETER1
	{
		...
		[FieldOffset(0)]
		public D3D12_ROOT_PARAMETER_TYPE ParameterType;
		/// <summary>A <c>D3D12_ROOT_DESCRIPTOR_TABLE1</c> structure that describes the layout of a descriptor table as a collection of descriptor ranges that appear one after the other in a descriptor heap.</summary>
		[FieldOffset(4)]
		public D3D12_ROOT_DESCRIPTOR_TABLE1 DescriptorTable;
		/// <summary>A <c>D3D12_ROOT_CONSTANTS</c> structure that describes constants inline in the root signature that appear in shaders as one constant buffer.</summary>
		[FieldOffset(4)]
		public D3D12_ROOT_CONSTANTS Constants;
		/// <summary>A <c>D3D12_ROOT_DESCRIPTOR1</c> structure that describes descriptors inline in the root signature that appear in shaders.</summary>
		[FieldOffset(4)]
	...
	/// <remarks>
	/// <para>This structure is used by the <c>D3D12SerializeRootSignature</c> function and is returned by the <c>ID3D12RootSignatureDeserializer::GetRootSignatureDesc</c> method.</para>
	...
	// typedef struct D3D12_ROOT_SIGNATURE_DESC { UINT NumParameters; const D3D12_ROOT_PARAMETER *pParameters; UINT NumStaticSamplers; const D3D12_STATIC_SAMPLER_DESC *pStaticSamplers; D3D12_ROOT_SIGNATURE_FLAGS Flags; } D3D12_ROOT_SIGNATURE_DESC;
	...
	public struct D3D12_ROOT_SIGNATURE_DESC
	{
		...
		/// <summary>A combination of <c>D3D12_ROOT_SIGNATURE_FLAGS</c>-typed values that are combined by using a bitwise OR operation. The resulting value specifies options for the root signature layout.</summary>
	...
	// typedef struct D3D12_ROOT_SIGNATURE_DESC1 { UINT NumParameters; const D3D12_ROOT_PARAMETER1 *pParameters; UINT NumStaticSamplers; const D3D12_STATIC_SAMPLER_DESC *pStaticSamplers; D3D12_ROOT_SIGNATURE_FLAGS Flags; } D3D12_ROOT_SIGNATURE_DESC1;
	...
	/// <remarks>This structure is primarily intended to be used when creating pipeline state stream descriptions that contain multiple contiguous render target format descriptions.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_rt_format_array
	...
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_sample_position
	// typedef struct D3D12_SAMPLE_POSITION { INT8 X; INT8 Y; } D3D12_SAMPLE_POSITION;
	...
	public struct D3D12_SAMPLER_DESC
	{
		...
		/// <summary>A <c>D3D12_TEXTURE_ADDRESS_MODE</c>-typed value that specifies the method to use for resolving a u texture coordinate that is outside the 0 to 1 range.</summary>
		public D3D12_TEXTURE_ADDRESS_MODE AddressU;
		/// <summary>A <c>D3D12_TEXTURE_ADDRESS_MODE</c>-typed value that specifies the method to use for resolving a v texture coordinate that is outside the 0 to 1 range.</summary>
		public D3D12_TEXTURE_ADDRESS_MODE AddressV;
		/// <summary>A <c>D3D12_TEXTURE_ADDRESS_MODE</c>-typed value that specifies the method to use for resolving a w texture coordinate that is outside the 0 to 1 range.</summary>
		...
		/// <summary>A <c>D3D12_COMPARISON_FUNC</c>-typed value that specifies a function that compares sampled data against existing sampled data.</summary>
		public D3D12_COMPARISON_FUNC ComparisonFunc;
		/// <summary>RGBA border color to use if <c>D3D12_TEXTURE_ADDRESS_MODE_BORDER</c> is specified for <b>AddressU</b>, <b>AddressV</b>, or <b>AddressW</b>. Range must be between 0.0 and 1.0 inclusive.</summary>
	...
	/// <summary>Opaque data structure describing driver versioning for a serialized acceleration structure. Pass this structure into a call to <c>ID3D12Device5::CheckDriverMatchingIdentifier</c> to determine if a previously serialized acceleration structure is compatible with the current driver/device, and can therefore be deserialized and used for raytracing.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_serialized_data_driver_matching_identifier
	// typedef struct D3D12_SERIALIZED_DATA_DRIVER_MATCHING_IDENTIFIER { GUID DriverOpaqueGUID; BYTE DriverOpaqueVersioningData[16]; } D3D12_SERIALIZED_DATA_DRIVER_MATCHING_IDENTIFIER;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_serialized_raytracing_acceleration_structure_header
	...
	/// <remarks>
	/// <para>The <c>D3D12_GRAPHICS_PIPELINE_STATE_DESC</c> and <c>D3D12_COMPUTE_PIPELINE_STATE_DESC</c> objects contain <b>D3D12_SHADER_BYTECODE</b> structures that describe various shader types.</para>
	/// <para>When loading a shader from FXC/DXC, this may be the entire compiled blob as is loaded from disk.</para>
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_shader_bytecode
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_shader_cache_session_desc
	// typedef struct D3D12_SHADER_CACHE_SESSION_DESC { GUID Identifier; D3D12_SHADER_CACHE_MODE Mode; D3D12_SHADER_CACHE_FLAGS Flags; UINT MaximumInMemoryCacheSizeBytes; UINT MaximumInMemoryCacheEntries; UINT MaximumValueFileSizeBytes; UINT64 Version; } D3D12_SHADER_CACHE_SESSION_DESC;
	...
	public struct D3D12_SHADER_RESOURCE_VIEW_DESC
	{
		...
		[FieldOffset(0)]
		public DXGI_FORMAT Format;
		/// <summary>A <c>D3D12_SRV_DIMENSION</c>-typed value that specifies the resource type of the view. This type is the same as the resource type of the underlying resource. This member also determines which _SRV to use in the union below.</summary>
		[FieldOffset(4)]
		public D3D12_SRV_DIMENSION ViewDimension;
		/// <summary>A value, constructed using the <c>D3D12_ENCODE_SHADER_4_COMPONENT_MAPPING</c> macro. The <b>D3D12_SHADER_COMPONENT_MAPPING</b> enumeration specifies what values from memory should be returned when the texture is accessed in a shader via this shader resource view (SRV). For example, it can route component 1 (green) from memory, or the constant <c>0</c>, into component 2 (<c>.b</c>) of the value given to the shader.</summary>
		[FieldOffset(8)]
		...
		/// <summary>A <c>D3D12_RAYTRACING_ACCELERATION_STRUCTURE_SRV</c> structure that views the resource as a raytracing acceleration structure.</summary>
		[FieldOffset(16)]
	...
	/// <remarks>Specify an array of <b>D3D12_SO_DECLARATION_ENTRY</b> structures in the <b>pSODeclaration</b> member of a <c>D3D12_STREAM_OUTPUT_DESC</c> structure.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_so_declaration_entry
	// typedef struct D3D12_SO_DECLARATION_ENTRY { UINT Stream; LPCSTR SemanticName; UINT SemanticIndex; BYTE StartComponent; BYTE ComponentCount; BYTE OutputSlot; } D3D12_SO_DECLARATION_ENTRY;
	...
	public struct D3D12_SO_DECLARATION_ENTRY
	{
		...
		public uint Stream;
		/// <summary>Type of output element; possible values include: <b>"POSITION"</b>, <b>"NORMAL"</b>, or <b>"TEXCOORD0"</b>. Note that if <b>SemanticName</b> is <b>NULL</b> then <b>ComponentCount</b> can be greater than 4 and the described entry will be a gap in the stream out where no data will be written.</summary>
		[MarshalAs(UnmanagedType.LPStr)]
		public string SemanticName;
		...
		public uint SemanticIndex;
		/// <summary>The component of the entry to begin writing out to. Valid values are 0 to 3. For example, if you only wish to output to the y and z components of a position, <b>StartComponent</b> is 1 and <b>ComponentCount</b> is 2.</summary>
		public byte StartComponent;
		/// <summary>The number of components of the entry to write out to. Valid values are 1 to 4. For example, if you only wish to output to the y and z components of a position, <b>StartComponent</b> is 1 and <b>ComponentCount</b> is 2. Note that if <b>SemanticName</b> is <b>NULL</b> then <b>ComponentCount</b> can be greater than 4 and the described entry will be a gap in the stream out where no data will be written.</summary>
		public byte ComponentCount;
		/// <summary>The associated stream output buffer that is bound to the pipeline. The valid range for <b>OutputSlot</b> is 0 to 3.</summary>
		public byte OutputSlot;
	}

	/// <summary>Defines general properties of a state object.</summary>
	/// <remarks>The presence of this subobject in a state object is optional. If present, all exports in the state object must be associated with the same subobject (or one with a matching definition). This consistency requirement does not apply across existing collections that are included in a larger state object.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_state_object_config
	// typedef struct D3D12_STATE_OBJECT_CONFIG { D3D12_STATE_OBJECT_FLAGS Flags; } D3D12_STATE_OBJECT_CONFIG;
	[PInvokeData("d3d12.h", MSDNShortId = "NS:d3d12.D3D12_STATE_OBJECT_CONFIG")]
	[StructLayout(LayoutKind.Sequential)]
	public struct D3D12_STATE_OBJECT_CONFIG
	{
		/// <summary>A value from the <c>D3D12_STATE_OBJECT_FLAGS</c> flags enumeration that specifies the requirements for the state object.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_state_object_desc
	// typedef struct D3D12_STATE_OBJECT_DESC { D3D12_STATE_OBJECT_TYPE Type; UINT NumSubobjects; const D3D12_STATE_SUBOBJECT *pSubobjects; } D3D12_STATE_OBJECT_DESC;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_state_subobject
	// typedef struct D3D12_STATE_SUBOBJECT { D3D12_STATE_SUBOBJECT_TYPE Type; const void *pDesc; } D3D12_STATE_SUBOBJECT;
	...
	public struct D3D12_STATIC_SAMPLER_DESC
	{
		...
		public D3D12_FILTER Filter;
		/// <summary>Specifies the <c>D3D12_TEXTURE_ADDRESS_MODE</c> mode to use for resolving a <i>u</i> texture coordinate that is outside the 0 to 1 range.</summary>
		public D3D12_TEXTURE_ADDRESS_MODE AddressU;
		/// <summary>Specifies the <c>D3D12_TEXTURE_ADDRESS_MODE</c> mode to use for resolving a <i>v</i> texture coordinate that is outside the 0 to 1 range.</summary>
		public D3D12_TEXTURE_ADDRESS_MODE AddressV;
		/// <summary>Specifies the <c>D3D12_TEXTURE_ADDRESS_MODE</c> mode to use for resolving a <i>w</i> texture coordinate that is outside the 0 to 1 range.</summary>
		public D3D12_TEXTURE_ADDRESS_MODE AddressW;
		...
		public D3D12_COMPARISON_FUNC ComparisonFunc;
		/// <summary>One member of <c>D3D12_STATIC_BORDER_COLOR</c>, the border color to use if D3D12_TEXTURE_ADDRESS_MODE_BORDER is specified for AddressU, AddressV, or AddressW. Range must be between 0.0 and 1.0 inclusive.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_stream_output_buffer_view
	// typedef struct D3D12_STREAM_OUTPUT_BUFFER_VIEW { D3D12_GPU_VIRTUAL_ADDRESS BufferLocation; UINT64 SizeInBytes; D3D12_GPU_VIRTUAL_ADDRESS BufferFilledSizeLocation; } D3D12_STREAM_OUTPUT_BUFFER_VIEW;
	[PInvokeData("d3d12.h", MSDNShortId = "NS:d3d12.D3D12_STREAM_OUTPUT_BUFFER_VIEW")]
	[StructLayout(LayoutKind.Sequential, CharSet = CharSet.Unicode)]
	public struct D3D12_STREAM_OUTPUT_BUFFER_VIEW
	{
		/// <summary>A D3D12_GPU_VIRTUAL_ADDRESS (a UINT64) that points to the stream output buffer. If <b>SizeInBytes</b> is 0, this member isn't used and can be any value.</summary>
		public D3D12_GPU_VIRTUAL_ADDRESS BufferLocation;
		/// <summary>The size of the stream output buffer in bytes.</summary>
		public ulong SizeInBytes;
		/// <summary>The location of the value of how much data has been filled into the buffer, as a D3D12_GPU_VIRTUAL_ADDRESS (a UINT64). This member can't be NULL; a filled size location must be supplied (which the hardware will increment as data is output). If <b>SizeInBytes</b> is 0, this member isn't used and can be any value.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_stream_output_desc
	// typedef struct D3D12_STREAM_OUTPUT_DESC { const D3D12_SO_DECLARATION_ENTRY *pSODeclaration; UINT NumEntries; const UINT *pBufferStrides; UINT NumStrides; UINT RasterizedStream; } D3D12_STREAM_OUTPUT_DESC;
	...
	/// <remarks>Depending on the flags specified in the optional <c>D3D12_STATE_OBJECT_CONFIG</c> subobject for opting into cross linkage, the exports being associated don’t necessarily have to be present in the current state object, or one that has been seen yet, to be resolved later, on raytracing pipeline state object (RTPSO) creation for example.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_subobject_to_exports_association
	// typedef struct D3D12_SUBOBJECT_TO_EXPORTS_ASSOCIATION { const D3D12_STATE_SUBOBJECT *pSubobjectToAssociate; UINT NumExports; LPCWSTR *pExports; } D3D12_SUBOBJECT_TO_EXPORTS_ASSOCIATION;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_subresource_data
	...
	public struct D3D12_SUBRESOURCE_FOOTPRINT
	{
		...
		public uint Depth;
		/// <summary>The row pitch, or width, or physical size, in bytes, of the subresource data. This must be a multiple of D3D12_TEXTURE_DATA_PITCH_ALIGNMENT (256), and must be greater than or equal to the size of the data within a row.</summary>
		public uint RowPitch;
	}

	/// <summary>Describes subresource data.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_subresource_info
	// typedef struct D3D12_SUBRESOURCE_INFO { UINT64 Offset; UINT RowPitch; UINT DepthPitch; } D3D12_SUBRESOURCE_INFO;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_subresource_range_uint64
	// typedef struct D3D12_SUBRESOURCE_RANGE_UINT64 { UINT Subresource; D3D12_RANGE_UINT64 Range; } D3D12_SUBRESOURCE_RANGE_UINT64;
	...
	/// <remarks>This structure is used by the <c>GetResourceTiling</c> method.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_subresource_tiling
	// typedef struct D3D12_SUBRESOURCE_TILING { UINT WidthInTiles; UINT16 HeightInTiles; UINT16 DepthInTiles; UINT StartTileIndexInOverallResource; } D3D12_SUBRESOURCE_TILING;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex1d_array_dsv
	// typedef struct D3D12_TEX1D_ARRAY_DSV { UINT MipSlice; UINT FirstArraySlice; UINT ArraySize; } D3D12_TEX1D_ARRAY_DSV;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex1d_array_rtv
	// typedef struct D3D12_TEX1D_ARRAY_RTV { UINT MipSlice; UINT FirstArraySlice; UINT ArraySize; } D3D12_TEX1D_ARRAY_RTV;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex1d_array_srv
	// typedef struct D3D12_TEX1D_ARRAY_SRV { UINT MostDetailedMip; UINT MipLevels; UINT FirstArraySlice; UINT ArraySize; FLOAT ResourceMinLODClamp; } D3D12_TEX1D_ARRAY_SRV;
	[PInvokeData("d3d12.h", MSDNShortId = "NS:d3d12.D3D12_TEX1D_ARRAY_SRV")]
	[StructLayout(LayoutKind.Sequential)]
	public struct D3D12_TEX1D_ARRAY_SRV
	{
		/// <summary>Index of the most detailed mipmap level to use; this number is between 0 and <b>MipLevels</b> (from the original Texture1D for which <c>ID3D12Device::CreateShaderResourceView</c> creates a view) -1.</summary>
		...
		public uint ArraySize;
		/// <summary>
		///   <para>Specifies the minimum mipmap level that you can access. Specifying 0.0f means that you can access all of the mipmap levels. Specifying 3.0f means that you can access mipmap levels from 3.0f to MipCount - 1.</para>
		///   <para>We recommend that you don't set MostDetailedMip and ResourceMinLODClamp at the same time. Instead, set one of those two members to 0 (to get default behavior). That's because MipLevels is interpreted differently in conjunction with different fields:</para>
		///   <list type="bullet">
		///     <item>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex1d_array_uav
	// typedef struct D3D12_TEX1D_ARRAY_UAV { UINT MipSlice; UINT FirstArraySlice; UINT ArraySize; } D3D12_TEX1D_ARRAY_UAV;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex1d_dsv
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex1d_rtv
	...
	/// <para>As an example, assuming <b>MostDetailedMip</b> = 6 and <b>MipLevels</b> = 2, the view will have access to 2 mipmap levels, 6 and 7, of the original texture for which <c>ID3D12Device::CreateShaderResourceView</c> creates the view. In this situation, <b>MostDetailedMip</b> is greater than the <b>MipLevels</b> in the view. However, <b>MostDetailedMip</b> is not greater than the <b>MipLevels</b> in the original resource.</para>
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex1d_srv
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex1d_uav
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex2d_array_dsv
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex2d_array_rtv
	...
	/// <remarks>This structure is one member of a shader-resource-view description, <c>D3D12_SHADER_RESOURCE_VIEW_DESC</c>.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex2d_array_srv
	...
	{
		/// <summary>Index of the most detailed mipmap level to use; this number is between 0 and <b>MipLevels</b> -1 (where <b>MipLevels</b> is from the original Texture2D for which <c>ID3D12Device::CreateShaderResourceView</c> creates a view).</summary>
		public uint MostDetailedMip;
		/// <summary>
		///   <para>The maximum number of mipmap levels for the view of the texture. See the remarks in <c>D3D12_TEX1D_SRV</c>.</para>
		///   <para>Set to -1 to indicate all the mipmap levels from <b>MostDetailedMip</b> on down to least detailed.</para>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex2d_array_uav
	// typedef struct D3D12_TEX2D_ARRAY_UAV { UINT MipSlice; UINT FirstArraySlice; UINT ArraySize; UINT PlaneSlice; } D3D12_TEX2D_ARRAY_UAV;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex2d_dsv
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex2d_rtv
	...
	{
		/// <summary>The index of the most detailed mipmap level to use; this number is between 0 and <b>MipLevels</b> (from the original Texture2D for which <c>ID3D12Device::CreateShaderResourceView</c> creates a view) -1.</summary>
		public uint MostDetailedMip;
		/// <summary>The maximum number of mipmap levels for the view of the texture. See the remarks in <c>D3D12_TEX1D_SRV</c>. Set to -1 to indicate all the mipmap levels from <b>MostDetailedMip</b> on down to least detailed.</summary>
		public uint MipLevels;
		/// <summary>The index (plane slice number) of the plane to use in the texture.</summary>
		public uint PlaneSlice;
		/// <summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex2d_uav
	...
	/// <remarks>Use this structure with a <c>D3D12_DEPTH_STENCIL_VIEW_DESC</c> structure to view the resource as an array of multi sampled 2D textures.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex2dms_array_dsv
	// typedef struct D3D12_TEX2DMS_ARRAY_DSV { UINT FirstArraySlice; UINT ArraySize; } D3D12_TEX2DMS_ARRAY_DSV;
	...
	/// <remarks>Use this structure with a <c>D3D12_RENDER_TARGET_VIEW_DESC</c> structure to view the resource as an array of multi sampled 2D textures.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex2dms_array_rtv
	// typedef struct D3D12_TEX2DMS_ARRAY_RTV { UINT FirstArraySlice; UINT ArraySize; } D3D12_TEX2DMS_ARRAY_RTV;
	...
	// typedef struct D3D12_TEX2DMS_ARRAY_SRV { UINT FirstArraySlice; UINT ArraySize; } D3D12_TEX2DMS_ARRAY_SRV;
	...
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex2dms_dsv
	// typedef struct D3D12_TEX2DMS_DSV { UINT UnusedField_NothingToDefine; } D3D12_TEX2DMS_DSV;
	...
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex2dms_rtv
	// typedef struct D3D12_TEX2DMS_RTV { UINT UnusedField_NothingToDefine; } D3D12_TEX2DMS_RTV;
	...
	// typedef struct D3D12_TEX2DMS_SRV { UINT UnusedField_NothingToDefine; } D3D12_TEX2DMS_SRV;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex3d_rtv
	...
	/// <remarks>This structure is one member of a shader-resource-view description, <c>D3D12_SHADER_RESOURCE_VIEW_DESC</c>.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex3d_srv
	...
	{
		/// <summary>Index of the most detailed mipmap level to use; this number is between 0 and <b>MipLevels</b> (from the original Texture3D for which <c>ID3D12Device::CreateShaderResourceView</c> creates a view) -1.</summary>
		public uint MostDetailedMip;
		/// <summary>
		///   <para>The maximum number of mipmap levels for the view of the texture. See the remarks in <c>D3D12_TEX1D_SRV</c>.</para>
		///   <para>Set to -1 to indicate all the mipmap levels from <b>MostDetailedMip</b> on down to least detailed.</para>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tex3d_uav
	...
	/// <remarks>This structure is one member of a shader-resource-view description, <c>D3D12_SHADER_RESOURCE_VIEW_DESC</c>.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_texcube_array_srv
	// typedef struct D3D12_TEXCUBE_ARRAY_SRV { UINT MostDetailedMip; UINT MipLevels; UINT First2DArrayFace; UINT NumCubes; FLOAT ResourceMinLODClamp; } D3D12_TEXCUBE_ARRAY_SRV;
	[PInvokeData("d3d12.h", MSDNShortId = "NS:d3d12.D3D12_TEXCUBE_ARRAY_SRV")]
	[StructLayout(LayoutKind.Sequential)]
	public struct D3D12_TEXCUBE_ARRAY_SRV
	{
		/// <summary>Index of the most detailed mipmap level to use; this number is between 0 and <b>MipLevels</b> (from the original TextureCube for which <c>ID3D12Device::CreateShaderResourceView</c> creates a view) -1.</summary>
		public uint MostDetailedMip;
		/// <summary>
		///   <para>The maximum number of mipmap levels for the view of the texture. See the remarks in <c>D3D12_TEX1D_SRV</c>.</para>
		///   <para>Set to -1 to indicate all the mipmap levels from <b>MostDetailedMip</b> on down to least detailed.</para>
	...
	/// <remarks>This structure is one member of a shader-resource-view description, <c>D3D12_SHADER_RESOURCE_VIEW_DESC</c>.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_texcube_srv
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_texture_copy_location
	// typedef struct D3D12_TEXTURE_COPY_LOCATION { ID3D12Resource *pResource; D3D12_TEXTURE_COPY_TYPE Type; union { D3D12_PLACED_SUBRESOURCE_FOOTPRINT PlacedFootprint; UINT SubresourceIndex; }; } D3D12_TEXTURE_COPY_LOCATION;
	...
	public struct D3D12_TEXTURE_COPY_LOCATION
	{
		...
		/// <summary>Specifies a texture layout, with offset, dimensions, and pitches, for the hardware to understand how to treat a section of a buffer resource as a multi-dimensional texture. To fill-in the correct data for a <c>CopyTextureRegion</c> call, see <c>D3D12_PLACED_SUBRESOURCE_FOOTPRINT</c>.</summary>
	...
	/// <remarks>This structure is used by the <c>GetResourceTiling</c> method.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_tile_shape
	// typedef struct D3D12_TILE_SHAPE { UINT WidthInTexels; UINT HeightInTexels; UINT DepthInTexels; } D3D12_TILE_SHAPE;
	...
	public struct D3D12_UNORDERED_ACCESS_VIEW_DESC
	{
		...
		[FieldOffset(0)]
		public DXGI_FORMAT Format;
		/// <summary>A <c>D3D12_UAV_DIMENSION</c>-typed value that specifies the resource type of the view. This type specifies how the resource will be accessed. This member also determines which _UAV to use in the union below.</summary>
		[FieldOffset(4)]
	...
	/// <summary>Represents versioned Device Removed Extended Data (DRED) data, so that debuggers and debugger extensions can access DRED data.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_versioned_device_removed_extended_data
	// typedef struct D3D12_VERSIONED_DEVICE_REMOVED_EXTENDED_DATA { D3D12_DRED_VERSION Version; union { D3D12_DEVICE_REMOVED_EXTENDED_DATA Dred_1_0; D3D12_DEVICE_REMOVED_EXTENDED_DATA1 Dred_1_1; D3D12_DEVICE_REMOVED_EXTENDED_DATA2 Dred_1_2; D3D12_DEVICE_REMOVED_EXTENDED_DATA3 Dred_1_3; }; } D3D12_VERSIONED_DEVICE_REMOVED_EXTENDED_DATA;
	...
	{
		/// <summary>A <c>D3D12_DRED_VERSION</c> value, specifying a DRED version. This value determines which inner data member (of the union) is active.</summary>
		[FieldOffset(0)]
	...
	// typedef struct D3D12_VERSIONED_ROOT_SIGNATURE_DESC { D3D_ROOT_SIGNATURE_VERSION Version; union { D3D12_ROOT_SIGNATURE_DESC Desc_1_0; D3D12_ROOT_SIGNATURE_DESC1 Desc_1_1; D3D12_ROOT_SIGNATURE_DESC2 Desc_1_2; }; } D3D12_VERSIONED_ROOT_SIGNATURE_DESC;
	...
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_view_instance_location
	// typedef struct D3D12_VIEW_INSTANCE_LOCATION { UINT ViewportArrayIndex; UINT RenderTargetArrayIndex; } D3D12_VIEW_INSTANCE_LOCATION;
	...
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_view_instancing_desc
	// typedef struct D3D12_VIEW_INSTANCING_DESC { UINT ViewInstanceCount; const D3D12_VIEW_INSTANCE_LOCATION *pViewInstanceLocations; D3D12_VIEW_INSTANCING_FLAGS Flags; } D3D12_VIEW_INSTANCING_DESC;
	...
	/// <remarks>Pass an array of these structures to the <i>pViewports</i> parameter in a call to <c>ID3D12GraphicsCommandList::RSSetViewports</c> to set viewports for the display.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_viewport
	// typedef struct D3D12_VIEWPORT { FLOAT TopLeftX; FLOAT TopLeftY; FLOAT Width; FLOAT Height; FLOAT MinDepth; FLOAT MaxDepth; } D3D12_VIEWPORT;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/ns-d3d12-d3d12_writebufferimmediate_parameter
	// typedef struct D3D12_WRITEBUFFERIMMEDIATE_PARAMETER { D3D12_GPU_VIRTUAL_ADDRESS Dest; UINT32 Value; } D3D12_WRITEBUFFERIMMEDIATE_PARAMETER;
