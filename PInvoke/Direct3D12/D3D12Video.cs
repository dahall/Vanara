...
public static partial class D3D12
{
	...
	{
		/// <summary>
		/// <para>Check if a decode profile, bitstream encryption, resolution, and format are supported. The result is a D3D12_VIDEO_DECODE_TIER indicating the level of support. The associated data structure is D3D12_FEATURE_DATA_VIDEO_DECODE_SUPPORT.</para>
		/// </summary>
		D3D12_FEATURE_VIDEO_DECODE_SUPPORT,
		/// <summary>
		/// <para>Retrieve the list of decode profiles supported by the adapter. Call CheckFeatureSupport specifying the feature D3D12_FEATURE_VIDEO_DECODE_PROFILE_COUNT to get the number of profiles before calling CheckFeatureSupport for the D3D12_FEATURE_VIDEO_DECODE_PROFILES feature. The associated data structure is D3D12_FEATURE_DATA_VIDEO_DECODE_PROFILES.</para>
		/// </summary>
		...
		/// features. The associated data structure is
		///  D3D12_FEATURE_DATA_VIDEO_PROCESS_REFERENCE_INFO.</para>
		/// </summary>
		D3D12_FEATURE_VIDEO_PROCESS_REFERENCE_INFO,
		/// <summary>
		/// <para>Checks the allocation size of a video decoder heap. The associated data structure is D3D12_FEATURE_DATA_VIDEO_DECODER_HEAP_SIZE. For information on residency budgeting for heaps, see Residency.</para>
		/// </summary>
		D3D12_FEATURE_VIDEO_DECODER_HEAP_SIZE,
		/// <summary>
		/// <para>Checks the allocation size of a video processor heap. The associated data structure is D3D12_FEATURE_DATA_VIDEO_PROCESSOR_SIZE. For information on residency budgeting for heaps, see Residency.</para>
		/// </summary>
		...
		///  D3D12_FEATURE_DATA_VIDEO_DECODE_HISTOGRAM.</para>
		/// </summary>
		...
		/// <summary>
		/// <para>Value: 20 
		/// Retrieves the supported resolutions, search block sizes, and precision for motion estimation. The associated data structure is
		///  D3D12_FEATURE_DATA_VIDEO_MOTION_ESTIMATOR.</para>
		/// </summary>
		...
		/// <summary>
		/// <para>Value: 23 Retrieves a list of D3D12_VIDEO_EXTENSION_COMMAND_INFO structures describing video extension commands. The associated data structure is D3D12_FEATURE_DATA_VIDEO_EXTENSION_COMMAND_COUNT.</para>
		/// </summary>
		...
		/// <summary>
		/// <para>Value: 25 Retrieves a list of D3D12_VIDEO_EXTENSION_COMMAND_PARAMETER_INFO 
		/// structures describing video extension command parameters for the specified parameter stage. The associated data structure is
		///  D3D12_FEATURE_DATA_VIDEO_EXTENSION_COMMAND_PARAMETERS.</para>
		/// </summary>
	...
	public enum D3D12_VIDEO_DECODE_TIER
	{
		...
		/// operations, the decoder may set the
		///  D3D12_VIDEO_DECODE_CONFIGURATION_FLAG_REFERENCE_ONLY_ALLOCATIONS_REQUIRED configuration flag in the D3D12_FEATURE_DATA_VIDEO_DECODE_SUPPORT structure when queried for profile support. This flag indicates that the application must allocate references with the D3D12_RESOURCE_FLAG_VIDEO_DECODE_REFERENCE_ONLY flag. The application should use the D3D12_VIDEO_DECODE_CONVERSION_ARGUMENTS 
		/// structure to set a reference-only output if the output is needed as a future reference frame. The output frame passed to
		/// </para>
		...
		/// </para>
		/// <para>- The first slice must begin on a 128 Byte boundary. The offset set in the D3D12_VIDEO_DECODE_COMPRESSED_BITSTREAM structure must be a multiple of 128 Bytes.</para>
		/// <para>- Decoding is supported from buffers allocated with D3D12_MEMORY_POOL_L0. This is always system memory, but still a D3D12 buffer.</para>
		/// <para>
	...
	public enum D3D12_VIDEO_ENCODER_AV1_COMP_PREDICTION_TYPE
	{
		...
		/// <summary>Indicates that the mode info for inter blocks contains the syntax element comp_mode that indicates whether to use single or compound reference prediction. Equivalent to AV1 syntax reference_select equal to 1.</summary>
	...
	public enum D3D12_VIDEO_ENCODER_AV1_FEATURE_FLAGS : uint
	{
		...
		/// <summary>Indicates if support is available for interintra, where the mode info for inter blocks may contain the syntax element interintra. Equal to 0 specifies that the syntax element interintra will not be present.</summary>
		D3D12_VIDEO_ENCODER_AV1_FEATURE_FLAG_INTERINTRA_COMPOUND = 0x8,
		/// <summary>Indicates if support is available for masked compound, where the mode info for inter blocks may contain the syntax element compound_type. Equal to 0 specifies that the syntax element compound_type will not be present.</summary>
		D3D12_VIDEO_ENCODER_AV1_FEATURE_FLAG_MASKED_COMPOUND = 0x10,
		/// <summary>Equal to 1 indicates that the syntax element motion_mode may be present. If equal to 0 indicates that the syntax element motion_mode will not be present (this means that LOCALWARP cannot be signaled if this flag is equal to 0).</summary>
		D3D12_VIDEO_ENCODER_AV1_FEATURE_FLAG_WARPED_MOTION = 0x20,
		/// <summary>Indicates if support is available for dual filter mode, where the inter prediction filter type may be specified independently in the horizontal and vertical directions. If the flag is equal to 0, only one filter type may be specified, which is then used in both directions.</summary>
		...
		/// <summary>Indicates if support is available for frame level control on palette encoding; Equal to 0 indicates that palette encoding is never used.</summary>
		...
		/// <summary>Indicates if support is available for use_ref_frame_mvs to be configured on a per frame basis. Equal to 0 specifies that use_ref_frame_mvs syntax element will not be used.</summary>
		D3D12_VIDEO_ENCODER_AV1_FEATURE_FLAG_FRAME_REFERENCE_MOTION_VECTORS = 0x4000,
		/// <summary>Indicates if support is available for usage of tools based on the values of order hints. Equal to 0 indicates that tools based on order hints are not supported and can’t be enabled.</summary>
		D3D12_VIDEO_ENCODER_AV1_FEATURE_FLAG_ORDER_HINT_TOOLS = 0x8000,
		/// <summary>Indicates if the driver can perform segmentation without API Client input and return segmentation_params() information in D3D12_VIDEO_ENCODER_AV1_POST_ENCODE_VALUES. Driver will write the segment map in the compressed bitstream.</summary>
		D3D12_VIDEO_ENCODER_AV1_FEATURE_FLAG_AUTO_SEGMENTATION = 0x10000,
		/// <summary>Indicates if the driver supports the API Client passing customized segmentation segmentation_params() as well as the segment map and driver will honor exactly.</summary>
	...
	public enum D3D12_VIDEO_ENCODER_AV1_PICTURE_CONTROL_FLAGS
	{
		...
		/// <summary>Related to AV1 syntax skip_mode_present. skip_mode element will be present for this frame if this flag is set. Please check support in AV1 query caps before enabling this feature.</summary>
		D3D12_VIDEO_ENCODER_AV1_PICTURE_CONTROL_FLAG_ENABLE_SKIP_MODE = 0x8,

		/// <summary>Related to AV1 syntax use_ref_frame_mvs. Equal to 1 specifies that motion vector information from a previous frame can be used when encoding the current frame. use_ref_frame_mvs equal to 0 specifies that this information will not be used.</summary>
		D3D12_VIDEO_ENCODER_AV1_PICTURE_CONTROL_FLAG_FRAME_REFERENCE_MOTION_VECTORS = 0x10,

		/// <summary>Equal to 1 specifies that force_integer_mv may be enabled on a per frame basis. Equal to 0 specifies that force_integer_mv syntax element will not be used. Please check support in AV1 query caps before enabling this feature.</summary>
		...
		/// <summary>Enables automatic (performed by driver without API Client input) segmentation for the current frame. Requires D3D12_VIDEO_ENCODER_AV1_FEATURE_FLAG_AUTO_SEGMENTATION. This flag must not be combined with D3D12_VIDEO_ENCODER_AV1_PICTURE_CONTROL_FLAG_ENABLE_FRAME_SEGMENTATION_CUSTOM.</summary>
		D3D12_VIDEO_ENCODER_AV1_PICTURE_CONTROL_FLAG_ENABLE_FRAME_SEGMENTATION_AUTO = 0x200,

		/// <summary>Enables customized segmentation with the API Client sending the driver segmentation config and segment map. Requires D3D12_VIDEO_ENCODER_AV1_FEATURE_FLAG_CUSTOM_SEGMENTATION. This flag must not be combined with D3D12_VIDEO_ENCODER_AV1_PICTURE_CONTROL_FLAG_ENABLE_FRAME_SEGMENTATION_AUTO.</summary>
	...
	/// <summary>
	///   <para>Specifies for which AV1 encoding features, the underlying encoder is able to override (partially or totally) the associated AV1 syntax values or honor API Client exact configuration input otherwise.</para>
	///   <para>When the bitflag is SET for a given feature, the driver receives the related API Client input and is able to override all or certain parameters of the associated structure with the given reported flag, which will then write back in D3D12_VIDEO_ENCODER_AV1_POST_ENCODE_VALUES with the final values for the API Client to re-pack the AV1 headers accordingly. API Client can compare this to the associated input structure to determine the driver changes (if any).</para>
	///   <para>When the bitflag is NOT SET for a given feature, the driver honors the related API Client input exactly and copies the input values in D3D12_VIDEO_ENCODER_AV1_POST_ENCODE_VALUES. This way the client can always copy the post encode values to pack the headers directly.</para>
	/// </summary>
	[PInvokeData("d3d12video.h")]
	[Flags]
	public enum D3D12_VIDEO_ENCODER_AV1_POST_ENCODE_VALUES_FLAGS
	{
		...
		/// <remarks>
		///   <list type="bullet">
		///     <item>When API Client selects D3D12_VIDEO_ENCODER_AV1_COMP_PREDICTION_TYPE_COMPOUND_REFERENCE and this flag is set, the driver will return D3D12_VIDEO_ENCODER_AV1_COMP_PREDICTION_TYPE in post encode values. The returned value must be used to code reference_select = 0 (SINGLE) or reference_select = 1 (COMPOUND) syntax accordingly.</item>
		///     <item>When API Client selects D3D12_VIDEO_ENCODER_AV1_COMP_PREDICTION_TYPE_COMPOUND_SINGLE and this flag is set, the driver will return D3D12_VIDEO_ENCODER_AV1_COMP_PREDICTION_TYPE_COMPOUND_SINGLE and reference_select must be coded as 0 (SINGLE).</item>
		///   </list>
		/// </remarks>
		D3D12_VIDEO_ENCODER_AV1_POST_ENCODE_VALUES_FLAG_COMPOUND_PREDICTION_MODE = 0x40,
		/// <summary>Related to PrimaryRefFrame element in D3D12VIDEO_ENCODER_AV1_PICTURE_CONTROL_CODEC_DATA. Used to code AV1 element syntax _primary_ref_frame in uncompressed_header().</summary>
		D3D12_VIDEO_ENCODER_AV1_POST_ENCODE_VALUES_FLAG_PRIMARY_REF_FRAME = 0x80,
		/// <summary>
		///   <para>When the flag is reported by the driver, the driver may reorder/remap (but not change the number of references) of the D3D12_VIDEO_ENCODER_AV1_POST_ENCODE_VALUES.ReferenceIndices array output, based on the user input D3D12_VIDEO_ENCODER_AV1_PICTURE_CONTROL_CODEC_DATA.ReferenceIndices. Otherwise, driver must copy each array entry of this post encode output parameter as-is from D3D12_VIDEO_ENCODER_AV1_PICTURE_CONTROL_CODEC_DATA.ReferenceIndices.</para>
	...
	/// <summary>Corresponds to the the size of loop restoration units in units of samples in the current plane. The enum values are based on lr_unit_shift and lr_uv_shift in lr_params() AV1 syntax and the RESTORATION_TILESIZE_MAX(256) AV1 spec constant.</summary>
	[PInvokeData("d3d12video.h")]
	...
	public enum D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_H264_FLAGS
	{
		...
		///  D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT_H264_FLAG_CONSTRAINED_INTRAPREDICTION_SUPPORT. This refers to constrained_intra_pred_flag in the picture parameter set (PPS).</para>
		/// </summary>
	...
	public enum D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT_HEVC_FLAGS
	{
		...
		/// <summary>
		/// <para>When this flag is set, indicates that when encoding frames with type D3D12_VIDEO_ENCODER_FRAME_TYPE_HEVC_P_FRAME 
		/// , they will be written as low delay B-Frames in the compressed bitstream. When this flag is not set, indicates that P frames
		/// will be written in the compressed bitstream.
		///  Note 
		/// When operating under this mode, it is the caller's responsibility to code the correct frame type in AUD_NUT and other parts of
		/// the HEVC bitstream, taking into account that P frames will be treated as generalized B frames with only references to past
		/// frames in POC order.
		/// </para>
		/// </summary>
	...
	public enum D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA_H264_FLAGS
	{
		...
		/// . This mode restricts the motion vector search range to the region box of the current slice, i.e. motion vectors outside the
		/// slice boundary can't be used.
		/// </para>
		/// </summary>
	...
	public enum D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA_HEVC_FLAGS
	{
		...
		///  D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT_FLAGS_HEVC_INTRA_SLICE_CONSTRAINED_ENCODING_SUPPORT 
		/// . This mode restricts the motion vector search range to the region box of the current slice, i.e. motion vectors outside the
		/// slice boundary can't be used.
		/// </para>
		/// </summary>
	...
	public enum D3D12_VIDEO_ENCODER_RATE_CONTROL_FLAGS
	{
		...
		/// <summary>
		/// <para>If the selected rate control is D3D12_VIDEO_ENCODER_RATE_CONTROL_MODE_ABSOLUTE_QP_MAP , this flag has no effect since the QP values in</para>
		...
		/// incremented/decremented on top of the QP values decided by the rate control algorithm or the baseline QP constant set in CQP mode.
		///  Note 
		/// Using delta QP adjustment along with some active rate control modes may violate bitrate constraints as it's explicitly altering
		/// the QP values that were selected by rate control budgeting algorithm.
		/// </para>
		/// </summary>
		D3D12_VIDEO_ENCODER_RATE_CONTROL_FLAG_ENABLE_DELTA_QP = 0x1,
		/// <summary>
		/// <para>If D3D12_VIDEO_ENCODER_SUPPORT_FLAGS 
		/// is supported, Enables the rate control algorithm to optimize bitrate usage by selecting QP values based on statistics collected
		/// by doing frame analysis on a first pass.
		/// </para>
		/// </summary>
		...
		/// <summary>
		/// <para>When D3D12_VIDEO_ENCODER_SUPPORT_FLAG_RATE_CONTROL_MAX_FRAME_SIZE 
		/// is supported, the rate control algorithm will limit the maximum size per frame to the specified parameter in the rate control configuration.
		/// </para>
		/// </summary>
	...
	{
		/// <summary>
		/// <para>No rate control budgeting. Each EncodeFrame call will interpret the the QP values in the pRateControlQPMap field of the D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA_H264 or D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA_HEVC structure as a map of absolute QP values.</para>
		/// </summary>
	...
	public enum D3D12_VIDEO_EXTENSION_COMMAND_PARAMETER_FLAGS
	{
		...
		/// <summary>
		/// <para>Value: 0x1 The resource parameter is read. This flag is for ID3D12Resource only and is not valid for simple data type parameters.</para>
		/// </summary>
		D3D12_VIDEO_EXTENSION_COMMAND_PARAMETER_FLAG_READ = 0x1,
		/// <summary>
		/// <para>Value: 0x2 The resource parameter is written. This flag is for ID3D12Resource only and is not valid for simple data type parameters.</para>
		/// </summary>
	...
	public enum D3D12_VIDEO_EXTENSION_COMMAND_PARAMETER_TYPE
	{
		...
		/// <summary>
		/// <para>Value: 10 A D3D12DDI_HRESOURCE handle. The caller must use resource barriers to transition to the state appropriate for the parameter.</para>
		/// </summary>
	...
	public enum D3D12_VIDEO_PROCESS_ALPHA_FILL_MODE
	{
		...
		///  AlphaFillModeSourceStreamIndex member of D3D12_VIDEO_PROCESS_OUTPUT_STREAM_ARGUMENTS.</para>
		/// <para>
	...
	public enum D3D12_VIDEO_PROCESS_DEINTERLACE_FLAGS : uint
	{
		...
		///  PastFrames and FutureFrames output fields of the D3D12_FEATURE_DATA_VIDEO_PROCESS_REFERENCE_INFO populated by a call to</para>
		/// <para>ID3D12VideoDevice::CheckFeatureSupport</para>
		/// <para>when the feature specified is D3D12_FEATURE_VIDEO_PROCESS_REFERENCE_INFO. If the video processor doesn’t have the necessary number of reference frames, it falls back to bob deinterlacing.</para>
		/// </summary>
	...
	public enum D3D12_VIDEO_PROCESS_FEATURE_FLAGS
	{
		...
		/// <summary>
		/// <para>The video processor can set alpha values on the output pixels. The alpha fill mode is used in D3D12_VIDEO_PROCESS_OUTPUT_STREAM_DESC. D3D12_VIDEO_PROCESS_ALPHA_FILL_MODE_OPAQUE 
		/// must be always supported. The background, destination, and source stream modes are only supported when the driver reports D3D12_VIDEO_PROCESS_FEATURE_FLAG_ALPHA_FILL.
		/// </para>
		/// </summary>
		D3D12_VIDEO_PROCESS_FEATURE_FLAG_ALPHA_FILL = 0x1,
		/// <summary>
		/// <para>The video processor can perform luma keying. Luma keying is configured via the D3D12_VIDEO_PROCESS_LUMA_KEY member of the D3D12_VIDEO_PROCESS_INPUT_STREAM_ARGUMENTS structure. For more information see &lt;a href=ns-d3d12video-d3d12_video_process_luma_key""&gt;D3D12_VIDEO_PROCESS_LUMA_KEY.</para>
		/// </summary>
		...
		/// <summary>
		/// <para>Alpha blending and a planar alpha may be set in the AlphaBlending member of the D3D12_VIDEO_PROCESS_INPUT_STREAM_ARGUMENTS structure. For more information see D3D12_VIDEO_PROCESS_ALPHA_BLENDING.</para>
		/// </summary>
		D3D12_VIDEO_PROCESS_FEATURE_FLAG_ALPHA_BLENDING = 0x20,
		/// <summary>
		/// <para>The driver supports changing the pixel aspect ratio. If the driver does not report this capability, then the SourceAspectRatio and DestinationAspectRatio members of D3D12_VIDEO_PROCESS_INPUT_STREAM_ARGUMENTS structure must indicate a 1:1 aspect ratio.</para>
		/// </summary>
	...
	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_DECODE_CONVERSION_SUPPORT</c>. Retrieves the list of supported profiles. Check if a colorspace conversion, format conversion, and scale are supported.</summary>
	/// <remarks>If the colorspace and format conversion is supported, ScaleFlags will have the <c>D3D12_VIDEO_SCALE_SUPPORT_FLAGS</c> set. Callers should check the <c>D3D12_VIDEO_SIZE_RANGE</c> field to determine if the requested scale is supported.</remarks>
	...
	// typedef struct D3D12_FEATURE_DATA_VIDEO_DECODE_CONVERSION_SUPPORT { UINT NodeIndex; D3D12_VIDEO_DECODE_CONFIGURATION Configuration; D3D12_VIDEO_SAMPLE DecodeSample; D3D12_VIDEO_FORMAT OutputFormat; DXGI_RATIONAL FrameRate; UINT BitRate; D3D12_VIDEO_DECODE_CONVERSION_SUPPORT_FLAGS SupportFlags; D3D12_VIDEO_SCALE_SUPPORT ScaleSupport; } D3D12_FEATURE_DATA_VIDEO_DECODE_CONVERSION_SUPPORT;
	...
	public struct D3D12_FEATURE_DATA_VIDEO_DECODE_CONVERSION_SUPPORT
	{
		...
		public uint BitRate;
		/// <summary>A combination of values from the <c>D3D12_VIDEO_DECODE_CONVERSION_SUPPORT_FLAGS</c> indicating the support for the specified conversion.</summary>
	...
	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_DECODE_FORMAT</c>. Retrieves the list of supported formats.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_feature_data_video_decode_formats
	// typedef struct D3D12_FEATURE_DATA_VIDEO_DECODE_FORMATS { UINT NodeIndex; D3D12_VIDEO_DECODE_CONFIGURATION Configuration; UINT FormatCount; DXGI_FORMAT *pOutputFormats; } D3D12_FEATURE_DATA_VIDEO_DECODE_FORMATS;
	...
	public struct D3D12_FEATURE_DATA_VIDEO_DECODE_FORMATS
	{
		...
		public D3D12_VIDEO_DECODE_CONFIGURATION Configuration;
		/// <summary>The number of formats to retrieve. This number must match the value returned from a call <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_DECODE_FORMAT_COUNT</c>.</summary>
	...
	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_DECODE_HISTOGRAM</c>. Retrieves the histogram capabilities for the specified decoder configuration.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_feature_data_video_decode_histogram
	// typedef struct D3D12_FEATURE_DATA_VIDEO_DECODE_HISTOGRAM { UINT NodeIndex; GUID DecodeProfile; UINT Width; UINT Height; DXGI_FORMAT DecodeFormat; D3D12_VIDEO_DECODE_HISTOGRAM_COMPONENT_FLAGS Components; UINT BinCount; UINT CounterBitDepth; } D3D12_FEATURE_DATA_VIDEO_DECODE_HISTOGRAM;
	...
	public struct D3D12_FEATURE_DATA_VIDEO_DECODE_HISTOGRAM
	{
		/// <summary>For single GPU operation, set this to zero. If there are multiple GPU nodes, set a bit to identify the node (the device's physical adapter) to which the command queue applies. Each bit in the mask corresponds to a single node. Only 1 bit may be set.</summary>
		public uint NodeIndex;
		/// <summary>A GUID representing the decode profile for which histogram capabilities will be queried. Get a list of available profile GUIDs by calling <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_DECODE_PROFILES</c>.</summary>
		...
		/// <summary>A bitwise OR combination of values from the <c>D3D12_VIDEO_DECODE_HISTOGRAM_COMPONENT_FLAGS</c> enumeration specifying the components of a DXGI_FORMAT for which histogram support will be queried.</summary>
	...
	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_DECODE_PROFILES</c>. Retrieves the list of supported profiles.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_feature_data_video_decode_profiles
	// typedef struct D3D12_FEATURE_DATA_VIDEO_DECODE_PROFILES { UINT NodeIndex; UINT ProfileCount; GUID *pProfiles; } D3D12_FEATURE_DATA_VIDEO_DECODE_PROFILES;
	...
	public struct D3D12_FEATURE_DATA_VIDEO_DECODE_PROFILES
	{
		/// <summary>For single GPU operation, set this to zero. If there are multiple GPU nodes, set a bit to identify the node (the device's physical adapter) to which the command queue applies. Each bit in the mask corresponds to a single node. Only 1 bit may be set.</summary>
		public uint NodeIndex;
		/// <summary>The number of profiles to retrieve. This number must match the value returned from a call <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_DECODE_PROFILE_COUNT</c>.</summary>
	...
	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_DECODE_SUPPORT</c>. Retrieves support information for video decoding.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_feature_data_video_decode_support
	// typedef struct D3D12_FEATURE_DATA_VIDEO_DECODE_SUPPORT { UINT NodeIndex; D3D12_VIDEO_DECODE_CONFIGURATION Configuration; UINT Width; UINT Height; DXGI_FORMAT DecodeFormat; DXGI_RATIONAL FrameRate; UINT BitRate; D3D12_VIDEO_DECODE_SUPPORT_FLAGS SupportFlags; D3D12_VIDEO_DECODE_CONFIGURATION_FLAGS ConfigurationFlags; D3D12_VIDEO_DECODE_TIER DecodeTier; } D3D12_FEATURE_DATA_VIDEO_DECODE_SUPPORT;
	...
	public struct D3D12_FEATURE_DATA_VIDEO_DECODE_SUPPORT
	{
		/// <summary>For single GPU operation, set this to zero. If there are multiple GPU nodes, set a bit to identify the node (the device's physical adapter) to which the command queue applies. Each bit in the mask corresponds to a single node. Only 1 bit may be set.</summary>
		public uint NodeIndex;
		/// <summary>A <c>D3D12_VIDEO_DECODE_CONFIGURATION</c> structure specifying the decode profile, bitstream encryption, and interlace type of the source stream.</summary>
		public D3D12_VIDEO_DECODE_CONFIGURATION Configuration;
		/// <summary>The decode width of the source stream.</summary>
		public uint Width;
		/// <summary>The decode height of the source stream</summary>
		public uint Height;
		/// <summary>The <c>DXGI_FORMAT</c> to use as the decode format. This format is the output format if no decoder conversion is specified.</summary>
		public DXGI_FORMAT DecodeFormat;
		...
		public uint BitRate;
		/// <summary>A combination of values from the <c>D3D12_VIDEO_DECODE_SUPPORT_FLAGS</c> enumeration indicating the support for video decoding. This value is populated by the call to <b>ID3D12Device::CheckFeatureSupport</b>.</summary>
		public D3D12_VIDEO_DECODE_SUPPORT_FLAGS SupportFlags;
		/// <summary>A combination of values from the <c>D3D12_VIDEO_DECODE_CONFIGURATION_FLAGS</c> enumeration describing the video decode configuration. This value is populated by the call to <b>ID3D12Device::CheckFeatureSupport</b>.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_feature_data_video_decoder_heap_size
	// typedef struct D3D12_FEATURE_DATA_VIDEO_DECODER_HEAP_SIZE { D3D12_VIDEO_DECODER_HEAP_DESC VideoDecoderHeapDesc; UINT64 MemoryPoolL0Size; UINT64 MemoryPoolL1Size; } D3D12_FEATURE_DATA_VIDEO_DECODER_HEAP_SIZE;
	...
	public struct D3D12_FEATURE_DATA_VIDEO_DECODER_HEAP_SIZE
	{
		...
		/// <summary>The allocation size of the video decoder heap in the L0 memory pool. L0 is the physical system memory pool. When the adapter is discrete/NUMA, this pool has greater bandwidth for the CPU and less bandwidth for the GPU. When the adapter is UMA, this pool is the only one which is valid. For more information, see <c>Residency</c>.</summary>
		public ulong MemoryPoolL0Size;
		/// <summary>The allocation size of the video decoder heap in the L1 memory pool. L1 is typically known as the physical video memory pool. L1 is only available when the adapter is discrete/NUMA, and has greater bandwidth for the GPU and cannot even be accessed by the CPU. When the adapter is UMA, this pool is not available. For more information, see <c>Residency</c>.</summary>
		public ulong MemoryPoolL1Size;
	}

	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_ENCODER_CODEC</c>. Retrieves a value indicating if the specified codec is supported for video encoding.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_feature_data_video_encoder_codec
	// typedef struct D3D12_FEATURE_DATA_VIDEO_ENCODER_CODEC { UINT NodeIndex; D3D12_VIDEO_ENCODER_CODEC Codec; BOOL IsSupported; } D3D12_FEATURE_DATA_VIDEO_ENCODER_CODEC;
	...
	public struct D3D12_FEATURE_DATA_VIDEO_ENCODER_CODEC
	{
		...
		public uint NodeIndex;
		/// <summary>A member of the <c>D3D12_VIDEO_ENCODER_CODEC</c> enumeration specifying the codec for which encoder support is being queried.</summary>
	...
	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT</c>. Retrieves a value indicating if the specified codec configuration support parameters are supported for the provided HEVC encoding configuration or retrieves the supported configuration for H.264 encoding.</summary>
	...
	// typedef struct D3D12_FEATURE_DATA_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT { UINT NodeIndex; D3D12_VIDEO_ENCODER_CODEC Codec; D3D12_VIDEO_ENCODER_PROFILE_DESC Profile; BOOL IsSupported; D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT CodecSupportLimits; } D3D12_FEATURE_DATA_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT;
	...
	public struct D3D12_FEATURE_DATA_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT
	{
		/// <summary>In multi-adapter operation, this indicates which physical adapter of the device this operation applies to.</summary>
		public uint NodeIndex;
		/// <summary>A member of the <c>D3D12_VIDEO_ENCODER_CODEC</c> enumeration specifying the codec for which rate control mode support is being queried.</summary>
		public D3D12_VIDEO_ENCODER_CODEC Codec;
		/// <summary>A <c>D3D12_VIDEO_ENCODER_PROFILE_DESC</c> structure specifying the profile for which intra refresh mode support is being queried.</summary>
		...
		[MarshalAs(UnmanagedType.Bool)] public bool IsSupported;
		/// <summary>A <c>D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT</c> structure. For HEVC, the caller populates this structure with the desired encoder configuration. For H.264, the <b>CheckFeatureSupport</b> call populates the structure with the supported configuration.</summary>
		public D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT CodecSupportLimits;
	}

	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_ENCODER_CODEC_PICTURE_CONTROL_SUPPORT</c>. Retrieves the picture control support for the specified codec and profile.</summary>
	...
	public struct D3D12_FEATURE_DATA_VIDEO_ENCODER_CODEC_PICTURE_CONTROL_SUPPORT
	{
		/// <summary>In multi-adapter operation, this indicates which physical adapter of the device this operation applies to.</summary>
		public uint NodeIndex;
		/// <summary>A member of the <c>D3D12_VIDEO_ENCODER_CODEC</c> enumeration specifying the codec for which picture control support is being queried.</summary>
		public D3D12_VIDEO_ENCODER_CODEC Codec;
		/// <summary>A <c>D3D12_VIDEO_ENCODER_PROFILE_DESC</c> structure specifying the profile for which picture control support is being queried.</summary>
		public D3D12_VIDEO_ENCODER_PROFILE_DESC Profile;
		/// <summary>Gets a boolean value indicating if the provided values are supported.</summary>
		[MarshalAs(UnmanagedType.Bool)] public bool IsSupported;
		/// <summary>Receives a <c>D3D12_VIDEO_ENCODER_CODEC_PICTURE_CONTROL_SUPPORT</c> structure representing the picture control support for the provided values.</summary>
		public D3D12_VIDEO_ENCODER_CODEC_PICTURE_CONTROL_SUPPORT PictureSupport;
	}

	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_ENCODER_FRAME_SUBREGION_LAYOUT_MODE</c>. Retrieves a value indicating if the specified frame subregion layout mode is supported for the specified code, profile, and level.</summary>
	...
	public struct D3D12_FEATURE_DATA_VIDEO_ENCODER_FRAME_SUBREGION_LAYOUT_MODE
	{
		/// <summary>In multi-adapter operation, this indicates which physical adapter of the device this operation applies to.</summary>
		public uint NodeIndex;
		/// <summary>A member of the <c>D3D12_VIDEO_ENCODER_CODEC</c> enumeration specifying the codec for which frame subregion layout mode support is being queried.</summary>
		public D3D12_VIDEO_ENCODER_CODEC Codec;
		/// <summary>A <c>D3D12_VIDEO_ENCODER_PROFILE_DESC</c> structure specifying the profile for which frame subregion layout mode support is being queried.</summary>
		public D3D12_VIDEO_ENCODER_PROFILE_DESC Profile;
		/// <summary>A <c>D3D12_VIDEO_ENCODER_LEVEL_SETTING</c> structure specifying the level for which frame subregion layout mode support is being queried.</summary>
		public D3D12_VIDEO_ENCODER_LEVEL_SETTING Level;
		/// <summary>A member of the <c>D3D12_VIDEO_ENCODER_FRAME_SUBREGION_LAYOUT_MODE</c> enumeration specifying the frame subregion layout mode for which support is being queried.</summary>
		public D3D12_VIDEO_ENCODER_FRAME_SUBREGION_LAYOUT_MODE SubregionMode;
		/// <summary>Receives a boolean value indicating if the specified frame subregion layout mode is supported for the specified codec.</summary>
		[MarshalAs(UnmanagedType.Bool)] public bool IsSupported;
	}

	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_ENCODER_HEAP_SIZE</c>. Retrieves a value indicating if the specified codec is supported for video encoding as well as the L0 and L1 sizes of the heap object.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_feature_data_video_encoder_heap_size
	// typedef struct D3D12_FEATURE_DATA_VIDEO_ENCODER_HEAP_SIZE { D3D12_VIDEO_ENCODER_HEAP_DESC HeapDesc; BOOL IsSupported; UINT64 MemoryPoolL0Size; UINT64 MemoryPoolL1Size; } D3D12_FEATURE_DATA_VIDEO_ENCODER_HEAP_SIZE;
	...
	{
		/// <summary>A <c>D3D12_VIDEO_ENCODER_PICTURE_RESOLUTION_DESC</c> structure specifying the creation properties for a video encoder heap. The driver should map these creation properties to size and assume the maximum resolution allowed for such heap.</summary>
		...
		[MarshalAs(UnmanagedType.Bool)] public bool IsSupported;
		/// <summary>Receives the L0 size of the heap object. Memory Pool L0 is the memory pool “closest” to the GPU. In the case of UMA adapters, this is the amount of system memory used. For discrete adapters, this is the amount of discrete memory used.</summary>
		public ulong MemoryPoolL0Size;
		/// <summary>Receives the L1 size of the heap object. Memory Pool L1 is the memory pool “second closest” to the GPU. In the case of UMA adapters, this value is zero. In the case of discrete adapters, this is the amount of system memory used.</summary>
		public ulong MemoryPoolL1Size;
	}

	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_ENCODER_INPUT_FORMAT</c>. Retrieves a value indicating if the specified codec, profile, and format are supported for video encoding.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_feature_data_video_encoder_input_format
	// typedef struct D3D12_FEATURE_DATA_VIDEO_ENCODER_INPUT_FORMAT { UINT NodeIndex; D3D12_VIDEO_ENCODER_CODEC Codec; D3D12_VIDEO_ENCODER_PROFILE_DESC Profile; DXGI_FORMAT Format; BOOL IsSupported; } D3D12_FEATURE_DATA_VIDEO_ENCODER_INPUT_FORMAT;
	...
	public struct D3D12_FEATURE_DATA_VIDEO_ENCODER_INPUT_FORMAT
	{
		...
		public D3D12_VIDEO_ENCODER_PROFILE_DESC Profile;
		/// <summary>
		///   <para>A member of the <c>DXGI_FORMAT</c> enumeration specifying the pixel format for which support is being queried. This format definition includes the subsampling and bit-depth modes settings for the video encoding session.</para>
		...
		///   </list>
		///   <para>
		///     <para>Note</para> <para>The host is expected to handle the input subsampling and color conversion stages of video encoding.</para></para>
		/// </summary>
	...
	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_ENCODER_INTRA_REFRESH_MODE</c>. Retrieves a value indicating if the specified intra refresh mode is supported for the specified codec, profile, and level.</summary>
	...
	public struct D3D12_FEATURE_DATA_VIDEO_ENCODER_INTRA_REFRESH_MODE
	{
		/// <summary>In multi-adapter operation, this indicates which physical adapter of the device this operation applies to.</summary>
		public uint NodeIndex;
		/// <summary>A member of the <c>D3D12_VIDEO_ENCODER_CODEC</c> enumeration specifying the codec for which intra refresh mode support is being queried.</summary>
		public D3D12_VIDEO_ENCODER_CODEC Codec;
		/// <summary>A <c>D3D12_VIDEO_ENCODER_PROFILE_DESC</c> structure specifying the profile for which intra refresh mode support is being queried.</summary>
		public D3D12_VIDEO_ENCODER_PROFILE_DESC Profile;
		/// <summary>A <c>D3D12_VIDEO_ENCODER_LEVEL_SETTING</c> structure specifying the level for which intra refresh mode support is being queried.</summary>
		public D3D12_VIDEO_ENCODER_LEVEL_SETTING Level;
		/// <summary>A member of the <c>D3D12_VIDEO_ENCODER_INTRA_REFRESH_MODE</c> enumeration specifying the intra refresh mode for which support is being queried.</summary>
		public D3D12_VIDEO_ENCODER_INTRA_REFRESH_MODE IntraRefreshMode;
		/// <summary>Receives a boolean value indicating if the specified intra refresh mode is supported for the specified codec, profile, and level.</summary>
		[MarshalAs(UnmanagedType.Bool)] public bool IsSupported;
	}

	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_ENCODER_OUTPUT_RESOLUTION</c>. Retrieves the list of supported resolutions for the specified codec.</summary>
	...
	// typedef struct D3D12_FEATURE_DATA_VIDEO_ENCODER_OUTPUT_RESOLUTION { UINT NodeIndex; D3D12_VIDEO_ENCODER_CODEC Codec; UINT ResolutionRatiosCount; BOOL IsSupported; D3D12_VIDEO_ENCODER_PICTURE_RESOLUTION_DESC MinResolutionSupported; D3D12_VIDEO_ENCODER_PICTURE_RESOLUTION_DESC MaxResolutionSupported; UINT ResolutionWidthMultipleRequirement; UINT ResolutionHeightMultipleRequirement; D3D12_VIDEO_ENCODER_PICTURE_RESOLUTION_RATIO_DESC *pResolutionRatios; } D3D12_FEATURE_DATA_VIDEO_ENCODER_OUTPUT_RESOLUTION;
	...
	public struct D3D12_FEATURE_DATA_VIDEO_ENCODER_OUTPUT_RESOLUTION
	{
		/// <summary>For single GPU operation, set this to zero. If there are multiple GPU nodes, set a bit to identify the node (the device's physical adapter) to which the command queue applies. Each bit in the mask corresponds to a single node. Only 1 bit may be set.</summary>
		public uint NodeIndex;
		/// <summary>A member of the <c>D3D12_VIDEO_ENCODER_CODEC</c> enumeration specifying the codec for which the supported resolutions are being queried.</summary>
		public D3D12_VIDEO_ENCODER_CODEC Codec;
		/// <summary>The number of resolution ratios to retrieve. This number must match the number in the <c>D3D12_FEATURE_DATA_VIDEO_ENCODER_OUTPUT_RESOLUTION_RATIOS_COUNT.ResolutionRatiosCount</c> field returned from a call to <c>ID3D12VideoDevice::CheckFeatureSupport</c> with <b>D3D12_FEATURE_VIDEO_ENCODER_OUTPUT_RESOLUTION_RATIOS_COUNT</b> specified as the feature.</summary>
		...
		/// <summary>Receives a list of <c>D3D12_VIDEO_ENCODER_PICTURE_RESOLUTION_RATIO_DESC</c> representing the supported resolution ratios for the specified codec as irreducible fractions. The caller must allocate the memory for this array based on the <b>ResolutionRatiosCount</b> field, and assign it to the query struct the call to ID3D12VideoDevice::CheckFeatureSupport.</summary>
		public ArrayPointer<D3D12_VIDEO_ENCODER_PICTURE_RESOLUTION_RATIO_DESC> pResolutionRatios;
	}

	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_ENCODER_OUTPUT_RESOLUTION_RATIOS_COUNT</c>. Retrieves the number of supported resolution ratios for the specified codec.</summary>
	...
	// typedef struct D3D12_FEATURE_DATA_VIDEO_ENCODER_OUTPUT_RESOLUTION_RATIOS_COUNT { UINT NodeIndex; D3D12_VIDEO_ENCODER_CODEC Codec; UINT ResolutionRatiosCount; } D3D12_FEATURE_DATA_VIDEO_ENCODER_OUTPUT_RESOLUTION_RATIOS_COUNT;
	...
	public struct D3D12_FEATURE_DATA_VIDEO_ENCODER_OUTPUT_RESOLUTION_RATIOS_COUNT
	{
		/// <summary>In multi-adapter operation, this indicates which physical adapter of the device this operation applies to.</summary>
		public uint NodeIndex;
		/// <summary>A member of the <c>D3D12_VIDEO_ENCODER_CODEC</c> enumeration specifying the codec for which the number of supported resolution ratios is being queried.</summary>
		public D3D12_VIDEO_ENCODER_CODEC Codec;
		/// <summary>Receives a UINT indicating the number of supported resolution ratios for the specified codec.</summary>
		public uint ResolutionRatiosCount;
	}
	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_ENCODER_PROFILE_LEVEL</c>. Retrieves a value indicating if the specified profile is supported for video encoding.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_feature_data_video_encoder_profile_level
	...
	public struct D3D12_FEATURE_DATA_VIDEO_ENCODER_PROFILE_LEVEL
	{
		/// <summary>For single GPU operation, set this to zero. If there are multiple GPU nodes, set a bit to identify the node (the device's physical adapter) to which the command queue applies. Each bit in the mask corresponds to a single node. Only 1 bit may be set.</summary>
		public uint NodeIndex;
		/// <summary>A member of the <c>D3D12_VIDEO_ENCODER_CODEC</c> enumeration specifying the codec for which the supported profile level is being queried.</summary>
		public D3D12_VIDEO_ENCODER_CODEC Codec;
	...
	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_ENCODER_RATE_CONTROL_MODE</c>. Retrieves a value indicating if the specified rate control mode is supported for video encoding with the specified codec.</summary>
	...
	// typedef struct D3D12_FEATURE_DATA_VIDEO_ENCODER_RATE_CONTROL_MODE { UINT NodeIndex; D3D12_VIDEO_ENCODER_CODEC Codec; D3D12_VIDEO_ENCODER_RATE_CONTROL_MODE RateControlMode; BOOL IsSupported; } D3D12_FEATURE_DATA_VIDEO_ENCODER_RATE_CONTROL_MODE;
	...
	public struct D3D12_FEATURE_DATA_VIDEO_ENCODER_RATE_CONTROL_MODE
	{
		...
		/// <summary>A member of the <c>D3D12_VIDEO_ENCODER_CODEC</c> enumeration specifying the codec for which rate control mode support is being queried.</summary>
		public D3D12_VIDEO_ENCODER_CODEC Codec;
		/// <summary>A member of the <c>D3D12_VIDEO_ENCODER_RATE_CONTROL_MODE</c> enumeration specifying the rate control mode for which support is being queried.</summary>
	...
	// typedef struct D3D12_FEATURE_DATA_VIDEO_ENCODER_RESOLUTION_SUPPORT_LIMITS { UINT MaxSubregionsNumber; UINT MaxIntraRefreshFrameDuration; UINT SubregionBlockPixelsSize; UINT QPMapRegionPixelsSize; } D3D12_FEATURE_DATA_VIDEO_ENCODER_RESOLUTION_SUPPORT_LIMITS;
	...
	{
		/// <summary>The maximum number of subregions per frame supported by the encoder for the associated resolution. For the mode <c>D3D12_VIDEO_ENCODER_FRAME_SUBREGION_LAYOUT_MODE_BYTES_PER_SUBREGION</c> this value must be the absolute maximum limit of subregions per frame to be coded.</summary>
		public uint MaxSubregionsNumber;
		/// <summary>The maximum number that can be used in <c>D3D12_VIDEO_ENCODER_INTRA_REFRESH.IntraRefreshDuration</c> for the associated resolution.</summary>
		...
		///   <para>
		///     <para>Note</para> <para>For HEVC, this indicates the resolution block alignment for the compressed bitstream. For example: If SubregionBlockPixelsSize = 32, then SPS.pic_width/height_in_luma_samples must be aligned to this value and SPS.conf_win_*_offset/conformance_window_flag indicate the difference between this aligned resolution and the current frame resolution indicated by <c>D3D12_RESOURCE_DESC.Dimension</c> of the input video texture. SubregionBlockPixelsSize must be aligned to <c>D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT_HEVC.MinLumaCodingUnitSize</c> (minCUSize), so SPS.pic_width/height_in_luma_samples ends up aligned to minCUSize as required by the HEVC codec spec.</para></para>
		/// </summary>
	...
	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_ENCODER_RESOURCE_REQUIREMENTS</c>. Retrieves values indicating resource requirements for video encoding with the specified encoding configuration.</summary>
	...
	public struct D3D12_FEATURE_DATA_VIDEO_ENCODER_RESOURCE_REQUIREMENTS
	{
		/// <summary>In multi-adapter operation, this indicates which physical adapter of the device this operation applies to.</summary>
		public uint NodeIndex;
		/// <summary>A member of the <c>D3D12_VIDEO_ENCODER_CODEC</c> enumeration specifying the codec for which resource requirements are being queried.</summary>
		public D3D12_VIDEO_ENCODER_CODEC Codec;
		/// <summary>A <c>D3D12_VIDEO_ENCODER_PROFILE_DESC</c> structure specifying the profile for which resource requirements are being queried.</summary>
		public D3D12_VIDEO_ENCODER_PROFILE_DESC Profile;
		...
		/// <summary>A <c>D3D12_VIDEO_ENCODER_PICTURE_RESOLUTION_DESC</c> structure representing the resolution for which resource requirements are being queried.</summary>
		...
		[MarshalAs(UnmanagedType.Bool)] public bool IsSupported;
		/// <summary>Receives a UINT indicating the alignment required in bytes for the resource to be passed in <c>D3D12_VIDEO_ENCODER_COMPRESSED_BITSTREAM.pBuffer</c> and <b>D3D12_VIDEO_ENCODER_COMPRESSED_BITSTREAM.Offset</b>. If no alignment is required, 1 should is returned to indicate 1 byte (trivial) alignment.</summary>
		public uint CompressedBitstreamBufferAccessAlignment;
		/// <summary>Receives a UINT indicating the alignment required in bytes for the resource to be passed in D3D12_VIDEO_ENCODER_OUTPUT_ARGUMENTS.pEncoderOutputMetadata. If no alignment required, 1 should be reported to convey 1 byte (trivial) alignment.</summary>
		public uint EncoderMetadataBufferAccessAlignment;
		/// <summary>Receives a UINT indicating the maximum size in bytes needed for the <c>ID3D12Resource</c> that will be allocated by the host and used as output in the <c>EncodeFrame</c> for output encoder metadata based on the input arguments.</summary>
		public uint MaxEncoderOutputMetadataBufferSize;
	}

	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_ENCODER_SUPPORT</c>. Retrieves values indicating support for the specified video encoding features and configuration values.</summary>
	/// <remarks>
	...
	/// <para>For example, there can be support for intra refresh when checking <c>D3D12_FEATURE_VIDEO_ENCODER_INTRA_REFRESH_MODE</c> and there can be support for B frames when checking <c>D3D12_VIDEO_ENCODER_CODEC_PICTURE_CONTROL_SUPPORT_H264.MaxL1ReferencesForB</c> &gt; 0. But it can be the case that intra refresh and B frames are not supported simultaneously. In this case, querying <b>D3D12_FEATURE_DATA_VIDEO_ENCODER_SUPPORT</b> with CodecGopSequence containing B frames and intra refresh row-based mode, the <b>D3D12_VIDEO_ENCODER_SUPPORT_FLAG_GENERAL_SUPPORT_OK</b> flag will be set off.</para>
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_feature_data_video_encoder_support
	...
	public struct D3D12_FEATURE_DATA_VIDEO_ENCODER_SUPPORT
	{
		...
		/// <summary>A <c>D3D12_VIDEO_ENCODER_PROFILE_DESC</c> structure specifying the profile for which support is being queried.</summary>
		public DXGI_FORMAT InputFormat;
		/// <summary>A <c>D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION</c> structure representing the codec configuration for which support is being queried.</summary>
		public D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION CodecConfiguration;
		/// <summary>A <c>D3D12_VIDEO_ENCODER_SEQUENCE_GOP_STRUCTURE</c> structure representing the GOP structure for which support is being queried.</summary>
		...
		/// <summary>A member of the <c>D3D12_VIDEO_ENCODER_FRAME_SUBREGION_LAYOUT_MODE</c> enumeration, specifying the subregion layout mode for which support is being queried.</summary>
		...
		/// <summary>A pointer to an array of <c>D3D12_VIDEO_ENCODER_PICTURE_RESOLUTION_DESC</c> specifying the picture resolutions for which support is being queried.</summary>
		public ArrayPointer<D3D12_VIDEO_ENCODER_PICTURE_RESOLUTION_DESC> pResolutionList;
		/// <summary>A UINT specifying Maximum number of previous reference frames to be used when calling <c>EncodeFrame</c> for inter-frames. This value is used to calculate the suggested level returned in the SuggestedLevel field.</summary>
		public uint MaxReferenceFramesInDPB;
		/// <summary>Receives a bitwise OR combination of flags from the <c>D3D12_VIDEO_ENCODER_VALIDATION_FLAGS</c> enumeration that provide additional details if the <c>D3D12_VIDEO_ENCODER_SUPPORT_FLAG_GENERAL_SUPPORT_OK</c> flag is not set in the SupportFlags field. See <b>Remarks</b> for more information.</summary>
		public D3D12_VIDEO_ENCODER_VALIDATION_FLAGS ValidationFlags;
		/// <summary>Receives a bitwise OR combination of flags from the <c>D3D12_VIDEO_ENCODER_SUPPORT_FLAGS</c> enumeration specifying support details for the specified encoder features and configuration values.</summary>
		public D3D12_VIDEO_ENCODER_SUPPORT_FLAGS SupportFlags;
		/// <summary>Receives a <c>D3D12_VIDEO_ENCODER_PROFILE_DESC</c> specifying the recommended profile for the specified encoder features and configuration values.</summary>
		public D3D12_VIDEO_ENCODER_PROFILE_DESC SuggestedProfile;
		/// <summary>Receives a <c>D3D12_VIDEO_ENCODER_LEVEL_SETTING</c> specifying the recommended profile for the specified encoder features and configuration values. The recommended level assumes the maximum resolution from the list provided in pResolutionList.</summary>
		public D3D12_VIDEO_ENCODER_LEVEL_SETTING SuggestedLevel;
		/// <summary>Receives a pointer to an array of <c>D3D12_FEATURE_DATA_VIDEO_ENCODER_RESOLUTION_SUPPORT_LIMITS</c> structures specifying resolution-dependent support limits corresponding to the resolutions provided in pResolutionList.</summary>
		public ArrayPointer<D3D12_FEATURE_DATA_VIDEO_ENCODER_RESOLUTION_SUPPORT_LIMITS> pResolutionDependentSupport;
	}

	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_EXTENSION_COMMAND_COUNT</c>. Retrieves the supported number of video extension commands.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_feature_data_video_extension_command_count
	// typedef struct D3D12_FEATURE_DATA_VIDEO_EXTENSION_COMMAND_COUNT { UINT NodeIndex; UINT CommandCount; } D3D12_FEATURE_DATA_VIDEO_EXTENSION_COMMAND_COUNT;
	...
	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_EXTENSION_COMMAND_PARAMETER_COUNT</c>. Retrieves the number of video extension command parameters for the specified parameter stage.</summary>
	...
	public struct D3D12_FEATURE_DATA_VIDEO_EXTENSION_COMMAND_PARAMETER_COUNT
	{
		...
		/// <summary>A member of the <c>D3D12_VIDEO_EXTENSION_COMMAND_PARAMETER_STAGE</c> enumeration specifying the parameter stage for which the parameter count is retrieved.</summary>
	...
	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_EXTENSION_COMMAND_PARAMETERS</c>. Retrieves the list of video extension command parameters for the specified parameter stage.</summary>
	...
	// typedef struct D3D12_FEATURE_DATA_VIDEO_EXTENSION_COMMAND_PARAMETERS { GUID CommandId; D3D12_VIDEO_EXTENSION_COMMAND_PARAMETER_STAGE Stage; UINT ParameterCount; D3D12_VIDEO_EXTENSION_COMMAND_PARAMETER_INFO *pParameterInfos; } D3D12_FEATURE_DATA_VIDEO_EXTENSION_COMMAND_PARAMETERS;
	...
	public struct D3D12_FEATURE_DATA_VIDEO_EXTENSION_COMMAND_PARAMETERS
	{
		...
		public Guid CommandId;
		/// <summary>A member of the <c>D3D12_VIDEO_EXTENSION_COMMAND_PARAMETER_STAGE</c> enumeration specifying the parameter stage for which the parameters are retrieved.</summary>
		public D3D12_VIDEO_EXTENSION_COMMAND_PARAMETER_STAGE Stage;
		/// <summary>The supported number of video extension command parameters. This value must be the count returned by a call to <c>ID3D12VideoDevice::CheckFeatureSupport</c> with <c>D3D12_FEATURE_VIDEO_EXTENSION_COMMAND_PARAMETER_COUNT</c> specified as the feature.</summary>
		public uint ParameterCount;
		/// <summary>Receives a list of <c>D3D12_VIDEO_EXTENSION_COMMAND_PARAMETER_INFO</c> structures describing video extension command parameters.</summary>
		public ManagedArrayPointer<D3D12_VIDEO_EXTENSION_COMMAND_PARAMETER_INFO> pParameterInfos;
	}

	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_EXTENSION_COMMAND_SIZE</c>. Checks the allocation size of a video extension command.</summary>
	...
	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_EXTENSION_COMMAND_SUPPORT</c>. Retrieves video extension command support using command-defined input and output structures.</summary>
	...
	// typedef struct D3D12_FEATURE_DATA_VIDEO_EXTENSION_COMMAND_SUPPORT { UINT NodeIndex; GUID CommandId; const void *pInputData; SIZE_T InputDataSizeInBytes; void *pOutputData; SIZE_T OutputDataSizeInBytes; } D3D12_FEATURE_DATA_VIDEO_EXTENSION_COMMAND_SUPPORT;
	...
	public struct D3D12_FEATURE_DATA_VIDEO_EXTENSION_COMMAND_SUPPORT
	{
		...
		public Guid CommandId;
		/// <summary>Input data for the capability query allocated by the caller with a size of InputDataSizeInBytes. This struct is enumerable as the <c>D3D12_VIDEO_EXTENSION_COMMAND_PARAMETER_STAGE_CAPS_INPUT</c> parameter stage.</summary>
		public IntPtr pInputData;
		/// <summary>The byte size of the input data allocation.</summary>
		public SizeT InputDataSizeInBytes;
		/// <summary>Output data for the capability query allocated by the caller with a size of OutputDataSizeInBytes. This struct is enumerable as the <c>D3D12_VIDEO_EXTENSION_COMMAND_PARAMETER_STAGE_CAPS_OUTPUT</c> parameter stage.</summary>
		public IntPtr pOutputData;
		/// <summary>The byte size of the output data allocation.</summary>
		public SizeT OutputDataSizeInBytes;
	}

	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_EXTENSION_COMMANDS</c>. Retrieves the list of video extension commands from the driver.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_feature_data_video_extension_commands
	// typedef struct D3D12_FEATURE_DATA_VIDEO_EXTENSION_COMMANDS { UINT NodeIndex; UINT CommandCount; D3D12_VIDEO_EXTENSION_COMMAND_INFO *pCommandInfos; } D3D12_FEATURE_DATA_VIDEO_EXTENSION_COMMANDS;
	...
	public struct D3D12_FEATURE_DATA_VIDEO_EXTENSION_COMMANDS
	{
		/// <summary>In multi-adapter operation, this indicates which physical adapter of the device this operation applies to.</summary>
		public uint NodeIndex;
		/// <summary>The supported number of video extension commands. This value must be the count returned by a call to <c>ID3D12VideoDevice::CheckFeatureSupport</c> with <c>D3D12_FEATURE_VIDEO_EXTENSION_COMMAND_COUNT</c> specified as the feature.</summary>
		public uint CommandCount;
	...
	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_MOTION_ESTIMATOR</c>. Retrieves the motion estimation capabilities for a video encoder.</summary>
	/// <remarks>When the format is not supported with motion estimation, BlockSizeFlags will be set to <c>D3D12_VIDEO_MOTION_ESTIMATOR_SEARCH_BLOCK_SIZE_FLAG_NONE</c>, PrecisionFlags will be set to <c>D3D12_VIDEO_MOTION_ESTIMATOR_VECTOR_PRECISION_FLAG_NONE</c>, and the SizeRange will be set to all zeros.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_feature_data_video_motion_estimator
	// typedef struct D3D12_FEATURE_DATA_VIDEO_MOTION_ESTIMATOR { UINT NodeIndex; DXGI_FORMAT InputFormat; D3D12_VIDEO_MOTION_ESTIMATOR_SEARCH_BLOCK_SIZE_FLAGS BlockSizeFlags; D3D12_VIDEO_MOTION_ESTIMATOR_VECTOR_PRECISION_FLAGS PrecisionFlags; D3D12_VIDEO_SIZE_RANGE SizeRange; } D3D12_FEATURE_DATA_VIDEO_MOTION_ESTIMATOR;
	...
	public struct D3D12_FEATURE_DATA_VIDEO_MOTION_ESTIMATOR
	{
		...
		public DXGI_FORMAT InputFormat;
		/// <summary>A bitwise OR combination of values from the <c>D3D12_VIDEO_MOTION_ESTIMATOR_SEARCH_BLOCK_SIZE_FLAGS</c> enumeration specifying the encoder's supported search block sizes for motion estimation.</summary>
		public D3D12_VIDEO_MOTION_ESTIMATOR_SEARCH_BLOCK_SIZE_FLAGS BlockSizeFlags;
		/// <summary>A bitwise OR combination of values from the <c>D3D12_VIDEO_MOTION_ESTIMATOR_VECTOR_PRECISION_FLAGS</c> enumeration specifying the encoder's supported vector precision for motion estimation.</summary>
		public D3D12_VIDEO_MOTION_ESTIMATOR_VECTOR_PRECISION_FLAGS PrecisionFlags;
		/// <summary>A <c>D3D12_VIDEO_SIZE_RANGE</c> structure representing the minimum and maximum input size supported by the driver. The driver sets the fields of this structure to zero if motion estimation is unsupported.</summary>
		public D3D12_VIDEO_SIZE_RANGE SizeRange;
	}

	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_MOTION_ESTIMATOR_PROTECTED_RESOURCES</c>. Retrieves the protected resources support for video motion estimation.</summary>
	...
	// typedef struct D3D12_FEATURE_DATA_VIDEO_MOTION_ESTIMATOR_PROTECTED_RESOURCES { UINT NodeIndex; D3D12_VIDEO_PROTECTED_RESOURCE_SUPPORT_FLAGS SupportFlags; } D3D12_FEATURE_DATA_VIDEO_MOTION_ESTIMATOR_PROTECTED_RESOURCES;
	...
	public struct D3D12_FEATURE_DATA_VIDEO_MOTION_ESTIMATOR_PROTECTED_RESOURCES
	{
		/// <summary>In multi-adapter operation, identifies the physical adapter of the device this operation applies to.</summary>
		public uint NodeIndex;
		/// <summary>A member of the <c>D3D12_VIDEO_PROTECTED_RESOURCE_SUPPORT_FLAGS</c> enumeration specifying support for protected resources.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_feature_data_video_motion_estimator_size
	// typedef struct D3D12_FEATURE_DATA_VIDEO_MOTION_ESTIMATOR_SIZE { UINT NodeIndex; DXGI_FORMAT InputFormat; D3D12_VIDEO_MOTION_ESTIMATOR_SEARCH_BLOCK_SIZE BlockSize; D3D12_VIDEO_MOTION_ESTIMATOR_VECTOR_PRECISION Precision; D3D12_VIDEO_SIZE_RANGE SizeRange; BOOL Protected; UINT64 MotionVectorHeapMemoryPoolL0Size; UINT64 MotionVectorHeapMemoryPoolL1Size; UINT64 MotionEstimatorMemoryPoolL0Size; UINT64 MotionEstimatorMemoryPoolL1Size; } D3D12_FEATURE_DATA_VIDEO_MOTION_ESTIMATOR_SIZE;
	...
	public struct D3D12_FEATURE_DATA_VIDEO_MOTION_ESTIMATOR_SIZE
	{
		...
		public DXGI_FORMAT InputFormat;
		/// <summary>A value from the <c>D3D12_VIDEO_MOTION_ESTIMATOR_SEARCH_BLOCK_SIZE</c> specifying the search block size for motion estimation.</summary>
		public D3D12_VIDEO_MOTION_ESTIMATOR_SEARCH_BLOCK_SIZE BlockSize;
		/// <summary>A value from the <c>D3D12_VIDEO_MOTION_ESTIMATOR_VECTOR_PRECISION</c> specifying the search block size for motion estimation.</summary>
		public D3D12_VIDEO_MOTION_ESTIMATOR_VECTOR_PRECISION Precision;
		/// <summary>A <c>D3D12_VIDEO_SIZE_RANGE</c> structure representing the minimum and maximum input and reference frame size, in pixels, used by the motion estimator.</summary>
		public D3D12_VIDEO_SIZE_RANGE SizeRange;
	...
	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_PROCESS_MAX_INPUT_STREAMS</c>. Retrieves the maximum number of enabled input streams supported by the video processor.</summary>
	...
	/// <summary>Provides data for calls to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_PROCESS_REFERENCE_INFO</c>. Retrieves the number of past and future reference frames required for the specified deinterlace mode, filter, rate conversion, or auto processing features.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_feature_data_video_process_reference_info
	// typedef struct D3D12_FEATURE_DATA_VIDEO_PROCESS_REFERENCE_INFO { UINT NodeIndex; D3D12_VIDEO_PROCESS_DEINTERLACE_FLAGS DeinterlaceMode; D3D12_VIDEO_PROCESS_FILTER_FLAGS Filters; D3D12_VIDEO_PROCESS_FEATURE_FLAGS FeatureSupport; DXGI_RATIONAL InputFrameRate; DXGI_RATIONAL OutputFrameRate; BOOL EnableAutoProcessing; UINT PastFrames; UINT FutureFrames; } D3D12_FEATURE_DATA_VIDEO_PROCESS_REFERENCE_INFO;
	...
	public struct D3D12_FEATURE_DATA_VIDEO_PROCESS_REFERENCE_INFO
	{
		/// <summary>An integer indicating which physical adapter of the device the operation applies to, in a multi-adapter operation.</summary>
		public uint NodeIndex;
		/// <summary>A member of the <c>D3D12_VIDEO_PROCESS_DEINTERLACE_FLAGS</c> enumeration specifying the deinterlacing mode for which the required past and future reference frame counts are retrieved.</summary>
		public D3D12_VIDEO_PROCESS_DEINTERLACE_FLAGS DeinterlaceMode;
		/// <summary>A bitwise OR combination of values from the <c>D3D12_VIDEO_PROCESS_FILTER_FLAGS</c> enumeration specifying the filters for which the required past and future reference frame counts are retrieved.</summary>
		public D3D12_VIDEO_PROCESS_FILTER_FLAGS Filters;
		/// <summary>A bitwise OR combination of values from the <c>D3D12_VIDEO_PROCESS_FEATURE_FLAGS</c> enumeration specifying the features for which the required past and future reference frame counts are retrieved.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_feature_data_video_process_support
	// typedef struct D3D12_FEATURE_DATA_VIDEO_PROCESS_SUPPORT { UINT NodeIndex; D3D12_VIDEO_SAMPLE InputSample; D3D12_VIDEO_FIELD_TYPE InputFieldType; D3D12_VIDEO_FRAME_STEREO_FORMAT InputStereoFormat; DXGI_RATIONAL InputFrameRate; D3D12_VIDEO_FORMAT OutputFormat; D3D12_VIDEO_FRAME_STEREO_FORMAT OutputStereoFormat; DXGI_RATIONAL OutputFrameRate; D3D12_VIDEO_PROCESS_SUPPORT_FLAGS SupportFlags; D3D12_VIDEO_SCALE_SUPPORT ScaleSupport; D3D12_VIDEO_PROCESS_FEATURE_FLAGS FeatureSupport; D3D12_VIDEO_PROCESS_DEINTERLACE_FLAGS DeinterlaceSupport; D3D12_VIDEO_PROCESS_AUTO_PROCESSING_FLAGS AutoProcessingSupport; D3D12_VIDEO_PROCESS_FILTER_FLAGS FilterSupport; D3D12_VIDEO_PROCESS_FILTER_RANGE FilterRangeSupport[32]; } D3D12_FEATURE_DATA_VIDEO_PROCESS_SUPPORT;
	...
	public struct D3D12_FEATURE_DATA_VIDEO_PROCESS_SUPPORT
	{
		...
		public DXGI_RATIONAL OutputFrameRate;
		/// <summary>A member of the <c>D3D12_VIDEO_PROCESS_SUPPORT_FLAGS</c> indicating whether the requested format and colorspace conversion is supported. This value is populated by the call to <b>ID3D12Device::CheckFeatureSupport</b>.</summary>
		public D3D12_VIDEO_PROCESS_SUPPORT_FLAGS SupportFlags;
		/// <summary>A <c>D3D12_VIDEO_SCALE_SUPPORT</c> structure specifying the supported scaling capabilities. This value is populated by the call to <b>ID3D12Device::CheckFeatureSupport</b>.</summary>
		public D3D12_VIDEO_SCALE_SUPPORT ScaleSupport;
		/// <summary>A bitwise OR combination of values from the <c>D3D12_VIDEO_PROCESS_FEATURE_FLAGS</c> enumeration specifying the supported video processing features. This value is populated by the call to <b>ID3D12Device::CheckFeatureSupport</b>.</summary>
		public D3D12_VIDEO_PROCESS_FEATURE_FLAGS FeatureSupport;
		/// <summary>A member of the <c>D3D12_VIDEO_PROCESS_DEINTERLACE_FLAGS</c> enumeration specifying the supported deinterlacing capabilities. This value is populated by the call to <b>ID3D12Device::CheckFeatureSupport</b>.</summary>
		public D3D12_VIDEO_PROCESS_DEINTERLACE_FLAGS DeinterlaceSupport;
		/// <summary>A member of the <c>D3D12_VIDEO_PROCESS_AUTO_PROCESSING_FLAGS</c> specifying the supported automatic processing capabilities. This value is populated by the call to <b>ID3D12Device::CheckFeatureSupport</b>.</summary>
		public D3D12_VIDEO_PROCESS_AUTO_PROCESSING_FLAGS AutoProcessingSupport;
		/// <summary>A bitwise OR combination of values from the <c>D3D12_VIDEO_PROCESS_FILTER_FLAGS</c> enumeration specifying the supported video filtering features. This value is populated by the call to <b>ID3D12Device::CheckFeatureSupport</b>.</summary>
		public D3D12_VIDEO_PROCESS_FILTER_FLAGS FilterSupport;
		/// <summary>An array of <c>D3D12_VIDEO_PROCESS_FILTER_RANGE</c> structures representing the filter range values. This value is populated by the call to <b>ID3D12Device::CheckFeatureSupport</b>. The calling application must allocate the memory for the filter range list before calling <b>CheckFeatureSupport</b>.</summary>
		[MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
		public D3D12_VIDEO_PROCESS_FILTER_RANGE[] FilterRangeSupport;
	}

	/// <summary>Describes the allocation size of a video decoder heap.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_feature_data_video_processor_size
	// typedef struct D3D12_FEATURE_DATA_VIDEO_PROCESSOR_SIZE { UINT NodeMask; const D3D12_VIDEO_PROCESS_OUTPUT_STREAM_DESC *pOutputStreamDesc; UINT NumInputStreamDescs; const D3D12_VIDEO_PROCESS_INPUT_STREAM_DESC *pInputStreamDescs; UINT64 MemoryPoolL0Size; UINT64 MemoryPoolL1Size; } D3D12_FEATURE_DATA_VIDEO_PROCESSOR_SIZE;
	...
	public struct D3D12_FEATURE_DATA_VIDEO_PROCESSOR_SIZE
	{
		...
		/// <summary>A pointer to a list of D3D12_VIDEO_PROCESS_INPUT_STREAM_DESC(ns-d3d12video-d3d12_video_process_input_stream_desc) structures the input streams.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_query_data_video_decode_statistics
	// typedef struct D3D12_QUERY_DATA_VIDEO_DECODE_STATISTICS { UINT64 Status; UINT64 NumMacroblocksAffected; DXGI_RATIONAL FrameRate; UINT BitRate; } D3D12_QUERY_DATA_VIDEO_DECODE_STATISTICS;
	...
	public struct D3D12_QUERY_DATA_VIDEO_DECODE_STATISTICS
	{
		...
		public DXGI_RATIONAL FrameRate;
		/// <summary>
		///   <para>When the <b>Status</b> returned is <c>D3D12_VIDEO_DECODE_STATUS_RATE_EXCEEDED</c>, this field reports the bitrate that would succeed. This value may be used to recreate the decoder and try again. A value of zero here is valid to indicate that the worst case bit rate should be assumed.</para>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_resolve_video_motion_vector_heap_input
	// typedef struct D3D12_RESOLVE_VIDEO_MOTION_VECTOR_HEAP_INPUT { ID3D12VideoMotionVectorHeap *pMotionVectorHeap; UINT PixelWidth; UINT PixelHeight; } D3D12_RESOLVE_VIDEO_MOTION_VECTOR_HEAP_INPUT;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_resolve_video_motion_vector_heap_output
	// typedef struct D3D12_RESOLVE_VIDEO_MOTION_VECTOR_HEAP_OUTPUT { ID3D12Resource *pMotionVectorTexture2D; D3D12_RESOURCE_COORDINATE MotionVectorCoordinate; } D3D12_RESOLVE_VIDEO_MOTION_VECTOR_HEAP_OUTPUT;
	...
	{
		/// <summary>An <c>ID3D12Resource</c> representing the output resource for resolved motion vectors. Motion vectors are resolved to <c>DXGI_FORMAT_R16G16_SINT</c> 2D textures. The resolved data is a signed 16-byte integer with quarter PEL units with the X vector component stored in the R component and the Y vector component stored in the G component. Motion vectors are stored in a 2D layout that corresponds to the pixel layout of the original input textures.</summary>
		[MarshalAs(UnmanagedType.IUnknown)]
		public ID3D12Resource pMotionVectorTexture2D;
		/// <summary>A <c>D3D12_RESOURCE_COORDINATE</c> structure specifying the output origin of the motion vectors. The remaining sub-region must be large enough to store all motion vectors per block specified by the input pixel with and pixel height and the specified <c>D3D12_VIDEO_MOTION_ESTIMATOR_SEARCH_BLOCK_SIZE</c>.</summary>
		public D3D12_RESOURCE_COORDINATE MotionVectorCoordinate;
	}

	/// <summary>Describes the coordinates of a resource.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_resource_coordinate
	// typedef struct D3D12_RESOURCE_COORDINATE { UINT64 X; UINT Y; UINT Z; UINT SubresourceIndex; } D3D12_RESOURCE_COORDINATE;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_decode_compressed_bitstream
	// typedef struct D3D12_VIDEO_DECODE_COMPRESSED_BITSTREAM { ID3D12Resource *pBuffer; UINT64 Offset; UINT64 Size; } D3D12_VIDEO_DECODE_COMPRESSED_BITSTREAM;
	...
	public struct D3D12_VIDEO_DECODE_COMPRESSED_BITSTREAM
	{
		...
		/// <summary>The offset to the beginning of the first slice. This offset has alignment requirements based on the tier value of the video decoder. For more information on decoding tiers, see <c>D3D12_VIDEO_DECODE_TIER</c>.</summary>
		public ulong Offset;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_decode_configuration
	// typedef struct D3D12_VIDEO_DECODE_CONFIGURATION { GUID DecodeProfile; D3D12_BITSTREAM_ENCRYPTION_TYPE BitstreamEncryption; D3D12_VIDEO_FRAME_CODED_INTERLACE_TYPE InterlaceType; } D3D12_VIDEO_DECODE_CONFIGURATION;
	...
	{
		/// <summary>A GUID identifying the profile for the decoder, such as D3D12_VIDEO_DECODE_PROFILE_H264 or D3D12_VIDEO_DECODE_PROFILE_HEVC_MAIN. For a list of supported GUIDs, see <c>Direct3D 12 Video GUIDs</c>.</summary>
		public Guid DecodeProfile;
		/// <summary>A member of the <c>D3D12_BITSTREAM_ENCRYPTION_TYPE</c> enumeration specifying the type of bitstream encryption. For no encryption, use D3D12_BITSTREAM_ENCRYPTION_TYPE_NONE.</summary>
		public D3D12_BITSTREAM_ENCRYPTION_TYPE BitstreamEncryption;
		/// <summary>A member of the <c>D3D12_VIDEO_FRAME_CODED_INTERLACE_TYPE</c> enumeration the desired interlace type used by the coded frames.</summary>
		public D3D12_VIDEO_FRAME_CODED_INTERLACE_TYPE InterlaceType;
	}

	/// <summary>Specifies the parameters for decode output conversion. <c>D3D12_VIDEO_DECODE_CONVERSION_ARGUMENTS1</c> is used for the same purpose, but provides additional fields for output width and output height.</summary>
	/// <remarks>
	...
	/// <para>The source and destination resolution and format are communicated by the resource properties of decode textures and the output buffer specified in <c>ID3D12VideoCommandList::DecodeFrame</c>.</para>
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_decode_conversion_arguments
	// typedef struct D3D12_VIDEO_DECODE_CONVERSION_ARGUMENTS { BOOL Enable; ID3D12Resource *pReferenceTexture2D; UINT ReferenceSubresource; DXGI_COLOR_SPACE_TYPE OutputColorSpace; DXGI_COLOR_SPACE_TYPE DecodeColorSpace; } D3D12_VIDEO_DECODE_CONVERSION_ARGUMENTS;
	...
	public struct D3D12_VIDEO_DECODE_CONVERSION_ARGUMENTS
	{
		...
		/// <summary>A pointer to an <c>ID3D12Resource</c> containing the native decoding output. When downsampling is enabled, the output at native decode resolution, color space, and format may be required for future decode submissions (as reference frames, for instance).</summary>
		[MarshalAs(UnmanagedType.IUnknown)]
	...
	/// <summary>Specifies the parameters for decode output conversion. <c>D3D12_VIDEO_DECODE_CONVERSION_ARGUMENTS</c> is used for the same purpose, but does not contain fields for output width and output height.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_decode_conversion_arguments1
	// typedef struct D3D12_VIDEO_DECODE_CONVERSION_ARGUMENTS1 { BOOL Enable; ID3D12Resource *pReferenceTexture2D; UINT ReferenceSubresource; DXGI_COLOR_SPACE_TYPE OutputColorSpace; DXGI_COLOR_SPACE_TYPE DecodeColorSpace; UINT OutputWidth; UINT OutputHeight; } D3D12_VIDEO_DECODE_CONVERSION_ARGUMENTS1;
	...
	/// <summary>Represents the decode parameters for a frame. Parameter definitions are specified by the codec specification for each decode profile.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_decode_frame_argument
	// typedef struct D3D12_VIDEO_DECODE_FRAME_ARGUMENT { D3D12_VIDEO_DECODE_ARGUMENT_TYPE Type; UINT Size; void *pData; } D3D12_VIDEO_DECODE_FRAME_ARGUMENT;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_decode_input_stream_arguments
	// typedef struct D3D12_VIDEO_DECODE_INPUT_STREAM_ARGUMENTS { UINT NumFrameArguments; D3D12_VIDEO_DECODE_FRAME_ARGUMENT FrameArguments[10]; D3D12_VIDEO_DECODE_REFERENCE_FRAMES ReferenceFrames; D3D12_VIDEO_DECODE_COMPRESSED_BITSTREAM CompressedBitstream; ID3D12VideoDecoderHeap *pHeap; } D3D12_VIDEO_DECODE_INPUT_STREAM_ARGUMENTS;
	...
	public struct D3D12_VIDEO_DECODE_INPUT_STREAM_ARGUMENTS
	{
		...
		/// <summary>A <c>D3D12_VIDEO_DECODE_COMPRESSED_BITSTREAM</c> structure representing the compressed bitstream in a single continuous buffer.</summary>
	...
	/// </list>
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_decode_output_histogram
	...
	{
		/// <summary>The offset location in pBuffer to write the component histogram. Must be 256-byte aligned. Set to zero when a component is disabled.</summary>
		public ulong Offset;
		/// <summary>And <c>ID3D12Resource</c> representing the target buffer for hardware to write the components histogram. Set to a nullptr when the component histogram is disabled.</summary>
		[MarshalAs(UnmanagedType.IUnknown)]
		public ID3D12Resource pBuffer;
	}

	/// <summary>Specifies the parameters for the output stream for a video decode operation. <c>D3D12_VIDEO_DECODE_OUTPUT_STREAM_ARGUMENTS1</c> is used for the same purpose, but provides an additional field for histograms.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_decode_output_stream_arguments
	// typedef struct D3D12_VIDEO_DECODE_OUTPUT_STREAM_ARGUMENTS { ID3D12Resource *pOutputTexture2D; UINT OutputSubresource; D3D12_VIDEO_DECODE_CONVERSION_ARGUMENTS ConversionArguments; } D3D12_VIDEO_DECODE_OUTPUT_STREAM_ARGUMENTS;
	...
	{
		/// <summary>An <c>ID3D12Resource</c> representing the output texture. If decode conversion is enabled, this texture will contain the post-conversion output. If decode conversion is not enabled, this texture will contain the decode output.</summary>
		[MarshalAs(UnmanagedType.IUnknown)]
		public ID3D12Resource pOutputTexture2D;
		/// <summary>The index of the output subresource of pOutputTexture2D to use. This allows you to specify array indices if the output is an array.</summary>
	...
	/// <summary>Specifies the parameters for the output stream for a video decode operation. <c>D3D12_VIDEO_DECODE_OUTPUT_STREAM_ARGUMENTS</c> is used for the same purpose, but does not provide a field for histograms.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_decode_output_stream_arguments1
	// typedef struct D3D12_VIDEO_DECODE_OUTPUT_STREAM_ARGUMENTS1 { ID3D12Resource *pOutputTexture2D; UINT OutputSubresource; D3D12_VIDEO_DECODE_CONVERSION_ARGUMENTS1 ConversionArguments; D3D12_VIDEO_DECODE_OUTPUT_HISTOGRAM Histograms[4]; } D3D12_VIDEO_DECODE_OUTPUT_STREAM_ARGUMENTS1;
	...
	public struct D3D12_VIDEO_DECODE_OUTPUT_STREAM_ARGUMENTS1
	{
		...
		/// <summary>An array of <c>D3D12_VIDEO_DECODE_OUTPUT_HISTOGRAM</c> structures that are populated with histogram data. The maximum size of the array is 4.</summary>
	...
	/// <remarks>Reference textures may have limitations such as a requirement to allocate reference buffers as a texture array. For information on the requirements for different decoder configurations, see <c>D3D12_VIDEO_DECODE_TIER</c>.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_decode_reference_frames
	// typedef struct D3D12_VIDEO_DECODE_REFERENCE_FRAMES { UINT NumTexture2Ds; ID3D12Resource **ppTexture2Ds; UINT *pSubresources; ID3D12VideoDecoderHeap **ppHeaps; } D3D12_VIDEO_DECODE_REFERENCE_FRAMES;
	...
	public struct D3D12_VIDEO_DECODE_REFERENCE_FRAMES
	{
		...
		/// <summary>
		///   <para>An array of subresource indices for the reference textures specified in ppTexture2Ds. NULL indicates that subresource 0 should be assumed for each resource.</para>
		...
		///   <para>The video device driver uses the "PicEntry" indices defined in the DXVA spec for the codec to dereference this array to find the subresource index to use with the corresponding resource. For example, in HEVC, the Driver uses <c>DXVA_PicEntry_HEVC::Index7Bits</c> as an index for this array.</para>
		/// </summary>
		public ArrayPointer<uint> pSubresources;
		/// <summary>An array of <c>ID3D12VideoDecoderHeap</c> objects. This field is used with formats that support non-key frame resolution changes, allowing the caller to pass in the previous resolution's heap, relative to the reference it's being used for, in addition to the current resolution heap.</summary>
		public IntPtr ppHeaps;
	}

	/// <summary>Describes a <c>ID3D12VideoDecoder</c>. Pass this structure into <c>ID3D12VideoDevice::CreateVideoDecoder</c> to create an instance of <b>ID3D12VideoDecoder</b>.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_decoder_desc
	// typedef struct D3D12_VIDEO_DECODER_DESC { UINT NodeMask; D3D12_VIDEO_DECODE_CONFIGURATION Configuration; } D3D12_VIDEO_DECODER_DESC;
	...
	/// <summary>Describes a <c>ID3D12VideoDecoderHeap</c>. Pass this structure into <c>ID3D12VideoDevice::CreateVideoDecoderHeap</c> to create an instance of <b>ID3D12VideoDecoderHeap</b>.</summary>
	/// <remarks>
	/// <para>The BitRate and FrameRate parameters may be used by drivers to inform heuristics such as intermediate allocation sizes. Decoding a frame may fail if these values are insufficient for the video stream. Use <c>D3D12_QUERY_DATA_VIDEO_DECODE_STATISTICS</c> to determine if the video decode succeeded. If decode fails due to insufficient BitRate and FrameRate parameters, the Status field of this query is set to <c>D3D12_VIDEO_DECODE_STATUS_RATE_EXCEEDED</c>. This query also returns new BitRate and FrameRate values that would succeed.</para>
	/// <para>The BitRate and FrameRate parameters may also be set to zero. Drivers make worst-case assumptions when these values are used which may result in higher memory consumption with some adapters.</para>
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_decoder_heap_desc
	// typedef struct D3D12_VIDEO_DECODER_HEAP_DESC { UINT NodeMask; D3D12_VIDEO_DECODE_CONFIGURATION Configuration; UINT DecodeWidth; UINT DecodeHeight; DXGI_FORMAT Format; DXGI_RATIONAL FrameRate; UINT BitRate; UINT MaxDecodePictureBufferCount; } D3D12_VIDEO_DECODER_HEAP_DESC;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encode_reference_frames
	// typedef struct D3D12_VIDEO_ENCODE_REFERENCE_FRAMES { UINT NumTexture2Ds; ID3D12Resource **ppTexture2Ds; UINT *pSubresources; } D3D12_VIDEO_ENCODE_REFERENCE_FRAMES;
	...
	public struct D3D12_VIDEO_ENCODE_REFERENCE_FRAMES
	{
		...
		public IntPtr ppTexture2Ds;
		/// <summary>An array of subresource indices for the reference textures specified in ppTexture2Ds. NULL indicates that subresource 0 should be assumed for each resource. With texture arrays within a single resource, the subresource indices point to the array index of the first resource plane. With an array of textures in individual resources, the subresource index is typically zero.</summary>
		public ArrayPointer<uint> pSubresources;
	}
	...
	{
		/// <summary>Defines the set of enabled features. Flags can be combined based on the reported capabilities/requirements by the driver.</summary>
	...
	{
		/// <summary>Output param. Indicates which features are supported for the codec. Supported features can be set or not by the API Client.</summary>
		...
		/// <summary>
		///   <para>Output parameter. Indicates which values can be selected as input parameters for FrameRestorationType and LoopRestorationPixelSize in D3D12_VIDEO_ENCODER_AV1_RESTORATION_CONFIG.</para>
		///   <para>The first array indexing corresponds to the restoration filter type: | Index i in SupportedRestorationParams[i][j] | Filter type | |————–|————-| | 0 | SWITCHABLE | | 1 | WIENER | | 2 | SGRPROJ |</para>
		...
		///   <para>The second array indexing corresponds to the planes: | Index j in SupportedRestorationParams[i][j] | Plane | |————–|————-| | 0 | Y plane | | 1 | U plane | | 2 | V plane |</para>
		///   <para>The value returned in SupportedRestorationParams[i][j] is a bitflag mask indicating whether the i-th filter in the j-th plane is either:</para>
		///   <list type="number">
		...
		/// <summary>Output parameter. Indicates which values can be selected on input parameters of type D3D12_VIDEO_ENCODER_AV1_TX_MODE for different D3D12_VIDEO_ENCODER_AV1_FRAME_TYPE.
		/// <note>Driver must support at least 1 mode for each frame type(ie.mask value cannot be 0).</note></summary>
		[MarshalAs(UnmanagedType.ByValArray, SizeConst = 4)]
		public D3D12_VIDEO_ENCODER_AV1_TX_MODE_FLAGS[] SupportedTxModes;
		/// <summary>Output parameter. Indicates the block size for the segment map. This is both the for input blocks in D3D12_VIDEO_ENCODER_AV1_SEGMENTATION_MAP for custom segmentation or the block size of the segment map written in the compressed bitstream by the driver in auto segmentation.</summary>
		public D3D12_VIDEO_ENCODER_AV1_SEGMENTATION_BLOCK_SIZE SegmentationBlockSize;
		/// <summary>Specifies for which AV1 encoding features, the underlying encoder is able to override the associated AV1 syntax values or accept API Client configurable input exactly.</summary>
		public D3D12_VIDEO_ENCODER_AV1_POST_ENCODE_VALUES_FLAGS PostEncodeValuesFlags;
		/// <summary>Specifies the maximum number of temporal layers that can be supported. The reported values must be in the range [1..MaxTemporalIdSupported + 1]. A reported value 1, there is no temporal scalability support.</summary>
		public uint MaxTemporalLayers;
		/// <summary>Specifies the maximum number of spatial layers that can be supported. The reported values must be in the range [1..MaxSpatialIdSupported + 1]. A reported value 1, there is no spatial scalability support.</summary>
	...
	{
		/// <summary>Input parameter. Indicates if the returned values by the driver in superblock units need to be expressed as 128x128 superblocks. Otherwise the superblock default size 64x64 must be used.</summary>
		...
		/// <summary>Output parameter. Indicates more details when D3D12_FEATURE_DATA_VIDEO_ENCODER_FRAME_SUBREGION_LAYOUT_CONFIG.IsSupported is false.</summary>
	...
	public struct D3D12_VIDEO_ENCODER_AV1_PICTURE_CONTROL_CODEC_DATA
	{
		...
		/// <summary>Indicates the configuration for super resolution. Has to be greater or equal than D3D12_VIDEO_ENCODER_AV1_SUPERRES_DENOM_MIN when super resolution is enabled.</summary>
		public uint SuperResDenominator;
		/// <summary>Current frame order_hint AV1 syntax. For this API purposes, OrderHint must be always passed even when not coding the order hint in the AV1 bitstream, and it must reflect the display order of the frame.</summary>
		public uint OrderHint;
		/// <summary>
		///   <para>The unique picture index for this frame that will be used to uniquely identify it as a reference for future frames. This parameter is not related in any way to the AV1 standard syntax, but merely used for API client implementation tracking instead.</para>
		///   <para>API Client should initialize this value at 0 for the first D3D12_VIDEO_ENCODER_AV1_FRAME_TYPE_KEY_FRAME and increment it by one on each subsequent frame until the next D3D12_VIDEO_ENCODER_AV1_FRAME_TYPE_KEY_FRAME, when it should be reset to zero and follow the same process.</para>
		///   <para>Note: OrderHint cannot be used for this purpose as it has a max range of [0..2^(OrderHintBitsMinus1+1)], which can wrap around and not work as unique identifier of the frames and their references.</para>
		/// </summary>
		public uint PictureIndex;
		/// <summary>Picture temporal layer index plus one. A value of zero indicates temporal scalability not used. This value must be within the range [0..D3D12_VIDEO_ENCODER_AV1_CODEC_CONFIGURATION_SUPPORT.MaxTemporalLayers].</summary>
		public uint TemporalLayerIndexPlus1;
		/// <summary>Picture spatial layer index plus one. A value of zero indicates spatial scalability not used. This value must be within the range [0..D3D12_VIDEO_ENCODER_AV1_CODEC_CONFIGURATION_SUPPORT.MaxSpatialLayers].</summary>
		public uint SpatialLayerIndexPlus1;
		/// <summary>
		///   <para>Describes the current state snapshot of the complete (ie. including frames that are not used by current frame but used by future frames, etc) DPB buffer kept in D3D12_VIDEO_ENCODER_PICTURE_CONTROL_DESC.ReferenceFrames. The reference indices (ie. last, altref, etc) map from past/future references into this descriptors array. The AV1 codec allows up to 8 references in the DPB.</para>
		///   <para>This array of descriptors, in turn, maps a reference picture for this frame into a resource index in the reconstructed pictures array D3D12_VIDEO_ENCODER_PICTURE_CONTROL_DESC.ReferenceFrames.</para>
		///   <para>The size of this array always matches D3D12_VIDEO_ENCODER_PICTURE_CONTROL_DESC.ReferenceFrames.NumTextures for the associated EncodeFrame command.</para>
		/// </summary>
		[MarshalAs(UnmanagedType.ByValArray, SizeConst = 8)]
		public D3D12_VIDEO_ENCODER_AV1_REFERENCE_PICTURE_DESCRIPTOR[] ReferenceFramesReconPictureDescriptors;
		/// <summary>
		///   <para>Corresponds to the ref_frame_idx[i] AV1 syntax. For a reference type i, ReferenceIndices[i] indicates an index between [0..7] into ReferenceFramesReconPictureDescriptors where the current frame i-th reference type is stored in the DPB. In other words ReferenceFramesReconPictureDescriptors[ReferenceIndices[i]] contains the DPB descriptor for the i-th reference type.</para>
		...
		/// <summary>
		///   <para>Corresponds to the AV1 element syntax primary_ref_frame in uncompressed_header(). Specifies which reference frame contains the CDF values and other state that must be loaded at the start of the frame. The allowed range is [0..7] and the values corresponds as follows:</para>
		///   <list type="table">
		///     <listheader>
		...
		/// <summary>Array containing in row/column scan order, the QP map values to use on each squared region for this frame. The QP map dimensions can be calculated using the current resolution and D3D12_FEATURE_DATA_VIDEO_ENCODER_RESOLUTION_SUPPORT_LIMITS.QPMapRegionPixelsSize conveying the squared region sizes. The range for Delta QP values is [-255;255].</summary>
		public ArrayPointer<short> pRateControlQPMap;
		/// <summary>Only used when D3D12_VIDEO_ENCODER_AV1_PICTURE_CONTROL_FLAG_ENABLE_FRAME_SEGMENTATION_CUSTOM is set for current frame.</summary>
		public D3D12_VIDEO_ENCODER_AV1_SEGMENTATION_CONFIG CustomSegmentation;
		/// <summary>Only used when D3D12_VIDEO_ENCODER_AV1_PICTURE_CONTROL_FLAG_ENABLE_FRAME_SEGMENTATION_CUSTOM is set for current frame. Segment map to be used if CustomSegmentation.UpdateMap is set. Otherwise, the segment map is inherited from ref frame.</summary>
	...
	///   <list type="bullet">
	///     <item>Input parameters: RowCount, ColCount, RowHeights, ColWidths within the reported tile caps. The integer values must match the AV1 codec standard expectations (ie. power of two, etc).</item>
	...
	///   <list type="bullet">
	///     <item>Input parameters: RowCount, ColCount. The integer values must match the AV1 codec standard expectations (ie. power of two, etc).</item>
	...
	public struct D3D12_VIDEO_ENCODER_AV1_SEGMENTATION_CONFIG
	{
		...
		/// <summary>
		///   <para>When using D3D12_VIDEO_ENCODER_AV1_PICTURE_CONTROL_FLAG_ENABLE_FRAME_SEGMENTATION_AUTO and the driver writes it back on post encode values, NumSegments = 0 indicated that segmentation_enabled must be 0 in the frame header. Otherwise, the API client codes segmentation_params() in the frame header accordingly with the other parameters in this structure.</para>
		///   <para>When using D3D12_VIDEO_ENCODER_AV1_PICTURE_CONTROL_FLAG_ENABLE_FRAME_SEGMENTATION_CUSTOM, indicates the input number of segments.</para>
		/// </summary>
	...
	public struct D3D12_VIDEO_ENCODER_AV1_SEGMENTATION_MAP
	{
		...
		/// <summary>In raster order, contains the AV1 syntax segment_id between [0..7] for each block in the frame. The block size is SegmentationBlockSize as reported by driver in D3D12_VIDEO_ENCODER_AV1_CODEC_CONFIGURATION_SUPPORT.</summary>
	...
	{
		/// <summary>Indicates the distance between intra-only frames (or key frames) in the video sequence, or the number of pictures on a sequence of inter-frame pictures. If set to 0, only the first frame will be an key-frame.</summary>
		public uint IntraDistance;
		/// <summary>Indicates the period for inter-frames to be inserted within the inter frame structure. Note that if IntraDistance is set to 0 for infinite inter frame structure, this value must be greater than zero.</summary>
	...
	public struct D3D12_VIDEO_ENCODER_CODEC_AV1_LOOP_FILTER_CONFIG
	{
		...
		/// <summary>Related to AV1 syntax loop_filter_delta_enabled. Requires D3D12_VIDEO_ENCODER_AV1_FEATURE_FLAG_LOOP_FILTER_DELTAS supported/enabled.</summary>
	...
	public struct D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION
	{
		...
		/// <summary>A pointer to a <c>D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_H264</c> structure containing codec configuration parameters for H.264 encoding.</summary>
		...
		/// <summary>A pointer to a <c>D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_HEVC</c> structure containing codec configuration parameters for HEVC encoding.</summary>
	...
	{
		/// <summary>A bitwise OR combination of flags from the <c>D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_H264_FLAGS</c> enumeration defining the set of enabled codec features.</summary>
		public D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_H264_FLAGS ConfigurationFlags;
		/// <summary>A value from the <c>D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_H264_DIRECT_MODES</c> enumeration specifying direct modes.</summary>
		public D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_H264_DIRECT_MODES DirectModeConfig;
		/// <summary>A value from the <c>D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_H264_SLICES_DEBLOCKING_MODES</c> enumeration specifying configuration related to the disable_deblocking_filter_idc syntax from the H.264 specification.</summary>
	...
	{
		/// <summary>A bitwise OR combination of flags from the <c>D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_HEVC_FLAGS</c> enumeration defining the set of enabled codec features.</summary>
		public D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_HEVC_FLAGS ConfigurationFlags;
		/// <summary>A value from the <c>D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_HEVC_CUSIZE</c> enumeration indicating the minimum luma coding block size to be used in the encoder. This value matches what the caller will code in SPS.</summary>
		public D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_HEVC_CUSIZE MinLumaCodingUnitSize;
		/// <summary>A value from the <c>D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_HEVC_CUSIZE</c> enumeration indicating the maximum luma coding block size to be used in the encoder. This value matches what the caller will code in SPS.</summary>
		public D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_HEVC_CUSIZE MaxLumaCodingUnitSize;
		/// <summary>A value from the <c>D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_HEVC_TUSIZE</c> enumeration indicating the minimum luma transform block size to be used in the encoder. This value matches the pixel size of what the user will code in SPS.log2_min_luma_transform_block_size_minus2.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_codec_configuration_support
	// typedef struct D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT { UINT DataSize; union { D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT_H264 *pH264Support; D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT_HEVC *pHEVCSupport; D3D12_VIDEO_ENCODER_AV1_CODEC_CONFIGURATION_SUPPORT *pAV1Support; }; } D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT;
	[PInvokeData("d3d12video.h", MSDNShortId = "NS:d3d12video.D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT")]
	[StructLayout(LayoutKind.Sequential)]
	public struct D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT
	{
		...
		/// <summary>A pointer to a <c>D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT_H264</c> structure containing codec configuration support parameters for H.264 encoding.</summary>
		...
		/// <summary>A pointer to a <c>D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT_HEVC</c> structure containing codec configuration support parameters for HEVC encoding.</summary>
	...
	{
		/// <summary>A bitwise OR combination of flags from the <c>D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT_H264_FLAGS</c> specifying which optional features are supported for the codec.</summary>
		public D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT_H264_FLAGS SupportFlags;
		/// <summary>A bitwise OR combination of flags specifying the allowed modes supported for disable_deblocking_filter_idc syntax for H264 spec.</summary>
	...
	{
		/// <summary>A bitwise OR combination of flags from the <c>D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT_HEVC_FLAGS</c> specifying which optional features are supported for the codec.</summary>
		public D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT_HEVC_FLAGS SupportFlags;
		/// <summary>The minimum luma coding block size requested. This value must match what the caller will code in the sequence parameter set (SPS).</summary>
		public D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_HEVC_CUSIZE MinLumaCodingUnitSize;
		/// <summary>The maximum luma coding block size requested. This value matches what the user will code in SPS.</summary>
		public D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_HEVC_CUSIZE MaxLumaCodingUnitSize;
		/// <summary>The minimum luma transform block size requested. This value matches the pixel size of what the user will code in SPS.log2_min_luma_transform_block_size_minus2.</summary>
		public D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_HEVC_TUSIZE MinLumaTransformUnitSize;
		/// <summary>The maximum luma transform block size requested. This value must be consistent with the pixel size the user will code in SPS.log2_diff_max_min_luma_transform_block_size. The variable MaxTbLog2SizeY is set equal to log2_min_luma_transform_block_size_minus2 + 2 + log2_diff_max_min_luma_transform_block_size.</summary>
		public D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_HEVC_TUSIZE MaxLumaTransformUnitSize;
		/// <summary>The maximum hierarchy depth for transform units of coding units coded in inter prediction mode. The value of max_transform_hierarchy_depth_inter shall be in the range of 0 to CtbLog2SizeY ? MinTbLog2SizeY, inclusive.</summary>
		public byte max_transform_hierarchy_depth_inter;
		/// <summary>Specifies the maximum hierarchy depth for transform units of coding units coded in intra prediction mode. The value of max_transform_hierarchy_depth_intra shall be in the range of 0 to CtbLog2SizeY ? MinTbLog2SizeY, inclusive.</summary>
		public byte max_transform_hierarchy_depth_intra;
	}
	...
	public struct D3D12_VIDEO_ENCODER_CODEC_PICTURE_CONTROL_SUPPORT
	{
		...
		/// <summary>A pointer to a <c>D3D12_VIDEO_ENCODER_CODEC_PICTURE_CONTROL_SUPPORT_H264</c> structure representing the picture control support structure for H.264 encoding.</summary>
		...
		/// <summary>A pointer to a <c>D3D12_VIDEO_ENCODER_CODEC_PICTURE_CONTROL_SUPPORT_HEVC</c> structure representing the picture control support structure for HEVC encoding.</summary>
	...
	// typedef struct D3D12_VIDEO_ENCODER_CODEC_PICTURE_CONTROL_SUPPORT_H264 { UINT MaxL0ReferencesForP; UINT MaxL0ReferencesForB; UINT MaxL1ReferencesForB; UINT MaxLongTermReferences; UINT MaxDPBCapacity; } D3D12_VIDEO_ENCODER_CODEC_PICTURE_CONTROL_SUPPORT_H264;
	...
	{
		/// <summary>The maximum value allowed in the slice headers for (num_ref_idx_l0_active_minus1 +1) when encoding P frames. This is equivalent to the maximum size of an L0 for a P frame supported.</summary>
		public uint MaxL0ReferencesForP;
		/// <summary>The maximum value allowed in the slice headers for (num_ref_idx_l0_active_minus1 +1) when encoding B frames. This is equivalent to the maximum size of an L0 for a B frame supported.</summary>
		public uint MaxL0ReferencesForB;
		/// <summary>The maximum value allowed in the slice headers for (num_ref_idx_l1_active_minus1 +1) when encoding B frames. This is equivalent to the maximum size of an L1 for a B frame supported.</summary>
		...
		/// <summary>The maximum number of unique pictures that can be used from the DPB the caller manages (number of unique indices in L0 union L1) for a given <c>EncodeFrame</c> command on the underlying hardware.</summary>
	...
	// typedef struct D3D12_VIDEO_ENCODER_CODEC_PICTURE_CONTROL_SUPPORT_HEVC { UINT MaxL0ReferencesForP; UINT MaxL0ReferencesForB; UINT MaxL1ReferencesForB; UINT MaxLongTermReferences; UINT MaxDPBCapacity; } D3D12_VIDEO_ENCODER_CODEC_PICTURE_CONTROL_SUPPORT_HEVC;
	...
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_compressed_bitstream
	// typedef struct D3D12_VIDEO_ENCODER_COMPRESSED_BITSTREAM { ID3D12Resource *pBuffer; UINT64 FrameStartOffset; } D3D12_VIDEO_ENCODER_COMPRESSED_BITSTREAM;
	...
	{
		/// <summary>A pointer to a <c>ID3D12Resource</c> containing the compressed bitstream buffer. Note that the resource buffer size is not the available size for this encoding operation because FrameStartOffset needs to be taken into account against this size.</summary>
		[MarshalAs(UnmanagedType.IUnknown)]
		public ID3D12Resource pBuffer;
		/// <summary>A UINT64 specifying th offset into the compressed bitstream where the encoder may start adding the current frame output.</summary>
		public ulong FrameStartOffset;
	}

	/// <summary>Describes an <c>ID3D12VideoEncoder</c>. Pass this structure into <c>ID3D12VideoDevice3::CreateVideoEncoder</c> to create an instance of <b>ID3D12VideoEncoder</b>.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_desc
	...
	public struct D3D12_VIDEO_ENCODER_DESC
	{
		...
		public D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION CodecConfiguration;
		/// <summary>A value from the <c>D3D12_VIDEO_ENCODER_MOTION_ESTIMATION_PRECISION_MODE</c> enumeration the maximum number of motion vectors allowed.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_encodeframe_input_arguments
	...
	public struct D3D12_VIDEO_ENCODER_ENCODEFRAME_INPUT_ARGUMENTS
	{
		...
		/// <summary>A UINT64 specifying the number of bytes added to the final bitstream between the end of the last <b>EncodeFrame</b> compressed bitstream output and the current call output. This is intended to capture the size of any headers or metadata messages added by the client to the final bitstream which are used as a hint by the rate control algorithms to keep track of the full bitstream size.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_encodeframe_output_arguments
	// typedef struct D3D12_VIDEO_ENCODER_ENCODEFRAME_OUTPUT_ARGUMENTS { D3D12_VIDEO_ENCODER_COMPRESSED_BITSTREAM Bitstream; D3D12_VIDEO_ENCODER_RECONSTRUCTED_PICTURE ReconstructedPicture; D3D12_VIDEO_ENCODER_ENCODE_OPERATION_METADATA_BUFFER EncoderOutputMetadata; } D3D12_VIDEO_ENCODER_ENCODEFRAME_OUTPUT_ARGUMENTS;
	...
	public struct D3D12_VIDEO_ENCODER_ENCODEFRAME_OUTPUT_ARGUMENTS
	{
		...
		/// <summary>A <c>D3D12_VIDEO_ENCODER_RECONSTRUCTED_PICTURE</c> representing a reconstructed picture generated from the input frame. This resource is only needed if the encoded picture is marked to be used as a reference picture in the corresponding picture control structure for this encode operation, NULL can be set otherwise as the reconstructed picture will not be written in the output.</summary>
		public D3D12_VIDEO_ENCODER_RECONSTRUCTED_PICTURE ReconstructedPicture;
		/// <summary>A <c>D3D12_VIDEO_ENCODER_ENCODE_OPERATION_METADATA_BUFFER</c> representing encoding metadata returned by the encoder in hardware-specific layout. This data must be resolved into a readable format using <c>ID3D12VIDEOCOMMANDLIST2::ResolveEncoderOutputMetadata</c>.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_frame_subregion_metadata
	// typedef struct D3D12_VIDEO_ENCODER_FRAME_SUBREGION_METADATA { UINT64 bSize; UINT64 bStartOffset; UINT64 bHeaderSize; } D3D12_VIDEO_ENCODER_FRAME_SUBREGION_METADATA;
	...
	public struct D3D12_VIDEO_ENCODER_FRAME_SUBREGION_METADATA
	{
		...
		///   <para>For example, let pFrameSubregionsSizes be an array of bSize for each slice. With this information, along with pFrameSubregionsSizes, the user can extract individual subregions from the output bitstream buffer by calculating i-th subregion start offset as <c>pBuffer + FrameStartOffset + sum j = (0, (i-1)){ pFrameSubregionsSizes[j] } + pFrameSubregionsStartOffsets[i]</c> and reading <c>pFrameSubregionsSizes[i]</c> bytes.</para>
		/// </summary>
	...
	/// <summary>Describes a <c>ID3D12VideoEncoderHeap</c>. Pass this structure into <c>ID3D12VideoDevice3::CreateVideoEncoderHeap</c> to create an instance of <b>ID3D12VideoEncoderHeap</b>.</summary>
	/// <remarks>If support for resolution dynamic reconfiguration is not supported, specify only one resolution in pResolutionList, denoting the desired target resolution.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_heap_desc
	// typedef struct D3D12_VIDEO_ENCODER_HEAP_DESC { UINT NodeMask; D3D12_VIDEO_ENCODER_HEAP_FLAGS Flags; D3D12_VIDEO_ENCODER_CODEC EncodeCodec; D3D12_VIDEO_ENCODER_PROFILE_DESC EncodeProfile; D3D12_VIDEO_ENCODER_LEVEL_SETTING EncodeLevel; UINT ResolutionsListCount; const D3D12_VIDEO_ENCODER_PICTURE_RESOLUTION_DESC *pResolutionList; } D3D12_VIDEO_ENCODER_HEAP_DESC;
	...
	public struct D3D12_VIDEO_ENCODER_HEAP_DESC
	{
		/// <summary>The node mask specifying the physical adapter on which the video processor will be used. For single GPU operation, set this to zero. If there are multiple GPU nodes, set a bit to identify the node, i.e. the device's physical adapter, to which the command queue applies. Each bit in the mask corresponds to a single node. Only 1 bit may be set.</summary>
		public uint NodeMask;
		/// <summary>A bitwise or combination of values from the <c>D3D12_VIDEO_ENCODER_HEAP_FLAGS</c> enumeration specifying encoder heap creation options.</summary>
		...
		public D3D12_VIDEO_ENCODER_CODEC EncodeCodec;
		/// <summary>A <c>D3D12_VIDEO_ENCODER_PROFILE_DESC</c> specifying the profile for the selected codec in the associated encoder object.</summary>
		public D3D12_VIDEO_ENCODER_PROFILE_DESC EncodeProfile;
		/// <summary>A <c>D3D12_VIDEO_ENCODER_LEVEL_SETTING</c> specifying the level for the selected codec in the associated encoder object.</summary>
		...
		public uint ResolutionsListCount;
		/// <summary>Pointer to an array of <c>D3D12_VIDEO_ENCODER_PICTURE_RESOLUTION_DESC</c> specifying the list of resolutions requested to be supported.</summary>
		public ArrayPointer<D3D12_VIDEO_ENCODER_PICTURE_RESOLUTION_DESC> pResolutionList;
	}

	/// <summary>Represents intra refresh settings for video encoding.</summary>
	/// <remarks>When triggering an intra-refresh session, the host informs the current frame number relative to the [0..IntraRefreshDuration) session by setting IntraRefreshFrameIndex in the picture control structures.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_intra_refresh
	// typedef struct D3D12_VIDEO_ENCODER_INTRA_REFRESH { D3D12_VIDEO_ENCODER_INTRA_REFRESH_MODE Mode; UINT IntraRefreshDuration; } D3D12_VIDEO_ENCODER_INTRA_REFRESH;
	...
	public struct D3D12_VIDEO_ENCODER_INTRA_REFRESH
	{
		...
		/// <summary>A UINT64 specifying the duration of the intra-refresh session, as a number of frames . For D3D12_VIDEO_ENCODER_INTRA_REFRESH_MODE_ROW_BASED, this value and the frame height define the size of the I rows for the duration of the IR session.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_level_tier_constraints_hevc
	// typedef struct D3D12_VIDEO_ENCODER_LEVEL_TIER_CONSTRAINTS_HEVC { D3D12_VIDEO_ENCODER_LEVELS_HEVC Level; D3D12_VIDEO_ENCODER_TIER_HEVC Tier; } D3D12_VIDEO_ENCODER_LEVEL_TIER_CONSTRAINTS_HEVC;
	...
	/// <remarks><b>D3D12_VIDEO_ENCODER_OUTPUT_METADATA</b> and its child structures are all aligned to a 64-bit access boundary for use with <c>SetPredication</c>.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_output_metadata
	// typedef struct D3D12_VIDEO_ENCODER_OUTPUT_METADATA { UINT64 EncodeErrorFlags; D3D12_VIDEO_ENCODER_OUTPUT_METADATA_STATISTICS EncodeStats; UINT64 EncodedBitstreamWrittenBytesCount; UINT64 WrittenSubregionsCount; } D3D12_VIDEO_ENCODER_OUTPUT_METADATA;
	...
	{
		/// <summary>A <b>UINT64</b> representing a bitwise OR combination of values from the <c>D3D12_VIDEO_ENCODER_ENCODE_ERROR_FLAG</c> enumeration specifying information about the encode execution status.</summary>
		...
		/// <summary>Output field that receives a <b>UINT64</b> indicating how many bytes were into <c>D3D12_VIDEO_ENCODER_COMPRESSED_BITSTREAM.pBuffer</c> plus the value of <c>D3D12_VIDEO_ENCODER_COMPRESSED_BITSTREAM.FrameStartOffset</c>.</summary>
		...
		///   <para>This value is coherent with the settings specified in <c>D3D12_VIDEO_ENCODER_SEQUENCE_CONTROL_DESC.pFrameSubregionsLayoutData</c>. If a number of subregions was specified, WrittenSubregionsCount should match that value. If another mode was used, this field is how the driver reports the final number of subregions. If the output is a full frame, then there is only 1 subregion.</para>
		/// </summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_output_metadata_statistics
	// typedef struct D3D12_VIDEO_ENCODER_OUTPUT_METADATA_STATISTICS { UINT64 AverageQP; UINT64 IntraCodingUnitsCount; UINT64 InterCodingUnitsCount; UINT64 SkipCodingUnitsCount; UINT64 AverageMotionEstimationXDirection; UINT64 AverageMotionEstimationYDirection; } D3D12_VIDEO_ENCODER_OUTPUT_METADATA_STATISTICS;
	...
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_picture_control_codec_data
	// typedef struct D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA { UINT DataSize; union { D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA_H264 *pH264PicData; D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA_HEVC *pHEVCPicData; D3D12_VIDEO_ENCODER_AV1_PICTURE_CONTROL_CODEC_DATA *pAV1PicData; }; } D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA;
	[PInvokeData("d3d12video.h", MSDNShortId = "NS:d3d12video.D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA")]
	[StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
	public struct D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA
	{
		...
		private IntPtr union;
		/// <summary>A pointer to a <c>D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA_H264</c> representing the picture level control elements for H.264 encoding.</summary>
		...
		/// <summary>A pointer to a <c>D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA_HEVC</c> representing the picture level control elements for H.264 encoding.</summary>
	...
	{
		/// <summary>A bitwise OR combination of values from the <c>D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA_H264_FLAGS</c> enumeration specifying configuration flags for the frame being encoded.</summary>
		public D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA_H264_FLAGS Flags;
		/// <summary>A value from the <c>D3D12_VIDEO_ENCODER_FRAME_TYPE_H264</c> enumeration specifying the picture type. Make sure that the codec-specific flags support the specified type. This selection must be kept in sync with the GOP structure configuration set by the host. Note that the GOP is defined in display order and this pic type selection must follow the GOP, but in encode order.</summary>
		...
		/// <summary>A <b>UINT</b> specifying the picture layer number in temporal hierarchy. Check for the maximum number of layers in <c>D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT_H264</c>.</summary>
		...
		/// <summary>A pointer to a <b>UINT</b> array specifying the list of past frame reference frames to be used for this frame. Each integer value in this array indices into pReferenceFramesReconPictureDescriptors to reference pictures kept in the DPB.</summary>
		...
		/// <summary>A pointer to a <b>UINT</b> array specifying the list of future frame reference frames to be used for this frame. Each integer value in this array indices into pReferenceFramesReconPictureDescriptors to reference pictures kept in the DPB.</summary>
		...
		/// <summary>
		///   <para>A pointer to a <b>UINT</b> array that describes the current state of the DPB buffer kept in <c>D3D12_VIDEO_ENCODER_PICTURE_CONTROL_DESC.ReferenceFrames</c>. The pList0ReferenceFrames and pList1ReferenceFrames lists indices map from past/future references into this descriptors array.</para>
		...
		///     </item>
		///     <item>
		///       <description>Sliding window reference picture marking mode: A marking mode providing a first-in first-out mechanism for short-term reference pictures.</description>
		///       <description>Adaptive reference picture marking mode: A reference picture marking mode providing syntax elements to specify marking of reference pictures as "unused for reference" and to assign long-term frame indices.</description>
		///     </item>
		///   </list>
		/// </summary>
		public byte adaptive_ref_pic_marking_mode_flag;
		/// <summary>A <b>UINT</b> specifying the number of reference pictures marking operations associated with the current frame. Requires that adaptive_ref_pic_marking_mode_flag is set to 1.</summary>
		public uint RefPicMarkingOperationsCommandsCount;
		/// <summary>A pointer to an array of <c>D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA_H264_REFERENCE_PICTURE_MARKING_OPERATION</c> structures representing the list of reference pictures marking operations associated with the current frame. The operations described by this list need to be reflected in the DPB descriptors accordingly during the encoding session.</summary>
		...
		/// <summary>A pointer to an array of <c>D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA_H264_REFERENCE_PICTURE_LIST_MODIFICATION_OPERATION</c> structures representing the list of reference picture list modification operations for the pList0ReferenceFrames list.</summary>
		...
		/// <summary>A pointer to an array of <c>D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA_H264_REFERENCE_PICTURE_LIST_MODIFICATION_OPERATION</c> structures representing the list of reference picture list modification operations for the pList1ReferenceFrames list.</summary>
		...
		/// <summary>A <b>UINT</b> specifying the number of elements present in pRateControlQPMap. This should match the number of coding blocks in the frame, rounding up the frame resolution to the closest aligned values.</summary>
		public uint QPMapValuesCount;
		/// <summary>A pointer to an array of <b>Int8</b> containing, in row/column scan order, the QP map values to use on each squared region for this frame. The QP map dimensions can be calculated using the current resolution and <c>D3D12_FEATURE_DATA_VIDEO_ENCODER_RESOLUTION_SUPPORT_LIMITS.QPMapRegionPixelsSize</c> conveying the squared region sizes.</summary>
	...
	/// <remarks>For more information, refer to H264 specification for more details: section 7.4.3.1 "Reference picture list modification semantics". .</remarks>
	...
	// typedef struct D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA_H264_REFERENCE_PICTURE_LIST_MODIFICATION_OPERATION { UCHAR modification_of_pic_nums_idc; UINT abs_diff_pic_num_minus1; UINT long_term_pic_num; } D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA_H264_REFERENCE_PICTURE_LIST_MODIFICATION_OPERATION;
	...
	public struct D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA_H264_REFERENCE_PICTURE_LIST_MODIFICATION_OPERATION
	{
		...
		/// <summary>Specifies the absolute difference between the picture number of the picture being moved to the current index in the list and the picture number prediction value.</summary>
	...
	/// <summary>Describes changes in the reference pictures as memory operations as a tuple of an operation identifier and associated parameters needed for the operation.</summary>
	/// <remarks>
	...
	/// <para>– If <b>max_long_term_frame_idx_plus1</b> is equal to 0, MaxLongTermFrameIdx is set equal to "no long-term frame indices". – Otherwise (<b>max_long_term_frame_idx_plus1</b> is greater than 0), MaxLongTermFrameIdx is set equal to max_long_term_frame_idx_plus1 ? 1.</para>
	/// <para>The operation of marking the current frame as a short-term reference is given by a flag present in the <c>D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA_H264_FLAGS</c> structure with its corresponding <b>PictureOrderCountNumber</b> and <b>FrameDecodingOrderNumber</b> values indicated in the associated picture control structure.</para>
	/// <para>The variable max_num_ref_frames is indicated in this API by the maximum DPB capacity reported.</para>
	/// <para>Note that for marking an IDR frame as long-term reference, the proposed explicit mechanism is to mark it as short term reference first, by setting <c>D3D12_VIDEO_ENCODER_PICTURE_CONTROL_FLAG_USED_AS_REFERENCE_PICTURE</c> when calling <c>EncodeFrame</c> for the IDR frame, and later promoting it to be a long-term reference frame using memory management operation '3' to mark a short-term reference picture as "used for long-term reference" and assign a long-term frame index to it.</para>
	...
	public struct D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA_H264_REFERENCE_PICTURE_MARKING_OPERATION
	{
		...
		/// <summary>Used with <b>memory_management_control_operation</b> equal to 3 or 1 to assign a long-term frame index to a short-term reference picture or to mark a short-term reference picture as "unused for reference".</summary>
		public uint difference_of_pic_nums_minus1;
		/// <summary>Used with <b>memory_management_control_operation</b> equal to 2 to mark a long-term reference picture as "unused for reference".</summary>
		public uint long_term_pic_num;
		...
		/// <summary>The value minus 1 specifies the maximum value of long-term frame index allowed for long-term reference pictures (until receipt of another value of <b>max_long_term_frame_idx_plus1</b>).</summary>
	...
	/// <description>(MaxReferenceFramesInDPB/<c>previous reference frames</c>/ + 1 /<c>additional current frame recon pic</c>/) - 1/*<c>minus1 for header</c>/;</description>
	/// <description>None</description>
	/// </item>
	/// <item>
	...
	/// <description>1 if sending down D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA_HEVC lists modifications. Otherwise set as 0.</description>
	/// <description>None</description>
	/// </item>
	/// <item>
	...
	{
		/// <summary>A bitwise OR combination of values from the <c>D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA_HEVC_FLAGS</c> enumeration specifying configuration flags for the frame being encoded.</summary>
		public D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA_HEVC_FLAGS Flags;
		/// <summary>A value from the <c>D3D12_VIDEO_ENCODER_FRAME_TYPE_HEVC</c> enumeration specifying the picture type. Make sure that the codec-specific flags support the specified type. This selection must be kept in sync with the GOP structure configuration set by the host. Note that the GOP is defined in display order and this pic type selection must follow the GOP, but in encode order.</summary>
		public D3D12_VIDEO_ENCODER_FRAME_TYPE_HEVC FrameType;
		/// <summary>A <b>UINT</b> specifying the value to be used in the slice headers of the current frame to reference the PPS.</summary>
		public uint slice_pic_parameter_set_id;
		/// <summary>A <b>UINT</b> specifying the current frame display order.</summary>
		public uint PictureOrderCountNumber;
		/// <summary>A <b>UINT</b> specifying the picture layer number in temporal hierarchy. Check for the maximum number of layers in <c>D3D12_VIDEO_ENCODER_CODEC_CONFIGURATION_SUPPORT_HEVC</c>.</summary>
		public uint TemporalLayerIndex;
	...
	/// <para>If coding temporal layers, a picture can only use as references pictures on lower TemporalLayerIndex than its own. The temporal layer indices are specified in the picture control structure and in the reference picture descriptors.</para>
	/// <para>The HW limitations for the number of reference pictures are expressed in terms of the maximum number of elements present in L0 (MaxL0ReferencesForP/MaxL0ReferencesForB) and L1 (MaxL1ReferencesForB) lists and limiting by MaxDPBCapacity the maximum number of unique indices in (L0 union L1) that map into the value of pReferenceFramesReconPictureDescriptors provided in <c>D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA</c>.</para>
	/// <para>There's no limitation to the number of DPB entries being passed in pReferenceFramesReconPictureDescriptors, but instead in the number of entries on that array being references by the L0 and L1 lists. This allows the user to track the state of a DPB in pReferenceFramesReconPictureDescriptors within the restrictions defined by the codec standard limitations and only use a subset restricted by the hardware limitations when calling <b>EncodeFrame</b>. For example, for HEVC encoding, the caller could keep track of the latest 15 encoded pictures in pReferenceFramesReconPictureDescriptors but only use a subset of the pictures that falls within the hardware restrictions, by assigning a limited number of unique indices in the L0 and L1 lists.</para>
	...
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_picture_control_desc
	// typedef struct D3D12_VIDEO_ENCODER_PICTURE_CONTROL_DESC { UINT IntraRefreshFrameIndex; D3D12_VIDEO_ENCODER_PICTURE_CONTROL_FLAGS Flags; D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA PictureControlCodecData; D3D12_VIDEO_ENCODE_REFERENCE_FRAMES ReferenceFrames; } D3D12_VIDEO_ENCODER_PICTURE_CONTROL_DESC;
	...
	{
		/// <summary>When requesting an intra-refresh wave for IntraRefreshFramesDuration frames by specifying the <c>D3D12_VIDEO_ENCODER_SEQUENCE_CONTROL_FLAG_REQUEST_INTRA_REFRESH</c> flag, this value indicates, for the current picture, the index of the frame in the intra-refresh wave. The value range is set by the host between 0 and <c>IntraRefreshFramesDuration</c> to hint the status of the intra-refresh session to the driver.</summary>
		public uint IntraRefreshFrameIndex;
		/// <summary>A bitwise OR combination of values from the <c>D3D12_VIDEO_ENCODER_PICTURE_CONTROL_FLAGS</c> enumeration specifying the picture control descriptor flags.</summary>
		public D3D12_VIDEO_ENCODER_PICTURE_CONTROL_FLAGS Flags;
		/// <summary>A <c>D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA</c> structure containing codec-specific picture control data. Depending of the selected rate control mode the QP values are interpreted differently.</summary>
		public D3D12_VIDEO_ENCODER_PICTURE_CONTROL_CODEC_DATA PictureControlCodecData;
		/// <summary>A <c>D3D12_VIDEO_ENCODE_REFERENCE_FRAMES</c> structure containing the reconstructed pictures from the past encoding operations outputs.</summary>
	...
	public struct D3D12_VIDEO_ENCODER_PICTURE_CONTROL_SUBREGIONS_LAYOUT_DATA
	{
		...
		private IntPtr union;
		/// <summary>A <c>D3D12_VIDEO_ENCODER_PICTURE_CONTROL_SUBREGIONS_LAYOUT_DATA_SLICES</c> Defines subregions as slices for H.264 encoding.</summary>
		...
		/// <summary>A <c>D3D12_VIDEO_ENCODER_PICTURE_CONTROL_SUBREGIONS_LAYOUT_DATA_SLICES</c> Defines subregions as slices for HEVC encoding.</summary>
	...
	/// <remarks>For modes that imply a fixed number of slices, the number of slices selected must be less than indicated by <c>D3D12_FEATURE_DATA_VIDEO_ENCODER_RESOLUTION_SUPPORT_LIMITS.MaxSubregionsNumber</c> and the selected resolution.</remarks>
	...
	// typedef struct D3D12_VIDEO_ENCODER_PICTURE_CONTROL_SUBREGIONS_LAYOUT_DATA_SLICES { union { UINT MaxBytesPerSlice; UINT NumberOfCodingUnitsPerSlice; UINT NumberOfRowsPerSlice; UINT NumberOfSlicesPerFrame; }; } D3D12_VIDEO_ENCODER_PICTURE_CONTROL_SUBREGIONS_LAYOUT_DATA_SLICES;
	...
	public struct D3D12_VIDEO_ENCODER_PICTURE_CONTROL_SUBREGIONS_LAYOUT_DATA_SLICES
	{
		...
		/// <summary>The number of squared blocks to be used per slice. The size in pixels of the squared regions can be calculated using the current resolution and <c>D3D12_FEATURE_DATA_VIDEO_ENCODER_RESOLUTION_SUPPORT_LIMITS.SubregionBlockPixelsSize</c> for the current frame resolution. This field is used exclusively with <c>D3D12_VIDEO_ENCODER_FRAME_SUBREGION_LAYOUT_MODE_SQUARE_UNITS_PER_SUBREGION_ROW_UNALIGNED</c>.</summary>
		[FieldOffset(0)]
		public uint NumberOfCodingUnitsPerSlice;
		/// <summary>The number of squared blocks rows per slice for the frame to be divided into. The size in pixels of the squared regions can be calculated using the current resolution and <c>D3D12_FEATURE_DATA_VIDEO_ENCODER_RESOLUTION_SUPPORT_LIMITS.SubregionBlockPixelsSize</c> for the current frame resolution. This field is used exclusively with <c>D3D12_VIDEO_ENCODER_FRAME_SUBREGION_LAYOUT_MODE_UNIFORM_PARTITIONING_ROWS_PER_SUBREGION</c>.</summary>
		[FieldOffset(0)]
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_picture_resolution_desc
	// typedef struct D3D12_VIDEO_ENCODER_PICTURE_RESOLUTION_DESC { UINT Width; UINT Height; } D3D12_VIDEO_ENCODER_PICTURE_RESOLUTION_DESC;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_profile_desc
	// typedef struct D3D12_VIDEO_ENCODER_PROFILE_DESC { UINT DataSize; union { D3D12_VIDEO_ENCODER_PROFILE_H264 *pH264Profile; D3D12_VIDEO_ENCODER_PROFILE_HEVC *pHEVCProfile; D3D12_VIDEO_ENCODER_AV1_PROFILE *pAV1Profile; }; } D3D12_VIDEO_ENCODER_PROFILE_DESC;
	[PInvokeData("d3d12video.h", MSDNShortId = "NS:d3d12video.D3D12_VIDEO_ENCODER_PROFILE_DESC")]
	[StructLayout(LayoutKind.Explicit)]
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_rate_control
	// typedef struct D3D12_VIDEO_ENCODER_RATE_CONTROL { D3D12_VIDEO_ENCODER_RATE_CONTROL_MODE Mode; D3D12_VIDEO_ENCODER_RATE_CONTROL_FLAGS Flags; D3D12_VIDEO_ENCODER_RATE_CONTROL_CONFIGURATION_PARAMS ConfigParams; DXGI_RATIONAL TargetFrameRate; } D3D12_VIDEO_ENCODER_RATE_CONTROL;
	...
	public struct D3D12_VIDEO_ENCODER_RATE_CONTROL
	{
		...
		/// <summary>
		///   <para>A <c>D3D12_VIDEO_ENCODER_RATE_CONTROL_CONFIGURATION_PARAMS</c> structure representing rate control configuration parameters corresponding to the specified Mode. Note that for absolute QP matrix mode, the configuration arguments are provided per EncodeFrame basis.</para>
		///   <para>If the selected rate control mode is <b>D3D12_VIDEO_ENCODER_RATE_CONTROL_MODE_ABSOLUTE_QP_MAP</b>, the QP values in pRateControlQPMap are treated as absolute QP values.</para>
		///   <para>For the other rate control modes, the QP values in pRateControlQPMap are interpreted as a delta QP map to be used for the current frame encode operation. The values provided in the map are incremented/decremented on top of the QP values decided by the rate control algorithm or the baseline QP constant set in CQP mode.</para>
		/// </summary>
		public D3D12_VIDEO_ENCODER_RATE_CONTROL_CONFIGURATION_PARAMS ConfigParams;
		/// <summary>A <c>DXGI_RATIONAL</c> specifying the target frame rate for the encoded stream. This value is a hint for the rate control budgeting algorithm.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_rate_control_cbr
	// typedef struct D3D12_VIDEO_ENCODER_RATE_CONTROL_CBR { UINT InitialQP; UINT MinQP; UINT MaxQP; UINT64 MaxFrameBitSize; UINT64 TargetBitRate; UINT64 VBVCapacity; UINT64 InitialVBVFullness; } D3D12_VIDEO_ENCODER_RATE_CONTROL_CBR;
	...
	{
		/// <summary>When <c>D3D12_VIDEO_ENCODER_RATE_CONTROL_FLAG_ENABLE_INITIAL_QP</c> is enabled, allows the Initial QP to be used by the rate control algorithm.</summary>
		public uint InitialQP;
		/// <summary>When <c>D3D12_VIDEO_ENCODER_RATE_CONTROL_FLAG_ENABLE_QP_RANGE</c> is enabled, limits QP range of the rate control algorithm.</summary>
		public uint MinQP;
		/// <summary>When <c>D3D12_VIDEO_ENCODER_RATE_CONTROL_FLAG_ENABLE_QP_RANGE</c> is enabled, limits QP range of the rate control algorithm.</summary>
		public uint MaxQP;
		/// <summary>Maximum size in bits for each frame to be coded. When <c>D3D12_VIDEO_ENCODER_RATE_CONTROL_FLAG_ENABLE_MAX_FRAME_SIZE</c> is enabled, limits each frame maximum size in the rate control algorithm.</summary>
		...
		/// <summary>When <c>D3D12_VIDEO_ENCODER_SUPPORT_FLAG_RATE_CONTROL_VBV_SIZE_CONFIG_AVAILABLE</c> is enabled, specifies the capacity in bits of the Video Buffer Verifier to be used in the rate control algorithm.</summary>
		public ulong VBVCapacity;
		/// <summary>When <c>D3D12_VIDEO_ENCODER_SUPPORT_FLAG_RATE_CONTROL_VBV_SIZE_CONFIG_AVAILABLE</c> is enabled, specifies the initial fullness in bits of the Video Buffer Verifier to be used in the rate control algorithm.</summary>
	...
	public struct D3D12_VIDEO_ENCODER_RATE_CONTROL_CONFIGURATION_PARAMS
	{
		...
		private IntPtr union;
		/// <summary>A <c>D3D12_VIDEO_ENCODER_RATE_CONTROL_CQP</c> structure representing the rate control structure definition for constant quantization parameter mode.</summary>
		...
		/// <summary>A <c>D3D12_VIDEO_ENCODER_RATE_CONTROL_CBR</c> structure representing the rate control structure definition for constant bitrate mode.</summary>
		...
		/// <summary>A <c>D3D12_VIDEO_ENCODER_RATE_CONTROL_VBR</c> structure representing the rate control structure definition for variable bitrate mode.</summary>
		...
		/// <summary>A <c>D3D12_VIDEO_ENCODER_RATE_CONTROL_QVBR</c> structure representing the rate control structure definition for constant quality target with constrained bitrate mode.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_rate_control_cqp
	// typedef struct D3D12_VIDEO_ENCODER_RATE_CONTROL_CQP { UINT ConstantQP_FullIntracodedFrame; UINT ConstantQP_InterPredictedFrame_PrevRefOnly; UINT ConstantQP_InterPredictedFrame_BiDirectionalRef; } D3D12_VIDEO_ENCODER_RATE_CONTROL_CQP;
	...
	public struct D3D12_VIDEO_ENCODER_RATE_CONTROL_CQP
	{
		...
		/// <summary>A UINT64 specifying the quantization parameter that should be used for each encoded frame that has inter-picture references to pictures (in display order) before the current one.</summary>
		public uint ConstantQP_InterPredictedFrame_PrevRefOnly;
		/// <summary>A UINT64 specifying the quantization parameter that should be used for each encoded frame that has inter-picture references to pictures (in display order) both from previous and next frames.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_rate_control_qvbr
	// typedef struct D3D12_VIDEO_ENCODER_RATE_CONTROL_QVBR { UINT InitialQP; UINT MinQP; UINT MaxQP; UINT64 MaxFrameBitSize; UINT64 TargetAvgBitRate; UINT64 PeakBitRate; UINT ConstantQualityTarget; } D3D12_VIDEO_ENCODER_RATE_CONTROL_QVBR;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_rate_control_vbr
	// typedef struct D3D12_VIDEO_ENCODER_RATE_CONTROL_VBR { UINT InitialQP; UINT MinQP; UINT MaxQP; UINT64 MaxFrameBitSize; UINT64 TargetAvgBitRate; UINT64 PeakBitRate; UINT64 VBVCapacity; UINT64 InitialVBVFullness; } D3D12_VIDEO_ENCODER_RATE_CONTROL_VBR;
	...
	public struct D3D12_VIDEO_ENCODER_RATE_CONTROL_VBR
	{
		...
		/// <summary>When <c>D3D12_VIDEO_ENCODER_RATE_CONTROL_FLAG_ENABLE_QP_RANGE</c> is enabled, limits QP range of the rate control algorithm.</summary>
		public uint MaxQP;
		/// <summary>The maximum size, in bits, for each frame to be coded. When <c>D3D12_VIDEO_ENCODER_RATE_CONTROL_FLAG_ENABLE_MAX_FRAME_SIZE</c> is enabled, limits each frame maximum size in the rate control algorithm.</summary>
		public ulong MaxFrameBitSize;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_reconstructed_picture
	// typedef struct D3D12_VIDEO_ENCODER_RECONSTRUCTED_PICTURE { ID3D12Resource *pReconstructedPicture; UINT ReconstructedPictureSubresource; } D3D12_VIDEO_ENCODER_RECONSTRUCTED_PICTURE;
	...
	// typedef struct D3D12_VIDEO_ENCODER_REFERENCE_PICTURE_DESCRIPTOR_H264 { UINT ReconstructedPictureResourceIndex; BOOL IsLongTermReference; UINT LongTermPictureIdx; UINT PictureOrderCountNumber; UINT FrameDecodingOrderNumber; UINT TemporalLayerIndex; } D3D12_VIDEO_ENCODER_REFERENCE_PICTURE_DESCRIPTOR_H264;
	...
	{
		/// <summary>Maps the current reference picture described by this structure to a resource in the <c>D3D12_VIDEO_ENCODER_PICTURE_CONTROL_DESC.ReferenceFrames</c> array.</summary>
		public uint ReconstructedPictureResourceIndex;
		...
		public uint PictureOrderCountNumber;
		/// <summary>The frame decode order with semantic as indicated by the slice header frame_num syntax element associated with the encoded reference picture.</summary>
		public uint FrameDecodingOrderNumber;
	...
	{
		/// <summary>A <b>UINT</b> that maps the current reference picture described by this structure to a resource in the <c>D3D12_VIDEO_ENCODER_PICTURE_CONTROL_DESC.ReferenceFrames</c> array.</summary>
		public uint ReconstructedPictureResourceIndex;
		/// <summary>A <b>BOOL</b> indicating whether this descriptor entry is being used by the current picture by being indexed from either L0 and/or L1 lists.</summary>
	...
	public struct D3D12_VIDEO_ENCODER_RESOLVE_METADATA_INPUT_ARGUMENTS
	{
		...
		/// <summary>A <c>D3D12_VIDEO_ENCODER_PROFILE_DESC</c> specifying the profile for the selected codec in the associated encode operation.</summary>
		...
		/// <summary>A <c>D3D12_VIDEO_ENCODER_ENCODE_OPERATION_METADATA_BUFFER</c> representing the associated opaque metadata buffer received from <c>EncodeFrame</c>.</summary>
	...
	// typedef struct D3D12_VIDEO_ENCODER_RESOLVE_METADATA_OUTPUT_ARGUMENTS { D3D12_VIDEO_ENCODER_ENCODE_OPERATION_METADATA_BUFFER ResolvedLayoutMetadata; } D3D12_VIDEO_ENCODER_RESOLVE_METADATA_OUTPUT_ARGUMENTS;
	...
	public struct D3D12_VIDEO_ENCODER_RESOLVE_METADATA_OUTPUT_ARGUMENTS
	{
		...
		///   <para>This buffer must be read back to the CPU by the caller and cast to a <c>D3D12_VIDEO_ENCODER_OUTPUT_METADATA</c> structure. The remaining data in the buffer, corresponds to <b>D3D12_VIDEO_ENCODER_OUTPUT_METADATA.WrittenSubregionsCount</b> packed entries of type <c>D3D12_VIDEO_ENCODER_FRAME_SUBREGION_METADATA</c>.</para>
		/// </summary>
	...
	{
		/// <summary>A bitwise OR combination of values from the <c>D3D12_VIDEO_ENCODER_SEQUENCE_CONTROL_FLAGS</c> enumeration specifying the sequence control descriptor flags for the current operation.</summary>
		public D3D12_VIDEO_ENCODER_SEQUENCE_CONTROL_FLAGS Flags;
		/// <summary>
		///   <para>A <c>D3D12_VIDEO_ENCODER_INTRA_REFRESH</c> structure specifying the parameters for the intra-refresh mode that should be used when triggering intra-refresh sessions.</para>
		///   <para>The use of bidirectional reference frames (B Frames) is mutually exclusive with intra-refresh. Callers should verify that the GOP structure doesn't contain B frames if intra-refresh modes are active. When querying for <c>D3D12_FEATURE_DATA_VIDEO_ENCODER_SUPPORT</c> with an intra-refresh mode other than <c>D3D12_VIDEO_ENCODER_INTRA_REFRESH_MODE_NONE</c>, the specified <c>D3D12_VIDEO_ENCODER_SEQUENCE_GOP_STRUCTURE</c> set must not contain B frames, or the query will return no support.</para>
		...
		///   <para>Check support for resolution dynamic reconfiguration in <c>D3D12_VIDEO_ENCODER_SUPPORT_FLAG_RESOLUTION_RECONFIGURATION_AVAILABLE</c>. If no support is given for this, PictureTargetResolution indicates the target resolution of the full encoding session and must not be changed during the encoding session. Otherwise this indicates the resolution used for the associated <b>EncodeFrame</b> command.</para>
		...
		public D3D12_VIDEO_ENCODER_PICTURE_RESOLUTION_DESC PictureTargetResolution;
		/// <summary>
		///   <para>A value from the <c>D3D12_VIDEO_ENCODER_FRAME_SUBREGION_LAYOUT_MODE</c> enumeration specifying which layout mode is being used and therefore which union members to use in pFrameSubregionsLayoutData.</para>
		...
		/// <summary>A <c>D3D12_VIDEO_ENCODER_PICTURE_CONTROL_SUBREGIONS_LAYOUT_DATA</c> structure specifying picture subregions layout optional codec-specific data. If the specified SelectedLayoutMode value indicates that such that there are no subregions, null is expected.</summary>
		public D3D12_VIDEO_ENCODER_PICTURE_CONTROL_SUBREGIONS_LAYOUT_DATA FrameSubregionsLayoutData;
		/// <summary>
		///   <para>A <c>D3D12_VIDEO_ENCODER_SEQUENCE_GOP_STRUCTURE</c> structure specifying the current GOP used in the video sequence, in display order. Calls to <c>EncodeFrame</c> must follow this GOP but in encode order after B-Frames reordering.</para>
		///   <para>Check support for GOP dynamic reconfiguration in <c>D3D12_VIDEO_ENCODER_SUPPORT_FLAG_SEQUENCE_GOP_RECONFIGURATION_AVAILABLE</c>. If no support is given for this, CodecGopSequence indicates the target GOP pattern (in display order) of the full encoding session and must not be changed during the encoding session. Otherwise this indicates the new GOP subregion mode used starting at the associated <b>EncodeFrame</b> command.</para>
		/// </summary>
		public D3D12_VIDEO_ENCODER_SEQUENCE_GOP_STRUCTURE CodecGopSequence;
	}

	/// <summary>Represents the GOP structure for multiple video codecs.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_sequence_gop_structure
	// typedef struct D3D12_VIDEO_ENCODER_SEQUENCE_GOP_STRUCTURE { UINT DataSize; union { D3D12_VIDEO_ENCODER_SEQUENCE_GOP_STRUCTURE_H264 *pH264GroupOfPictures; D3D12_VIDEO_ENCODER_SEQUENCE_GOP_STRUCTURE_HEVC *pHEVCGroupOfPictures; D3D12_VIDEO_ENCODER_AV1_SEQUENCE_STRUCTURE *pAV1SequenceStructure; }; } D3D12_VIDEO_ENCODER_SEQUENCE_GOP_STRUCTURE;
	[PInvokeData("d3d12video.h", MSDNShortId = "NS:d3d12video.D3D12_VIDEO_ENCODER_SEQUENCE_GOP_STRUCTURE")]
	[StructLayout(LayoutKind.Sequential)]
	...
	// typedef struct D3D12_VIDEO_ENCODER_SEQUENCE_GOP_STRUCTURE_H264 { UINT GOPLength; UINT PPicturePeriod; UCHAR pic_order_cnt_type; UCHAR log2_max_frame_num_minus4; UCHAR log2_max_pic_order_cnt_lsb_minus4; } D3D12_VIDEO_ENCODER_SEQUENCE_GOP_STRUCTURE_H264;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_encoder_sequence_gop_structure_hevc
	// typedef struct D3D12_VIDEO_ENCODER_SEQUENCE_GOP_STRUCTURE_HEVC { UINT GOPLength; UINT PPicturePeriod; UCHAR log2_max_pic_order_cnt_lsb_minus4; } D3D12_VIDEO_ENCODER_SEQUENCE_GOP_STRUCTURE_HEVC;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_extension_command_desc
	// typedef struct D3D12_VIDEO_EXTENSION_COMMAND_DESC { UINT NodeMask; GUID CommandId; } D3D12_VIDEO_EXTENSION_COMMAND_DESC;
	...
	/// <summary>Describes a video extension command.</summary>
	/// <remarks>An array of this structure is provided in a <c>D3D12_FEATURE_DATA_VIDEO_EXTENSION_COMMANDS</c> structure returned from a call to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_EXTENSION_COMMANDS</c>.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_extension_command_info
	...
	public struct D3D12_VIDEO_EXTENSION_COMMAND_INFO
	{
		...
		/// <summary>A member of the <c>D3D12_COMMAND_LIST_SUPPORT_FLAGS</c> enumeration. Indicates the video command queue that the video extension targets. Only one value from the enumeration can be set.</summary>
	...
	/// <remarks>An array of this structure is provided in a <c>D3D12_FEATURE_DATA_VIDEO_EXTENSION_COMMAND_PARAMETERS</c> structure returned from a call to <c>ID3D12VideoDevice::CheckFeatureSupport</c> when the feature specified is <c>D3D12_FEATURE_VIDEO_EXTENSION_COMMAND_PARAMETERS</c>.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_extension_command_parameter_info
	// typedef struct D3D12_VIDEO_EXTENSION_COMMAND_PARAMETER_INFO { LPCWSTR Name; D3D12_VIDEO_EXTENSION_COMMAND_PARAMETER_TYPE Type; D3D12_VIDEO_EXTENSION_COMMAND_PARAMETER_FLAGS Flags; } D3D12_VIDEO_EXTENSION_COMMAND_PARAMETER_INFO;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_format
	// typedef struct D3D12_VIDEO_FORMAT { DXGI_FORMAT Format; DXGI_COLOR_SPACE_TYPE ColorSpace; } D3D12_VIDEO_FORMAT;
	...
	/// <summary>Describes a <c>ID3D12VideoMotionEstimator</c>. Pass this structure into <c>ID3D12VideoDevice1::CreateVideoMotionEstimator</c> to create an instance of <b>ID3D12VideoMotionEstimator</b>.</summary>
	/// <remarks>Call <c>ID3D12VideoDevice::CheckFeatureSupport</c> and specify <c>D3D12_FEATURE_VIDEO_MOTION_ESTIMATOR</c> as the feature to determine supported values.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_motion_estimator_desc
	// typedef struct D3D12_VIDEO_MOTION_ESTIMATOR_DESC { UINT NodeMask; DXGI_FORMAT InputFormat; D3D12_VIDEO_MOTION_ESTIMATOR_SEARCH_BLOCK_SIZE BlockSize; D3D12_VIDEO_MOTION_ESTIMATOR_VECTOR_PRECISION Precision; D3D12_VIDEO_SIZE_RANGE SizeRange; } D3D12_VIDEO_MOTION_ESTIMATOR_DESC;
	...
	public struct D3D12_VIDEO_MOTION_ESTIMATOR_DESC
	{
		...
		public DXGI_FORMAT InputFormat;
		/// <summary>A value from the <c>D3D12_VIDEO_MOTION_ESTIMATOR_SEARCH_BLOCK_SIZE</c> enumeration specifying the search block size the video motion estimator will use.</summary>
		public D3D12_VIDEO_MOTION_ESTIMATOR_SEARCH_BLOCK_SIZE BlockSize;
		/// <summary>A value from the <c>D3D12_VIDEO_MOTION_ESTIMATOR_VECTOR_PRECISION</c> enumeration specifying the vector precision the video motion estimator will use.</summary>
		public D3D12_VIDEO_MOTION_ESTIMATOR_VECTOR_PRECISION Precision;
		/// <summary>A <c>D3D12_VIDEO_SIZE_RANGE</c> structure representing the minimum and maximum input and reference frame size, in pixels, that the motion estimator will accept.</summary>
		public D3D12_VIDEO_SIZE_RANGE SizeRange;
	}

	/// <summary>Specifies the input parameters for calls to <c>ID3D12VideoEncodeCommandList::EstimateMotion</c>.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_motion_estimator_input
	// typedef struct D3D12_VIDEO_MOTION_ESTIMATOR_INPUT { ID3D12Resource *pInputTexture2D; UINT InputSubresourceIndex; ID3D12Resource *pReferenceTexture2D; UINT ReferenceSubresourceIndex; ID3D12VideoMotionVectorHeap *pHintMotionVectorHeap; } D3D12_VIDEO_MOTION_ESTIMATOR_INPUT;
	...
	/// <remarks>Call <c>ID3D12VideoEncodeCommandList::ResolveMotionVectorHeap</c> to translate the motion vector output of the <c>EstimateMotion</c> method from hardware-dependent formats into a consistent format defined by the video motion estimation APIs.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_motion_estimator_output
	// typedef struct D3D12_VIDEO_MOTION_ESTIMATOR_OUTPUT { ID3D12VideoMotionVectorHeap *pMotionVectorHeap; } D3D12_VIDEO_MOTION_ESTIMATOR_OUTPUT;
	...
	{
		/// <summary>An <c>ID3D12VideoMotionVectorHeap</c> containing the resolved motion estimation vectors. Motion vectors are resolved to a <c>DXGI_FORMAT_R16G16_SINT</c> 2D texture. The resolved data is a signed 16-byte integer with quarter PEL units with the X vector component stored in the R component and the Y vector component stored in the G component. Motion vectors are stored in a 2D layout that corresponds to the pixel layout of the original input textures.</summary>
		[MarshalAs(UnmanagedType.IUnknown)]
		public ID3D12VideoMotionVectorHeap pMotionVectorHeap;
	}

	/// <summary>Describes a <c>ID3D12VideoMotionEstimatorHeap</c>. Pass this structure into <c>ID3D12VideoDevice1::CreateVideoMotionVectorHeap</c> to create an instance of <b>ID3D12VideoMotionEstimatorHeap</b>.</summary>
	/// <remarks>Call <c>ID3D12VideoDevice::CheckFeatureSupport</c> and specify <c>D3D12_FEATURE_VIDEO_MOTION_ESTIMATOR</c> as the feature to determine supported values.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_motion_vector_heap_desc
	// typedef struct D3D12_VIDEO_MOTION_VECTOR_HEAP_DESC { UINT NodeMask; DXGI_FORMAT InputFormat; D3D12_VIDEO_MOTION_ESTIMATOR_SEARCH_BLOCK_SIZE BlockSize; D3D12_VIDEO_MOTION_ESTIMATOR_VECTOR_PRECISION Precision; D3D12_VIDEO_SIZE_RANGE SizeRange; } D3D12_VIDEO_MOTION_VECTOR_HEAP_DESC;
	...
	/// <summary>Specifies alpha blending parameters for video processing. Used by the [D3D12_VIDEO_PROCESS_INPUT_STREAM_ARGUMENTS]ns-d3d12video-d3d12_video_process_input_stream_arguments) structure.</summary>
	/// <remarks>
	...
	/// </list>
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_process_alpha_blending
	// typedef struct D3D12_VIDEO_PROCESS_ALPHA_BLENDING { BOOL Enable; FLOAT Alpha; } D3D12_VIDEO_PROCESS_ALPHA_BLENDING;
	...
	public struct D3D12_VIDEO_PROCESS_ALPHA_BLENDING
	{
		...
		[MarshalAs(UnmanagedType.Bool)] public bool Enable;
		/// <summary>The planar alpha value. The value can range from 0.0 (transparent) to 1.0 (opaque). If Enable is FALSe, this parameter is ignored.</summary>
		public float Alpha;
	}

	/// <summary>Defines the range of supported values for an image filter.</summary>
	/// <remarks>
	/// <para>The multiplier enables the filter range to have a fractional step value. For example, a hue filter might have an actual range of [–180.0 ... +180.0] with a step size of 0.25. The device would report the following range and multiplier:</para>
	/// <list type="bullet">
	/// <item>
	...
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_process_filter_range
	// typedef struct D3D12_VIDEO_PROCESS_FILTER_RANGE { INT Minimum; INT Maximum; INT Default; FLOAT Multiplier; } D3D12_VIDEO_PROCESS_FILTER_RANGE;
	...
	public struct D3D12_VIDEO_PROCESS_FILTER_RANGE
	{
		...
		public int Default;
		/// <summary>A multiplier. Use the following formula to translate the filter setting into the actual filter value: <c>Actual Value = Set Value × Multiplier.</c></summary>
		public float Multiplier;
	}

	/// <summary>Contains input information for the video processor blend functionality.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_process_input_stream
	// typedef struct D3D12_VIDEO_PROCESS_INPUT_STREAM { ID3D12Resource *pTexture2D; UINT Subresource; D3D12_VIDEO_PROCESS_REFERENCE_SET ReferenceSet; } D3D12_VIDEO_PROCESS_INPUT_STREAM;
	...
	public struct D3D12_VIDEO_PROCESS_INPUT_STREAM
	{
		...
		/// <summary>A <c>D3D12_VIDEO_PROCESS_REFERENCE_SET</c> containing the set of references for video processing. Some video processing algorithms require forward or backward frame references. For more information, see <c>D3D12_FEATURE_VIDEO_PROCESS_REFERENCE_INFO</c>.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_process_input_stream_arguments
	// typedef struct D3D12_VIDEO_PROCESS_INPUT_STREAM_ARGUMENTS { D3D12_VIDEO_PROCESS_INPUT_STREAM InputStream[2]; D3D12_VIDEO_PROCESS_TRANSFORM Transform; D3D12_VIDEO_PROCESS_INPUT_STREAM_FLAGS Flags; D3D12_VIDEO_PROCESS_INPUT_STREAM_RATE RateInfo; INT FilterLevels[32]; D3D12_VIDEO_PROCESS_ALPHA_BLENDING AlphaBlending; } D3D12_VIDEO_PROCESS_INPUT_STREAM_ARGUMENTS;
	...
	{
		/// <summary>An array of <c>D3D12_VIDEO_PROCESS_INPUT_STREAM</c> structures containing the set of references for video processing. If the stereo format is <c>D3D12_VIDEO_PROCESS_STEREO_FORMAT_SEPARATE</c>, then two sets of input streams must be supplied. For all other stereo formats, the first set of reference must be supplied, and the second should be zero initialized.</summary>
		[MarshalAs(UnmanagedType.ByValArray, SizeConst = 2)]
		public D3D12_VIDEO_PROCESS_INPUT_STREAM InputStream;
		/// <summary>A <c>D3D12_VIDEO_PROCESS_TRANSFORM</c> structure specifying the flip, rotation, scale and destination translation for the video input.</summary>
		public D3D12_VIDEO_PROCESS_TRANSFORM Transform;
		/// <summary>A value from the <c>D3D12_VIDEO_PROCESS_INPUT_STREAM_FLAGS</c> enumeration specifying the options for the input stream.</summary>
		public D3D12_VIDEO_PROCESS_INPUT_STREAM_FLAGS Flags;
		/// <summary>A <c>D3D12_VIDEO_PROCESS_INPUT_STREAM_RATE</c> structure specifying the framerate and input and output indices for framerate conversion and deinterlacing.</summary>
		public D3D12_VIDEO_PROCESS_INPUT_STREAM_RATE RateInfo;
		/// <summary>The level to apply for each enabled filter. The filter level is specified in the order that filters appear in the <c>D3D12_VIDEO_PROCESS_FILTER_FLAGS</c> enumeration. Specify 0 if a filter is not enabled or the filter index is reserved.</summary>
		[MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
		public int[] FilterLevels;
		/// <summary>A <c>D3D12_VIDEO_PROCESS_ALPHA_BLENDING</c> structure specifying the planar alpha for an input stream on the video processor.</summary>
		public D3D12_VIDEO_PROCESS_ALPHA_BLENDING AlphaBlending;
	}
	/// <summary>Specifies input stream arguments for an input stream passed to <c>ID3D12VideoProcessCommandList1::ProcessFrames1</c>, which supports changing the field type for each call.</summary>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_process_input_stream_arguments1
	// typedef struct D3D12_VIDEO_PROCESS_INPUT_STREAM_ARGUMENTS1 { D3D12_VIDEO_PROCESS_INPUT_STREAM InputStream[2]; D3D12_VIDEO_PROCESS_TRANSFORM Transform; D3D12_VIDEO_PROCESS_INPUT_STREAM_FLAGS Flags; D3D12_VIDEO_PROCESS_INPUT_STREAM_RATE RateInfo; INT FilterLevels[32]; D3D12_VIDEO_PROCESS_ALPHA_BLENDING AlphaBlending; D3D12_VIDEO_FIELD_TYPE FieldType; } D3D12_VIDEO_PROCESS_INPUT_STREAM_ARGUMENTS1;
	...
	public struct D3D12_VIDEO_PROCESS_INPUT_STREAM_ARGUMENTS1
	{
		...
		/// <summary>A <c>D3D12_VIDEO_PROCESS_ALPHA_BLENDING</c> structure specifying the planar alpha for an input stream on the video processor.</summary>
		public D3D12_VIDEO_PROCESS_ALPHA_BLENDING AlphaBlending;
		/// <summary>A value from the <c>D3D12_VIDEO_FIELD_TYPE</c> enumeration specfying the interlaced field type of the input source. When working with mixed content, use <c>ID3D12VideoProcessCommandList1::ProcessFrames1</c> which supports changing the field type for each call.</summary>
	...
	{
		/// <summary>A value from the <c>DXGI_FORMAT</c> enumeration specifying the format of the input stream. In the case of stereo, this format is the format of both inputs.</summary>
		public DXGI_FORMAT Format;
		/// <summary>A value from the <c>DXGI_COLOR_SPACE_TYPE</c> enumeration specifying the color space of the video processor input and reference surfaces.</summary>
		public DXGI_COLOR_SPACE_TYPE ColorSpace;
		...
		public DXGI_RATIONAL FrameRate;
		/// <summary>A <c>D3D12_VIDEO_SIZE_RANGE</c> structure representing the size of the source rectangle. This argument specifies the input range size this video processor must support for <c>ProcessFrames</c>. If a source size exceeds the range, the video processor must be recreated.</summary>
		public D3D12_VIDEO_SIZE_RANGE SourceSizeRange;
		/// <summary>A <c>D3D12_VIDEO_SIZE_RANGE</c> structure representing the size of the destination rectangle. This argument specifies the destination range size this video processor must support for <c>ProcessFrames</c>. If a source size exceeds the range, the video processor must be recreated.</summary>
		public D3D12_VIDEO_SIZE_RANGE DestinationSizeRange;
		/// <summary>A boolean value specifying whether the video processor should support all <c>D3D12_VIDEO_PROCESS_ORIENTATION</c> for <c>ProcessFrames</c>.</summary>
		[MarshalAs(UnmanagedType.Bool)] public bool EnableOrientation;
		/// <summary>A bitwise OR combination of one or more flags from the <c>D3D12_VIDEO_PROCESS_FILTER_FLAGS</c> enumeration specifying the filters to enable.</summary>
		public D3D12_VIDEO_PROCESS_FILTER_FLAGS FilterFlags;
		/// <summary>A value from the <c>D3D12_VIDEO_FRAME_STEREO_FORMAT</c> enumeration specifies whether the stream is stereo or not. A value of <b>D3D12_VIDEO_PROCESS_STEREO_FORMAT_SEPARATE</b> indicates that there will be two sets of input textures, and two sets of references for the stereo interlaced case.</summary>
		...
		public D3D12_VIDEO_PROCESS_DEINTERLACE_FLAGS DeinterlaceMode;
		/// <summary>A boolean value specifying whether alpha blending is enabled. Alpha blending settings are provided to <c>ProcessFrames</c> with AlphaBlending the field of the <c>D3D12_VIDEO_PROCESS_INPUT_STREAM_ARGUMENTS</c> structure.</summary>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_process_input_stream_rate
	// typedef struct D3D12_VIDEO_PROCESS_INPUT_STREAM_RATE { UINT OutputIndex; UINT InputFrameOrField; } D3D12_VIDEO_PROCESS_INPUT_STREAM_RATE;
	...
	/// <summary>Specifies the settings used for luma keying. This value is used with the <c>D3D12_VIDEO_PROCESS_INPUT_STREAM_DESC</c> structure.</summary>
	/// <remarks>
	/// <para>The values of Lower and Upper give the lower and upper bounds of the luma key, using a nominal range of [0...1]. Given a format with n bits per channel, these values are converted to luma values as follows:</para>
	...
	/// </remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_process_luma_key
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_process_output_stream
	// typedef struct D3D12_VIDEO_PROCESS_OUTPUT_STREAM { ID3D12Resource *pTexture2D; UINT Subresource; } D3D12_VIDEO_PROCESS_OUTPUT_STREAM;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_process_output_stream_arguments
	// typedef struct D3D12_VIDEO_PROCESS_OUTPUT_STREAM_ARGUMENTS { D3D12_VIDEO_PROCESS_OUTPUT_STREAM OutputStream[2]; D3D12_RECT TargetRectangle; } D3D12_VIDEO_PROCESS_OUTPUT_STREAM_ARGUMENTS;
	...
	public struct D3D12_VIDEO_PROCESS_OUTPUT_STREAM_DESC
	{
		...
		public DXGI_COLOR_SPACE_TYPE ColorSpace;
		/// <summary>A value from the <c>D3D12_VIDEO_PROCESS_ALPHA_FILL_MODE</c> enumeration specifying the alpha fill mode for data that the video processor writes to the render target.</summary>
		public D3D12_VIDEO_PROCESS_ALPHA_FILL_MODE AlphaFillMode;
		/// <summary>The zero-based index of an input stream. This parameter is used if AlphaFillMode is <c>D3D12_VIDEO_PROCESS_ALPHA_FILL_MODE_SOURCE_STREAM</c>. Otherwise, the parameter is ignored.</summary>
		public uint AlphaFillModeSourceStreamIndex;
		/// <summary>
		///   <para>The video processor uses the background color to fill areas of the target rectangle that do not contain a video image. Areas outside the target rectangle are not affected. The meaning of the values are specified by the ColorSpace parameter.</para>
		///   <list type="table">
		///     <listheader>
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_process_reference_set
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_process_transform
	// typedef struct D3D12_VIDEO_PROCESS_TRANSFORM { D3D12_RECT SourceRectangle; D3D12_RECT DestinationRectangle; D3D12_VIDEO_PROCESS_ORIENTATION Orientation; } D3D12_VIDEO_PROCESS_TRANSFORM;
	...
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_sample
	// typedef struct D3D12_VIDEO_SAMPLE { UINT Width; UINT Height; D3D12_VIDEO_FORMAT Format; } D3D12_VIDEO_SAMPLE;
	...
	/// <remarks>By default, all possible output size combinations that exist between the maximum size and minimum size for the extent, inclusive, are supported. ScaleSupportFlags may add additional restrictions to the supported scale sizes. When scaling is not supported, the minimum and maximum sizes should both be set to the input size and no flags should be specified.</remarks>
	// https://learn.microsoft.com/en-us/windows/win32/api/d3d12video/ns-d3d12video-d3d12_video_scale_support
	// typedef struct D3D12_VIDEO_SCALE_SUPPORT { D3D12_VIDEO_SIZE_RANGE OutputSizeRange; D3D12_VIDEO_SCALE_SUPPORT_FLAGS Flags; } D3D12_VIDEO_SCALE_SUPPORT;
	...
	public struct D3D12_VIDEO_SCALE_SUPPORT
	{
		...
		/// <summary>A member of the <c>D3D12_VIDEO_SCALE_SUPPORT_FLAGS</c> enumeration specifying the supported scaling capabilities of the scaler.</summary>
	...
	{
		/// <summary>The largest output width to which content can be scaled. The largest value allowed is <b>D3D12_REQ_TEXTURE2D_U_OR_V_DIMENSION</b> (16384).</summary>
		public uint MaxWidth;
		/// <summary>The largest output height to which content can be scaled. The largest value allowed is <b>D3D12_REQ_TEXTURE2D_U_OR_V_DIMENSION</b> (16384).</summary>
