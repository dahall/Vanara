...
public static partial class D3D12
{
	...
	public interface ID3D12Device9 : ID3D12Device8
	{
		...
		[PreserveSig]
		HRESULT CreateShaderCacheSession(in D3D12_SHADER_CACHE_SESSION_DESC pDesc, in Guid riid,
		...
		[PreserveSig]
		HRESULT CreateCommandQueue1(in D3D12_COMMAND_QUEUE_DESC pDesc, in Guid CreatorID, in Guid riid,
	...
	public interface ID3D12Device12 : ID3D12Device11
	{
		...
		[PreserveSig]
		new void CreateShaderResourceView([In] ID3D12Resource pResource, IntPtr pDesc, D3D12_CPU_DESCRIPTOR_HANDLE DestDescriptor);

		[PreserveSig]
		new void CreateUnorderedAccessView([In] ID3D12Resource pResource, ID3D12Resource pCounterResource, IntPtr pDesc, D3D12_CPU_DESCRIPTOR_HANDLE DestDescriptor);

		[PreserveSig]
		new void CreateRenderTargetView([In] ID3D12Resource pResource, IntPtr pDesc, D3D12_CPU_DESCRIPTOR_HANDLE DestDescriptor);

		[PreserveSig]
		new void CreateDepthStencilView([In] ID3D12Resource pResource, IntPtr pDesc, D3D12_CPU_DESCRIPTOR_HANDLE DestDescriptor);

		[PreserveSig]
		new void CreateSampler(ref D3D12_SAMPLER_DESC pDesc, D3D12_CPU_DESCRIPTOR_HANDLE DestDescriptor);

		[PreserveSig]
		new void CopyDescriptors(int NumDestDescriptorRanges, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 0)] D3D12_CPU_DESCRIPTOR_HANDLE[] pDestDescriptorRangeStarts, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 0)] uint[] pDestDescriptorRangeSizes, int NumSrcDescriptorRanges, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 3)] D3D12_CPU_DESCRIPTOR_HANDLE[] pSrcDescriptorRangeStarts, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 3)] uint[] pSrcDescriptorRangeSizes, D3D12_DESCRIPTOR_HEAP_TYPE DescriptorHeapsType);

		[PreserveSig]
		new void CopyDescriptorsSimple(uint NumDescriptors, D3D12_CPU_DESCRIPTOR_HANDLE DestDescriptorRangeStart, D3D12_CPU_DESCRIPTOR_HANDLE SrcDescriptorRangeStart, D3D12_DESCRIPTOR_HEAP_TYPE DescriptorHeapsType);

		[PreserveSig]
		new void GetResourceAllocationInfo(out D3D12_RESOURCE_ALLOCATION_INFO size, uint visibleMask, int numResourceDescs, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 2)] D3D12_RESOURCE_DESC[] pResourceDescs);
		...
		[PreserveSig]
		new HRESULT MakeResident(int NumObjects, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 0)][In] ID3D12Pageable[] ppObjects);
		...
		[PreserveSig]
		new HRESULT EnumerateMetaCommands(ref int pNumMetaCommands, [Out, MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 0)] D3D12_META_COMMAND_DESC[] pDescs);
		...
		[PreserveSig]
		new D3D12_DRIVER_MATCHING_IDENTIFIER_STATUS CheckDriverMatchingIdentifier(D3D12_SERIALIZED_DATA_TYPE SerializedDataType, ref D3D12_SERIALIZED_DATA_DRIVER_MATCHING_IDENTIFIER pIdentifierToCheck);
