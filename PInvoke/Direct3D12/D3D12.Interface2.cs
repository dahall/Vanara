...
public static partial class D3D12
{
	/// <summary>
	/// <para>
	/// Represents a virtual adapter. This interface extends <c>ID3D12Device1</c> to create pipeline state objects from pipeline state
	/// stream descriptions.
	/// </para>
	/// <note>This interface was introduced in Windows 10 Creators Update. Applications targeting Windows 10 Creators Update should use this
	/// interface instead of earlier or later versions. Applications targeting an earlier or later version of Windows 10 should use the
	/// appropriate version of the <b>ID3D12Device</b> interface.</note>
	/// </summary>
	...
	/// heaps in system memory that persist even in the event of a GPU-fault or device-removed scenario.
	/// </para>
	/// <note>This interface, introduced in the Windows 10 Fall Creators Update, is the latest version of the <c>ID3D12Device</c> interface.
	/// Applications targeting the Windows 10 Fall Creators Update and later should use this interface instead of earlier versions.</note>
	/// </summary>
	/// <remarks>Use <c>D3D12CreateDevice</c> to create a device.</remarks>
	...
	public interface ID3D12Device3 : ID3D12Device2
	{
		...
		[PreserveSig]
		HRESULT OpenExistingHeapFromAddress([In] IntPtr pAddress, in Guid riid, [MarshalAs(UnmanagedType.IUnknown, IidParameterIndex = 1)] out object? ppvHeap);
		...
		[PreserveSig]
		HRESULT OpenExistingHeapFromFileMapping([In] IntPtr hFileMapping, in Guid riid, [MarshalAs(UnmanagedType.IUnknown, IidParameterIndex = 1)] out object? ppvHeap);
		...
		// FenceValueToSignal );
		[PreserveSig]
		HRESULT EnqueueMakeResident(D3D12_RESIDENCY_FLAGS Flags, int NumObjects,
	...
	/// <para>This interface extends <c>ID3D12Device3</c>.</para>
	/// </summary>
	...
	public interface ID3D12Device4 : ID3D12Device3
	{
		...
		// **ppCommandList );
		[PreserveSig]
		HRESULT CreateCommandList1(uint nodeMask, D3D12_COMMAND_LIST_TYPE type, D3D12_COMMAND_LIST_FLAGS flags, in Guid riid,
		...
		[PreserveSig]
		HRESULT CreateProtectedResourceSession(in D3D12_PROTECTED_RESOURCE_SESSION_DESC pDesc, in Guid riid,
		...
		[PreserveSig]
		HRESULT CreateCommittedResource1(in D3D12_HEAP_PROPERTIES pHeapProperties, D3D12_HEAP_FLAGS HeapFlags, in D3D12_RESOURCE_DESC pDesc,
		...
		// **ppvHeap );
		[PreserveSig]
		HRESULT CreateHeap1(in D3D12_HEAP_DESC pDesc, [In, Optional] ID3D12ProtectedResourceSession? pProtectedSession, in Guid riid,
		...
		[PreserveSig]
		HRESULT CreateReservedResource1(in D3D12_RESOURCE_DESC pDesc, D3D12_RESOURCE_STATES InitialState,
		...
		[PreserveSig]
		D3D12_RESOURCE_ALLOCATION_INFO GetResourceAllocationInfo1(uint visibleMask, int numResourceDescs,
			[In, MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 1)] D3D12_RESOURCE_DESC[] pResourceDescs,
	...
	/// <summary>
	/// <para>Represents a virtual adapter.</para>
	/// <para>This interface extends <c>ID3D12Device4</c>.</para>
	/// <note>This interface, introduced in Windows 10, version 1809, is the latest version of the <c>ID3D12Device</c> interface.
	/// Applications targeting Windows 10, version 1809 and later should use this interface instead of earlier versions.</note>
	/// </summary>
	...
	public interface ID3D12Device5 : ID3D12Device4
	{
		...
		// HRESULT CreateLifetimeTracker( [in] ID3D12LifetimeOwner *pOwner, [in] REFIID riid, [out] void **ppvTracker );
		[PreserveSig]
		HRESULT CreateLifetimeTracker([In] ID3D12LifetimeOwner pOwner, in Guid riid, [MarshalAs(UnmanagedType.IUnknown, IidParameterIndex = 1)] out object? ppvTracker);
		...
		/// <returns>None</returns>
		/// <remarks>
		/// <para>Because device removal triggers all fences to be signaled to <c>UINT64_MAX</c>, you can create a callback for device removal using an event.</para>
		/// <para><c>HANDLE deviceRemovedEvent = CreateEventW(NULL, FALSE, FALSE, NULL); assert(deviceRemovedEvent != NULL); _deviceFence-&gt;SetEventOnCompletion(UINT64_MAX, deviceRemoved); HANDLE waitHandle; RegisterWaitForSingleObject( &amp;waitHandle, deviceRemovedEvent, OnDeviceRemoved, _device.Get(), // Pass the device as our context INFINITE, // No timeout 0 // No flags ); void OnDeviceRemoved(PVOID context, BOOLEAN) { ID3D12Device* removedDevice = (ID3D12Device*)context; HRESULT removedReason = removedDevice-&gt;GetDeviceRemovedReason(); // Perform app-specific device removed operation, such as logging or inspecting DRED output } </c></para>
		/// </remarks>
		// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/nf-d3d12-id3d12device5-removedevice
		// void RemoveDevice();
		[PreserveSig]
		void RemoveDevice();
		...
		/// <para>A pointer to a <c>UINT</c> containing the number of meta commands to query for. This field determines the size of the <i>pDescs</i> array, unless <i>pDescs</i> is <b>nullptr</b>.</para>
		/// </param>
		/// <param name="pDescs">
		/// <para>Type: [out, optional] <b><c>D3D12_META_COMMAND_DESC</c>*</b></para>
		/// <para>An optional pointer to an array of <c>D3D12_META_COMMAND_DESC</c> containing the descriptions of the available meta commands. Pass <c>nullptr</c> to have the number of available meta commands returned in <i>pNumMetaCommands</i>.</para>
		/// </param>
		/// <returns>
		/// <para>Type: <b><c>HRESULT</c></b></para>
		/// <para>If this method succeeds, it returns <b>S_OK</b>. Otherwise, it returns an <c>HRESULT</c> error code.</para>
		/// </returns>
		// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/nf-d3d12-id3d12device5-enumeratemetacommands
		// HRESULT EnumerateMetaCommands( UINT *pNumMetaCommands, D3D12_META_COMMAND_DESC *pDescs );
		[PreserveSig]
		HRESULT EnumerateMetaCommands(ref int pNumMetaCommands, [Out, Optional, MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 0)] D3D12_META_COMMAND_DESC[]? pDescs);
		...
		/// <para>A <c>D3D12_META_COMMAND_PARAMETER_STAGE</c> specifying the stage of the parameters that you wish to be included in the query.</para>
		/// </param>
		/// <param name="pTotalStructureSizeInBytes">
		/// <para>Type: <b><c>UINT</c>*</b></para>
		/// <para>An optional pointer to a <c>UINT</c> containing the size of the structure containing the parameter values, which you pass when creating/initializing/executing the meta command, as appropriate.</para>
		/// </param>
		/// <param name="pParameterCount">
		/// <para>Type: <b><c>UINT</c>*</b></para>
		/// <para>A pointer to a <c>UINT</c> containing the number of parameters to query for. This field determines the size of the <i>pParameterDescs</i> array, unless <i>pParameterDescs</i> is <b>nullptr</b>.</para>
		/// </param>
		/// <param name="pParameterDescs">
		/// <para>Type: <b>D3D12_META_COMMAND_PARAMETER_DESC*</b></para>
		/// <para>An optional pointer to an array of <c>D3D12_META_COMMAND_PARAMETER_DESC</c> containing the descriptions of the parameters. Pass <b>nullptr</b> to have the parameter count returned in <i>pParameterCount</i>.</para>
		/// </param>
		/// <returns>
		...
		[PreserveSig]
		HRESULT EnumerateMetaCommandParameters(in Guid CommandId, D3D12_META_COMMAND_PARAMETER_STAGE Stage, out uint pTotalStructureSizeInBytes,
		...
		/// <param name="NodeMask">
		/// <para>Type: <b><c>UINT</c></b></para>
		/// <para>For single-adapter operation, set this to zero. If there are multiple adapter nodes, set a bit to identify the node (one of the device's physical adapters) to which the meta command applies. Each bit in the mask corresponds to a single node. Only one bit must be set. See <c>Multi-adapter systems</c>.</para>
		/// </param>
		...
		/// <param name="riid">
		/// <para>Type: <b>REFIID</b></para>
		/// <para>A reference to the globally unique identifier (GUID) of the interface that you wish to be returned in <i>ppMetaCommand</i>. This is expected to be the GUID of <c>ID3D12MetaCommand</c>.</para>
		/// </param>
		/// <param name="ppMetaCommand">
		/// <para>Type: <b>void**</b></para>
		/// <para>A pointer to a memory block that receives a pointer to the meta command. This is the address of a pointer to an <c>ID3D12MetaCommand</c>, representing the meta command created.</para>
		/// </param>
		/// <returns>
		/// <para>Type: <b>HRESULT</b></para>
		/// <para>If this method succeeds, it returns <b>S_OK</b>. Otherwise, it returns an <b>HRESULT</b> error code.</para>
		...
		/// </list>
		/// </returns>
		// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/nf-d3d12-id3d12device5-createmetacommand
		// HRESULT CreateMetaCommand( [in] REFGUID CommandId, [in] UINT NodeMask, [in, optional] const void *pCreationParametersData, [in] SIZE_T CreationParametersDataSizeInBytes, REFIID riid, [out] void **ppMetaCommand );
		[PreserveSig]
		HRESULT CreateMetaCommand(in Guid CommandId, uint NodeMask, [In, Optional] IntPtr pCreationParametersData, [In] SIZE_T CreationParametersDataSizeInBytes,
		...
		/// </list>
		/// </returns>
		// https://learn.microsoft.com/en-us/windows/win32/api/d3d12/nf-d3d12-id3d12device5-createstateobject
		// HRESULT CreateStateObject( [in] const D3D12_STATE_OBJECT_DESC *pDesc, REFIID riid, [out] void **ppStateObject );
		[PreserveSig]
		HRESULT CreateStateObject(in D3D12_STATE_OBJECT_DESC pDesc, in Guid riid, [MarshalAs(UnmanagedType.IUnknown, IidParameterIndex = 1)] out object? ppStateObject);

		/// <summary>Query the driver for resource requirements to build an acceleration structure.</summary>
		/// <param name="pDesc">
		/// <para>Description of the acceleration structure build. This structure is shared with <c>BuildRaytracingAccelerationStructure</c>. For more information, see <c>D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS</c>.</para>
		/// <para>The implementation is allowed to look at all the CPU parameters in this struct and nested structs. It may not inspect/dereference any GPU virtual addresses, other than to check to see if a pointer is NULL or not, such as the optional transform in <c>D3D12_RAYTRACING_GEOMETRY_TRIANGLES_DESC</c>, without dereferencing it. In other words, the calculation of resource requirements for the acceleration structure does not depend on the actual geometry data (such as vertex positions), rather it can only depend on overall properties, such as the number of triangles, number of instances etc.</para>
		/// </param>
		/// <param name="pInfo">The result of the query (in a <c>D3D12_RAYTRACING_ACCELERATION_STRUCTURE_PREBUILD_INFO</c> structure).</param>
		/// <returns>None</returns>
		/// <remarks>
		/// <para>The input acceleration structure description is the same as what goes into <c>BuildRaytracingAccelerationStructure</c>. The result of this function lets the application provide the correct amount of output storage and scratch storage to <b>BuildRaytracingAccelerationStructure</b> given the same geometry.</para>
		...
		// void GetRaytracingAccelerationStructurePrebuildInfo( [in] const D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS *pDesc, [out] D3D12_RAYTRACING_ACCELERATION_STRUCTURE_PREBUILD_INFO *pInfo );
		[PreserveSig]
		...
		/// <summary>Reports the compatibility of serialized data, such as a serialized raytracing acceleration structure resulting from a call to <c>CopyRaytracingAccelerationStructure</c> with mode <c>D3D12_RAYTRACING_ACCELERATION_STRUCTURE_COPY_MODE_SERIALIZE</c>, with the current device/driver.</summary>
		...
		/// <param name="pIdentifierToCheck">Identifier from the header of the serialized data to check with the driver. For more information, see <c>D3D12_SERIALIZED_DATA_DRIVER_MATCHING_IDENTIFIER</c>.</param>
		...
		// D3D12_DRIVER_MATCHING_IDENTIFIER_STATUS CheckDriverMatchingIdentifier( [in] D3D12_SERIALIZED_DATA_TYPE SerializedDataType, [in] const D3D12_SERIALIZED_DATA_DRIVER_MATCHING_IDENTIFIER *pIdentifierToCheck );
		[PreserveSig]
