...
public static partial class Shell32
{
	...
	public static extern HRESULT SHBindToObject([Optional] IShellFolder? psf, PIDL pidl, [Optional] IBindCtx? pbc, in Guid riid, [MarshalAs(UnmanagedType.IUnknown, IidParameterIndex = 3)] out object? ppv);
	...
	// IShellFolder *psf, PCUIDLIST_RELATIVE pidl, IBindCtx *pbc, REFIID riid, void **ppv );
	[PInvokeData("shlobj_core.h", MSDNShortId = "acc16097-8301-4118-8cb5-00aa2705306a")]
	public static TIntf? SHBindToObject<TIntf>([Optional] IShellFolder? psf, PIDL pidl, [Optional] IBindCtx? pbc) where TIntf : class =>
	...
	/// </remarks>
	/// <param name="hwnd">Specifies the owner window that receives messages (such as WM_MENUCOMMAND or WM_COMMAND) generated by the context menu.</param>
	...
	public static IContextMenu SHCreateDefaultContextMenuEx(HWND hwnd, IShellFolder? pContainingFolder, out IDisposable disposables, params IShellItem[] items)
	{
		var pidls = Array.ConvertAll(items, si => { SHGetIDListFromObject(si, out var pidl).ThrowIfFailed(); return pidl!; });
		var pcm = SHCreateDefaultContextMenuEx(hwnd, pContainingFolder, out disposables, pidls);
	...
	/// </param>
	/// <returns>An <see cref="IContextMenu"/> instance that provides access to the default context menu for the specified items.</returns>
	public static IContextMenu SHCreateDefaultContextMenuEx(HWND hwnd, IShellFolder? pContainingFolder, out IDisposable disposables, params PIDL[] pidls)
	{
		DisposingList dl = [];
		disposables = dl;
		pidls ??= [];

		// Handle unique case of null pContainingFolder and no children and get Desktop's context menu
		if (pContainingFolder is null && pidls.Length == 0)
		{
			SHGetDesktopFolder(out pContainingFolder).ThrowIfFailed();
			pContainingFolder.CreateViewObject(hwnd, out IContextMenu? pcmd).ThrowIfFailed();
			return pcmd!;
		}

		SHCreateDefaultContextMenu(new DEFCONTEXTMENU(hwnd, pContainingFolder, pidls, null, out var mem), out IContextMenu? pcm).ThrowIfFailed();
	...
	public static extern HRESULT SHParseDisplayName([MarshalAs(UnmanagedType.LPWStr)] string pszName,
		[In, Optional] IBindCtx? pbc, out PIDL ppidl, SFGAO sfgaoIn, out SFGAO psfgaoOut);
	...
	[PInvokeData("Shlobj.h", MSDNShortId = "bb762236")]
	public static extern HRESULT SHParseDisplayName([MarshalAs(UnmanagedType.LPWStr)] string pszName,
		[In, Optional] IBindCtx? pbc, out PIDL ppidl, [Optional] SFGAO sfgaoIn, [Out, Optional] IntPtr psfgaoOut);
	...
	public struct DEFCONTEXTMENU
	{
		...
		/// <param name="hwnd">
		/// Specifies the owner window that receives messages (such as WM_MENUCOMMAND or WM_COMMAND) generated by the context menu.
		/// </param>
		...
		/// <param name="mem">
		/// When this method returns, contains a IDisposable representing the allocated instances for the item identifier list. This
		/// parameter is passed uninitialized.
		/// </param>
		/// <exception cref="ArgumentNullException">Thrown if pShellFolder is null, or if childIds is null or empty.</exception>
		public DEFCONTEXTMENU(HWND hwnd, IShellFolder? pShellFolder, PIDL[]? childIds, HKEY[]? extKeys, out IDisposable mem) : this()
		{
			this.hwnd = hwnd;
			psf = pShellFolder; // ?? throw new ArgumentNullException(nameof(pShellFolder));

			DisposingList disposingList = [];
			mem = disposingList;
			// If a shell folder is provided, use it. Otherwise, bind to the parent of the child items or desktop if no child items are provided.
			if (psf is not null)
			{
				SHGetIDListFromObject(psf, out var fpidl).ThrowIfFailed();
				disposingList.Add(fpidl);
				pidlFolder = (IntPtr)fpidl;
				// If childIds is null or empty, use the folder's own PIDL as the child item.
				if (childIds is null || childIds.Length == 0)
					childIds = [fpidl];
			}
			else
			{
				var parent = childIds is null ? default : PIDL.FindCommonParent(childIds);
				pidlFolder = (IntPtr)(parent ?? PIDL.Desktop);
				disposingList.Add(psf = SHBindToObject<IShellFolder>(null, pidlFolder)!);
			}

			cidl = (uint?)childIds?.Length ?? 0U;
			if (cidl > 0)
			{
				SafeNativeArray<IntPtr> arpidls = new((int)cidl);
				disposingList.Add(arpidls);
				for (int i = 0; i < cidl; i++)
				{
					var ap = childIds![i].GetRelativeTo(pidlFolder);
					arpidls.AddSubReference([ap]);
					arpidls[i] = (IntPtr)ap;
				}
				apidl = (IntPtr)arpidls;
			}
			...
			if (extKeys != null && extKeys.Length > 0)
			{
				...
				cKeys = (uint)extKeys.Length;
				disposingList.Add(aKeys.DestructiveAssign(extKeys));
			}
		}
	}
