...
namespace Vanara.PInvoke
{
	public static partial class Shell32
	{
		...
		public enum FILEOP_FLAGS : uint
		{
			...
			/// <para>If the source file parameter does not contain fully qualified path and file names, this flag is ignored.</para></summary>
			...
			/// <summary>Do not display a message to the user if an error occurs. If this flag is set without FOFX_EARLYFAILURE, any error is treated as if the user had chosen Ignore or Continue in a dialog box. It halts the current action, sets a flag to indicate that an action was aborted, and proceeds with the rest of the operation.</summary>
			...
			/// <summary>If FOFX_EARLYFAILURE is set together with FOF_NOERRORUI, the entire set of operations is stopped upon encountering any error in any operation. This flag is valid only when FOF_NOERRORUI is set.</summary>
			...
			/// <summary>Introduced in Windows Vista SP1. The user expects a requirement for rights elevation, so do not display a dialog box asking for a confirmation of the elevation.</summary>
		...
		/// <summary>
		/// Exposes methods to copy, move, rename, create, and delete Shell items as well as methods to provide progress and error dialogs. This interface
		/// replaces the SHFileOperation function.
		/// </summary>
		[PInvokeData("Shobjidl.h")]
		...
		public interface IFileOperation
		{
			...
			/// <returns>When this method returns, this parameter points to a returned token that uniquely identifies this connection. The calling application uses this token later to delete the connection by passing it to IFileOperation::Unadvise. If the call to Advise fails, this value is meaningless.</returns>
			...
			/// <param name="dwCookie">The connection token that identifies the connection to delete. This value was originally retrieved by Advise when the connection was made.</param>
			void Unadvise(uint dwCookie);
			/// <summary>Sets parameters for the current operation.</summary>
			/// <param name="dwOperationFlags">Flags that control the file operation. This member can be a combination of the following flags. FOF flags are defined in Shellapi.h and FOFX flags are defined in Shobjidl.h.</param>
			...
			/// <param name="pproparray">Pointer to an IPropertyChangeArray, which accesses a collection of IPropertyChange objects that specify the properties to be set and their new values.</param>
			...
			/// <param name="punkItems">Pointer to the IUnknown of the IShellItemArray, IDataObject, or IEnumShellItems object which represents the group of items. You can also point to an IPersistIDList object to represent a single item, effectively accomplishing the same function as IFileOperation::ApplyPropertiesToItem.</param>
			...
			/// <param name="pfopsItem">Pointer to an IFileOperationProgressSink object to be used for status and failure notifications. If you call IFileOperation::Advise for the overall operation, progress status and error notifications for the rename operation are included there, so set this parameter to NULL.</param>
			...
			/// <param name="pUnkItems">Pointer to the IUnknown of the IShellItemArray, IDataObject, or IEnumShellItems object which represents the group of items to be renamed. You can also point to an IPersistIDList object to represent a single item, effectively accomplishing the same function as IFileOperation::RenameItem.</param>
			...
			/// <param name="pfopsItem">Pointer to an IFileOperationProgressSink object to be used for progress status and error notifications for this specific move operation. If you call IFileOperation::Advise for the overall operation, progress status and error notifications for the move operation are included there, so set this parameter to NULL.</param>
			...
			/// <param name="punkItems">Pointer to the IUnknown of the IShellItemArray, IDataObject, or IEnumShellItems object which represents the group of items to be moved. You can also point to an IPersistIDList object to represent a single item, effectively accomplishing the same function as IFileOperation::MoveItem.</param>
			/// <param name="psiDestinationFolder">Pointer to an IShellItem that specifies the destination folder to contain the moved items.</param>
			...
			/// <param name="pszCopyName">Pointer to a new name for the item after it has been copied. This is a null-terminated Unicode string and can be NULL. If NULL, the name of the destination item is the same as the source.</param>
			/// <param name="pfopsItem">Pointer to an IFileOperationProgressSink object to be used for progress status and error notifications for this specific copy operation. If you call IFileOperation::Advise for the overall operation, progress status and error notifications for the copy operation are included there, so set this parameter to NULL.</param>
			...
			/// <param name="punkItems">Pointer to the IUnknown of the IShellItemArray, IDataObject, or IEnumShellItems object which represents the group of items to be copied. You can also point to an IPersistIDList object to represent a single item, effectively accomplishing the same function as IFileOperation::CopyItem.</param>
			/// <param name="psiDestinationFolder">Pointer to an IShellItem that specifies the destination folder to contain the copy of the items.</param>
			...
			/// <param name="pfopsItem">Pointer to an IFileOperationProgressSink object to be used for progress status and error notifications for this specific delete operation. If you call IFileOperation::Advise for the overall operation, progress status and error notifications for the delete operation are included there, so set this parameter to NULL.</param>
			...
			/// <para>This parameter is normally NULL to specify a new, blank file.</para></param>
			...
			/// <remarks>This method is called last to execute those actions that have been specified earlier by calling their individual methods. For instance, RenameItem does not rename the item, it simply sets the parameters. The actual renaming is done when you call PerformOperations.</remarks>
			void PerformOperations();
			/// <summary>Gets a value that states whether any file operations initiated by a call to IFileOperation::PerformOperations were stopped before they were complete. The operations could be stopped either by user action or silently by the system.</summary>
			/// <returns>When this method returns, points to TRUE if any file operations were aborted before they were complete; otherwise, FALSE.</returns>
