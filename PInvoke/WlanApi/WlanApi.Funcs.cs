...
namespace Vanara.PInvoke
{
	...
	public static partial class WlanApi
	{
		...
		///// </param>
		///// <param name="dwFlags">
		///// <para>A set of flags that control the type of networks returned in the list. This parameter can be a combination of these possible values.</para>
		///// <list type="table">
		///// <listheader>
		///// <term>Value</term>
		///// <term>Meaning</term>
		...
		///// <param name="ppAvailableNetworkList">
		///// <para>A pointer to storage for a pointer to receive the returned list of visible networks in a WLAN_AVAILABLE_NETWORK_LIST structure.</para>
		///// <para>The buffer for the WLAN_AVAILABLE_NETWORK_LIST returned is allocated by the <c>WlanGetAvailableNetworkList</c> function if the call succeeds.</para>
		///// </param>
		///// <returns>
		///// <para>If the function succeeds, the return value is ERROR_SUCCESS.</para>
		///// <para>If the function fails, the return value may be one of the following return codes.</para>
		...
		///// </returns>
		///// <remarks>
		///// <para>The <c>WlanGetAvailableNetworkList</c> function allocates memory for the list of available networks returned in the buffer pointed to by the ppAvailableNetworkList parameter when the function succeeds. The memory used for the buffer pointed to by ppAvailableNetworkList parameter should be released by calling the WlanFreeMemory function after the buffer is no longer needed.</para>
		///// <para>There is a hotfix available for Wireless LAN API for Windows XP with SP2 that can help improve the performance of applications that call WlanFreeMemory and <c>WlanGetAvailableNetworkList</c> many times. For more information, see Help and Knowledge Base article 940541, entitled "FIX: The private bytes of the application continuously increase when an application calls the WlanGetAvailableNetworkList function and the WlanFreeMemory function on a Windows XP Service Pack 2-based computer", in the Help and Support Knowledge Base at https://go.microsoft.com/fwlink/p/?linkid=102216.</para>
		///// <para>Examples</para>
		///// <para>The following example enumerates the wireless LAN interfaces on the local computer, retrieves the list of available networks on each wireless LAN interface, and prints values from the retrieved WLAN_AVAILABLE_NETWORK_LIST that contains the WLAN_AVAILABLE_NETWORK entries.</para>
		///// <para><c>Note</c> This example will fail to load on Windows Server 2008 and Windows Server 2008 R2 if the Wireless LAN Service is not installed and started.</para>
		///// </remarks>
		//// https://docs.microsoft.com/en-us/windows/win32/api/wlanapi/nf-wlanapi-wlangetavailablenetworklist
		//// DWORD WlanGetAvailableNetworkList( HANDLE hClientHandle, const GUID *pInterfaceGuid, DWORD dwFlags, PVOID pReserved, PWLAN_AVAILABLE_NETWORK_LIST *ppAvailableNetworkList );
		//[PInvokeData("wlanapi.h", MSDNShortId = "27353a1b-2a3c-4c3b-b512-917d010ee8dd")]
		//public static Win32Error WlanGetAvailableNetworkList(HWLANSESSION hClientHandle, in Guid pInterfaceGuid, uint dwFlags, out WLAN_AVAILABLE_NETWORK[] ppAvailableNetworkList)
		//{
		//	var ret = WlanGetAvailableNetworkList(hClientHandle, pInterfaceGuid, dwFlags, default, out var mem);
		//	if (ret.Succeeded)
		//	{
		//		ppAvailableNetworkList = mem.DangerousGetHandle().ToStructure<WLAN_AVAILABLE_NETWORK_LIST>().Network;
		//		mem?.Dispose();
		//	}
		//	else
		...
		///// <summary>The <c>WlanGetFilterList</c> function retrieves a group policy or user permission list.</summary>
		///// <param name="hClientHandle">The client's session handle, obtained by a previous call to the WlanOpenHandle function.</param>
		///// <param name="wlanFilterListType">A WLAN_FILTER_LIST_TYPE value that specifies the type of filter list. All user defined and group policy filter lists can be queried.</param>
		///// <param name="ppNetworkList">Pointer to a DOT11_NETWORK_LIST structure that contains the list of permitted or denied networks.</param>
		///// <returns>
		///// <para>If the function succeeds, the return value is ERROR_SUCCESS.</para>
		///// <para>If the function fails, the return value may be one of the following return codes.</para>
		...
		///// <item>
		///// <term>ERROR_NOT_SUPPORTED</term>
		///// <term>This function was called from an unsupported platform. This value will be returned if this function was called from a Windows XP with SP3 or Wireless LAN API for Windows XP with SP2 client.</term>
		///// </item>
		///// <item>
		///// <term>RPC_STATUS</term>
		///// <term>Various error codes.</term>
		...
		//// DWORD WlanGetFilterList( HANDLE hClientHandle, WLAN_FILTER_LIST_TYPE wlanFilterListType, PVOID pReserved, PDOT11_NETWORK_LIST *ppNetworkList );
		//[PInvokeData("wlanapi.h", MSDNShortId = "3ea88e52-34bb-47a6-b345-c789d1d8047d")]
		//public static Win32Error WlanGetFilterList(HWLANSESSION hClientHandle, WLAN_FILTER_LIST_TYPE wlanFilterListType, out DOT11_NETWORK[] ppNetworkList)
		//{
		//	var ret = WlanGetFilterList(hClientHandle, wlanFilterListType, default, out var mem);
		//	if (ret.Succeeded)
		//	{
		//		ppNetworkList = mem.DangerousGetHandle().ToStructure<DOT11_NETWORK_LIST>().Network;
		//		mem?.Dispose();
		//	}
		//	else
		//		ppNetworkList = null;
