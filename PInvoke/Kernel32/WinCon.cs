...
namespace Vanara.PInvoke
{
	public static partial class Kernel32
	{
		...
		/// <summary>
		/// An application-defined function used with the SetConsoleCtrlHandler function. A console process uses this function to handle control signals received
		...
		public enum CONSOLE_MODE : uint
		{
			...
			NONE = 0U,
			/// <summary>
			/// Characters read by the ReadFile or ReadConsole function are written to the active screen buffer as they are read. This mode can be used only if
			/// the ENABLE_LINE_INPUT mode is also enabled.
			/// </summary>
			ENABLE_ECHO_INPUT = 4U,
			/// <summary>
			/// When enabled, text entered in a console window will be inserted at the current cursor location and all text following that location will not be
			/// overwritten. When disabled, all following text will be overwritten.
			/// </summary>
			ENABLE_INSERT_MODE = 32U,
			/// <summary>
			/// The ReadFile or ReadConsole function returns only when a carriage return character is read. If this mode is disabled, the functions return when
			...
			/// <summary>
			/// CTRL+C is processed by the system and is not placed in the input buffer. If the input buffer is being read by ReadFile or ReadConsole, other
			/// control keys are processed by the system and are not returned in the ReadFile or ReadConsole buffer. If the ENABLE_LINE_INPUT mode is also
			/// enabled, backspace, carriage return, and line feed characters are handled by the system.
			/// </summary>
			ENABLE_PROCESSED_INPUT = 1U,
			/// <summary>
			/// Characters written by the WriteFile or WriteConsole function or echoed by the ReadFile or ReadConsole function are examined for ASCII control
			/// sequences and the correct action is performed. Backspace, tab, bell, carriage return, and line feed characters are processed.
			/// </summary>
			ENABLE_PROCESSED_OUTPUT = ENABLE_PROCESSED_INPUT,
			/// <summary>
			/// This flag enables the user to use the mouse to select and edit text. To enable this mode, use . To disable this mode, use ENABLE_EXTENDED_FLAGS
			/// without this flag.
			/// </summary>
			ENABLE_QUICK_EDIT_MODE = 64U,
			/// <summary>
			/// User interactions that change the size of the console screen buffer are reported in the console&amp;#39;s input buffer. Information about these
			/// events can be read from the input buffer by applications using the ReadConsoleInput function, but not by those using ReadFile or ReadConsole.
			/// </summary>
			ENABLE_WINDOW_INPUT = 8U,
			/// <summary>
			/// Setting this flag directs the Virtual Terminal processing engine to convert user input received by the console window into Console Virtual
			/// Terminal Sequences that can be retrieved by a supporting application through WriteFile or WriteConsole functions. The typical usage of this flag
			/// is intended in conjunction with ENABLE_VIRTUAL_TERMINAL_PROCESSING on the output handle to connect to an application that communicates
			/// exclusively via virtual terminal sequences.
			/// </summary>
			ENABLE_VIRTUAL_TERMINAL_INPUT = 0x0200,
			/// <summary>
			/// When writing with WriteFile or WriteConsole or echoing with ReadFile or ReadConsole, the cursor moves to the beginning of the next row when it
			/// reaches the end of the current row. This causes the rows displayed in the console window to scroll up automatically when the cursor advances
			/// beyond the last row in the window. It also causes the contents of the console screen buffer to scroll up (discarding the top row of the console
			/// screen buffer) when the cursor advances beyond the last row in the console screen buffer. If this mode is disabled, the last character in the row
			/// is overwritten with any subsequent characters.
			/// </summary>
			ENABLE_WRAP_AT_EOL_OUTPUT = ENABLE_LINE_INPUT,
			/// <summary>
			/// When writing with WriteFile or WriteConsole, characters are parsed for VT100 and similar control character sequences that control cursor
			/// movement, color/font mode, and other operations that can also be performed via the existing Console APIs. For more information, see Console
			/// Virtual Terminal Sequences.
			/// </summary>
			ENABLE_VIRTUAL_TERMINAL_PROCESSING = 0x0004,
			/// <summary>
			/// When writing with WriteFile or WriteConsole, this adds an additional state to end-of-line wrapping that can delay the cursor move and buffer
			/// scroll operations.
			/// <para>
			/// Normally when ENABLE_WRAP_AT_EOL_OUTPUT is set and text reaches the end of the line, the cursor will immediately move to the next line and the
			/// contents of the buffer will scroll up by one line. In contrast with this flag set, the scroll operation and cursor move is delayed until the next
			/// character arrives. The written character will be printed in the final position on the line and the cursor will remain above this character as if
			/// ENABLE_WRAP_AT_EOL_OUTPUT was off, but the next printable character will be printed as if ENABLE_WRAP_AT_EOL_OUTPUT is on. No overwrite will
			/// occur. Specifically, the cursor quickly advances down to the following line, a scroll is performed if necessary, the character is printed, and
			/// the cursor advances one more position.
			/// </para>
			/// <para>
			/// The typical usage of this flag is intended in conjunction with setting ENABLE_VIRTUAL_TERMINAL_PROCESSING to better emulate a terminal emulator
			/// where writing the final character on the screen (in the bottom right corner) without triggering an immediate scroll is the desired behavior.
			/// </para>
			/// </summary>
			DISABLE_NEWLINE_AUTO_RETURN = 0x0008,
			/// <summary>
			/// The APIs for writing character attributes including WriteConsoleOutput and WriteConsoleOutputAttribute allow the usage of flags from character
			/// attributes to adjust the color of the foreground and background of text. Additionally, a range of DBCS flags was specified with the COMMON_LVB
			/// prefix. Historically, these flags only functioned in DBCS code pages for Chinese, Japanese, and Korean languages. With exception of the leading
			/// byte and trailing byte flags, the remaining flags describing line drawing and reverse video (swap foreground and background colors) can be useful
			/// for other languages to emphasize portions of output.
			/// <para>
			/// With exception of the leading byte and trailing byte flags, the remaining flags describing line drawing and reverse video (swap foreground and
			...
			/// <para>
			/// It is off by default to maintain compatibility with known applications that have historically taken advantage of the console ignoring these flags
			/// on non-CJK machines to store bits in these fields for their own purposes or by accident.
			/// </para>
			/// <para>
			/// Note that using the ENABLE_VIRTUAL_TERMINAL_PROCESSING mode can result in LVB grid and reverse video flags being set while this flag is still off
			/// if the attached application requests underlining or inverse video via Console Virtual Terminal Sequences.
			/// </para>
			/// </summary>
		...
		{
			/// <summary>
			/// Generates a CTRL+C signal. This signal cannot be generated for process groups. If dwProcessGroupId is nonzero, this function will succeed, but
			/// the CTRL+C signal will not be received by processes within the specified process group.
			/// </summary>
			CTRL_C_EVENT = 0,
			/// <summary>Generates a CTRL+BREAK signal.</summary>
			CTRL_BREAK_EVENT = 1,
			/// <summary>
			/// A signal that the system sends to all processes attached to a console when the user closes the console (either by clicking Close on the console
			/// window's window menu, or by clicking the End Task button command from Task Manager).
			/// </summary>
			CTRL_CLOSE_EVENT = 2,
			/// <summary>
			/// A signal that the system sends to all console processes when a user is logging off. This signal does not indicate which user is logging off, so
			/// no assumptions can be made.
			/// <para>
			/// Note that this signal is received only by services. Interactive applications are terminated at logoff, so they are not present when the system
			/// sends this signal.
			/// </para>
			/// </summary>
			CTRL_LOGOFF_EVENT = 5,
			/// <summary>
			/// A signal that the system sends when the system is shutting down. Interactive applications are not present by the time the system sends this
			/// signal, therefore it can be received only be services in this situation. Services also have their own notification mechanism for shutdown events.
		...
		public enum EVENT_TYPE : ushort
		{
			...
			KEY_EVENT = 0x0001,
			/// <summary>The Event member contains a MOUSE_EVENT_RECORD structure with information about a mouse movement or button press event.</summary>
			MOUSE_EVENT = 0x0002,
			/// <summary>The Event member contains a WINDOW_BUFFER_SIZE_RECORD structure with information about the new size of the console screen buffer.</summary>
		...
		public enum GET_CONSOLE_DISPLAY_MODE
		{
			...
			/// <summary>Full-screen console communicating directly with the video hardware. This mode is set after the console is in CONSOLE_FULLSCREEN mode to
			/// indicate that the transition to full-screen mode has completed./summary>
		...
		public enum MOUSE_EVENT_FLAG
		{
			...
			DOUBLE_CLICK = 0x0002,
			/// <summary>
			/// The horizontal mouse wheel was moved. If the high word of the dwButtonState member contains a positive value, the wheel was rotated to the right.
			...
			MOUSE_MOVED = 0x0001,
			/// <summary>
			/// The vertical mouse wheel was moved. If the high word of the dwButtonState member contains a positive value, the wheel was rotated forward, away
		...
		/// lpSecurityAttributes is <c>NULL</c>, the handle cannot be inherited. The <c>lpSecurityDescriptor</c> member of the structure specifies a security
		/// descriptor for the new console screen buffer. If lpSecurityAttributes is <c>NULL</c>, the console screen buffer gets a default security descriptor.
		...
		/// </returns>
		// HANDLE WINAPI CreateConsoleScreenBuffer( _In_ DWORD dwDesiredAccess, _In_ DWORD dwShareMode, _In_opt_ const SECURITY_ATTRIBUTES *lpSecurityAttributes,
		...
		/// A handle to the console screen buffer. The handle must have the <c>GENERIC_WRITE</c> access right. For more information, see Console Buffer Security
		/// and Access Rights.
		/// </param>
		/// <param name="wAttribute">The attributes to use when writing to the console screen buffer. For more information, see Character Attributes.</param>
		/// <param name="nLength">The number of character cells to be set to the specified color attributes.</param>
		/// <param name="dwWriteCoord">A <c>COORD</c> structure that specifies the character coordinates of the first cell whose attributes are to be set.</param>
		...
		/// <para>If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		// BOOL WINAPI FillConsoleOutputAttribute( _In_ HANDLE hConsoleOutput, _In_ WORD wAttribute, _In_ DWORD nLength, _In_ COORD dwWriteCoord, _Out_ LPDWORD
		// lpNumberOfAttrsWritten );
		[DllImport(Lib.Kernel32, SetLastError = true, ExactSpelling = true)]
		[PInvokeData("Wincon.h", MSDNShortId = "")]
		[return: MarshalAs(UnmanagedType.Bool)]
		public static extern bool FillConsoleOutputAttribute(IntPtr hConsoleOutput, CHARACTER_ATTRIBUTE wAttribute, uint nLength, COORD dwWriteCoord, out uint lpNumberOfAttrsWritten);
		...
		/// <para>If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		// BOOL WINAPI FillConsoleOutputCharacter( _In_ HANDLE hConsoleOutput, _In_ TCHAR cCharacter, _In_ DWORD nLength, _In_ COORD dwWriteCoord, _Out_ LPDWORD
		...
		/// A handle to the console input buffer. The handle must have the <c>GENERIC_WRITE</c> access right. For more information, see Console Buffer Security
		/// and Access Rights.
		/// </param>
		/// <returns>
		/// <para>If the function succeeds, the return value is nonzero.</para>
		...
		/// <term>
		/// Generates a CTRL+C signal. This signal cannot be generated for process groups. If dwProcessGroupId is nonzero, this function will succeed, but the
		...
		/// <para>
		/// The identifier of the process group to receive the signal. A process group is created when the <c>CREATE_NEW_PROCESS_GROUP</c> flag is specified in a
		/// call to the <c>CreateProcess</c> function. The process identifier of the new process is also the process group identifier of a new process group. The
		/// process group includes all processes that are descendants of the root process. Only those processes in the group that share the same console as the
		/// calling process receive the signal. In other words, if a process in the group creates a new console, that process does not receive the signal, nor do
		/// its descendants.
		/// </para>
		...
		/// <para>If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		// DWORD WINAPI GetConsoleAlias( _In_ LPTSTR lpSource, _Out_ LPTSTR lpTargetBuffer, _In_ DWORD TargetBufferLength, _In_ LPTSTR lpExeName );
		[DllImport(Lib.Kernel32, SetLastError = true, CharSet = CharSet.Auto)]
		[PInvokeData("Wincon.h", MSDNShortId = "")]
		[return: MarshalAs(UnmanagedType.Bool)]
		...
		/// <para>The format of the data is as follows: Source1=Target1\0Source2=Target2\0... SourceN=TargetN\0, where N is the number of console aliases defined.</para>
		/// </param>
		...
		/// <summary>
		/// Retrieves the input code page used by the console associated with the calling process. A console uses its input code page to translate keyboard input
		/// into the corresponding character value.
		/// </summary>
		...
		/// and Access Rights.
		/// </param>
		/// <param name="lpConsoleCursorInfo">A pointer to a <c>CONSOLE_CURSOR_INFO</c> structure that receives information about the console's cursor.</param>
		/// <returns>
		/// <para>If the function succeeds, the return value is nonzero.</para>
		/// <para>If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		...
		/// <term>
		/// Full-screen console communicating directly with the video hardware. This mode is set after the console is in CONSOLE_FULLSCREEN mode to indicate that
		/// the transition to full-screen mode has completed.
		/// </term>
		/// </item>
		/// </list>
		/// </para>
		...
		/// and Access Rights.
		/// </param>
		/// <param name="nFont">The index of the font whose size is to be retrieved. This index is obtained by calling the <c>GetCurrentConsoleFont</c> function.</param>
		/// <returns>
		/// <para>
		/// If the function succeeds, the return value is a <c>COORD</c> structure that contains the width and height of each character in the font, in logical
		/// units. The <c>X</c> member contains the width, while the <c>Y</c> member contains the height.
		/// </para>
		...
		/// <para>
		/// To compile an application that uses this function, define <c>_WIN32_WINNT</c> as 0x0500 or later. For more information, see Using the Windows Headers.
		/// </para>
		/// </returns>
		...
		/// A handle to the console input buffer or the console screen buffer. The handle must have the <c>GENERIC_READ</c> access right. For more information,
		...
		/// <para>
		/// If the hConsoleHandle parameter is an input handle, the mode can be one or more of the following values. When a console is created, all input modes
		/// except <c>ENABLE_WINDOW_INPUT</c> are enabled by default.
		/// </para>
		/// <para>
		/// <list type="table">
		/// <listheader>
		...
		/// <term>
		/// Characters read by the ReadFile or ReadConsole function are written to the active screen buffer as they are read. This mode can be used only if the
		...
		/// <term>
		/// The ReadFile or ReadConsole function returns only when a carriage return character is read. If this mode is disabled, the functions return when one
		...
		/// <term>
		/// If the mouse pointer is within the borders of the console window and the window has the keyboard focus, mouse events generated by mouse movement and
		/// button presses are placed in the input buffer. These events are discarded by ReadFile or ReadConsole, even when this mode is enabled.
		/// </term>
		/// </item>
		/// <item>
		/// <term>ENABLE_PROCESSED_INPUT 0x0001</term>
		/// <term>
		/// CTRL+C is processed by the system and is not placed in the input buffer. If the input buffer is being read by ReadFile or ReadConsole, other control
		/// keys are processed by the system and are not returned in the ReadFile or ReadConsole buffer. If the ENABLE_LINE_INPUT mode is also enabled,
		...
		/// <term>
		/// Setting this flag directs the Virtual Terminal processing engine to convert user input received by the console window into Console Virtual Terminal
		/// Sequences that can be retrieved by a supporting application through WriteFile or WriteConsole functions. The typical usage of this flag is intended
		/// in conjunction with ENABLE_VIRTUAL_TERMINAL_PROCESSING on the output handle to connect to an application that communicates exclusively via virtual
		/// terminal sequences.
		/// </term>
		/// </item>
		/// <item/>
		...
		/// </para>
		/// <para>
		/// If the hConsoleHandle parameter is a screen buffer handle, the mode can be one or more of the following values. When a screen buffer is created, both
		...
		/// <term>
		/// Characters written by the WriteFile or WriteConsole function or echoed by the ReadFile or ReadConsole function are parsed for ASCII control
		/// sequences, and the correct action is performed. Backspace, tab, bell, carriage return, and line feed characters are processed.
		/// </term>
		/// </item>
		/// <item>
		...
		/// <term>
		/// When writing with WriteFile or WriteConsole or echoing with ReadFile or ReadConsole, the cursor moves to the beginning of the next row when it
		/// reaches the end of the current row. This causes the rows displayed in the console window to scroll up automatically when the cursor advances beyond
		/// the last row in the window. It also causes the contents of the console screen buffer to scroll up (discarding the top row of the console screen
		/// buffer) when the cursor advances beyond the last row in the console screen buffer. If this mode is disabled, the last character in the row is
		...
		/// <term>
		/// When writing with WriteFile or WriteConsole, characters are parsed for VT100 and similar control character sequences that control cursor movement,
		/// color/font mode, and other operations that can also be performed via the existing Console APIs. For more information, see Console Virtual Terminal Sequences.
		/// </term>
		/// </item>
		/// <item>
		/// <term>DISABLE_NEWLINE_AUTO_RETURN 0x0008</term>
		/// <term>
		/// When writing with WriteFile or WriteConsole, this adds an additional state to end-of-line wrapping that can delay the cursor move and buffer scroll
		/// operations. Normally when ENABLE_WRAP_AT_EOL_OUTPUT is set and text reaches the end of the line, the cursor will immediately move to the next line
		/// and the contents of the buffer will scroll up by one line. In contrast with this flag set, the scroll operation and cursor move is delayed until the
		/// next character arrives. The written character will be printed in the final position on the line and the cursor will remain above this character as if
		/// ENABLE_WRAP_AT_EOL_OUTPUT was off, but the next printable character will be printed as if ENABLE_WRAP_AT_EOL_OUTPUT is on. No overwrite will occur.
		/// Specifically, the cursor quickly advances down to the following line, a scroll is performed if necessary, the character is printed, and the cursor
		/// advances one more position. The typical usage of this flag is intended in conjunction with setting ENABLE_VIRTUAL_TERMINAL_PROCESSING to better
		/// emulate a terminal emulator where writing the final character on the screen (in the bottom right corner) without triggering an immediate scroll is
		...
		/// The APIs for writing character attributes including WriteConsoleOutput and WriteConsoleOutputAttribute allow the usage of flags from character
		/// attributes to adjust the color of the foreground and background of text. Additionally, a range of DBCS flags was specified with the COMMON_LVB
		/// prefix. Historically, these flags only functioned in DBCS code pages for Chinese, Japanese, and Korean languages. With exception of the leading byte
		/// and trailing byte flags, the remaining flags describing line drawing and reverse video (swap foreground and background colors) can be useful for
		/// other languages to emphasize portions of output. With exception of the leading byte and trailing byte flags, the remaining flags describing line
		/// drawing and reverse video (swap foreground and background colors) can be useful for other languages to emphasize portions of output. Setting this
		/// console mode flag will allow these attributes to be used in every code page on every language. It is off by default to maintain compatibility with
		/// known applications that have historically taken advantage of the console ignoring these flags on non-CJK machines to store bits in these fields for
		/// their own purposes or by accident. Note that using the ENABLE_VIRTUAL_TERMINAL_PROCESSING mode can result in LVB grid and reverse video flags being
		/// set while this flag is still off if the attached application requests underlining or inverse video via Console Virtual Terminal Sequences.
		/// </term>
		/// </item>
		/// <item/>
		/// <item/>
		...
		/// <summary>
		/// Retrieves the output code page used by the console associated with the calling process. A console uses its output code page to translate the
		/// character values written by the various output functions into the images displayed in the console window.
		/// </summary>
		/// <returns>The return value is a code that identifies the code page. For a list of identifiers, see Code Page Identifiers.</returns>
		...
		/// <returns>
		/// <para>
		/// If the function succeeds, the return value is less than or equal to dwProcessCount and represents the number of process identifiers stored in the
		/// lpdwProcessList buffer.
		/// </para>
		/// <para>
		/// If the buffer is too small to hold all the valid process identifiers, the return value is the required number of array elements. The function will
		/// have stored no identifiers in the buffer. In this situation, use the return value to allocate a buffer that is large enough to store the entire list
		/// and call the function again.
		/// </para>
		/// <para>
		/// If the return value is zero, the function has failed, because every console has at least one process associated with it. To get extended error
		...
		/// and Access Rights.
		/// </param>
		/// <param name="lpConsoleScreenBufferInfo">A pointer to a <c>CONSOLE_SCREEN_BUFFER_INFO</c> structure that receives the console screen buffer information.</param>
		/// <returns>
		/// <para>If the function succeeds, the return value is nonzero.</para>
		/// <para>If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		...
		/// and Access Rights.
		/// </param>
		/// <param name="lpConsoleScreenBufferInfoEx">A <c>CONSOLE_SCREEN_BUFFER_INFOEX</c> structure that receives the requested console screen buffer information.</param>
		/// <returns>
		/// <para>If the function succeeds, the return value is nonzero.</para>
		/// <para>If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		// BOOL WINAPI GetConsoleScreenBufferInfoEx( _In_ HANDLE hConsoleOutput, _Out_ PCONSOLE_SCREEN_BUFFER_INFOEX lpConsoleScreenBufferInfoEx );
		[DllImport(Lib.Kernel32, SetLastError = true, ExactSpelling = true)]
		[PInvokeData("Wincon.h", MSDNShortId = "")]
		[return: MarshalAs(UnmanagedType.Bool)]
		...
		/// <param name="lpConsoleTitle">
		/// <para>
		/// A pointer to a buffer that receives a null-terminated string containing the title. If the buffer is too small to store the title, the function stores
		/// as many characters of the title as will fit in the buffer, ending with a null terminator.
		/// </para>
		/// <para>
		/// The storage for this buffer is allocated from a shared heap for the process that is 64 KB in size. The maximum size of the buffer will depend on heap usage.
		/// </para>
		...
		/// <returns>
		/// The return value is a handle to the window used by the console associated with the calling process or <c>NULL</c> if there is no such associated console.
		/// </returns>
		...
		/// If this parameter is <c>TRUE</c>, font information is retrieved for the maximum window size. If this parameter is <c>FALSE</c>, font information is
		/// retrieved for the current window size.
		/// </param>
		...
		[PInvokeData("Wincon.h", MSDNShortId = "")]
		[return: MarshalAs(UnmanagedType.Bool)]
		public static extern bool GetCurrentConsoleFont(IntPtr hConsoleOutput, [MarshalAs(UnmanagedType.Bool)] bool bMaximumWindow, out CONSOLE_FONT_INFO lpConsoleCurrentFont);
		...
		/// <para>If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		// BOOL WINAPI GetCurrentConsoleFontEx( _In_ HANDLE hConsoleOutput, _In_ BOOL bMaximumWindow, _Out_ PCONSOLE_FONT_INFOEX lpConsoleCurrentFontEx );
		[DllImport(Lib.Kernel32, SetLastError = true, ExactSpelling = true)]
		[PInvokeData("Wincon.h", MSDNShortId = "")]
		public static extern bool GetCurrentConsoleFontEx(IntPtr hConsoleOutput, [MarshalAs(UnmanagedType.Bool)] bool bMaximumWindow, out CONSOLE_FONT_INFOEX lpConsoleCurrentFontEx);
		...
		/// <returns>
		/// <para>
		/// If the function succeeds, the return value is a <c>COORD</c> structure that specifies the number of character cell rows ( <c>X</c> member) and
		/// columns ( <c>Y</c> member) in the largest possible console window. Otherwise, the members of the structure are zero.
		/// </para>
		...
		/// <param name="hConsoleInput">
		/// A handle to the console input buffer. The handle must have the <c>GENERIC_READ</c> access right. For more information, see Console Buffer Security
		/// and Access Rights.
		/// </param>
		...
		/// <para>If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		// BOOL WINAPI PeekConsoleInput( _In_ HANDLE hConsoleInput, _Out_ PINPUT_RECORD lpBuffer, _In_ DWORD nLength, _Out_ LPDWORD lpNumberOfEventsRead );
		[DllImport(Lib.Kernel32, SetLastError = true, CharSet = CharSet.Auto)]
		[PInvokeData("Wincon.h", MSDNShortId = "")]
		[return: MarshalAs(UnmanagedType.Bool)]
		...
		/// <para>
		/// A pointer to a <c>CONSOLE_READCONSOLE_CONTROL</c> structure that specifies a control character to signal the end of the read operation. This
		...
		[PInvokeData("Wincon.h", MSDNShortId = "")]
		[return: MarshalAs(UnmanagedType.Bool)]
		public static extern bool ReadConsole(IntPtr hConsoleInput, StringBuilder lpBuffer, uint nNumberOfCharsToRead, out uint lpNumberOfCharsRead, IntPtr pInputControl = default(IntPtr));
		...
		[PInvokeData("Wincon.h", MSDNShortId = "")]
		[return: MarshalAs(UnmanagedType.Bool)]
		public static extern bool ReadConsoleInput(IntPtr hConsoleInput, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 2)] INPUT_RECORD[] lpBuffer, uint nLength, out uint lpNumberOfEventsRead);

		/// <summary>
		/// Reads character and color attribute data from a rectangular block of character cells in a console screen buffer, and the function writes the data to
		/// a rectangular block at a specified location in the destination buffer.
		/// </summary>
		/// <param name="hConsoleOutput">
		/// A handle to the console screen buffer. The handle must have the <c>GENERIC_READ</c> access right. For more information, see Console Buffer Security
		/// and Access Rights.
		...
		/// <param name="lpBuffer">
		/// <para>
		/// A pointer to a destination buffer that receives the data read from the console screen buffer. This pointer is treated as the origin of a
		/// two-dimensional array of <c>CHAR_INFO</c> structures whose size is specified by the dwBufferSize parameter.
		/// </para>
		/// <para>
		/// The storage for this buffer is allocated from a shared heap for the process that is 64 KB in size. The maximum size of the buffer will depend on heap usage.
		/// </para>
		...
		/// The size of the lpBuffer parameter, in character cells. The <c>X</c> member of the <c>COORD</c> structure is the number of columns; the <c>Y</c>
		/// member is the number of rows.
		/// </param>
		/// <param name="dwBufferCoord">
		/// The coordinates of the upper-left cell in the lpBuffer parameter that receives the data read from the console screen buffer. The <c>X</c> member of
		/// the <c>COORD</c> structure is the column, and the <c>Y</c> member is the row.
		/// </param>
		/// <param name="lpReadRegion">
		/// A pointer to a <c>SMALL_RECT</c> structure. On input, the structure members specify the upper-left and lower-right coordinates of the console screen
		/// buffer rectangle from which the function is to read. On output, the structure members specify the actual rectangle that was used.
		/// </param>
		/// <returns>
		/// <para>If the function succeeds, the return value is nonzero.</para>
		/// <para>If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		// BOOL WINAPI ReadConsoleOutput( _In_ HANDLE hConsoleOutput, _Out_ PCHAR_INFO lpBuffer, _In_ COORD dwBufferSize, _In_ COORD dwBufferCoord, _Inout_
		...
		/// <summary>Copies a specified number of character attributes from consecutive cells of a console screen buffer, beginning at a specified location.</summary>
		/// <param name="hConsoleOutput">
		/// A handle to the console screen buffer. The handle must have the <c>GENERIC_READ</c> access right. For more information, see Console Buffer Security
		/// and Access Rights.
		/// </param>
		...
		/// The coordinates of the first cell in the console screen buffer from which to read, in characters. The <c>X</c> member of the <c>COORD</c> structure
		/// is the column, and the <c>Y</c> member is the row.
		/// </param>
		...
		/// <para>If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		// BOOL WINAPI ReadConsoleOutputAttribute( _In_ HANDLE hConsoleOutput, _Out_ LPWORD lpAttribute, _In_ DWORD nLength, _In_ COORD dwReadCoord, _Out_
		// LPDWORD lpNumberOfAttrsRead );
		[DllImport(Lib.Kernel32, SetLastError = true, ExactSpelling = true)]
		[PInvokeData("Wincon.h", MSDNShortId = "")]
		[return: MarshalAs(UnmanagedType.Bool)]
		public static extern bool ReadConsoleOutputAttribute(IntPtr hConsoleOutput, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 2)] CHARACTER_ATTRIBUTE[] lpAttribute, uint nLength, COORD dwReadCoord, out uint lpNumberOfAttrsRead);
		...
		/// </param>
		/// <param name="dwReadCoord">
		/// The coordinates of the first cell in the console screen buffer from which to read, in characters. The X member of the COORD structure is the column,
		...
		/// <summary>
		/// Moves a block of data in a screen buffer. The effects of the move can be limited by specifying a clipping rectangle, so the contents of the console
		...
		/// A pointer to a <c>SMALL_RECT</c> structure whose members specify the upper-left and lower-right coordinates of the console screen buffer rectangle to
		/// be moved.
		/// </param>
		/// <param name="lpClipRectangle">
		/// A pointer to a <c>SMALL_RECT</c> structure whose members specify the upper-left and lower-right coordinates of the console screen buffer rectangle
		/// that is affected by the scrolling. This pointer can be <c>NULL</c>.
		/// </param>
		...
		/// </param>
		/// <param name="lpFill">
		/// A pointer to a <c>CHAR_INFO</c> structure that specifies the character and color attributes to be used in filling the cells within the intersection
		/// of lpScrollRectangle and lpClipRectangle that were left empty as a result of the move.
		/// </param>
		/// <returns>
		/// <para>If the function succeeds, the return value is nonzero.</para>
		/// <para>If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		...
		/// <summary>
		/// Sets the input code page used by the console associated with the calling process. A console uses its input code page to translate keyboard input into
		...
		/// <summary>
		/// <para>Adds or removes an application-defined <c>HandlerRoutine</c> function from the list of handler functions for the calling process.</para>
		/// <para>
		...
		/// <para>
		/// If the HandlerRoutine parameter is <c>NULL</c>, a <c>TRUE</c> value causes the calling process to ignore CTRL+C input, and a <c>FALSE</c> value
		...
		/// </para>
		/// </param>
		/// <param name="lpNewScreenBufferDimensions">A pointer to a <c>COORD</c> structure that receives the new dimensions of the screen buffer, in characters.</param>
		/// <returns>
		/// <para>If the function succeeds, the return value is nonzero.</para>
		/// <para>If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		...
		[PInvokeData("Wincon.h", MSDNShortId = "")]
		[return: MarshalAs(UnmanagedType.Bool)]
		public static extern bool SetConsoleDisplayMode(IntPtr hConsoleOutput, SET_CONSOLE_DISPLAY_MODE dwFlags, out COORD lpNewScreenBufferDimensions);

		/// <summary>Sets the history settings for the calling process's console.</summary>
		/// <param name="lpConsoleHistoryInfo">A pointer to a <c>CONSOLE_HISTORY_INFO</c> structure that contains the history settings for the process's console.</param>
		/// <returns>
		/// <para>If the function succeeds, the return value is nonzero.</para>
		/// <para>If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		...
		/// <param name="hConsoleHandle">
		/// A handle to the console input buffer or a console screen buffer. The handle must have the <c>GENERIC_READ</c> access right. For more information, see
		/// Console Buffer Security and Access Rights.
		/// </param>
		/// <param name="dwMode">
		/// <para>
		/// The input or output mode to be set. If the hConsoleHandle parameter is an input handle, the mode can be one or more of the following values. When a
		/// console is created, all input modes except <c>ENABLE_WINDOW_INPUT</c> are enabled by default.
		/// </para>
		/// <para>
		/// <list type="table">
		/// <listheader>
		...
		/// <term>
		/// When enabled, text entered in a console window will be inserted at the current cursor location and all text following that location will not be
		/// overwritten. When disabled, all following text will be overwritten. To enable this mode, use . To disable this mode, use ENABLE_EXTENDED_FLAGS
		/// without this flag.
		/// </term>
		/// </item>
		/// <item>
		...
		/// <term>
		/// Setting this flag directs the Virtual Terminal processing engine to convert user input received by the console window into Console Virtual Terminal
		/// Sequences that can be retrieved by a supporting application through ReadFile or ReadConsole functions. The typical usage of this flag is intended in
		/// conjunction with ENABLE_VIRTUAL_TERMINAL_PROCESSING on the output handle to connect to an application that communicates exclusively via virtual
		/// terminal sequences.
		/// </term>
		/// </item>
		/// <item/>
		...
		/// <summary>
		/// Sets the output code page used by the console associated with the calling process. A console uses its output code page to translate the character
		/// values written by the various output functions into the images displayed in the console window.
		/// </summary>
		...
		/// and Access Rights.
		/// </param>
		/// <param name="lpConsoleScreenBufferInfoEx">A <c>CONSOLE_SCREEN_BUFFER_INFOEX</c> structure that contains the console screen buffer information.</param>
		/// <returns>
		/// <para>If the function succeeds, the return value is nonzero.</para>
		/// <para>If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		// BOOL WINAPI SetConsoleScreenBufferInfoEx( _In_ HANDLE hConsoleOutput, _In_ PCONSOLE_SCREEN_BUFFER_INFOEX lpConsoleScreenBufferInfoEx );
		[DllImport(Lib.Kernel32, SetLastError = true, ExactSpelling = true)]
		[PInvokeData("Wincon.h", MSDNShortId = "")]
		...
		/// </param>
		/// <param name="dwSize">
		/// A <c>COORD</c> structure that specifies the new size of the console screen buffer, in character rows and columns. The specified width and height
		/// cannot be less than the width and height of the console screen buffer's window. The specified dimensions also cannot be less than the minimum size
		/// allowed by the system. This minimum depends on the current font size for the console (selected by the user) and the <c>SM_CXMIN</c> and
		/// <c>SM_CYMIN</c> values returned by the <c>GetSystemMetrics</c> function.
		/// </param>
		/// <returns>
		/// <para>If the function succeeds, the return value is nonzero.</para>
		/// <para>If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		...
		/// <summary>
		/// Sets the attributes of characters written to the console screen buffer by the <c>WriteFile</c> or <c>WriteConsole</c> function, or echoed by the
		/// <c>ReadFile</c> or <c>ReadConsole</c> function. This function affects text written after the function call.
		/// </summary>
		/// <param name="hConsoleOutput">
		/// A handle to the console screen buffer. The handle must have the <c>GENERIC_READ</c> access right. For more information, see Console Buffer Security
		/// and Access Rights.
		...
		/// If this parameter is <c>TRUE</c>, the coordinates specify the new upper-left and lower-right corners of the window. If it is <c>FALSE</c>, the
		/// coordinates are relative to the current window-corner coordinates.
		/// </param>
		/// <param name="lpConsoleWindow">A pointer to a <c>SMALL_RECT</c> structure that specifies the new upper-left and lower-right corners of the window.</param>
		/// <returns>
		/// <para>If the function succeeds, the return value is nonzero.</para>
		/// <para>If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		...
		/// </param>
		/// <param name="bMaximumWindow">
		/// If this parameter is <c>TRUE</c>, font information is set for the maximum window size. If this parameter is <c>FALSE</c>, font information is set for
		...
		[DllImport(Lib.Kernel32, SetLastError = true, ExactSpelling = true)]
		[PInvokeData("Wincon.h", MSDNShortId = "")]
		public static extern bool SetCurrentConsoleFontEx(IntPtr hConsoleOutput, [MarshalAs(UnmanagedType.Bool)] bool bMaximumWindow, ref CONSOLE_FONT_INFOEX lpConsoleCurrentFontEx);
		...
		/// The number of characters to be written. If the total size of the specified number of characters exceeds the available heap, the function fails with <c>ERROR_NOT_ENOUGH_MEMORY</c>.
		/// </param>
		...
		[PInvokeData("Wincon.h", MSDNShortId = "")]
		[return: MarshalAs(UnmanagedType.Bool)]
		public static extern bool WriteConsole(IntPtr hConsoleOutput, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 2)] char[] lpBuffer, uint nNumberOfCharsToWrite, out uint lpNumberOfCharsWritten, IntPtr lpReserved = default(IntPtr));
		...
		[PInvokeData("Wincon.h", MSDNShortId = "")]
		[return: MarshalAs(UnmanagedType.Bool)]
		public static extern bool WriteConsoleInput(IntPtr hConsoleInput, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 2)] INPUT_RECORD[] lpBuffer, uint nLength, out uint lpNumberOfEventsWritten);

		/// <summary>
		/// Writes character and color attribute data to a specified rectangular block of character cells in a console screen buffer. The data to be written is
		/// taken from a correspondingly sized rectangular block at a specified location in the source buffer.
		/// </summary>
		/// <param name="hConsoleOutput">
		/// A handle to the console screen buffer. The handle must have the <c>GENERIC_WRITE</c> access right. For more information, see Console Buffer Security
		/// and Access Rights.
		...
		/// <param name="lpBuffer">
		/// <para>
		/// The data to be written to the console screen buffer. This pointer is treated as the origin of a two-dimensional array of <c>CHAR_INFO</c> structures
		...
		/// </param>
		/// <param name="dwBufferSize">
		/// The size of the buffer pointed to by the lpBuffer parameter, in character cells. The <c>X</c> member of the <c>COORD</c> structure is the number of
		/// columns; the <c>Y</c> member is the number of rows.
		/// </param>
		/// <param name="dwBufferCoord">
		/// The coordinates of the upper-left cell in the buffer pointed to by the lpBuffer parameter. The <c>X</c> member of the <c>COORD</c> structure is the
		/// column, and the <c>Y</c> member is the row.
		/// </param>
		/// <param name="lpWriteRegion">
		/// A pointer to a <c>SMALL_RECT</c> structure. On input, the structure members specify the upper-left and lower-right coordinates of the console screen
		/// buffer rectangle to write to. On output, the structure members specify the actual rectangle that was used.
		/// </param>
		/// <returns>
		/// <para>If the function succeeds, the return value is nonzero.</para>
		/// <para>If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		// BOOL WINAPI WriteConsoleOutput( _In_ HANDLE hConsoleOutput, _In_ const CHAR_INFO *lpBuffer, _In_ COORD dwBufferSize, _In_ COORD dwBufferCoord, _Inout_
		...
		/// <summary>Copies a number of character attributes to consecutive cells of a console screen buffer, beginning at a specified location.</summary>
		/// <param name="hConsoleOutput">
		/// A handle to the console screen buffer. The handle must have the <c>GENERIC_WRITE</c> access right. For more information, see Console Buffer Security
		/// and Access Rights.
		/// </param>
		/// <param name="lpAttribute">The attributes to be used when writing to the console screen buffer. For more information, see Character Attributes.</param>
		/// <param name="nLength">
		...
		/// </param>
		/// <param name="dwWriteCoord">
		/// A <c>COORD</c> structure that specifies the character coordinates of the first cell in the console screen buffer to which the attributes will be written.
		/// </param>
		...
		/// <para>If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		// BOOL WINAPI WriteConsoleOutputAttribute( _In_ HANDLE hConsoleOutput, _In_ const WORD *lpAttribute, _In_ DWORD nLength, _In_ COORD dwWriteCoord, _Out_
		// LPDWORD lpNumberOfAttrsWritten );
		[DllImport(Lib.Kernel32, SetLastError = true, ExactSpelling = true)]
		[PInvokeData("Wincon.h", MSDNShortId = "")]
		[return: MarshalAs(UnmanagedType.Bool)]
		public static extern bool WriteConsoleOutputAttribute(IntPtr hConsoleOutput, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 2)] CHARACTER_ATTRIBUTE[] lpAttribute, uint nLength, COORD dwWriteCoord, out uint lpNumberOfAttrsWritten);
		...
		/// <param name="dwWriteCoord">
		/// A <c>COORD</c> structure that specifies the character coordinates of the first cell in the console screen buffer to which characters will be written.
		/// </param>
		/// <param name="lpNumberOfCharsWritten">A pointer to a variable that receives the number of characters actually written.</param>
		/// <returns>
		/// <para>If the function succeeds, the return value is nonzero.</para>
		/// <para>If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		// BOOL WINAPI WriteConsoleOutputCharacter( _In_ HANDLE hConsoleOutput, _In_ LPCTSTR lpCharacter, _In_ DWORD nLength, _In_ COORD dwWriteCoord, _Out_
		// LPDWORD lpNumberOfCharsWritten );
		[DllImport(Lib.Kernel32, SetLastError = true, CharSet = CharSet.Auto)]
		[PInvokeData("Wincon.h", MSDNShortId = "")]
		[return: MarshalAs(UnmanagedType.Bool)]
		public static extern bool WriteConsoleOutputCharacter(IntPtr hConsoleOutput, [In] string lpCharacter, uint nLength, COORD dwWriteCoord, out uint lpNumberOfCharsWritten);
		...
		{
			/// <summary>
			/// The percentage of the character cell that is filled by the cursor. This value is between 1 and 100. The cursor appearance varies, ranging from
			/// completely filling the cell to showing up as a horizontal line at the bottom of the cell.
			/// </summary>
			public uint dwSize;
		...
		public struct CONSOLE_FONT_INFO
		{
			...
			public uint nFont;
			/// <summary>A <c>COORD</c> structure that contains the width and height of each character in the font, in logical units. The <c>X</c> member contains the width, while the <c>Y</c> member contains the height.</summary>
			public COORD dwFontSize;
		}

		/// <summary>Contains extended information for a console font.</summary>
		// typedef struct _CONSOLE_FONT_INFOEX { ULONG cbSize; DWORD nFont; COORD dwFontSize; UINT FontFamily; UINT FontWeight; WCHAR FaceName[LF_FACESIZE]; }
		...
		public struct CONSOLE_FONT_INFOEX
		{
			...
			public uint nFont;
			/// <summary>
			/// A <c>COORD</c> structure that contains the width and height of each character in the font, in logical units. The <c>X</c> member contains the
			/// width, while the <c>Y</c> member contains the height.
			/// </summary>
			public COORD dwFontSize;
			/// <summary>
			/// The font pitch and family. For information about the possible values for this member, see the description of the <c>tmPitchAndFamily</c> member
			/// of the <c>TEXTMETRIC</c> structure.
			/// </summary>
		...
		public struct CONSOLE_HISTORY_INFO
		{
			...
			/// <summary>This parameter can be zero or HISTORY_NO_DUP_FLAG (0x1) to indicate duplicate entries will not be stored in the history buffer.</summary>
			public uint dwFlags;
		}
		...
		public struct CONSOLE_SCREEN_BUFFER_INFO
		{
			...
			/// <summary>
			/// The attributes of the characters written to a screen buffer by the <c>WriteFile</c> and <c>WriteConsole</c> functions, or echoed to a screen
			/// buffer by the <c>ReadFile</c> and <c>ReadConsole</c> functions. For more information, see Character Attributes.
			/// </summary>
			public CHARACTER_ATTRIBUTE wAttributes;
			/// <summary>
			/// A <c>SMALL_RECT</c> structure that contains the console screen buffer coordinates of the upper-left and lower-right corners of the display window.
			/// </summary>
			public SMALL_RECT srWindow;
			/// <summary>
			/// A <c>COORD</c> structure that contains the maximum size of the console window, in character columns and rows, given the current screen buffer
		...
		// typedef struct _CONSOLE_SCREEN_BUFFER_INFOEX { ULONG cbSize; COORD dwSize; COORD dwCursorPosition; WORD wAttributes; SMALL_RECT srWindow; COORD
		// dwMaximumWindowSize; WORD wPopupAttributes; BOOL bFullscreenSupported; COLORREF ColorTable[16]; } CONSOLE_SCREEN_BUFFER_INFOEX, *PCONSOLE_SCREEN_BUFFER_INFOEX;
		[PInvokeData("Wincon.h", MSDNShortId = "")]
		[StructLayout(LayoutKind.Sequential)]
		...
		/// <summary>Describes a focus event in a console <c>INPUT_RECORD</c> structure. These events are used internally and should be ignored.</summary>
		...
		/// <summary>
		/// Describes an input event in the console input buffer. These records can be read from the input buffer by using the <c>ReadConsoleInput</c> or
		/// <c>PeekConsoleInput</c> function, or written to the input buffer by using the <c>WriteConsoleInput</c> function.
		/// </summary>
		// typedef struct _INPUT_RECORD { WORD EventType; union { KEY_EVENT_RECORD KeyEvent; MOUSE_EVENT_RECORD MouseEvent; WINDOW_BUFFER_SIZE_RECORD
		// WindowBufferSizeEvent; MENU_EVENT_RECORD MenuEvent; FOCUS_EVENT_RECORD FocusEvent; } Event; } INPUT_RECORD;
		[PInvokeData("Wincon.h", MSDNShortId = "")]
		[StructLayout(LayoutKind.Sequential)]
		public struct INPUT_RECORD
		{
			...
			/// <term>The Event member contains a WINDOW_BUFFER_SIZE_RECORD structure with information about the new size of the console screen buffer.</term>
			/// </item>
			/// <item/>
			/// <item/>
			/// <item/>
			...
			/// <summary>The event information. The format of this member depends on the event type specified by the <c>EventType</c> member.</summary>
		...
		// typedef struct _KEY_EVENT_RECORD { BOOL bKeyDown; WORD wRepeatCount; WORD wVirtualKeyCode; WORD wVirtualScanCode; union { WCHAR UnicodeChar; CHAR
		// AsciiChar; } uChar; DWORD dwControlKeyState; } KEY_EVENT_RECORD;
		[PInvokeData("Wincon.h", MSDNShortId = "")]
		[StructLayout(LayoutKind.Sequential)]
		public struct KEY_EVENT_RECORD
		{
			...
			/// <summary>
			/// The repeat count, which indicates that a key is being held down. For example, when a key is held down, you might get five events with this member
			/// equal to 1, one event with this member equal to 5, or multiple events with this member greater than or equal to 1.
			/// </summary>
		...
		/// <summary>Describes a menu event in a console <c>INPUT_RECORD</c> structure. These events are used internally and should be ignored.</summary>
		...
		{
			/// <summary>A <c>COORD</c> structure that contains the location of the cursor, in terms of the console screen buffer's character-cell coordinates.</summary>
			public COORD dwMousePosition;
			/// <summary>
			/// <para>
			/// The status of the mouse buttons. The least significant bit corresponds to the leftmost mouse button. The next least significant bit corresponds
			/// to the rightmost mouse button. The next bit indicates the next-to-leftmost mouse button. The bits then correspond left to right to the mouse
			/// buttons. A bit is 1 if the button was pressed.
			/// </para>
			...
			/// <summary>
			/// <para>
			/// The type of mouse event. If this value is zero, it indicates a mouse button being pressed or released. Otherwise, this member is one of the
			/// following values.
			/// </para>
			/// <para>
			/// <list type="table">
			...
			/// <term>The second click (button press) of a double-click occurred. The first click is returned as a regular button-press event.</term>
			/// </item>
			/// <item>
		...
		{
			/// <summary>A <c>COORD</c> structure that contains the size of the console screen buffer, in character cell columns and rows.</summary>
			public COORD dwSize;
		}
	}
}