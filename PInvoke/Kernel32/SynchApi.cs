...
public static partial class Kernel32
{
	...
	public static extern SafeEventHandle CreateEventEx([In, Optional] SECURITY_ATTRIBUTES lpEventAttributes, [In, Optional] string lpName, [Optional] CREATE_EVENT_FLAGS dwFlags, ACCESS_MASK dwDesiredAccess);
...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		/// creates an auto-reset event object, and system automatically resets the event state to nonsignaled after a single waiting
		/// thread has been released.
		/// </param>
		/// <param name="name">
		/// <para>The name of the event object. The name is limited to <c>MAX_PATH</c> characters. Name comparison is case sensitive.</para>
		/// <para>
		/// If <paramref name="name"/> matches the name of an existing named event object, this function requests the
		/// <c>EVENT_ALL_ACCESS</c> access right. In this case, the bManualReset and bInitialState parameters are ignored because they
		/// have already been set by the creating process. If the <paramref name="attributes"/> parameter is not <see langword="null"/>,
		...
		/// The name can have a "Global\" or "Local\" prefix to explicitly create the object in the global or session namespace. The
		/// remainder of the name can contain any character except the backslash character (\). For more information, see Kernel Object
		/// Namespaces. Fast user switching is implemented using Terminal Services sessions. Kernel object names must follow the
		...
		/// <para>If the function fails, the return value is <c>Null</c>. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		public static SafeEventHandle Create(bool initialState = false, bool manualReset = false, string name = null, SECURITY_ATTRIBUTES attributes = default) => CreateEvent(attributes, manualReset, initialState, name);
		...
		/// <para>This function can open objects in a private namespace. For more information, see Object Namespaces.</para>
		/// <para>
		/// <c>Terminal Services:</c> The name can have a "Global\" or "Local\" prefix to explicitly open an object in the global or
		/// session namespace. The remainder of the name can contain any character except the backslash character (\). For more
		/// information, see Kernel Object Namespaces.
		/// </para>
		/// <para>
		/// <c>Note</c> Fast user switching is implemented using Terminal Services sessions. The first user to log on uses session 0, the
		/// next user to log on uses session 1, and so on. Kernel object names must follow the guidelines outlined for Terminal Services
		/// so that applications can support multiple users.
		/// </para>
		/// </param>
		/// <param name="inherit">
		/// If this value is <c>TRUE</c>, processes created by this process will inherit the handle. Otherwise, the processes do not
		/// inherit this handle.
		/// </param>
		/// <param name="access">
		/// The access to the event object. The function fails if the security descriptor of the specified object does not permit the
