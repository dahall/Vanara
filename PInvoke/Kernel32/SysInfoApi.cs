...
namespace Vanara.PInvoke
{
	public static partial class Kernel32
	{
		...
		// typedef enum _COMPUTER_NAME_FORMAT { ComputerNameNetBIOS, ComputerNameDnsHostname, ComputerNameDnsDomain, ComputerNameDnsFullyQualified,
		// ComputerNamePhysicalNetBIOS, ComputerNamePhysicalDnsHostname, ComputerNamePhysicalDnsDomain, ComputerNamePhysicalDnsFullyQualified, ComputerNameMax}
		// COMPUTER_NAME_FORMAT; https://msdn.microsoft.com/en-us/library/windows/desktop/ms724224(v=vs.85).aspx
		[PInvokeData("Winbase.h", MSDNShortId = "ms724224")]
		public enum COMPUTER_NAME_FORMAT
		{
			/// <summary>
			/// The NetBIOS name of the local computer or the cluster associated with the local computer. This name is limited to MAX_COMPUTERNAME_LENGTH + 1
			/// characters and may be a truncated version of the DNS host name. For example, if the DNS host name is "corporate-mail-server", the NetBIOS name
			/// would be "corporate-mail-".
			/// </summary>
			ComputerNameNetBIOS,
			...
			/// <para>
			/// This name is a combination of the DNS host name and the DNS domain name, using the form HostName.DomainName. For example, if the DNS host name is
			/// "corporate-mail-server" and the DNS domain name is "microsoft.com", the fully qualified DNS name is "corporate-mail-server.microsoft.com".
			/// </para>
			/// </summary>
			...
			/// <summary>
			/// The fully qualified DNS name that uniquely identifies the computer. On a cluster, this is the fully qualified DNS name of the local node on the
			/// cluster. The fully qualified DNS name is a combination of the DNS host name and the DNS domain name, using the form HostName.DomainName.
			/// </summary>
		...
		/// <summary>
		/// Represents the relationship between the processor set identified in the corresponding <c>SYSTEM_LOGICAL_PROCESSOR_INFORMATION</c> or
		/// <c>SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX</c> structure.
		/// </summary>
		// typedef enum _LOGICAL_PROCESSOR_RELATIONSHIP { RelationProcessorCore, RelationNumaNode, RelationCache, RelationProcessorPackage, RelationGroup,
		// RelationAll = 0xffff} LOGICAL_PROCESSOR_RELATIONSHIP; https://msdn.microsoft.com/en-us/library/windows/desktop/ms684197(v=vs.85).aspx
		...
		public enum LOGICAL_PROCESSOR_RELATIONSHIP : uint
		{
			...
			/// <para><c>Windows Server 2003:</c> This value is not supported until Windows Server 2003 with SP1 and Windows XP Professional x64 Edition.</para>
			/// </summary>
			RelationCache,
			/// <summary>
			/// <para>
			/// The specified logical processors share a physical package (a single package socketed or soldered onto a motherboard may contain multiple
			/// processor cores or threads, each of which is treated as a separate processor by the operating system).
			/// </para>
			/// <para><c>Windows Server 2003:</c> This value is not supported until Windows Server 2003 with SP1 and Windows XP Professional x64 Edition.</para>
			/// </summary>
			...
			/// <para>
			/// <c>Windows Server 2008, Windows Vista, Windows Server 2003 and Windows XP Professional x64 Edition:</c> This value is not supported until Windows
			/// Server 2008 R2.
			/// </para>
			/// </summary>
			RelationGroup,
		...
		/// <param name="nSize">
		/// <para>
		/// On input, specifies the size of the buffer, in <c>TCHARs</c>. On output, receives the number of <c>TCHARs</c> copied to the destination buffer, not
		/// including the terminating null character.
		/// </para>
		/// <para>
		/// If the buffer is too small, the function fails, <c>GetLastError</c> returns ERROR_MORE_DATA, and nSize receives the required buffer size, not
		/// including the terminating null character.
		/// </para>
		/// </param>
		/// <returns>
		...
		/// <para>
		/// If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>. Possible values include the following.
		/// </para>
		/// <para>
		/// <list type="table">
		/// <listheader>
		...
		public static extern bool DnsHostnameToComputerName([In] string Hostname, [Out] StringBuilder ComputerName, ref uint nSize);
		...
		/// <para>
		/// A pointer to a buffer that receives the list of firmware tables. If this parameter is <c>NULL</c>, the return value is the required buffer size.
		/// </para>
		...
		/// <returns>
		/// <para>If the function succeeds, the return value is the number of bytes written to the buffer. This value will always be less than or equal to BufferSize.</para>
		/// <para>
		/// If the function fails because the buffer is not large enough, the return value is the required buffer size, in bytes. This value is always greater
		/// than BufferSize.
		/// </para>
		...
		/// </returns>
		// UINT WINAPI EnumSystemFirmwareTables( _In_ DWORD FirmwareTableProviderSignature, _Out_ PVOID pFirmwareTableBuffer, _In_ DWORD BufferSize); https://msdn.microsoft.com/en-us/library/windows/desktop/ms724259(v=vs.85).aspx
		...
		/// <para>
		/// <c>GetComputerName</c> retrieves only the NetBIOS name of the local computer. To retrieve the DNS host name, DNS domain name, or the fully qualified
		/// DNS name, call the <c>GetComputerNameEx</c> function. Additional information is provided by the <c>IADsADSystemInfo</c> interface.
		/// </para>
		/// <para>
		...
		/// </summary>
		/// <param name="lpBuffer">
		/// A pointer to a buffer that receives the computer name or the cluster virtual server name. The buffer size should be large enough to contain
		/// MAX_COMPUTERNAME_LENGTH + 1 characters.
		/// </param>
		/// <param name="lpnSize">
		/// <para>
		/// On input, specifies the size of the buffer, in <c>TCHARs</c>. On output, the number of <c>TCHARs</c> copied to the destination buffer, not including
		/// the terminating null character.
		/// </para>
		/// <para>
		/// If the buffer is too small, the function fails and <c>GetLastError</c> returns ERROR_BUFFER_OVERFLOW. The lpnSize parameter specifies the size of the
		...
		[return: MarshalAs(UnmanagedType.Bool)]
		public static extern bool GetComputerName([Out] StringBuilder lpBuffer, ref uint nSize);
		...
		/// <param name="NameType">
		/// <para>
		/// The type of name to be retrieved. This parameter is a value from the <c>COMPUTER_NAME_FORMAT</c> enumeration type. The following table provides
		...
		/// <term>
		/// The name of the DNS domain assigned to the local computer. If the local computer is a node in a cluster, lpBuffer receives the DNS domain name of the
		...
		/// <term>
		/// The fully qualified DNS name that uniquely identifies the local computer. This name is a combination of the DNS host name and the DNS domain name,
		/// using the form HostName.DomainName. If the local computer is a node in a cluster, lpBuffer receives the fully qualified DNS name of the cluster
		/// virtual server.
		/// </term>
		/// </item>
		/// <item>
		...
		/// <term>
		/// The fully qualified DNS name that uniquely identifies the computer. If the local computer is a node in a cluster, lpBuffer receives the fully
		/// qualified DNS name of the local computer, not the name of the cluster virtual server. The fully qualified DNS name is a combination of the DNS host
		/// name and the DNS domain name, using the form HostName.DomainName.
		/// </term>
		/// </item>
		/// <item>
		/// <term>ComputerNamePhysicalDnsHostname</term>
		/// <term>
		/// The DNS host name of the local computer. If the local computer is a node in a cluster, lpBuffer receives the DNS host name of the local computer, not
		...
		/// <term>
		/// The NetBIOS name of the local computer. If the local computer is a node in a cluster, lpBuffer receives the NetBIOS name of the local computer, not
		/// the name of the cluster virtual server.
		/// </term>
		/// </item>
		/// </list>
		...
		/// <para>
		/// The length of the name may be greater than MAX_COMPUTERNAME_LENGTH characters because DNS allows longer names. To ensure that this buffer is large
		/// enough, set this parameter to <c>NULL</c> and use the required buffer size returned in the lpnSize parameter.
		/// </para>
		/// </param>
		/// <param name="lpnSize">
		/// <para>
		/// On input, specifies the size of the buffer, in <c>TCHARs</c>. On output, receives the number of <c>TCHARs</c> copied to the destination buffer, not
		/// including the terminating <c>null</c> character.
		/// </para>
		/// <para>
		/// If the buffer is too small, the function fails and <c>GetLastError</c> returns ERROR_MORE_DATA. This parameter receives the size of the buffer
		/// required, including the terminating <c>null</c> character.
		/// </para>
		...
		/// <para>
		/// If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>. Possible error codes include ERROR_INVALID_FUNCTION.
		/// </para>
		/// </returns>
		...
		public static extern uint GetFirmwareEnvironmentVariable([In] string lpName, [In] string lpGuid, IntPtr pBuffer, uint nSize);
		...
		/// <para>
		/// To retrieve information about logical processors and related hardware, including processor groups, use the <c>GetLogicalProcessorInformationEx</c> function.
		/// </para>
		/// </summary>
		/// <param name="Buffer">
		/// A pointer to a buffer that receives an array of <c>SYSTEM_LOGICAL_PROCESSOR_INFORMATION</c> structures. If the function fails, the contents of this
		/// buffer are undefined.
		/// </param>
		/// <param name="ReturnLength">
		/// On input, specifies the length of the buffer pointed to by Buffer, in bytes. If the buffer is large enough to contain all of the data, this function
		/// succeeds and ReturnLength is set to the number of bytes returned. If the buffer is not large enough to contain all of the data, the function fails,
		/// <c>GetLastError</c> returns ERROR_INSUFFICIENT_BUFFER, and ReturnLength is set to the buffer length required to contain all of the data. If the
		/// function fails with an error other than ERROR_INSUFFICIENT_BUFFER, the value of ReturnLength is undefined.
		/// </param>
		/// <returns>
		/// <para>
		/// If the function succeeds, the return value is TRUE and at least one <c>SYSTEM_LOGICAL_PROCESSOR_INFORMATION</c> structure is written to the output buffer.
		/// </para>
		...
		/// <para>The type of relationship to retrieve. This parameter can be one of the following <c>LOGICAL_PROCESSOR_RELATIONSHIP</c> values.</para>
		/// <para>
		/// <list type="table">
		/// <listheader>
		/// <term>Value</term>
		...
		/// </param>
		/// <param name="Buffer">
		/// A pointer to a buffer that receives an array of <c>SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX</c> structures. If the function fails, the contents of
		/// this buffer are undefined.
		/// </param>
		/// <param name="ReturnedLength">
		/// On input, specifies the length of the buffer pointed to by Buffer, in bytes. If the buffer is large enough to contain all of the data, this function
		/// succeeds and ReturnedLength is set to the number of bytes returned. If the buffer is not large enough to contain all of the data, the function fails,
		/// <c>GetLastError</c> returns ERROR_INSUFFICIENT_BUFFER, and ReturnedLength is set to the buffer length required to contain all of the data. If the
		/// function fails with an error other than ERROR_INSUFFICIENT_BUFFER, the value of ReturnedLength is undefined.
		/// </param>
		/// <returns>
		/// <para>
		/// If the function succeeds, the return value is TRUE and at least one <c>SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX</c> structure is written to the output buffer.
		/// </para>
		/// <para>If the function fails, the return value is FALSE. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		// BOOL GetLogicalProcessorInformationEx( _In_ LOGICAL_PROCESSOR_RELATIONSHIP RelationshipType, _Out_opt_ PSYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX
		// Buffer, _Inout_ PDWORD ReturnedLength); https://msdn.microsoft.com/en-us/library/windows/desktop/dd405488(v=vs.85).aspx
		[DllImport(Lib.Kernel32, SetLastError = true, ExactSpelling = true)]
		...
		/// <summary>
		/// Retrieves information about the current system to an application running under WOW64. If the function is called from a 64-bit application, it is
		/// equivalent to the <c>GetSystemInfo</c> function.
		/// </summary>
		...
		/// <para>
		/// If the function fails, it returns <c>FALSE</c> and does not modify the TotalMemoryInKilobytes parameter. To get extended error information, use the
		/// <c>GetLastError</c> function. Common errors are listed in the following table.
		/// </para>
		/// <para>
		/// <list type="table">
		/// <listheader>
		...
		/// <summary>
		/// Retrieves the cycle time each processor in the specified processor group spent executing deferred procedure calls (DPCs) and interrupt service
		/// routines (ISRs) since the processor became active.
		/// </summary>
		/// <param name="Group">The number of the processor group for which to retrieve the cycle time.</param>
		/// <param name="Buffer">
		/// A pointer to a buffer to receive a SYSTEM_PROCESSOR_CYCLE_TIME_INFORMATION structure for each processor in the group. On output, the DWORD64
		/// <c>CycleTime</c> member of this structure is set to the cycle time for one processor.
		/// </param>
		/// <param name="ReturnedLength">
		/// The size of the buffer, in bytes. When the function returns, this parameter contains the number of bytes written to Buffer. If the buffer is too
		/// small for the data, the function fails with ERROR_INSUFFICIENT_BUFFER and sets the ReturnedLength parameter to the required buffer size.
		/// </param>
		/// <returns>
		/// <para>If the function succeeds, the return value is a nonzero value.</para>
		...
		/// </returns>
		// BOOL GetProcessorSystemCycleTime( _In_ USHORT Group, _Out_ PSYSTEM_PROCESSOR_CYCLE_TIME_INFORMATION Buffer, _Inout_ PDWORD ReturnedLength); https://msdn.microsoft.com/en-us/library/windows/desktop/dd405497(v=vs.85).aspx
		[DllImport(Lib.Kernel32, SetLastError = true, ExactSpelling = true)]
		...
		/// <summary>
		/// <para>
		/// Retrieves the product type for the operating system on the local computer, and maps the type to the product types supported by the specified
		/// operating system.
		/// </para>
		/// <para>
		/// To retrieve product type information on versions of Windows prior to the minimum supported operating systems specified in the Requirements section,
		/// use the <c>GetVersionEx</c> function. You can also use the <c>OperatingSystemSKU</c> property of the <c>Win32_OperatingSystem</c> WMI class.
		/// </para>
		/// </summary>
		/// <param name="dwOSMajorVersion">
		/// <para>The major version number of the operating system. The minimum value is 6.</para>
		/// <para>
		/// The combination of the dwOSMajorVersion, dwOSMinorVersion, dwSpMajorVersion, and dwSpMinorVersion parameters describes the maximum target operating
		/// system version for the application. For example, Windows Vista and Windows Server 2008 are version 6.0.0.0 and Windows 7 and Windows Server 2008 R2
		/// are version 6.1.0.0.
		/// </para>
		/// </param>
		...
		/// <para>
		/// The product type. This parameter cannot be <c>NULL</c>. If the specified operating system is less than the current operating system, this information
		/// is mapped to the types supported by the specified operating system. If the specified operating system is greater than the highest supported operating
		/// system, this information is mapped to the types supported by the current operating system.
		/// </para>
		...
		/// </returns>
		// BOOL WINAPI GetProductInfo( _In_ DWORD dwOSMajorVersion, _In_ DWORD dwOSMinorVersion, _In_ DWORD dwSpMajorVersion, _In_ DWORD dwSpMinorVersion, _Out_
		// PDWORD pdwReturnedProductType); https://msdn.microsoft.com/en-us/library/windows/desktop/ms724358(v=vs.85).aspx
		[DllImport(Lib.Kernel32, SetLastError = false, ExactSpelling = true)]
		...
		/// <para>
		/// This function is provided primarily for compatibility. Applications should store code in the Program Files folder and persistent data in the
		/// Application Data folder in the user's profile. For more information, see <c>ShGetFolderPath</c>.
		/// </para>
		/// </summary>
		/// <param name="lpBuffer">
		/// A pointer to the buffer to receive the path. This path does not end with a backslash unless the system directory is the root directory. For example,
		/// if the system directory is named Windows\System32 on drive C, the path of the system directory retrieved by this function is C:\Windows\System32.
		/// </param>
		...
		/// character. If the length is greater than the size of the buffer, the return value is the size of the buffer required to hold the path, including the
		...
		/// <para>
		/// For example, FACP is an ACPI provider, as described in the Signature field of the DESCRIPTION_HEADER structure in the ACPI specification (see
		/// http://www.acpi.info). Therefore, use 'PCAF' to specify the FACP table, as shown in the following example:
		/// </para>
		/// <para>For more information, see the Remarks section of the <c>EnumSystemFirmwareTables</c> function.</para>
		/// </param>
		/// <param name="pFirmwareTableBuffer">
		/// <para>
		/// A pointer to a buffer that receives the requested firmware table. If this parameter is <c>NULL</c>, the return value is the required buffer size.
		/// </para>
		/// <para>For more information on the contents of this buffer, see the Remarks section.</para>
		/// </param>
		/// <param name="BufferSize">The size of the pFirmwareTableBuffer buffer, in bytes.</param>
		...
		/// <para>If the function fails for any other reason, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		// UINT WINAPI GetSystemFirmwareTable( _In_ DWORD FirmwareTableProviderSignature, _In_ DWORD FirmwareTableID, _Out_ PVOID pFirmwareTableBuffer, _In_
		// DWORD BufferSize); https://msdn.microsoft.com/en-us/library/windows/desktop/ms724379(v=vs.85).aspx
		[DllImport(Lib.Kernel32, SetLastError = true, ExactSpelling = true)]
		...
		/// A pointer to a <c>SYSTEMTIME</c> structure to receive the current system date and time. The lpSystemTime parameter must not be <c>NULL</c>. Using
		/// <c>NULL</c> will result in an access violation.
		/// </param>
		...
		/// A pointer to a variable that the function sets to the number of lpTimeIncrement100-nanosecond units added to the time-of-day clock for every period
		/// of time which actually passes as counted by the system. This value only has meaning if lpTimeAdjustmentDisabled is <c>FALSE</c>.
		/// </param>
		/// <param name="lpTimeIncrement">
		/// A pointer to a variable that the function sets to the interval in 100-nanosecond units at which the system will add lpTimeAdjustment to the
		/// time-of-day clock. This value only has meaning if lpTimeAdjustmentDisabled is <c>FALSE</c>.
		/// </param>
		...
		/// <para>
		/// A value of <c>TRUE</c> indicates that periodic time adjustment is disabled, and the system time-of-day clock advances at the normal rate. In this
		/// mode, the system may adjust the time of day using its own internal time synchronization mechanisms. These internal time synchronization mechanisms
		/// may cause the time-of-day clock to change during the normal course of the system operation, which can include noticeable jumps in time as deemed
		/// necessary by the system.
		/// </para>
		/// <para>
		/// A value of <c>FALSE</c> indicates that periodic time adjustment is being used to adjust the time-of-day clock. For each lpTimeIncrement period of
		/// time that actually passes, lpTimeAdjustment will be added to the time of day. If the lpTimeAdjustment value is smaller than lpTimeIncrement, the
		/// system time-of-day clock will advance at a rate slower than normal. If the lpTimeAdjustment value is larger than lpTimeIncrement, the time-of-day
		/// clock will advance at a rate faster than normal. If lpTimeAdjustment equals lpTimeIncrement, the time-of-day clock will advance at its normal speed.
		/// The lpTimeAdjustment value can be set by calling <c>SetSystemTimeAdjustment</c>. The lpTimeIncrement value is fixed by the system upon start, and
		/// does not change during system operation. In this mode, the system will not interfere with the time adjustment scheme, and will not attempt to
		/// synchronize time of day on its own via other techniques.
		/// </para>
		/// </param>
		/// <returns>
		/// <para>If the function succeeds, the return value is nonzero.</para>
		/// <para>If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		// BOOL WINAPI GetSystemTimeAdjustment( _Out_ PDWORD lpTimeAdjustment, _Out_ PDWORD lpTimeIncrement, _Out_ PBOOL lpTimeAdjustmentDisabled); https://msdn.microsoft.com/en-us/library/windows/desktop/ms724394(v=vs.85).aspx
		[DllImport(Lib.Kernel32, SetLastError = true, ExactSpelling = true)]
		...
		/// <param name="lpSystemTimeAsFileTime">A pointer to a <c>FILETIME</c> structure to receive the current system date and time in UTC format.</param>
		/// <returns>This function does not return a value.</returns>
		...
		/// <summary>
		/// The <c>GetSystemTimePreciseAsFileTime</c> function retrieves the current system date and time with the highest possible level of precision (&lt;1us).
		...
		/// </summary>
		/// <param name="lpBuffer">
		/// A pointer to the buffer to receive the path. This path does not end with a backslash unless the Windows directory is the root directory. For example,
		/// if the Windows directory is named Windows on drive C, the path of the Windows directory retrieved by this function is C:\Windows. If the system was
		/// installed in the root directory of drive C, the path retrieved is C:\.
		/// </param>
		/// <param name="uSize">The maximum size of the buffer specified by the lpBuffer parameter, in <c>TCHARs</c>.</param>
		/// <returns>
		/// <para>
		/// If the function succeeds, the return value is the length of the string copied to the buffer, in <c>TCHARs</c>, not including the terminating null character.
		/// </para>
		...
		/// <para>
		/// With the release of Windows 8.1, the behavior of the <c>GetVersion</c> API has changed in the value it will return for the operating system version.
		/// The value returned by the <c>GetVersion</c> function now depends on how the application is manifested.
		/// </para>
		/// <para>
		/// Applications not manifested for Windows 8.1 or Windows 10 will return the Windows 8 OS version value (6.2). Once an application is manifested for a
		/// given operating system version, <c>GetVersion</c> will always return the version that the application is manifested for in future releases. To
		/// manifest your applications for Windows 8.1 or Windows 10, refer to Targeting your application for Windows.
		/// </para>
		/// </summary>
		/// <returns>
		/// <para>
		...
		/// </para>
		/// <para>
		/// For all platforms, the low-order word contains the version number of the operating system. The low-order byte of this word specifies the major
		/// version number, in hexadecimal notation. The high-order byte specifies the minor version (revision) number, in hexadecimal notation. The high-order
		/// bit is zero, the next 7 bits represent the build number, and the low-order byte is 5.
		/// </para>
		/// </returns>
		...
		/// <para>
		/// With the release of Windows 8.1, the behavior of the <c>GetVersionEx</c> API has changed in the value it will return for the operating system
		/// version. The value returned by the <c>GetVersionEx</c> function now depends on how the application is manifested.
		/// </para>
		/// <para>
		/// Applications not manifested for Windows 8.1 or Windows 10 will return the Windows 8 OS version value (6.2). Once an application is manifested for a
		/// given operating system version, <c>GetVersionEx</c> will always return the version that the application is manifested for in future releases. To
		/// manifest your applications for Windows 8.1 or Windows 10, refer to Targeting your application for Windows.
		/// </para>
		/// </summary>
		...
		/// <para>
		/// Before calling the <c>GetVersionEx</c> function, set the <c>dwOSVersionInfoSize</c> member of the structure as appropriate to indicate which data
		/// structure is being passed to this function.
		/// </para>
		/// </param>
		/// <returns>
		/// <para>If the function succeeds, the return value is a nonzero value.</para>
		/// <para>
		/// If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>. The function fails if you specify an
		/// invalid value for the <c>dwOSVersionInfoSize</c> member of the <c>OSVERSIONINFO</c> or <c>OSVERSIONINFOEX</c> structure.
		/// </para>
		/// </returns>
		...
		/// <para>
		/// This function is provided primarily for compatibility with legacy applications. New applications should store code in the Program Files folder and
		/// persistent data in the Application Data folder in the user's profile. For more information, see <c>ShGetFolderPath</c>.
		/// </para>
		/// </summary>
		/// <param name="lpBuffer">
		/// A pointer to a buffer that receives the path. This path does not end with a backslash unless the Windows directory is the root directory. For
		/// example, if the Windows directory is named Windows on drive C, the path of the Windows directory retrieved by this function is C:\Windows. If the
		/// system was installed in the root directory of drive C, the path retrieved is C:\.
		/// </param>
		/// <param name="uSize">The maximum size of the buffer specified by the lpBuffer parameter, in <c>TCHARs</c>. This value should be set to <c>MAX_PATH</c>.</param>
		/// <returns>
		/// <para>
		/// If the function succeeds, the return value is the length of the string copied to the buffer, in <c>TCHARs</c>, not including the terminating null character.
		/// </para>
		...
		/// </summary>
		/// <param name="lpBuffer">
		/// A pointer to a <c>MEMORYSTATUS</c> structure. The <c>GlobalMemoryStatus</c> function stores information about current memory availability into this structure.
		/// </param>
		/// <returns>This function does not return a value.</returns>
		...
		/// <summary>
		/// Installs the certificate information specified in the resource file, which is linked into the ELAM driver at build time. This API is used by
		/// anti-malware vendors to launch the anti-malware software's user-mode service as protected. For more information, see Protecting Anti-Malware Services.
		/// </summary>
		/// <param name="ELAMFile">
		/// A handle to an ELAM driver file which contains the resource file with the certificate information. The handle to the ELAM driver file must be opened
		/// for read access only and must not be shared for write access.
		/// </param>
		/// <returns>
		...
		/// <summary>
		/// <para>
		/// Sets a new NetBIOS name for the local computer. The name is stored in the registry and the name change takes effect the next time the user restarts
		/// the computer.
		/// </para>
		/// <para>
		/// If the local computer is a node in a cluster, <c>SetComputerName</c> sets NetBIOS name of the local computer, not that of the cluster virtual server.
		/// </para>
		...
		/// <para>
		/// The standard character set includes letters, numbers, and the following symbols: ! @ # $ % ^ &amp; ' ) ( . - _ { } ~ . If this parameter contains one
		/// or more characters that are outside the standard character set, <c>SetComputerName</c> returns ERROR_INVALID_PARAMETER.
		/// </para>
		/// </param>
		/// <returns>
		/// <para>If the function succeeds, the return value is a nonzero value.</para>
		...
		[return: MarshalAs(UnmanagedType.Bool)]
		public static extern bool SetComputerName([In] string lpComputerName);

		/// <summary>
		/// Sets a new NetBIOS or DNS name for the local computer. Name changes made by <c>SetComputerNameEx</c> do not take effect until the user restarts the computer.
		/// </summary>
		/// <param name="NameType">
		/// <para>The type of name to be set. This parameter can be one of the following values from the <c>COMPUTER_NAME_FORMAT</c> enumeration type.</para>
		/// <para>
		/// <list type="table">
		/// <listheader>
		/// <term>Value</term>
		...
		/// <term>ComputerNamePhysicalDnsHostname</term>
		/// <term>
		/// Sets the NetBIOS and the Computer Name (the first label of the full DNS name) to the name specified in lpBuffer. If the name exceeds
		/// MAX_COMPUTERNAME_LENGTH characters, the NetBIOS name is truncated to MAX_COMPUTERNAME_LENGTH characters, not including the terminating null character.
		/// </term>
		/// </item>
		/// <item>
		/// <term>ComputerNamePhysicalNetBIOS</term>
		/// <term>
		/// Sets the NetBIOS name to the name specified in lpBuffer. The name cannot exceed MAX_COMPUTERNAME_LENGTH characters, not including the terminating
		/// null character. Warning: Using this option to set the NetBIOS name breaks the convention of interdependent NetBIOS and DNS names. Applications that
		/// use the DnsHostnameToComputerName function to derive the NetBIOS name from the first label of the DNS name will fail if this convention is broken.
		/// </term>
		/// </item>
		/// </list>
		/// </para>
		...
		[return: MarshalAs(UnmanagedType.Bool)]
		public static extern bool SetComputerNameEx(COMPUTER_NAME_FORMAT NameType, [In] string lpBuffer);
		...
		/// <param name="nSize">The size of the pBuffer buffer, in bytes. If this parameter is zero, the firmware environment variable is deleted.</param>
		/// <returns>
		/// <para>If the function succeeds, the return value is a nonzero value.</para>
		/// <para>
		/// If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>. Possible error codes include ERROR_INVALID_FUNCTION.
		...
		[return: MarshalAs(UnmanagedType.Bool)]
		public static extern bool SetFirmwareEnvironmentVariable([In] string lpName, [In] string lpGuid, [In] IntPtr pBuffer, uint nSize);
		...
		/// <summary>
		/// Enables or disables periodic time adjustments to the system's time-of-day clock. When enabled, such time adjustments can be used to synchronize the
		/// time of day with some other source of time information.
		/// </summary>
		/// <param name="dwTimeAdjustment">
		/// This value represents the number of 100-nanosecond units added to the system time-of-day for each lpTimeIncrement period of time that actually
		/// passes. Call <c>GetSystemTimeAdjustment</c> to obtain the lpTimeIncrement value. See remarks.
		/// </param>
		...
		/// <para>
		/// A value of <c>TRUE</c> specifies that periodic time adjustment is to be disabled. When disabled, the value of dwTimeAdjustment is ignored, and the
		/// system may adjust the time of day using its own internal time synchronization mechanisms. These internal time synchronization mechanisms may cause
		/// the time-of-day clock to change during the normal course of the system operation, which can include noticeable jumps in time as deemed necessary by
		/// the system.
		/// </para>
		/// <para>
		/// A value of <c>FALSE</c> specifies that periodic time adjustment is to be enabled, and will be used to adjust the time-of-day clock. The system will
		...
		/// <para>
		/// If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>. One way the function can fail is if the
		/// caller does not possess the SE_SYSTEMTIME_NAME privilege.
		/// </para>
		/// </returns>
		// BOOL WINAPI SetSystemTimeAdjustment( _In_ DWORD dwTimeAdjustment, _In_ BOOL bTimeAdjustmentDisabled); https://msdn.microsoft.com/en-us/library/windows/desktop/ms724943(v=vs.85).aspx
		[DllImport(Lib.Kernel32, SetLastError = true, ExactSpelling = true)]
		...
		/// <summary>
		/// Compares a set of operating system version requirements to the corresponding values for the currently running version of the system. This function is
		/// subject to manifest-based behavior. For more information, see the Remarks section.
		/// </summary>
		/// <param name="lpVersionInfo">
		/// <para>
		/// A pointer to an <c>OSVERSIONINFOEX</c> structure containing the operating system version requirements to compare. The dwTypeMask parameter indicates
		/// the members of this structure that contain information to compare.
		/// </para>
		/// <para>
		/// You must set the <c>dwOSVersionInfoSize</c> member of this structure to . You must also specify valid data for the members indicated by dwTypeMask.
		/// The function ignores structure members for which the corresponding dwTypeMask bit is not set.
		/// </para>
		/// </param>
		/// <param name="dwTypeMask">
		/// <para>
		/// A mask that indicates the members of the <c>OSVERSIONINFOEX</c> structure to be tested. This parameter can be one or more of the following values.
		/// </para>
		/// <para>
		/// <list type="table">
		/// <listheader>
		...
		/// <term>dwMajorVersionIf you are testing the major version, you must also test the minor version and the service pack major and minor versions.</term>
		/// </item>
		/// <item>
		...
		/// The type of comparison to be used for each <c>lpVersionInfo</c> member being compared. To build this value, call the <c>VerSetConditionMask</c>
		/// function or the <c>VER_SET_CONDITION</c> macro once for each <c>OSVERSIONINFOEX</c> member being compared.
		/// </param>
		/// <returns>
		...
		/// <summary>
		/// Sets the bits of a 64-bit value to indicate the comparison operator to use for a specified operating system version attribute. This function is used
		/// to build the dwlConditionMask parameter of the <c>VerifyVersionInfo</c> function.
		/// </summary>
		/// <param name="dwlConditionMask">
		/// <para>
		/// A value to be passed as the dwlConditionMask parameter of the <c>VerifyVersionInfo</c> function. The function stores the comparison information in
		/// the bits of this variable.
		/// </para>
		/// <para>
		/// Before the first call to <c>VerSetCondition</c>, initialize this variable to zero. For subsequent calls, pass in the variable used in the previous call.
		/// </para>
		/// </param>
		/// <param name="dwTypeBitMask">
		/// <para>
		/// A mask that indicates the member of the <c>OSVERSIONINFOEX</c> structure whose comparison operator is being set. This value corresponds to one of the
		/// bits specified in the dwTypeMask parameter for the <c>VerifyVersionInfo</c> function. This parameter can be one of the following values.
		/// </para>
		/// <para>
		/// <list type="table">
		/// <listheader>
		...
		/// <para>
		/// The operator to be used for the comparison. The <c>VerifyVersionInfo</c> function uses this operator to compare a specified attribute value to the
		...
		/// <summary>
		/// Contains information about the current state of both physical and virtual memory. The <c>GlobalMemoryStatus</c> function stores information in a
		/// <c>MEMORYSTATUS</c> structure.
		/// </summary>
		// typedef struct _MEMORYSTATUS { DWORD dwLength; DWORD dwMemoryLoad; SIZE_T dwTotalPhys; SIZE_T dwAvailPhys; SIZE_T dwTotalPageFile; SIZE_T
		// dwAvailPageFile; SIZE_T dwTotalVirtual; SIZE_T dwAvailVirtual;} MEMORYSTATUS, *LPMEMORYSTATUS; https://msdn.microsoft.com/en-us/library/windows/desktop/aa366772(v=vs.85).aspx
		[PInvokeData("WinBase.h", MSDNShortId = "aa366772")]
		[StructLayout(LayoutKind.Sequential)]
		public struct MEMORYSTATUS
		{
			/// <summary>
			/// The size of the <c>MEMORYSTATUS</c> data structure, in bytes. You do not need to set this member before calling the <c>GlobalMemoryStatus</c>
			/// function; the function sets it.
			/// </summary>
			public uint dwLength;
			/// <summary>
			/// A number between 0 and 100 that specifies the approximate percentage of physical memory that is in use (0 indicates no memory use and 100
			...
			/// <summary>
			/// The amount of physical memory currently available, in bytes. This is the amount of physical memory that can be immediately reused without having
			...
			/// <summary>
			/// The maximum amount of memory the current process can commit, in bytes. This value should be smaller than the system-wide available commit. To
			/// calculate this value, call <c>GetPerformanceInfo</c> and subtract the value of <c>CommitTotal</c> from <c>CommitLimit</c>.
			/// </summary>
			public SizeT dwAvailPageFile;
			/// <summary>
			/// The size of the user-mode portion of the virtual address space of the calling process, in bytes. This value depends on the type of process, the
			/// type of processor, and the configuration of the operating system. For example, this value is approximately 2 GB for most 32-bit processes on an
			/// x86 processor and approximately 3 GB for 32-bit processes that are large address aware running on a system with 4 GT RAM Tuning enabled.
			/// </summary>
		...
		/// <summary>
		/// Contains information about the current state of both physical and virtual memory, including extended memory. The <c>GlobalMemoryStatusEx</c> function
		...
		// DWORDLONG ullAvailPageFile; DWORDLONG ullTotalVirtual; DWORDLONG ullAvailVirtual; DWORDLONG ullAvailExtendedVirtual;} MEMORYSTATUSEX,
		...
		public struct MEMORYSTATUSEX
		{
			...
			/// <summary>
			/// The current committed memory limit for the system or the current process, whichever is smaller, in bytes. To get the system-wide committed memory
			/// limit, call <c>GetPerformanceInfo</c>.
			/// </summary>
			public ulong ullTotalPageFile;
			/// <summary>
			/// The maximum amount of memory the current process can commit, in bytes. This value is equal to or smaller than the system-wide available commit
			/// value. To calculate the system-wide available commit value, call <c>GetPerformanceInfo</c> and subtract the value of <c>CommitTotal</c> from the
			/// value of <c>CommitLimit</c>.
			/// </summary>
			public ulong ullAvailPageFile;
			/// <summary>
			/// The size of the user-mode portion of the virtual address space of the calling process, in bytes. This value depends on the type of process, the
			/// type of processor, and the configuration of the operating system. For example, this value is approximately 2 GB for most 32-bit processes on an
			/// x86 processor and approximately 3 GB for 32-bit processes that are large address aware running on a system with 4-gigabyte tuning enabled.
			/// </summary>
		...
		/// <summary>
		/// Contains operating system version information. The information includes major and minor version numbers, a build number, a platform identifier, and
		/// information about product suites and the latest Service Pack installed on the system. This structure is used with the <c>GetVersionEx</c> and
		/// <c>VerifyVersionInfo</c> functions.
		/// </summary>
		// typedef struct _OSVERSIONINFOEX { DWORD dwOSVersionInfoSize; DWORD dwMajorVersion; DWORD dwMinorVersion; DWORD dwBuildNumber; DWORD dwPlatformId;
		// TCHAR szCSDVersion[128]; WORD wServicePackMajor; WORD wServicePackMinor; WORD wSuiteMask; BYTE wProductType; BYTE wReserved;} OSVERSIONINFOEX,
		...
		public struct OSVERSIONINFOEX
		{
			...
			/// <summary>
			/// A null-terminated string, such as "Service Pack 3", that indicates the latest Service Pack installed on the system. If no Service Pack has been
			/// installed, the string is empty.
			/// </summary>
			[MarshalAs(UnmanagedType.ByValTStr, SizeConst = 128)]
			public string szCSDVersion;
			/// <summary>
			/// The major version number of the latest Service Pack installed on the system. For example, for Service Pack 3, the major version number is 3. If
			...
			/// <term>
			/// Windows Server 2008 Enterprise, Windows Server 2003, Enterprise Edition, or Windows 2000 Advanced Server is installed. Refer to the Remarks
			...
			/// <term>
			/// Microsoft Small Business Server was once installed on the system, but may have been upgraded to another version of Windows. Refer to the Remarks
			/// section for more information about this bit flag.
			/// </term>
			/// </item>
			/// <item>
			...
			/// <term>
			/// Microsoft Small Business Server is installed with the restrictive client license in force. Refer to the Remarks section for more information
			/// about this bit flag.
			/// </term>
			/// </item>
			/// <item>
			...
			/// <term>
			/// Terminal Services is installed. This value is always set.If VER_SUITE_TERMINAL is set but VER_SUITE_SINGLEUSERTS is not set, the system is
			...
			/// <term>
			/// The system is a domain controller and the operating system is Windows Server 2012 , Windows Server 2008 R2, Windows Server 2008, Windows Server
			...
			/// <term>
			/// The operating system is Windows Server 2012, Windows Server 2008 R2, Windows Server 2008, Windows Server 2003, or Windows 2000 Server.Note that a
			/// server that is also a domain controller is reported as VER_NT_DOMAIN_CONTROLLER, not VER_NT_SERVER.
			/// </term>
			/// </item>
			/// <item>
			/// <term>VER_NT_WORKSTATION0x0000001</term>
			/// <term>The operating system is Windows 8, Windows 7, Windows Vista, Windows XP Professional, Windows XP Home Edition, or Windows 2000 Professional.</term>
			/// </item>
			/// </list>
			/// </para>
			/// </summary>
		...
		/// <summary>
		/// Contains information about the current computer system. This includes the architecture and type of the processor, the number of processors in the
		/// system, the page size, and other such information.
		/// </summary>
		// typedef struct _SYSTEM_INFO { union { DWORD dwOemId; struct { WORD wProcessorArchitecture; WORD wReserved; }; }; DWORD dwPageSize; LPVOID
		// lpMinimumApplicationAddress; LPVOID lpMaximumApplicationAddress; DWORD_PTR dwActiveProcessorMask; DWORD dwNumberOfProcessors; DWORD dwProcessorType;
		// DWORD dwAllocationGranularity; WORD wProcessorLevel; WORD wProcessorRevision;} SYSTEM_INFO; https://msdn.microsoft.com/en-us/library/windows/desktop/ms724958(v=vs.85).aspx
		...
		public struct SYSTEM_INFO
		{
			...
			/// <summary>The page size and the granularity of page protection and commitment. This is the page size used by the <c>VirtualAlloc</c> function.</summary>
			...
			/// <summary>The granularity for the starting address at which virtual memory can be allocated. For more information, see <c>VirtualAlloc</c>.</summary>
			...
			/// <c>IsProcessorFeaturePresent</c> function.
			/// </para>
			...
			/// <term>
			/// The high byte is the model and the low byte is the stepping. For example, if the value is xxyy, the model number and stepping can be displayed as
			...
			/// <term>
			/// A value of the form xxyz. If xx is equal to 0xFF, y - 0xA is the model number, and z is the stepping identifier.If xx is not equal to 0xFF, xx +
			/// &amp;#39;A&amp;#39; is the stepping letter and yz is the minor stepping.
			/// </term>
			/// </item>
			/// <item>
			/// <term>ARM</term>
		...
		/// <summary>
		/// Describes the relationship between the specified processor set. This structure is used with the <c>GetLogicalProcessorInformation</c> function.
		/// </summary>
		// typedef struct _SYSTEM_LOGICAL_PROCESSOR_INFORMATION { ULONG_PTR ProcessorMask; LOGICAL_PROCESSOR_RELATIONSHIP Relationship; union { struct { BYTE
		// Flags; } ProcessorCore; struct { DWORD NodeNumber; } NumaNode; CACHE_DESCRIPTOR Cache; ULONGLONG Reserved[2]; };}
		...
		public struct SYSTEM_LOGICAL_PROCESSOR_INFORMATION
		{
			/// <summary>
			/// <para>
			/// The processor mask identifying the processors described by this structure. A processor mask is a bit vector in which each set bit represents an
			/// active processor in the relationship. At least one bit will be set.
			/// </para>
			...
			/// <summary>
			/// <para>
			/// The relationship between the processors identified by the value of the <c>ProcessorMask</c> member. This member can be one of the following
			/// <c>LOGICAL_PROCESSOR_RELATIONSHIP</c> values.
			/// </para>
			/// <para>
			/// <list type="table">
			/// <listheader>
			...
			/// <term>RelationCache2</term>
			/// <term>
			/// The specified logical processors share a cache. The Cache member contains additional information.Windows Server 2003: This value is not supported
			...
			/// <term>RelationProcessorPackage3</term>
			/// <term>
			/// The specified logical processors share a physical package. There is no additional information available.Windows Server 2003 and Windows XP
			/// Professional x64 Edition: This value is not supported until Windows Server 2003 with SP1 and Windows XP with SP3.
			/// </term>
			/// </item>
			/// </list>
			/// </para>
			...
			public struct ProcessorRelationUnion
			{
				...
				/// <summary>
				/// <para>
				/// A <c>CACHE_DESCRIPTOR</c> structure that identifies the characteristics of a particular cache. There is one record returned for each cache
				/// reported. Some or all caches may not be reported, depending on the mechanism used by the processor to identify its caches. Therefore, do not
				/// assume the absence of any particular caches. Caches are not necessarily shared among logical processors.
				/// </para>
				/// <para>This structure contains valid data only if the <c>Relationship</c> member is RelationCache.</para>
				/// <para><c>Windows Server 2003:</c> This member is not supported until Windows Server 2003 with SP1 and Windows XP Professional x64 Edition.</para>
				/// </summary>
				[FieldOffset(0)] public CACHE_DESCRIPTOR Cache;
				/// <summary>Reserved. Do not use.</summary>
				[FieldOffset(0), MarshalAs(UnmanagedType.ByValArray, SizeConst = 2)] private ulong[] Reserved;
			}
		}
