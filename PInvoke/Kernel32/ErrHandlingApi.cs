...
namespace Vanara.PInvoke
{
	public static partial class Kernel32
	{
		...
		/// <summary>
		/// An application-defined function that passes unhandled exceptions to the debugger, if the process is being debugged. Otherwise, it optionally displays
		/// an Application Error message box and causes the exception handler to be executed. This function can be called only from within the filter expression
		...
		/// This pointer is the return value of a call to the GetExceptionInformation function.
		/// </param>
		/// <returns>
		...
		/// <term>
		/// If the SEM_NOGPFAULTERRORBOX flag was specified in a previous call to SetErrorMode, no Application Error message box is displayed. The function
		/// returns control to the exception handler, which is free to take any appropriate action.
		/// </term>
		/// </item>
		/// </list>
		/// </returns>
		public delegate EXCEPTION_FLAG PTOP_LEVEL_EXCEPTION_FILTER([In] ref EXCEPTION_POINTERS ExceptionInfo);

		/// <summary>
		/// An application-defined function that serves as a vectored exception handler. Specify this address when calling the AddVectoredExceptionHandler function.
		/// </summary>
		/// <param name="ExceptionInfo">A pointer to an EXCEPTION_POINTERS structure that receives the exception record.</param>
		/// <returns>
		/// To return control to the point at which the exception occurred, return EXCEPTION_CONTINUE_EXECUTION (0xffffffff). To continue the handler search,
		/// return EXCEPTION_CONTINUE_SEARCH (0x0).
		/// </returns>
		...
		public enum EXCEPTION_FLAG : uint
		{
			...
			/// <summary>
			/// Proceed with normal execution of UnhandledExceptionFilter. That means obeying the SetErrorMode flags, or invoking the Application Error pop-up
			/// message box.
			/// </summary>
			EXCEPTION_CONTINUE_SEARCH = 0x0000,
			/// <summary>Indicates a noncontinuable exception.</summary>
			EXCEPTION_NONCONTINUABLE = 0x0001,
			/// <summary>Return from UnhandledExceptionFilter and execute the associated exception handler. This usually results in process termination.</summary>
			...
			/// <summary>
			/// Return from UnhandledExceptionFilter and continue execution from the point of the exception. Note that the filter function is free to modify the
			/// continuation state by modifying the exception information supplied through its LPEXCEPTION_POINTERS parameter.
			/// </summary>
		...
		public enum FAIL_FAST_FLAGS : uint
		{
			...
			NONE = 0,
			/// <summary>
			/// Causes RaiseFailFastException to set the ExceptionAddress of EXCEPTION_RECORD to the return address of this function (the next instruction in the
			/// caller after the call to RaiseFailFastException). This function will set the exception address only if ExceptionAddress is not NULL.
			/// </summary>
		...
		/// The order in which the handler should be called. If the parameter is nonzero, the handler is the first handler to be called. If the parameter is
		/// zero, the handler is the last handler to be called.
		/// </param>
		...
		/// <summary>
		/// Displays a message box and terminates the application when the message box is closed. If the system is running with a debug version of Kernel32.dll,
		/// the message box gives the user the opportunity to terminate the application or to cancel the message box and return to the application that called <c>FatalAppExit</c>.
		/// </summary>
		...
		public static extern void FatalAppExit(uint uAction, [In] string lpMessageText);
		...
		/// <term>
		/// The function allocates a buffer large enough to hold the formatted message, and places a pointer to the allocated buffer at the address specified by
		/// lpBuffer. The lpBuffer parameter is a pointer to an LPTSTR; you must cast the pointer to an LPTSTR (for example, ). The nSize parameter specifies the
		/// minimum number of TCHARs to allocate for an output message buffer. The caller should use the LocalFree function to free the buffer when it is no
		/// longer needed.If the length of the formatted message exceeds 128K bytes, then FormatMessage will fail and a subsequent call to GetLastError will
		/// return ERROR_MORE_DATA.In previous versions of Windows, this value was not available for use when compiling Windows Store apps. As of Windows 10 this
		/// value can be used. Windows Server 2003 and Windows XP: If the length of the formatted message exceeds 128K bytes, then FormatMessage will not
		/// automatically fail with an error of ERROR_MORE_DATA.Windows 10: LocalAlloc() has different options: LMEM_FIXED, and LMEM_MOVABLE. FormatMessage()
		/// uses LMEM_FIXED, so HeapFree can be used. If LMEM_MOVABLE is used, HeapFree cannot be used.
		/// </term>
		/// </item>
		/// <item>
		/// <term>FORMAT_MESSAGE_ARGUMENT_ARRAY0x00002000</term>
		/// <term>
		/// The Arguments parameter is not a va_list structure, but is a pointer to an array of values that represent the arguments.This flag cannot be used with
		/// 64-bit integer values. If you are using a 64-bit integer, you must use the va_list structure.
		/// </term>
		/// </item>
		/// <item>
		...
		/// <term>
		/// The lpSource parameter is a module handle containing the message-table resource(s) to search. If this lpSource handle is NULL, the current
		/// process&amp;#39;s application image file will be searched. This flag cannot be used with FORMAT_MESSAGE_FROM_STRING.If the module has no message
		/// table resource, the function fails with ERROR_RESOURCE_TYPE_NOT_FOUND.
		/// </term>
		/// </item>
		/// <item>
		...
		/// <term>
		/// The lpSource parameter is a pointer to a null-terminated string that contains a message definition. The message definition may contain insert
		/// sequences, just as the message text in a message table resource may. This flag cannot be used with FORMAT_MESSAGE_FROM_HMODULE or FORMAT_MESSAGE_FROM_SYSTEM.
		/// </term>
		/// </item>
		/// <item>
		...
		/// <term>
		/// The function should search the system message-table resource(s) for the requested message. If this flag is specified with
		/// FORMAT_MESSAGE_FROM_HMODULE, the function searches the system message table if the message is not found in the module specified by lpSource. This
		/// flag cannot be used with FORMAT_MESSAGE_FROM_STRING.If this flag is specified, an application can pass the result of the GetLastError function to
		...
		/// <term>
		/// Insert sequences in the message definition are to be ignored and passed through to the output buffer unchanged. This flag is useful for fetching a
		/// message for later formatting. If this flag is set, the Arguments parameter is ignored.
		/// </term>
		/// </item>
		/// </list>
		/// </para>
		...
		/// <item>
		/// <term>0</term>
		/// <term>There are no output line width restrictions. The function stores line breaks that are in the message definition text into the output buffer.</term>
		/// </item>
		/// <item>
		/// <term>FORMAT_MESSAGE_MAX_WIDTH_MASK0x000000FF</term>
		/// <term>
		/// The function ignores regular line breaks in the message definition text. The function stores hard-coded line breaks in the message definition text
		...
		/// </para>
		/// <para>
		/// If the low-order byte is a nonzero value other than <c>FORMAT_MESSAGE_MAX_WIDTH_MASK</c>, it specifies the maximum number of characters in an output
		/// line. The function ignores regular line breaks in the message definition text. The function never splits a string delimited by white space across a
		/// line break. The function stores hard-coded line breaks in the message definition text into the output buffer. Hard-coded line breaks are coded with
		...
		/// <para>
		/// If you pass a specific <c>LANGID</c> in this parameter, <c>FormatMessage</c> will return a message for that <c>LANGID</c> only. If the function
		/// cannot find a message for that <c>LANGID</c>, it sets Last-Error to <c>ERROR_RESOURCE_LANG_NOT_FOUND</c>. If you pass in zero, <c>FormatMessage</c>
		/// looks for a message for <c>LANGIDs</c> in the following order:
		/// </para>
		/// <para>
		/// If <c>FormatMessage</c> does not locate a message for any of the preceding <c>LANGIDs</c>, it returns any language message string that is present. If
		/// that fails, it returns <c>ERROR_RESOURCE_LANG_NOT_FOUND</c>.
		/// </para>
		/// </param>
		/// <param name="lpBuffer">
		/// <para>
		/// A pointer to a buffer that receives the null-terminated string that specifies the formatted message. If dwFlags includes
		/// <c>FORMAT_MESSAGE_ALLOCATE_BUFFER</c>, the function allocates a buffer using the <c>LocalAlloc</c> function, and places the pointer to the buffer at
		/// the address specified in lpBuffer.
		/// </para>
		/// <para>This buffer cannot be larger than 64K bytes.</para>
		/// </param>
		/// <param name="nSize">
		/// <para>
		/// If the <c>FORMAT_MESSAGE_ALLOCATE_BUFFER</c> flag is not set, this parameter specifies the size of the output buffer, in <c>TCHARs</c>. If
		/// <c>FORMAT_MESSAGE_ALLOCATE_BUFFER</c> is set, this parameter specifies the minimum number of <c>TCHARs</c> to allocate for an output buffer.
		/// </para>
		...
		/// array; a %2 indicates the second argument; and so on.
		/// </para>
		/// <para>
		...
		/// </para>
		/// <para>
		/// By default, the Arguments parameter is of type <c>va_list*</c>, which is a language- and implementation-specific data type for describing a variable
		/// number of arguments. The state of the <c>va_list</c> argument is undefined upon return from the function. To use the <c>va_list</c> again, destroy
		/// the variable argument list pointer using <c>va_end</c> and reinitialize it with <c>va_start</c>.
		/// </para>
		/// <para>
		/// If you do not have a pointer of type <c>va_list*</c>, then specify the <c>FORMAT_MESSAGE_ARGUMENT_ARRAY</c> flag and pass a pointer to an array of
		/// <c>DWORD_PTR</c> values; those values are input to the message formatted as the insert values. Each insert must have a corresponding element in the array.
		/// </para>
		/// </param>
		/// <returns>
		/// <para>If the function succeeds, the return value is the number of <c>TCHARs</c> stored in the output buffer, excluding the terminating null character.</para>
		...
		/// <term>The system does not display the critical-error-handler message box. Instead, the system sends the error to the calling process.</term>
		/// </item>
		/// <item>
		/// <term>SEM_NOALIGNMENTFAULTEXCEPT0x0004</term>
		/// <term>
		/// The system automatically fixes memory alignment faults and makes them invisible to the application. It does this for the calling process and any
		...
		/// <term>The system does not display a message box when it fails to find a file. Instead, the error is returned to the calling process.</term>
		/// </item>
		/// </list>
		/// </para>
		/// </returns>
		...
		/// <para>
		/// An application-defined exception code of the exception being raised. The filter expression and exception-handler block of an exception handler can
		/// use the <c>GetExceptionCode</c> function to retrieve this value.
		/// </para>
		/// <para>
		/// Note that the system will clear bit 28 of dwExceptionCode before displaying a message This bit is a reserved exception bit, used by the system for
		/// its own purposes.
		/// </para>
		/// </param>
		...
		/// The number of arguments in the lpArguments array. This value must not exceed EXCEPTION_MAXIMUM_PARAMETERS. This parameter is ignored if lpArguments
		/// is <c>NULL</c>.
		/// </param>
		/// <param name="lpArguments">
		/// An array of arguments. This parameter can be <c>NULL</c>. These arguments can contain any application-defined data that needs to be passed to the
		...
		/// <summary>
		/// Raises an exception that bypasses all exception handlers (frame or vector based). Raising this exception terminates the application and invokes
		/// Windows Error Reporting, if Windows Error Reporting is enabled.
		/// </summary>
		/// <param name="pExceptionRecord">
		/// <para>
		/// A pointer to an <c>EXCEPTION_RECORD</c> structure that contains the exception information. You must specify the <c>ExceptionAddress</c> and
		/// <c>ExceptionCode</c> members.
		/// </para>
		/// <para>
		/// If this parameter is <c>NULL</c>, the function creates an exception record and sets the <c>ExceptionCode</c> member to STATUS_FAIL_FAST_EXCEPTION.
		/// The function will also set the <c>ExceptionAddress</c> member if the dwFlags parameter contains the FAIL_FAST_GENERATE_EXCEPTION_ADDRESS flag.
		/// </para>
		/// </param>
		/// <param name="pContextRecord">
		/// A pointer to a <c>CONTEXT</c> structure that contains the context information. If <c>NULL</c>, this function generates the context (however, the
		/// context will not exactly match the context of the caller).
		/// </param>
		/// <param name="dwFlags">
		...
		/// </param>
		/// <returns>This function does not return a value.</returns>
		// VOID WINAPI RaiseFailFastException( _In_opt_ PEXCEPTION_RECORD pExceptionRecord, _In_opt_ PCONTEXT pContextRecord, _In_ DWORD dwFlags); https://msdn.microsoft.com/en-us/library/windows/desktop/dd941688(v=vs.85).aspx
		[DllImport(Lib.Kernel32, SetLastError = false, ExactSpelling = true)]
		...
		/// <param name="Handler">A pointer to a vectored exception handler previously registered using the <c>AddVectoredContinueHandler</c> function.</param>
		/// <returns>
		...
		/// <param name="Handler">A handle to the vectored exception handler previously registered using the <c>AddVectoredExceptionHandler</c> function.</param>
		/// <returns>
		/// <para>If the function succeeds, the return value is nonzero.</para>
		/// <para>If the function fails, the return value is zero.</para>
		/// </returns>
		...
		/// <term>SEM_FAILCRITICALERRORS0x0001</term>
		/// <term>
		/// The system does not display the critical-error-handler message box. Instead, the system sends the error to the calling process.Best practice is that
		/// all applications call the process-wide SetErrorMode function with a parameter of SEM_FAILCRITICALERRORS at startup. This is to prevent error mode
		/// dialogs from hanging the application.
		/// </term>
		/// </item>
		/// <item>
		/// <term>SEM_NOALIGNMENTFAULTEXCEPT0x0004</term>
		/// <term>
		/// The system automatically fixes memory alignment faults and makes them invisible to the application. It does this for the calling process and any
		/// descendant processes. This feature is only supported by certain processor architectures. For more information, see the Remarks section. After this
		/// value is set for a process, subsequent attempts to clear the value are ignored.
		/// </term>
		/// </item>
		/// <item>
		/// <term>SEM_NOGPFAULTERRORBOX0x0002</term>
		...
		/// <term>SEM_NOOPENFILEERRORBOX0x8000</term>
		/// <term>
		/// The OpenFile function does not display a message box when it fails to find a file. Instead, the error is returned to the caller. This error mode
		...
		/// <term>SEM_FAILCRITICALERRORS0x0001</term>
		/// <term>
		/// The system does not display the critical-error-handler message box. Instead, the system sends the error to the calling thread.Best practice is that
		/// all applications call the process-wide SetErrorMode function with a parameter of SEM_FAILCRITICALERRORS at startup. This is to prevent error mode
		/// dialogs from hanging the application.
		/// </term>
		/// </item>
		...
		/// </para>
		/// </param>
		/// <param name="lpOldMode">If the function succeeds, this parameter is set to the thread's previous error mode. This parameter can be <c>NULL</c>.</param>
		/// <returns>
		/// <para>If the function succeeds, the return value is nonzero.</para>
		/// <para>If the function fails, the return value is zero. To get extended error information, call <c>GetLastError</c>.</para>
		/// </returns>
		...
		/// <para>
		/// A pointer to a top-level exception filter function that will be called whenever the <c>UnhandledExceptionFilter</c> function gets control, and the
		/// process is not being debugged. A value of <c>NULL</c> for this parameter specifies default handling within <c>UnhandledExceptionFilter</c>.
		/// </para>
		/// <para>
		/// The filter function has syntax similar to that of <c>UnhandledExceptionFilter</c>: It takes a single parameter of type <c>LPEXCEPTION_POINTERS</c>,
		/// has a WINAPI calling convention, and returns a value of type <c>LONG</c>. The filter function should return one of the following values.
		/// </para>
		/// <para>
		/// <list type="table">
		/// <listheader>
		...
		/// </param>
		/// <returns>
		/// The <c>SetUnhandledExceptionFilter</c> function returns the address of the previous exception filter established with the function. A <c>NULL</c>
		/// return value means that there is no current top-level exception handler.
		/// </returns>
		...
		/// <summary>
		/// An application-defined function that passes unhandled exceptions to the debugger, if the process is being debugged. Otherwise, it optionally displays
		/// an <c>Application Error</c> message box and causes the exception handler to be executed. This function can be called only from within the filter
		/// expression of an exception handler.
		/// </summary>
		/// <param name="ExceptionInfo">
		/// A pointer to an <c>EXCEPTION_POINTERS</c> structure that specifies a description of the exception and the processor context at the time of the
		/// exception. This pointer is the return value of a call to the <c>GetExceptionInformation</c> function.
		/// </param>
		/// <returns>
		...
		public static extern EXCEPTION_FLAG UnhandledExceptionFilter([In] ref EXCEPTION_POINTERS ExceptionInfo);

		/// <summary>
		/// Contains an exception record with a machine-independent description of an exception and a context record with a machine-dependent description of the
		...
		public struct EXCEPTION_POINTERS
		{
			...
			/// <summary>
			/// A pointer to a <c>CONTEXT</c> structure that contains a processor-specific description of the state of the processor at the time of the exception.
			/// </summary>
			public IntPtr ContextRecord;
		}

		/// <summary>Describes an exception.</summary>
		// typedef struct _EXCEPTION_RECORD { DWORD ExceptionCode; DWORD ExceptionFlags; struct _EXCEPTION_RECORD *ExceptionRecord; PVOID ExceptionAddress; DWORD
		// NumberParameters; ULONG_PTR ExceptionInformation[EXCEPTION_MAXIMUM_PARAMETERS];} EXCEPTION_RECORD, *PEXCEPTION_RECORD;
		[PInvokeData("WinNT.h", MSDNShortId = "aa363082")]
		[StructLayout(LayoutKind.Sequential)]
		public struct EXCEPTION_RECORD
		{
			/// <summary>
			/// The exception flags. This member can be either zero, indicating a continuable exception, or <c>EXCEPTION_NONCONTINUABLE</c> indicating a
			/// noncontinuable exception. Any attempt to continue execution after a noncontinuable exception causes the <c>EXCEPTION_NONCONTINUABLE_EXCEPTION</c> exception.
			/// </summary>
			...
			/// <summary>
			/// A pointer to an associated <c>EXCEPTION_RECORD</c> structure. Exception records can be chained together to provide additional information when
			/// nested exceptions occur.
			/// </summary>
			public IntPtr ExceptionRecord;
			/// <summary>The address where the exception occurred.</summary>
			public IntPtr ExceptionAddress;
			/// <summary>
			/// The number of parameters associated with the exception. This is the number of defined elements in the <c>ExceptionInformation</c> array.
			/// </summary>
			public uint NumberParameters;
			/// <summary>
			/// <para>
			/// An array of additional arguments that describe the exception. The <c>RaiseException</c> function can specify this array of arguments. For most
			/// exception codes, the array elements are undefined. The following table describes the exception codes whose array elements are defined.
			/// </para>
			/// <list type="table">
			/// <listheader>
			...
			/// <term>
			/// The first element of the array contains a read-write flag that indicates the type of operation that caused the access violation. If this value is
			/// zero, the thread attempted to read the inaccessible data. If this value is 1, the thread attempted to write to an inaccessible address. If this
			/// value is 8, the thread causes a user-mode data execution prevention (DEP) violation.The second array element specifies the virtual address of the
			/// inaccessible data.
			/// </term>
			/// </item>
			/// <item>
			...
			/// The first element of the array contains a read-write flag that indicates the type of operation that caused the access violation. If this value is
			/// zero, the thread attempted to read the inaccessible data. If this value is 1, the thread attempted to write to an inaccessible address. If this
			/// value is 8, the thread causes a user-mode data execution prevention (DEP) violation. The second array element specifies the virtual address of
			/// the inaccessible data.The third array element specifies the underlying NTSTATUS code that resulted in the exception.
			/// </term>
			/// </item>
			/// </list>
			/// </summary>
