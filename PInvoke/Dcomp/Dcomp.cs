...
public static partial class Dcomp
{
	...
	[DllImport(Lib_Dcomp, SetLastError = false, ExactSpelling = true), DefaultDllImportSearchPaths(DllImportSearchPath.System32)]
	public static extern HRESULT DCompositionGetStatistics(COMPOSITION_FRAME_ID frameId, out COMPOSITION_FRAME_STATS frameStats, uint targetIdCount,
		[Out, Optional, MarshalAs(UnmanagedType.LPArray), SizeDef(nameof(targetIdCount), SizingMethod.Query, OutVarName = nameof(actualTargetIdCount))] COMPOSITION_TARGET_ID[]? targetIds, out uint actualTargetIdCount);
