...
namespace Vanara.PInvoke
{
	public static partial class CredUI
	{
		...
		/// <para>Beginning with Windows 8 and Windows Server 2012, the CredPackAuthenticationBuffer function converts an identity credential into an authentication buffer, which is a SEC_WINNT_AUTH_IDENTITY_EX2 structure. This buffer can be passed to LsaLogonUser, AcquireCredentialsHandle, or other identity provider interfaces.</para>
		/// </summary>
		...
		/// <para>For online identities, if the credential is a plaintext password, the user name format is ProviderName\UserName. If the credential is a SEC_WINNT_AUTH_IDENTITY_EX2 structure, the user name is an encoded string that is the UserName parameter output of a function call to SspiEncodeAuthIdentityAsStrings.</para>
		/// <para>For smart card or certificate credentials, the user name is an encoded string that is the output of a function call to CredMarshalCredential with the CertCredential option.</para>
		/// <para><c>Windows Server 2008 R2, Windows 7, Windows Server 2008 and Windows Vista:</c> Online identities are not supported.</para></param>
		/// <param name="pszPassword">A pointer to a null-terminated string that specifies the password to be converted.
		/// <para>For SEC_WINNT_AUTH_IDENTITY_EX2 credentials, the password is an encoded string that is in the ppszPackedCredentialsString output of a function call to SspiEncodeAuthIdentityAsStrings.</para>
		...
		[DllImport(Lib.CredUI, CharSet = CharSet.Unicode, EntryPoint = "CredUIConfirmCredentialsW")]
		...
		/// <para>Applications that target Windows Vista or Windows Server 2008 should call CredUIPromptForWindowsCredentials instead of this function, for the following reasons:</para>
		/// <list type="bullet">
		/// <item><term>CredUIPromptForWindowsCredentials is consistent with the current Windows user interface.</term></item>
		/// <item><term>CredUIPromptForWindowsCredentials is more extensible, allowing integration of additional authentication mechanisms such as biometrics and smart cards.</term></item>
		/// <item><term>CredUIPromptForWindowsCredentials is compliant with the Common Criteria specification.</term></item>
		/// </list>
		/// </summary>
		...
		/// <para>This function copies the user-supplied name to this buffer, copying a maximum of ulUserNameMaxChars characters. This format can be converted to UserName/Password format by using CredUIParseUsername. A marshaled format can be passed directly to a security support provider (SSP).</para>
		/// <para>If the CREDUI_FLAGS_DO_NOT_PERSIST flag is not specified, the value returned in this parameter is of a form that should not be inspected, printed, or persisted other than passing it to CredUIParseUsername. The subsequent results of CredUIParseUsername can be passed only to a client-side authentication function such as WNetAddConnection or an SSP function.</para>
		/// <para>If no domain or server is specified as part of this parameter, the value of the pszTargetName parameter is used as the domain to form a DomainName\UserName pair. On output, this parameter receives a string that contains that DomainName\UserName pair.</para></param>
		...
		/// <para>If the CREDUI_FLAGS_DO_NOT_PERSIST flag is specified and CREDUI_FLAGS_SHOW_SAVE_CHECK_BOX is not specified, the Save check box is not displayed, but is considered to be cleared.</para>
		/// <para>An application that needs to use CredUI to prompt the user for credentials, but does not need the credential management services provided by the credential manager, can use pfSave to receive the state of the Save check box after the user closes the dialog box. To do this, the caller must specify CREDUI_FLAGS_DO_NOT_PERSIST and CREDUI_FLAGS_SHOW_SAVE_CHECK_BOX in dwFlags. When CREDUI_FLAGS_DO_NOT_PERSIST and CREDUI_FLAGS_SHOW_SAVE_CHECK_BOX are set, the application is responsible for examining *pfSave after the function returns, and if *pfSave is TRUE, then the application must take the appropriate action to save the user credentials within the resources of the application.</para></param>
		...
		/// <returns>Status of the operation is returned.</returns>
		[DllImport(Lib.CredUI, CharSet = CharSet.Unicode, EntryPoint = "CredUIPromptForCredentialsW")]
		...
		/// <para>On output, this parameter specifies the authentication package for which the credentials in the ppvOutAuthBuffer buffer are serialized.</para></param>
		...
		/// <para>If the authentication buffer is a SEC_WINNT_AUTH_IDENTITY_EX2 structure, the function can decrypt the buffer if it is encrypted by using SspiEncryptAuthIdentityEx with the SEC_WINNT_AUTH_IDENTITY_ENCRYPT_SAME_LOGON option.</para>
		/// <para>If the authentication buffer is one of the marshaled KERB_*_LOGON structures, the function decrypts the password before returning it in the pszPassword buffer.</para></param>
		/// <param name="pAuthBuffer">A pointer to the authentication buffer to be converted.
		/// <para>This buffer is typically the output of the CredUIPromptForWindowsCredentials or CredPackAuthenticationBuffer function. This must be one of the following types:</para>
		/// <list type="bullet">
		/// <item><term>A SEC_WINNT_AUTH_IDENTITY_EX2 structure for identity credentials. The function does not accept other SEC_WINNT_AUTH_IDENTITY structures.</term></item>
		/// <item><term>A KERB_INTERACTIVE_LOGON or KERB_INTERACTIVE_UNLOCK_LOGON structure for password credentials.</term></item>
		/// <item><term>A KERB_CERTIFICATE_LOGON or KERB_CERTIFICATE_UNLOCK_LOGON structure for smart card certificate credentials.</term></item>
		/// <item><term>GENERIC_CRED for generic credentials.</term></item>
		/// </list>
		/// </param>
		...
		public struct CREDUI_INFO
		{
			...
			/// <summary>
			/// Initializes a new instance of the <see cref="CREDUI_INFO"/> struct.
			/// </summary>
