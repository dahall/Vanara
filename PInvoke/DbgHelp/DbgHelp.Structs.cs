...
namespace Vanara.PInvoke
{
	...
	public static partial class DbgHelp
	{
		...
		/// <remarks>
		/// <para>This structure supersedes the <c>ADDRESS</c> structure. For more information, see Updated Platform Support. <c>ADDRESS</c> is defined as follows in DbgHelp.h.</para>
		/// <para><code>#if !defined(_IMAGEHLP_SOURCE_) &amp;&amp; defined(_IMAGEHLP64) #define ADDRESS ADDRESS64 #define LPADDRESS LPADDRESS64 #else typedef struct _tagADDRESS { DWORD Offset; WORD Segment; ADDRESS_MODE Mode; } ADDRESS, *LPADDRESS; #endif </code></para>
		/// </remarks>
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-address64
		...
		{
			/// <summary>The offset into the segment, or a 32-bit virtual address. The interpretation of this value depends on the value contained in the <c>Mode</c> member.</summary>
			public ulong Offset;
		...
		/// <para><code>#if !defined(_IMAGEHLP_SOURCE_) &amp;&amp; defined(_IMAGEHLP64) #define ADDRESS ADDRESS64 #define LPADDRESS LPADDRESS64 #else typedef struct _tagADDRESS { DWORD Offset; WORD Segment; ADDRESS_MODE Mode; } ADDRESS, *LPADDRESS; #endif </code></para>
		/// </remarks>
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-address
		...
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-api_version
		...
		/// <para><c>Note</c> This structure is used by the MapDebugInformation and UnmapDebugInformation functions, which are provided only for backward compatibility.</para>
		/// </summary>
		/// <remarks>
		/// <para>The <c>LIST_ENTRY</c> structure is defined as follows:</para>
		/// <para><code>typedef struct _LIST_ENTRY { struct _LIST_ENTRY *Flink; struct _LIST_ENTRY *Blink; } LIST_ENTRY, *PLIST_ENTRY, *RESTRICTED_POINTER PRLIST_ENTRY; </code></para>
		/// </remarks>
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-image_debug_information
		...
		public struct IMAGE_DEBUG_INFORMATION
		{
			...
			/// <summary>The size of the memory allocated for the <c>IMAGE_DEBUG_INFORMATION</c> structure and all debugging information, in bytes.</summary>
...
// typedef struct _IMAGEHLP_CBA_EVENT { DWORD severity; DWORD code; PCHAR desc; PVOID object; } IMAGEHLP_CBA_EVENT, *PIMAGEHLP_CBA_EVENT;
[PInvokeData("dbghelp.h", MSDNShortId = "NS:dbghelp._IMAGEHLP_CBA_EVENT")]
		...
		/// <remarks>
		/// <para>This structure supersedes the <c>IMAGEHLP_DEFERRED_SYMBOL_LOAD</c> structure. For more information, see Updated Platform Support. <c>IMAGEHLP_DEFERRED_SYMBOL_LOAD</c> is defined as follows in DbgHelp.h.</para>
		/// <para><code>#if !defined(_IMAGEHLP_SOURCE_) &amp;&amp; defined(_IMAGEHLP64) #define IMAGEHLP_DEFERRED_SYMBOL_LOAD IMAGEHLP_DEFERRED_SYMBOL_LOAD64 #define PIMAGEHLP_DEFERRED_SYMBOL_LOAD PIMAGEHLP_DEFERRED_SYMBOL_LOAD64 #else typedef struct _IMAGEHLP_DEFERRED_SYMBOL_LOAD { DWORD SizeOfStruct; DWORD BaseOfImage; DWORD CheckSum; DWORD TimeDateStamp; CHAR FileName[MAX_PATH]; BOOLEAN Reparse; HANDLE hFile; } IMAGEHLP_DEFERRED_SYMBOL_LOAD, *PIMAGEHLP_DEFERRED_SYMBOL_LOAD; #endif </code></para>
		/// </remarks>
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-imagehlp_deferred_symbol_load64
		// typedef struct _IMAGEHLP_DEFERRED_SYMBOL_LOAD64 { DWORD SizeOfStruct; DWORD64 BaseOfImage; DWORD CheckSum; DWORD TimeDateStamp; CHAR FileName[MAX_PATH]; BOOLEAN Reparse; HANDLE hFile; DWORD Flags; } IMAGEHLP_DEFERRED_SYMBOL_LOAD64, *PIMAGEHLP_DEFERRED_SYMBOL_LOAD64;
		...
		{
			/// <summary>The size of the structure, in bytes. The caller must set this member to <code>sizeof(IMAGEHLP_DEFERRED_SYMBOL_LOAD64)</code>.</summary>
			...
			/// <summary>A handle to a file. This member is used with <c>CBA_DEFERRED_SYMBOL_LOAD_PARTIAL</c> and <c>IMAGEHLP_DEFERRED_SYMBOL_LOAD_FAILURE</c> callbacks.</summary>
			public HFILE hFile;
			/// <summary>
		...
		// typedef struct _IMAGEHLP_DEFERRED_SYMBOL_LOAD { DWORD SizeOfStruct; DWORD BaseOfImage; DWORD CheckSum; DWORD TimeDateStamp; CHAR FileName[MAX_PATH]; BOOLEAN Reparse; HANDLE hFile; } IMAGEHLP_DEFERRED_SYMBOL_LOAD, *PIMAGEHLP_DEFERRED_SYMBOL_LOAD;
		...
		public struct IMAGEHLP_DEFERRED_SYMBOL_LOAD
		{
			...
			/// <summary>The computed checksum of the image. This value can be zero.</summary>
			public uint CheckSum;
			/// <summary>The date and timestamp value. The value is represented in the number of seconds elapsed since midnight (00:00:00), January 1, 1970, Universal Coordinated Time, according to the system clock. The timestamp can be printed using the C run-time (CRT) function <c>ctime</c>.</summary>
			public uint TimeDateStamp;
			/// <summary>The image name. The name may or may not contain a full path.</summary>
			[MarshalAs(UnmanagedType.ByValTStr, SizeConst = 260 /*MAX_PATH*/)]
			public string FileName;
		...
		/// <remarks>
		/// <para>This structure supersedes the <c>IMAGEHLP_DUPLICATE_SYMBOL</c> structure. For more information, see Updated Platform Support. <c>IMAGEHLP_DUPLICATE_SYMBOL</c> is defined as follows in DbgHelp.h.</para>
		/// <para><code>#if !defined(_IMAGEHLP_SOURCE_) &amp;&amp; defined(_IMAGEHLP64) #define IMAGEHLP_DUPLICATE_SYMBOL IMAGEHLP_DUPLICATE_SYMBOL64 #define PIMAGEHLP_DUPLICATE_SYMBOL PIMAGEHLP_DUPLICATE_SYMBOL64 #else typedef struct _IMAGEHLP_DUPLICATE_SYMBOL { DWORD SizeOfStruct; DWORD NumberOfDups; PIMAGEHLP_SYMBOL Symbol; DWORD SelectedSymbol; } IMAGEHLP_DUPLICATE_SYMBOL, *PIMAGEHLP_DUPLICATE_SYMBOL; #endif </code></para>
		/// </remarks>
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-imagehlp_duplicate_symbol64
		// typedef struct _IMAGEHLP_DUPLICATE_SYMBOL64 { DWORD SizeOfStruct; DWORD NumberOfDups; PIMAGEHLP_SYMBOL64 Symbol; DWORD SelectedSymbol; } IMAGEHLP_DUPLICATE_SYMBOL64, *PIMAGEHLP_DUPLICATE_SYMBOL64;
		[PInvokeData("dbghelp.h", MSDNShortId = "NS:dbghelp._IMAGEHLP_DUPLICATE_SYMBOL64")]
		[StructLayout(LayoutKind.Sequential, CharSet = CharSet.Auto)]
		public struct IMAGEHLP_DUPLICATE_SYMBOL64
		{
			/// <summary>The size of the structure, in bytes. The caller must set this member to <code>sizeof(IMAGEHLP_DUPLICATE_SYMBOL64)</code>.</summary>
			public uint SizeOfStruct;
			/// <summary>The number of duplicate symbols.</summary>
			public uint NumberOfDups;
			/// <summary>A pointer to an array of symbols ( IMAGEHLP_SYMBOL64 structures). The number of entries in the array is specified by the <c>NumberOfDups</c> member.</summary>
		...
		public struct IMAGEHLP_GET_TYPE_INFO_PARAMS
		{
			...
			///       <term>Do not cache the data for later retrievals. It is good to use this flag if you will not be requesting the information again.</term>
			///     </item>
			///   </list>
			/// </summary>
			public uint Flags;
			...
			/// <summary>The filter for return values. For example, set this member to 1 &lt;&lt; <c>SymTagData</c> to return only results with a symbol tag of <c>SymTagData</c>. For a list of tags, see the <c>SymTagEnum</c> type defined in Dbghelp.h</summary>
			public ulong TagFilter;
			/// <summary>The number of elements specified in the arrays specified in the <c>ReqKinds</c>, <c>ReqOffsets</c>, and <c>ReqSizes</c> members. These arrays must be the same size.</summary>
			public uint NumReqs;
			/// <summary>An array of information types to be requested. Each element is one of the enumeration values in the IMAGEHLP_SYMBOL_TYPE_INFO enumeration type.</summary>
			public IntPtr ReqKinds;
			/// <summary>An array of offsets that specify where to store the data for each request within each element of <c>Buffer</c> array.</summary>
			...
			/// <summary>An array of records used for storing query results. Each record is separated by <c>ReqStride</c> bytes. Each type for which data is to be retrieved requires one record in the array. Within each record, there are <c>NumReqs</c> pieces of data stored as the result of individual queries. The data is stored within the record according to the offsets specified in <c>ReqOffsets</c>. The format of the data depends on the value of the <c>ReqKinds</c> member in use.</summary>
			...
			/// <summary>A bitmask indexed by <c>Buffer</c> element index that indicates which request data is valid. This member can be <c>NULL</c>.</summary>
			public IntPtr ReqsValid;
		}

		/// <summary>
		/// Flags for <see cref="IMAGEHLP_GET_TYPE_INFO_PARAMS"/>.
		/// </summary>
		[PInvokeData("dbghelp.h", MSDNShortId = "NS:dbghelp._IMAGEHLP_GET_TYPE_INFO_PARAMS")]
		public enum IMAGEHLP_GTI_FLAGS
		{
			///       <summary>Do not cache the data for later retrievals. It is good to use this flag if you will not be requesting the information again.</summary>
		...
		/// <remarks>
		/// <para>This structure supersedes the <c>IMAGEHLP_LINE</c> structure. For more information, see Updated Platform Support. <c>IMAGEHLP_LINE</c> is defined as follows in DbgHelp.h.</para>
		/// <para><code>#if !defined(_IMAGEHLP_SOURCE_) &amp;&amp; defined(_IMAGEHLP64) #define IMAGEHLP_LINE IMAGEHLP_LINE64 #define PIMAGEHLP_LINE PIMAGEHLP_LINE64 #else typedef struct _IMAGEHLP_LINE { DWORD SizeOfStruct; PVOID Key; DWORD LineNumber; PCHAR FileName; DWORD Address; } IMAGEHLP_LINE, *PIMAGEHLP_LINE; typedef struct _IMAGEHLP_LINEW { DWORD SizeOfStruct; PVOID Key; DWORD LineNumber; PCHAR FileName; DWORD64 Address; } IMAGEHLP_LINEW, *PIMAGEHLP_LINEW; #endif </code></para>
		/// </remarks>
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-imagehlp_line
		...
		{
			/// <summary>The size of the structure, in bytes. The caller must set this member to <code>sizeof(IMAGEHLP_LINE64)</code>.</summary>
			public uint SizeOfStruct;
			/// <summary>This member is reserved for use by the operating system.</summary>
			public IntPtr Key;
		...
		/// <para><code>#if !defined(_IMAGEHLP_SOURCE_) &amp;&amp; defined(_IMAGEHLP64) #define IMAGEHLP_LINE IMAGEHLP_LINE64 #define PIMAGEHLP_LINE PIMAGEHLP_LINE64 #else typedef struct _IMAGEHLP_LINE { DWORD SizeOfStruct; PVOID Key; DWORD LineNumber; PCHAR FileName; DWORD Address; } IMAGEHLP_LINE, *PIMAGEHLP_LINE; typedef struct _IMAGEHLP_LINEW { DWORD SizeOfStruct; PVOID Key; DWORD LineNumber; PCHAR FileName; DWORD64 Address; } IMAGEHLP_LINEW, *PIMAGEHLP_LINEW; #endif </code></para>
		/// </remarks>
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-imagehlp_line64
		...
		/// <para><code>#if !defined(_IMAGEHLP_SOURCE_) &amp;&amp; defined(_IMAGEHLP64) #define IMAGEHLP_LINE IMAGEHLP_LINE64 #define PIMAGEHLP_LINE PIMAGEHLP_LINE64 #else typedef struct _IMAGEHLP_LINE { DWORD SizeOfStruct; PVOID Key; DWORD LineNumber; PCHAR FileName; DWORD Address; } IMAGEHLP_LINE, *PIMAGEHLP_LINE; typedef struct _IMAGEHLP_LINEW { DWORD SizeOfStruct; PVOID Key; DWORD LineNumber; PCHAR FileName; DWORD64 Address; } IMAGEHLP_LINEW, *PIMAGEHLP_LINEW; #endif </code></para>
		/// </remarks>
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-imagehlp_linew64
		// typedef struct _IMAGEHLP_LINEW64 { DWORD SizeOfStruct; PVOID Key; DWORD LineNumber; PWSTR FileName; DWORD64 Address; } IMAGEHLP_LINEW64, *PIMAGEHLP_LINEW64;
		...
		/// <remarks>
		/// <para>This structure supersedes the <c>IMAGEHLP_MODULE</c> structure. For more information, see Updated Platform Support. <c>IMAGEHLP_MODULE</c> is defined as follows in DbgHelp.h.</para>
		/// <para><code>#if !defined(_IMAGEHLP_SOURCE_) &amp;&amp; defined(_IMAGEHLP64) #define IMAGEHLP_MODULE IMAGEHLP_MODULE64 #define PIMAGEHLP_MODULE PIMAGEHLP_MODULE64 #define IMAGEHLP_MODULEW IMAGEHLP_MODULEW64 #define PIMAGEHLP_MODULEW PIMAGEHLP_MODULEW64 #else typedef struct _IMAGEHLP_MODULE { DWORD SizeOfStruct; DWORD BaseOfImage; DWORD ImageSize; DWORD TimeDateStamp; DWORD CheckSum; DWORD NumSyms; SYM_TYPE SymType; CHAR ModuleName[32]; CHAR ImageName[256]; CHAR LoadedImageName[256]; } IMAGEHLP_MODULE, *PIMAGEHLP_MODULE; typedef struct _IMAGEHLP_MODULEW { DWORD SizeOfStruct; DWORD BaseOfImage; DWORD ImageSize; DWORD TimeDateStamp; DWORD CheckSum; DWORD NumSyms; SYM_TYPE SymType; WCHAR ModuleName[32]; WCHAR ImageName[256]; WCHAR LoadedImageName[256]; } IMAGEHLP_MODULEW, *PIMAGEHLP_MODULEW; #endif </code></para>
		/// </remarks>
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-imagehlp_module
		...
		{
			/// <summary>The size of the structure, in bytes. The caller must set this member to <code>sizeof(IMAGEHLP_MODULE64)</code>.</summary>
			public uint SizeOfStruct;
			/// <summary>The base virtual address where the image is loaded.</summary>
			public uint BaseOfImage;
			/// <summary>The size of the image, in bytes.</summary>
			...
			public uint CheckSum;
			/// <summary>The number of symbols in the symbol table. The value of this parameter is not meaningful when <c>SymPdb</c> is specified as the value of the SymType parameter.</summary>
			public uint NumSyms;
			/// <summary>
		...
		// typedef struct _IMAGEHLP_MODULE64 { DWORD SizeOfStruct; DWORD64 BaseOfImage; DWORD ImageSize; DWORD TimeDateStamp; DWORD CheckSum; DWORD NumSyms; SYM_TYPE SymType; CHAR ModuleName[32]; CHAR ImageName[256]; CHAR LoadedImageName[256]; CHAR LoadedPdbName[256]; DWORD CVSig; CHAR *CVData[MAX_PATH 3]; DWORD PdbSig; GUID PdbSig70; DWORD PdbAge; BOOL PdbUnmatched; BOOL DbgUnmatched; BOOL LineNumbers; BOOL GlobalSymbols; BOOL TypeInfo; BOOL SourceIndexed; BOOL Publics; DWORD MachineType; DWORD Reserved; } IMAGEHLP_MODULE64, *PIMAGEHLP_MODULE64;
		...
		public struct IMAGEHLP_MODULE64
		{
			...
			///   <para>
			///     <c>DbgHelp 6.1 and earlier: </c>This member is not supported.</para>
			/// </summary>
		...
		/// <para><code>#if !defined(_IMAGEHLP_SOURCE_) &amp;&amp; defined(_IMAGEHLP64) #define IMAGEHLP_MODULE IMAGEHLP_MODULE64 #define PIMAGEHLP_MODULE PIMAGEHLP_MODULE64 #define IMAGEHLP_MODULEW IMAGEHLP_MODULEW64 #define PIMAGEHLP_MODULEW PIMAGEHLP_MODULEW64 #else typedef struct _IMAGEHLP_MODULE { DWORD SizeOfStruct; DWORD BaseOfImage; DWORD ImageSize; DWORD TimeDateStamp; DWORD CheckSum; DWORD NumSyms; SYM_TYPE SymType; CHAR ModuleName[32]; CHAR ImageName[256]; CHAR LoadedImageName[256]; } IMAGEHLP_MODULE, *PIMAGEHLP_MODULE; typedef struct _IMAGEHLP_MODULEW { DWORD SizeOfStruct; DWORD BaseOfImage; DWORD ImageSize; DWORD TimeDateStamp; DWORD CheckSum; DWORD NumSyms; SYM_TYPE SymType; WCHAR ModuleName[32]; WCHAR ImageName[256]; WCHAR LoadedImageName[256]; } IMAGEHLP_MODULEW, *PIMAGEHLP_MODULEW; #endif </code></para>
		/// </remarks>
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-imagehlp_modulew64
		// typedef struct _IMAGEHLP_MODULEW64 { DWORD SizeOfStruct; DWORD64 BaseOfImage; DWORD ImageSize; DWORD TimeDateStamp; DWORD CheckSum; DWORD NumSyms; SYM_TYPE SymType; WCHAR ModuleName[32]; WCHAR ImageName[256]; WCHAR LoadedImageName[256]; WCHAR LoadedPdbName[256]; DWORD CVSig; WCHAR *CVData[MAX_PATH 3]; DWORD PdbSig; GUID PdbSig70; DWORD PdbAge; BOOL PdbUnmatched; BOOL DbgUnmatched; BOOL LineNumbers; BOOL GlobalSymbols; BOOL TypeInfo; BOOL SourceIndexed; BOOL Publics; DWORD MachineType; DWORD Reserved; } IMAGEHLP_MODULEW64, *PIMAGEHLP_MODULEW64;
		...
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-imagehlp_stack_frame
		...
		public struct IMAGEHLP_STACK_FRAME
		{
			...
			///   <para>
			///     <c>x86: </c>The program counter is EIP.</para>
			///   <para>
			///     <c>Intel Itanium: </c>The program counter is a combination of the bundle address and a slot indicator of 0, 4, or 8 for the slot within the bundle.</para>
			///   <para>
			///     <c>x64: </c>The program counter is RIP.</para>
			/// </summary>
			...
			///   <para>
			///     <c>x86: </c>The frame pointer is EBP.</para>
			///   <para>
			///     <c>Intel Itanium: </c>There is no frame pointer, but <c>AddrBStore</c> is used.</para>
			///   <para>
			///     <c>x64: </c>The frame pointer is RBP. AMD-64 does not always use this value.</para>
			/// </summary>
			public ulong FrameOffset;
			/// <summary>
			///   <para>The stack pointer.</para>
			///   <para>
			///     <c>x86: </c>The stack pointer is ESP.</para>
			///   <para>
			///     <c>Intel Itanium: </c>The stack pointer is SP.</para>
			///   <para>
			///     <c>x64: </c>The stack pointer is RSP.</para>
			/// </summary>
			public ulong StackOffset;
			/// <summary>
			///   <c>Intel Itanium: </c>The backing store address.</summary>
			public ulong BackingStoreOffset;
			/// <summary>
			///   <c>x86: </c>An FPO_DATA structure. If there is no function table entry, this member is <c>NULL</c>.</summary>
		...
		/// <remarks>
		/// <para>This structure supersedes the <c>IMAGEHLP_SYMBOL</c> structure. For more information, see Updated Platform Support. <c>IMAGEHLP_SYMBOL</c> is defined as follows in DbgHelp.h.</para>
		/// <para><code>#if !defined(_IMAGEHLP_SOURCE_) &amp;&amp; defined(_IMAGEHLP64) #define IMAGEHLP_SYMBOL IMAGEHLP_SYMBOL64 #define PIMAGEHLP_SYMBOL PIMAGEHLP_SYMBOL64 #else typedef struct _IMAGEHLP_SYMBOL { DWORD SizeOfStruct; DWORD Address; DWORD Size; DWORD Flags; DWORD MaxNameLength; CHAR Name[1]; } IMAGEHLP_SYMBOL, *PIMAGEHLP_SYMBOL; #endif </code></para>
		/// </remarks>
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-imagehlp_symbol
		// typedef struct _IMAGEHLP_SYMBOL { DWORD SizeOfStruct; DWORD Address; DWORD Size; DWORD Flags; DWORD MaxNameLength; CHAR Name[1]; } IMAGEHLP_SYMBOL, *PIMAGEHLP_SYMBOL;
		[PInvokeData("dbghelp.h", MSDNShortId = "NS:dbghelp._IMAGEHLP_SYMBOL")]
		[StructLayout(LayoutKind.Sequential, CharSet = CharSet.Ansi)]
		public struct IMAGEHLP_SYMBOL
		{
			/// <summary>The size of the structure, in bytes. The caller must set this member to <code>sizeof(IMAGEHLP_SYMBOL64)</code>.</summary>
			public uint SizeOfStruct;
			...
			/// <summary>This member is reserved for use by the operating system.</summary>
			public uint Flags;
			/// <summary>The maximum length of the string that the <c>Name</c> member can contain, in characters, not including the null-terminating character. Because symbol names can vary in length, this data structure is allocated by the caller. This member is used so the library knows how much memory is available for use by the symbol name.</summary>
			public uint MaxNameLength;
			/// <summary>The decorated or undecorated symbol name. If the buffer is not large enough for the complete name, it is truncated to <c>MaxNameLength</c> characters, including the null-terminating character.</summary>
		...
		/// <para><code>#if !defined(_IMAGEHLP_SOURCE_) &amp;&amp; defined(_IMAGEHLP64) #define IMAGEHLP_SYMBOL IMAGEHLP_SYMBOL64 #define PIMAGEHLP_SYMBOL PIMAGEHLP_SYMBOL64 #else typedef struct _IMAGEHLP_SYMBOL { DWORD SizeOfStruct; DWORD Address; DWORD Size; DWORD Flags; DWORD MaxNameLength; CHAR Name[1]; } IMAGEHLP_SYMBOL, *PIMAGEHLP_SYMBOL; #endif </code></para>
		/// </remarks>
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-imagehlp_symbol64
		// typedef struct _IMAGEHLP_SYMBOL64 { DWORD SizeOfStruct; DWORD64 Address; DWORD Size; DWORD Flags; DWORD MaxNameLength; CHAR Name[1]; } IMAGEHLP_SYMBOL64, *PIMAGEHLP_SYMBOL64;
		...
		/// <para><code>#if !defined(_IMAGEHLP_SOURCE_) &amp;&amp; defined(_IMAGEHLP64) #define IMAGEHLP_SYMBOL IMAGEHLP_SYMBOL64 #define PIMAGEHLP_SYMBOL PIMAGEHLP_SYMBOL64 #else typedef struct _IMAGEHLP_SYMBOL { DWORD SizeOfStruct; DWORD Address; DWORD Size; DWORD Flags; DWORD MaxNameLength; CHAR Name[1]; } IMAGEHLP_SYMBOL, *PIMAGEHLP_SYMBOL; #endif </code></para>
		/// </remarks>
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-imagehlp_symbolw64
		// typedef struct _IMAGEHLP_SYMBOLW64 { DWORD SizeOfStruct; DWORD64 Address; DWORD Size; DWORD Flags; DWORD MaxNameLength; WCHAR Name[1]; } IMAGEHLP_SYMBOLW64, *PIMAGEHLP_SYMBOLW64;
		...
		/// <remarks>
		/// <para>This structure supersedes the <c>KDHELP</c> structure. For more information, see Updated Platform Support. <c>KDHELP</c> is defined as follows in Dbghelp.h.</para>
		...
		/// </remarks>
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-kdhelp
		// typedef struct _KDHELP { DWORD Thread; DWORD ThCallbackStack; DWORD NextCallback; DWORD FramePointer; DWORD KiCallUserMode; DWORD KeUserCallbackDispatcher; DWORD SystemRangeStart; DWORD ThCallbackBStore; DWORD KiUserExceptionDispatcher; DWORD StackBase; DWORD StackLimit; DWORD Reserved[5]; } KDHELP, *PKDHELP;
		...
		public struct KDHELP
		{
			...
			/// <summary>
			///   <c>Intel Itanium: </c>The offset in the thread object to a pointer to the current callback backing store frame in the kernel stack.</summary>
		...
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-modload_cvmisc
		// typedef struct _MODLOAD_CVMISC { DWORD oCV; size_t cCV; DWORD oMisc; size_t cMisc; DWORD dtImage; DWORD cImage; } MODLOAD_CVMISC, *PMODLOAD_CVMISC;
		...
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-modload_data
		// typedef struct _MODLOAD_DATA { DWORD ssize; DWORD ssig; PVOID data; DWORD size; DWORD flags; } MODLOAD_DATA, *PMODLOAD_DATA;
		...
		/// <remarks>
		/// <para>An address map provides a translation from one image layout (A) to another (B). An array of OMAP structures, sorted by <c>rva</c>, defines an address map.</para>
		...
		/// </list>
		/// </remarks>
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-omap
		...
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-sourcefile
		// typedef struct _SOURCEFILE { DWORD64 ModBase; PCHAR FileName; } SOURCEFILE, *PSOURCEFILE;
		...
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-srccodeinfo
		// typedef struct _SRCCODEINFO { DWORD SizeOfStruct; PVOID Key; DWORD64 ModBase; CHAR Obj[MAX_PATH + 1]; CHAR FileName[MAX_PATH + 1]; DWORD LineNumber; DWORD64 Address; } SRCCODEINFO, *PSRCCODEINFO;
		...
		public struct STACKFRAME64
		{
			...
			///     <c>x86: </c>The program counter is EIP.</para>
			///   <para>
			///     <c>Intel Itanium: </c>The program counter is StIIP.</para>
			///   <para>
			///     <c>x64: </c>The program counter is RIP.</para>
			/// </summary>
			...
			///     <c>Intel Itanium: </c>There is no frame pointer, but <c>AddrBStore</c> is used.</para>
			///   <para>
			///     <c>x64: </c>The frame pointer is RBP or RDI. This value is not always used.</para>
			/// </summary>
			...
			/// <summary>
			///   <c>Intel Itanium: </c>An ADDRESS64 structure that specifies the backing store (RsBSP).</summary>
			public ADDRESS64 AddrBStore;
			/// <summary>On x86 computers, this member is an FPO_DATA structure. If there is no function table entry, this member is <c>NULL</c>.</summary>
		...
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-stackframe_ex
		// typedef struct _tagSTACKFRAME_EX { ADDRESS64 AddrPC; ADDRESS64 AddrReturn; ADDRESS64 AddrFrame; ADDRESS64 AddrStack; ADDRESS64 AddrBStore; PVOID FuncTableEntry; DWORD64 Params[4]; BOOL Far; BOOL Virtual; DWORD64 Reserved[3]; KDHELP64 KdHelp; DWORD StackFrameSize; DWORD InlineFrameContext; } STACKFRAME_EX, *LPSTACKFRAME_EX;
		...
		// typedef struct _SYMBOL_INFO { ULONG SizeOfStruct; ULONG TypeIndex; ULONG64 Reserved[2]; ULONG Index; ULONG Size; ULONG64 ModBase; ULONG Flags; ULONG64 Value; ULONG64 Address; ULONG Register; ULONG Scope; ULONG Tag; ULONG NameLen; ULONG MaxNameLen; CHAR Name[1]; } SYMBOL_INFO, *PSYMBOL_INFO;
		...
		public struct SYMBOL_INFO
		{
			...
			///       <term>The symbol address is an offset relative to the beginning of the intermediate language block. This applies to managed code only.</term>
			///     </item>
			///     <item>
			...
			/// <summary>
			///   <para>The DIA scope. For more information, see the Debug Interface Access SDK in the Visual Studio documentation. (This resource may not be available in some languages</para>
			...
			/// <summary>The name of the symbol. The name can be undecorated if the SYMOPT_UNDNAME option is used with the SymSetOptions function.</summary>
			[MarshalAs(UnmanagedType.ByValTStr, SizeConst = 1)]
			public string Name;
		}
		...
		public enum SYMFLAG : uint
		{
			...
			///       <summary>The symbol address is an offset relative to the beginning of the intermediate language block. This applies to managed code only.</summary>
		...
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-symsrv_index_info
		// typedef struct { DWORD sizeofstruct; char file[MAX_PATH + 1]; BOOL stripped; DWORD timestamp; DWORD size; char dbgfile[MAX_PATH + 1]; char pdbfile[MAX_PATH + 1]; GUID guid; DWORD sig; DWORD age; } SYMSRV_INDEX_INFO, *PSYMSRV_INDEX_INFO;
		[PInvokeData("dbghelp.h", MSDNShortId = "NS:dbghelp.__unnamed_struct_0")]
		[StructLayout(LayoutKind.Sequential, CharSet = CharSet.Auto)]
		public struct SYMSRV_INDEX_INFO
		{
			/// <summary>The size of the structure, in bytes. This member must be set to <code>sizeof(SYMSRV_INDEX_INFO)</code> or <code>sizeof(SYMSRV_INDEX_INFOW)</code>.</summary>
			public uint sizeofstruct;
			...
			public uint size;
			/// <summary>If the image file is stripped and there is a .dbg file, this member is the path to the .dbg file from the CV record.</summary>
			[MarshalAs(UnmanagedType.ByValTStr, SizeConst = 261 /*MAX_PATH + 1*/)]
			...
			/// <summary>The GUID of the .pdb file. If there is no GUID available, the signature of the .pdb file is copied into first <c>DWORD</c> of the GUID.</summary>
			public Guid guid;
			/// <summary>The signature of the .pdb file (for use with old-style .pdb files). This value can be 0 if it is a new-style .pdb file that uses a GUID-length signature.</summary>
			public uint sig;
		...
		// https://docs.microsoft.com/en-us/windows/win32/api/dbghelp/ns-dbghelp-ti_findchildren_params
		...
		public struct TI_FINDCHILDREN_PARAMS
		{
			...
			public uint Count;
			/// <summary>The zero-based index of the child from which the child indexes are to be retrieved. For example, in an array with five elements, if Start is two, this indicates the third array element. In most cases, this member is zero.</summary>
			public uint Start;
