...
namespace Vanara.PInvoke
{
	public static partial class User32
	{
		...
		public enum MouseWheelRouting : uint
		{
			...
			/// <summary>Mouse wheel input is delivered to the app with focus (desktop apps) or the app under the mouse cursor (Windows Store apps).</summary>
		...
		public enum SPI : uint
		{
			...
			/// <summary>
			/// Determines whether audio descriptions are enabled or disabled. The pvParam parameter is a pointer to an AUDIODESCRIPTION structure. Set the
			/// cbSize member of this structure and the uiParam parameter to sizeof(AUDIODESCRIPTION).
			/// <para>
			/// While it is possible for users who have visual impairments to hear the audio in video content, there is a lot of action in video that does not
			/// have corresponding audio. Specific audio description of what is happening in a video helps these users understand the content better. This flag
			/// enables you to determine whether audio descriptions have been enabled and in which language.
			/// </para>
			...
			/// <para>
			/// While it is possible for users who are visually impaired to hear the audio in video content, there is a lot of action in video that does not have
			/// corresponding audio. Specific audio description of what is happening in a video helps these users understand the content better. This flag
			/// enables you to enable or disable audio descriptions in the languages they are provided in.
			/// </para>
			/// <para>Windows Server 2003 and Windows XP/2000: This parameter is not supported.</para>
			/// </summary>
			...
			/// <summary>
			/// Determines whether the screen saver requires a password to display the Windows desktop. The pvParam parameter must point to a BOOL variable that
			/// receives TRUE if the screen saver requires a password, or FALSE otherwise. The uiParam parameter is ignored. Windows Server 2003 and Windows
			...
			/// <summary>
			/// Sets whether the screen saver requires the user to enter a password to display the Windows desktop. The uiParam parameter is a BOOL variable. The
			/// pvParam parameter is ignored. Set uiParam to TRUE to require a password, or FALSE to not require a password.
			...
			/// <summary>
			/// Retrieves the number of milliseconds that a thread can go without dispatching a message before the system considers it unresponsive. The pvParam
			/// parameter must point to an integer variable that receives the value.
			...
			/// <summary>
			/// Sets the hung application time-out to the value of the uiParam parameter. This value is the number of milliseconds that a thread can go without
			/// dispatching a message before the system considers it unresponsive.
			/// <para><c>Windows Server 2008, Windows Vista, Windows Server 2003 and Windows XP/2000:</c> This parameter is not supported.</para>
			/// </summary>
			[SPCorrespondingType(typeof(uint), CorrepsondingAction.Set, true)]
			...
			/// <summary>
			/// Retrieves the number of milliseconds that the system waits before terminating an application that does not respond to a shutdown request. The
			/// pvParam parameter must point to an integer variable that receives the value.
			/// <para><c>Windows Server 2008, Windows Vista, Windows Server 2003 and Windows XP/2000:</c> This parameter is not supported.</para>
			/// </summary>
			[SPCorrespondingType(typeof(uint), CorrepsondingAction.Get)]
			...
			/// <summary>
			/// Sets the application shutdown request time-out to the value of the uiParam parameter. This value is the number of milliseconds that the system
			/// waits before terminating an application that does not respond to a shutdown request.
			/// <para><c>Windows Server 2008, Windows Vista, Windows Server 2003 and Windows XP/2000:</c> This parameter is not supported.</para>
			/// </summary>
			[SPCorrespondingType(typeof(uint), CorrepsondingAction.Set, true)]
			...
			/// <summary>
			/// Retrieves the number of milliseconds that the service control manager waits before terminating a service that does not respond to a shutdown
			/// request. The pvParam parameter must point to an integer variable that receives the value.
			/// <para><c>Windows Server 2008, Windows Vista, Windows Server 2003 and Windows XP/2000:</c> This parameter is not supported.</para>
			/// </summary>
			[SPCorrespondingType(typeof(uint), CorrepsondingAction.Get)]
			...
			/// <summary>
			/// Sets the service shutdown request time-out to the value of the uiParam parameter. This value is the number of milliseconds that the system waits
			/// before terminating a service that does not respond to a shutdown request.
			/// <para><c>Windows Server 2008, Windows Vista, Windows Server 2003 and Windows XP/2000:</c> This parameter is not supported.</para>
			/// </summary>
			[SPCorrespondingType(typeof(uint), CorrepsondingAction.Set, true)]
			...
			/// <summary>
			/// Retrieves the threshold in pixels where docking behavior is triggered by using a mouse to drag a window to the edge of a monitor or monitor
			/// array. The default threshold is 1. The pvParam parameter must point to a DWORD variable that receives the value.
			...
			/// <summary>
			/// Sets the threshold in pixels where docking behavior is triggered by using a mouse to drag a window to the edge of a monitor or monitor array. The
			/// default threshold is 1. The pvParam parameter must point to a DWORD variable that contains the new value.
			...
			/// <summary>
			/// Sets the threshold in pixels where docking behavior is triggered by using a pen to drag a window to the edge of a monitor or monitor array. The
			/// default threshold is 30. The pvParam parameter must point to a DWORD variable that contains the new value.
			...
			/// <summary>
			/// Retrieves the threshold in pixels where undocking behavior is triggered by using a mouse to drag a window from the edge of a monitor or a monitor
			/// array toward the center. The default threshold is 20.
			/// <para>Use <c>SPI_GETWINARRANGING</c> to determine whether this behavior is enabled.</para>
			/// <para><c>Windows Server 2008, Windows Vista, Windows Server 2003 and Windows XP/2000:</c> This parameter is not supported.</para>
			/// </summary>
			[SPCorrespondingType(typeof(uint), CorrepsondingAction.Get)]
			...
			/// <summary>
			/// Sets the threshold in pixels where undocking behavior is triggered by using a mouse to drag a window from the edge of a monitor or monitor array
			/// to its center. The default threshold is 20. The pvParam parameter must point to a DWORD variable that contains the new value.
			/// <para><c>SPI_GETWINARRANGING</c> must be TRUE to enable this behavior.</para>
			/// <para><c>Windows Server 2008, Windows Vista, Windows Server 2003 and Windows XP/2000:</c> This parameter is not supported.</para>
			/// </summary>
			[SPCorrespondingType(typeof(uint), CorrepsondingAction.Set)]
			...
			/// <summary>
			/// Retrieves the threshold in pixels where undocking behavior is triggered by using a pen to drag a window from the edge of a monitor or monitor
			/// array toward its center. The default threshold is 30.
			/// <para>Use <c>SPI_GETWINARRANGING</c> to determine whether this behavior is enabled.</para>
			/// <para><c>Windows Server 2008, Windows Vista, Windows Server 2003 and Windows XP/2000:</c> This parameter is not supported.</para>
			/// </summary>
			[SPCorrespondingType(typeof(uint), CorrepsondingAction.Get)]
			...
			/// <summary>
			/// Sets the threshold in pixels where undocking behavior is triggered by using a pen to drag a window from the edge of a monitor or monitor array to
			/// its center. The default threshold is 30. The pvParam parameter must point to a DWORD variable that contains the new value.
			/// <para><c>SPI_GETWINARRANGING</c> must be TRUE to enable this behavior.</para>
			/// <para><c>Windows Server 2008, Windows Vista, Windows Server 2003 and Windows XP/2000:</c> This parameter is not supported.</para>
			/// </summary>
			[SPCorrespondingType(typeof(uint), CorrepsondingAction.Set)]
			...
			/// <summary>
			/// Retrieves the threshold in pixels from the top of a monitor or a monitor array where a vertically maximized window is restored when dragged with
			/// the mouse. The default threshold is 50.
			/// <para>Use <c>SPI_GETWINARRANGING</c> to determine whether this behavior is enabled.</para>
			/// <para><c>Windows Server 2008, Windows Vista, Windows Server 2003 and Windows XP/2000:</c> This parameter is not supported.</para>
			/// </summary>
			[SPCorrespondingType(typeof(uint), CorrepsondingAction.Get)]
			...
			/// <summary>
			/// Sets the threshold in pixels from the top of the monitor where a vertically maximized window is restored when dragged with the mouse. The default
			/// threshold is 50. The pvParam parameter must point to a DWORD variable that contains the new value.
			/// <para><c>SPI_GETWINARRANGING</c> must be TRUE to enable this behavior.</para>
			/// <para><c>Windows Server 2008, Windows Vista, Windows Server 2003 and Windows XP/2000:</c> This parameter is not supported.</para>
			/// </summary>
			[SPCorrespondingType(typeof(uint), CorrepsondingAction.Set)]
			...
			/// <summary>
			/// Retrieves the threshold in pixels from the top of a monitor or monitor array where a vertically maximized window is restored when dragged with
			/// the mouse. The default threshold is 50.
			/// <para>Use <c>SPI_GETWINARRANGING</c> to determine whether this behavior is enabled.</para>
			/// <para><c>Windows Server 2008, Windows Vista, Windows Server 2003 and Windows XP/2000:</c> This parameter is not supported.</para>
			/// </summary>
			...
			/// <summary>
			/// Sets the threshold in pixels from the top of the monitor where a vertically maximized window is restored when dragged with a pen. The default
			/// threshold is 50. The pvParam parameter must point to a DWORD variable that contains the new value.
			/// <para><c>SPI_GETWINARRANGING</c> must be TRUE to enable this behavior.</para>
			/// <para><c>Windows Server 2008, Windows Vista, Windows Server 2003 and Windows XP/2000:</c> This parameter is not supported.</para>
			/// </summary>
			...
			/// <summary>
			/// Determines whether a maximized window is restored when its caption bar is dragged. The pvParam parameter must point to a BOOL variable that
			...
			/// <summary>
			/// Determines whether a window is vertically maximized when it is sized to the top or bottom of a monitor or monitor array. The pvParam parameter
			...
			/// <summary>
			/// Determines whether a window is docked when it is moved to the top, left, or right edges of a monitor or monitor array. The pvParam parameter must
			...
			/// <summary>
			/// Sets whether a window is docked when it is moved to the top, left, or right docking targets on a monitor or monitor array. Set pvParam to TRUE
			...
			/// <summary>
			/// Retrieves a value that determines whether Windows 8 is displaying apps using the default scaling plateau for the hardware or going to the next
			/// higher plateau. This value is based on the current "Make everything on your screen bigger" setting, found in the Ease of Access section of PC
			/// settings: 1 is on, 0 is off.
			/// <para>
			/// Apps can provide text and image resources for each of several scaling plateaus: 100%, 140%, and 180%. Providing separate resources optimized for
			/// a particular scale avoids distortion due to resizing. Windows 8 determines the appropriate scaling plateau based on a number of factors,
			/// including screen size and pixel density. When "Make everything on your screen bigger" is selected (SPI_GETLOGICALDPIOVERRIDE returns a value of
			/// 1), Windows uses resources from the next higher plateau.For example, in the case of hardware that Windows determines should use a scale of
			/// SCALE_100_PERCENT, this override causes Windows to use the SCALE_140_PERCENT scale value, assuming that it does not violate other constraints.
			/// </para>
			/// <note type="note">You should not use this value.It might be altered or unavailable in subsequent versions of Windows. Instead, use the
			/// GetScaleFactorForDevice function or the DisplayProperties class to retrieve the preferred scaling factor.Desktop applications should use desktop
			/// logical DPI rather than scale factor.Desktop logical DPI can be retrieved through the GetDeviceCaps function.</note>
			/// </summary>
			[SPCorrespondingType(typeof(bool), CorrepsondingAction.Get)]
			...
			/// <summary>
			/// Determines whether overlapped content is enabled or disabled. The pvParam parameter must point to a BOOL variable that receives TRUE if enabled,
			/// or FALSE otherwise.
			/// <para>
			/// Display features such as background images, textured backgrounds, water marks on documents, alpha blending, and transparency can reduce the
			/// contrast between the foreground and background, making it harder for users with low vision to see objects on the screen. This flag enables you to
			/// determine whether such overlapped content has been disabled.
			/// </para>
			...
			/// <summary>
			/// Turns overlapped content (such as background images and watermarks) on or off. The pvParam parameter is a BOOL variable. Set pvParam to TRUE to
			/// disable overlapped content, or FALSE to enable overlapped content.
			/// <para>
			/// Display features such as background images, textured backgrounds, water marks on documents, alpha blending, and transparency can reduce the
			/// contrast between the foreground and background, making it harder for users with low vision to see objects on the screen. This flag enables you to
			/// determine whether such overlapped content has been disabled.
			...
			/// <summary>
			/// Determines whether animations are enabled or disabled. The pvParam parameter must point to a BOOL variable that receives TRUE if animations are
			/// enabled, or FALSE otherwise.
			/// <para>
			/// Display features such as flashing, blinking, flickering, and moving content can cause seizures in users with photo-sensitive epilepsy. This flag
			/// enables you to determine whether such animations have been disabled in the client area.
			/// </para>
			/// <para><c>Windows Server 2003 and Windows XP/2000:</c> This parameter is not supported.</para>
			/// </summary>
			[SPCorrespondingType(typeof(bool), CorrepsondingAction.Get)]
			...
			/// <summary>
			/// Turns client area animations on or off. The pvParam parameter is a BOOL variable. Set pvParam to TRUE to enable animations and other transient
			/// effects in the client area, or FALSE to disable them.
			/// <para>
			/// Display features such as flashing, blinking, flickering, and moving content can cause seizures in users with photo-sensitive epilepsy. This flag
			/// enables you to determine whether such animations have been disabled in the client area.
			/// </para>
			...
			/// <summary>
			/// <c>Starting with Windows 8:</c> Determines whether the active input settings have Local (per-thread, TRUE) or Global (session, FALSE) scope. The
			/// pvParam parameter must point to a BOOL variable.
			/// </summary>
			[SPCorrespondingType(typeof(bool), CorrepsondingAction.Get)]
			...
			/// <summary>
			/// <c>Starting with Windows 8:</c> Determines whether the active input settings have Local (per-thread, TRUE) or Global (session, FALSE) scope. The
			/// pvParam parameter must point to a BOOL variable, casted by PVOID.
			/// </summary>
			[SPCorrespondingType(typeof(bool), CorrepsondingAction.Get)]
			SPI_SETTHREADLOCALINPUTSETTINGS = 0x104F,

			/// <summary>
			/// <c>Starting with Windows 8:</c> Determines whether the system language bar is enabled or disabled. The pvParam parameter must point to a BOOL
			/// variable that receives TRUE if the language bar is enabled, or FALSE otherwise.
			/// </summary>
			[SPCorrespondingType(typeof(bool), CorrepsondingAction.Get)]
			SPI_GETSYSTEMLANGUAGEBAR = 0x1050,

			/// <summary>
			/// <c>Starting with Windows 8:</c> Turns the legacy language bar feature on or off. The pvParam parameter is a pointer to a BOOL variable. Set
			/// pvParam to TRUE to enable the legacy language bar, or FALSE to disable it. The flag is supported on Windows 8 where the legacy language bar is
			/// replaced by Input Switcher and therefore turned off by default. Turning the legacy language bar on is provided for compatibility reasons and has
			/// no effect on the Input Switcher.
			/// </summary>
			[SPCorrespondingType(typeof(bool), CorrepsondingAction.Set)]
			...
			/// <summary>
			/// Retrieves the font smoothing orientation. The pvParam parameter must point to a UINT that receives the information. The possible values are
			/// FE_FONTSMOOTHINGORIENTATIONBGR (blue-green-red) and FE_FONTSMOOTHINGORIENTATIONRGB (red-green-blue). Windows XP/2000: This parameter is not
			/// supported until Windows XP with SP2.
			/// </summary>
			[SPCorrespondingType(typeof(uint), CorrepsondingAction.Get)]
			...
			/// <summary>
			/// Sets the time that notification pop-ups should be displayed, in seconds. The pvParam parameter specifies the message duration. Users with visual
			/// impairments or cognitive conditions such as ADHD and dyslexia might need a longer time to read the text in notification messages. This flag
			/// enables you to set the message duration. Windows Server 2003 and Windows XP/2000: This parameter is not supported.
			/// </summary>
			[SPCorrespondingType(typeof(uint), CorrepsondingAction.Set)]
			SPI_SETMESSAGEDURATION = 0x2017,

			/// <summary>
			/// Retrieves the current contact visualization setting. The pvParam parameter must point to a ULONG variable that receives the setting. For more
			...
			/// <summary>
			/// Sets the current contact visualization setting. The pvParam parameter must point to a ULONG variable that identifies the setting. For more
			/// information, see Contact Visualization.
			/// </summary>
			...
			/// <summary>
			/// Retrieves the current gesture visualization setting. The pvParam parameter must point to a ULONG variable that receives the setting. For more
			...
			/// <summary>
			/// Sets the current gesture visualization setting. The pvParam parameter must point to a ULONG variable that identifies the setting. For more
			/// information, see Gesture Visualization.
			/// </summary>
			...
			/// <summary>
			/// Retrieves the routing setting for wheel button input. The routing setting determines whether wheel button input is sent to the app with focus
			/// (foreground) or the app under the mouse cursor. The pvParam parameter must point to a DWORD variable that receives the routing option. If the
			/// value is zero or MOUSEWHEEL_ROUTING_FOCUS, mouse wheel input is delivered to the app with focus. If the value is 1 or MOUSEWHEEL_ROUTING_HYBRID
			/// (default), mouse wheel input is delivered to the app with focus(desktop apps) or the app under the mouse cursor(Windows Store apps). The uiParam
			...
			/// <summary>
			/// Sets the routing setting for wheel button input. The routing setting determines whether wheel button input is sent to the app with focus
			/// (foreground) or the app under the mouse cursor. The pvParam parameter must point to a DWORD variable that receives the routing option. If the
			/// value is zero or MOUSEWHEEL_ROUTING_FOCUS, mouse wheel input is delivered to the app with focus. If the value is 1 or MOUSEWHEEL_ROUTING_HYBRID
			/// (default), mouse wheel input is delivered to the app with focus(desktop apps) or the app under the mouse cursor(Windows Store apps). Set the
			/// uiParam parameter to zero.
			/// </summary>
			[SPCorrespondingType(typeof(MouseWheelRouting), CorrepsondingAction.Set)]
			SPI_SETMOUSEWHEELROUTING = 0x201D,

			/// <summary>
			/// Retrieves the current pen gesture visualization setting. The pvParam parameter must point to a ULONG variable that receives the setting. For more
			...
			/// <summary>
			/// Sets the current pen gesture visualization setting. The pvParam parameter must point to a ULONG variable that identifies the setting. For more
			/// information, see Pen Visualization.
			/// </summary>
		...
		/// <summary>
		/// Contains information associated with audio descriptions. This structure is used with the <c>SystemParametersInfo</c> function when the
		/// SPI_GETAUDIODESCRIPTION or SPI_SETAUDIODESCRIPTION action value is specified.
		/// </summary>
		...
		/// </summary>
		// typedef struct tagNONCLIENTMETRICS { UINT cbSize; int iBorderWidth; int iScrollWidth; int iScrollHeight; int iCaptionWidth; int iCaptionHeight;
		// LOGFONT lfCaptionFont; int iSmCaptionWidth; int iSmCaptionHeight; LOGFONT lfSmCaptionFont; int iMenuWidth; int iMenuHeight; LOGFONT lfMenuFont;
		// LOGFONT lfStatusFont; LOGFONT lfMessageFont;#if (WINVER &gt;= 0x0600) int iPaddedBorderWidth;#endif } NONCLIENTMETRICS, *PNONCLIENTMETRICS,
		...
		public struct NONCLIENTMETRICS
		{
			...
			/// <summary>
			/// <para>
			/// The thickness of the padded border, in pixels. The default value is 4 pixels. The <c>iPaddedBorderWidth</c> and <c>iBorderWidth</c> members are
			/// combined for both resizable and nonresizable windows in the Windows Aero desktop experience. To compile an application that uses this member,
			/// define <c>_WIN32_WINNT</c> as 0x0600 or later. For more information, see Remarks.
			/// </para>
		...
		// typedef struct tagTouchPredictionParameters { UINT cbSize; UINT dwLatency; UINT dwSampleTime; UINT bUseHWTimeStamp;} TouchPredictionParameters, *PTouchPredictionParameters;
