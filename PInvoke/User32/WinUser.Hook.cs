...
public static partial class User32
{
	...
	/// <param name="nCode">
	/// A hook code that the hook procedure uses to determine the action to perform. The value of the hook code depends on the type of the
	/// hook; each type has its own characteristic set of hook codes.
	/// </param>
	...
	/// <summary>
	/// <para>
	/// An application-defined callback (or hook) function that the system calls in response to events generated by an accessible object. The
	/// hook function processes the event notifications as required. Clients install the hook function and request specific types of event
	...
	/// <para>
	/// Handle to an event hook function. This value is returned by SetWinEventHook when the hook function is installed and is specific to
	/// each instance of the hook function.
	/// </para>
	/// </param>
	/// <param name="winEvent">
	...
	/// <para>Type: <c>LONG</c></para>
	/// <para>
	/// Identifies whether the event was triggered by an object or a child element of the object. If this value is CHILDID_SELF, the event
	/// was triggered by the object; otherwise, this value is the child ID of the element that triggered the event.
	/// </para>
	/// </param>
	...
	[UnmanagedFunctionPointer(CallingConvention.Winapi)]
	public delegate void WinEventProc(HWINEVENTHOOK hWinEventHook, EventConstant winEvent, HWND hwnd, ObjectIdentifier idObject, int idChild, uint idEventThread, uint dwmsEventTime);

	/// <summary>
	/// <para>
	/// This topic describes the events that are generated by the operating system and by server applications. The constants are listed in
	/// alphabetical order.
	/// </para>
	/// <para>
	...
	/// </para>
	/// <para>
	/// For more information about events in general, see What Are WinEvents? and System Level and Object Level Events. For more information
	...
	public enum EventConstant : uint
	{
		...
		/// <summary>An object's KeyboardShortcut property has changed. Server applications send this event for their accessible objects.</summary>
		...
		/// <summary>
		/// A window object's scrolling has ended. Unlike EVENT_SYSTEM_SCROLLEND, this event is associated with the scrolling window. Whether
		/// the scrolling is horizontal or vertical scrolling, this event should be sent whenever the scroll action is completed.
		/// <para>
		...
		/// <para>
		/// Before sending the event for the parent object, servers must send it for all of an object's child objects. Servers must ensure
		/// that all child objects are fully created and ready to accept IAccessible calls from clients before the parent object sends this event.
		/// </para>
		/// <para>
		...
		/// <summary>
		/// An object's DefaultAction property has changed. The system sends this event for dialog boxes. Server applications send this event
		...
		/// Further calls on the interface pointer may return failure errors; to prevent this, servers create proxy objects and monitor their
		/// life spans.
		/// </para>
		/// </summary>
		...
		/// <summary>
		/// The user dropped an element on a drop target. The hwnd, idObject, and idChild parameters of the WinEventProc callback function
		...
		/// <summary>
		/// The user started to drag an element. The hwnd, idObject, and idChild parameters of the WinEventProc callback function identify
		...
		/// control, menu bar, pop-up menu, switch window, tab control, tree view control, and window object. Server applications send this
		...
		/// <summary>
		/// An object is hidden. The system sends this event for the following user interface elements: caret and cursor. Server applications
		...
		/// </para>
		/// <para>
		/// Hidden objects include the STATE_SYSTEM_INVISIBLE flag; shown objects do not include this flag. The EVENT_OBJECT_HIDE event also
		/// indicates that the STATE_SYSTEM_INVISIBLE flag is set. Therefore, servers do not send the EVENT_STATE_CHANGE event in this case.
		/// </para>
		/// </summary>
		...
		/// object from an accessibility framework (MSAA or UI Automation) that is different from that of the host. Changes in hosted objects
		/// that are from the same framework as the host should be handed with the structural change events, such as EVENT_OBJECT_CREATE for
		/// MSAA. For more info see comments within winuser.h.
		/// </summary>
		...
		/// <summary>
		/// An object has been invoked; for example, the user has clicked a button. This event is supported by common controls and is used by
		/// UI Automation.
		/// <para>For this event, the hwnd, ID, and idChild parameters of the WinEventProc callback function identify the item that is invoked.</para>
		/// </summary>
		...
		/// <summary>
		/// An object has changed location, shape, or size. The system sends this event for the following user interface elements: caret and
		/// window objects. Server applications send this event for their accessible objects.
		/// <para>
		...
		/// children that the object might have. For example, if the user resizes a window, the system sends this notification for the
		/// window, but not for the menu bar, title bar, scroll bar, or other objects that have also changed.
		/// </para>
		/// <para>
		...
		/// </para>
		/// <para>
		/// If an object's State property is set to STATE_SYSTEM_FLOATING, the server sends EVENT_OBJECT_LOCATIONCHANGE whenever the object
		/// changes location. If an object does not have this state, servers only trigger this event when the object moves in relation to its
		/// parent. For this event notification, the idChild parameter of the WinEventProc callback function identifies the child object that
		/// has changed.
		/// </para>
		/// </summary>
		...
		/// <summary>
		/// An object's Name property has changed. The system sends this event for the following user interface elements: check box, cursor,
		/// list-view control, push button, radio button, status bar control, tree view control, and window object. Server applications send
		...
		/// <summary>
		/// A container object has added, removed, or reordered its children. The system sends this event for the following user interface
		/// elements: header control, list-view control, toolbar control, and window object. Server applications send this event as
		/// appropriate for their accessible objects.
		/// <para>
		...
		/// list-view control, tab control, tree view control, and window object. Server applications send this event for their accessible objects.
		/// <para>
		/// This event signals a single selection: either a child is selected in a container that previously did not contain any selected
		/// children, or the selection has changed from one child to another.
		/// </para>
		/// <para>
		...
		/// the object that is selected. If the selected child is a window that also contains objects, the idChild parameter is OBJID_WINDOW.
		/// </para>
		/// </summary>
		...
		/// interface elements: list box, list-view control, and tree view control. Server applications send this event for their accessible objects.
		/// <para>
		/// The hwnd and idObject parameters of the WinEventProc callback function describe the container. The idChild parameter is the child
		...
		/// <para>
		/// This event is sent when the selected items within a control have changed substantially. The event informs the client that many
		/// selection changes have occurred, and it is sent instead of several EVENT_OBJECT_SELECTIONADD or EVENT_OBJECT_SELECTIONREMOVE
		/// events. The client queries for the selected items by calling the container object's IAccessible::get_accSelection method and
		/// enumerating the selected items.
		/// </para>
		/// <para>
		/// For this event notification, the hwnd and idObject parameters of the WinEventProc callback function describe the container in
		...
		/// </para>
		/// <para>
		/// Hidden objects include the STATE_SYSTEM_INVISIBLE flag; shown objects do not include this flag. The EVENT_OBJECT_SHOW event also
		/// indicates that the STATE_SYSTEM_INVISIBLE flag is cleared. Therefore, servers do not send the EVENT_STATE_CHANGE event in this case.
		/// </para>
		/// </summary>
		...
		/// <para>
		/// For this event notification, the idChild parameter of the WinEventProc callback function identifies the child object whose state
		/// has changed.
		/// </para>
		/// </summary>
		...
		/// <summary>
		/// The conversion target within an IME composition has changed. The conversion target is the subset of the IME composition which is
		/// actively selected as the target for user-initiated conversions.
		/// </summary>
		...
		/// <para>
		/// The hwnd, ID, and idChild parameters of the WinEventProc callback function describe the item that is contained in the updated
		/// text selection.
		/// </para>
		/// </summary>
		...
		/// <summary>
		/// An object's Value property has changed. The system sends this event for the user interface elements that include the scroll bar
		/// and the following controls: edit, header, hot key, progress bar, slider, and up-down. Server applications send this event for
		/// their accessible objects.
		/// </summary>
		...
		/// <summary>
		/// An application is about to exit drag-and-drop mode. Applications that support drag-and-drop operations must send this event; the
		/// system does not send this event.
		/// </summary>
		...
		/// <summary>
		/// The foreground window has changed. The system sends this event even if the foreground window has changed to another window in the
		/// same thread. Server applications never send this event.
		/// <para>
		...
		/// the menu bar or the control that activates the context menu. The hwnd parameter is the handle to the window that is related to
		/// the event. The idObject parameter is OBJID_MENU or OBJID_SYSMENU for a menu, or OBJID_WINDOW for a pop-up menu. The idChild
		...
		/// <summary>
		/// A pop-up menu has been displayed. The system sends this event for standard menus, which are identified by HMENU, and are created
		/// using menu-template resources or Win32 menu functions. Servers send this event for custom menus, which are user interface
		/// elements that function as menus but are not created in the standard way. This event is not sent consistently by the system.
		/// </summary>
		EVENT_SYSTEM_MENUPOPUPSTART = 0x0006,

		/// <summary>
		/// A menu item on the menu bar has been selected. The system sends this event for standard menus, which are identified by HMENU,
		/// created using menu-template resources or Win32 menu API elements. Servers send this event for custom menus, which are user
		/// interface elements that function as menus but are not created in the standard way.
		/// <para>
		/// For this event, the WinEventProc callback function's hwnd, idObject, and idChild parameters refer to the control that contains
		/// the menu bar or the control that activates the context menu. The hwnd parameter is the handle to the window related to the event.
		/// The idObject parameter is OBJID_MENU or OBJID_SYSMENU for a menu, or OBJID_WINDOW for a pop-up menu. The idChild parameter is CHILDID_SELF.
		/// </para>
		/// <para>
		...
		/// <summary>
		/// Scrolling has ended on a scroll bar. This event is sent by the system for standard scroll bar controls and for scroll bars that
		/// are attached to a window. Servers send this event for custom scroll bars, which are user interface elements that function as
		...
		/// <summary>
		/// Scrolling has started on a scroll bar. The system sends this event for standard scroll bar controls and for scroll bars attached
		/// to a window. Servers send this event for custom scroll bars, which are user interface elements that function as scroll bars but
		...
		/// audible (for example, due to the lack of a sound file or a sound card). Servers send this event whenever a custom UI element
		/// generates a sound.
		...
		/// <summary>
		/// The user has pressed ALT+TAB, which activates the switch window. This event is sent by the system, never by servers. The hwnd
		/// parameter of the WinEventProc callback function identifies the window to which the user is switching.
		/// <para>
	...
	public enum HC : int
	{
		...
		/// <summary>
		/// The hook procedure must prepare to copy the next mouse or keyboard message to the EVENTMSG structure pointed to by lParam. Upon
		/// receiving the HC_GETNEXT code, the hook procedure must copy the message to the structure.
		/// </summary>
		HC_SKIP = 2,
	...
	public enum HCBT : int
	{
		...
		/// <summary>
		/// A window is about to be created. The system calls the hook procedure before sending the WM_CREATE or WM_NCCREATE message to the
		/// window. If the hook procedure returns a nonzero value, the system destroys the window; the CreateWindow function returns NULL,
		/// but the WM_DESTROY message is not sent to the window. If the hook procedure returns zero, the window is created normally.
		/// <para>
		/// At the time of the HCBT_CREATEWND notification, the window has been created, but its final size and position may not have been
		/// determined and its parent window may not have been established. It is possible to send messages to the newly created window,
		/// although it has not yet received WM_NCCREATE or WM_CREATE messages. It is also possible to change the position in the z-order of
		/// the newly created window by modifying the hwndInsertAfter member of the CBT_CREATEWND structure.
		/// </para>
		/// </summary>
		HCBT_CREATEWND = 3,
		...
		/// <summary>
		/// The system has removed a keyboard message from the system message queue. Upon receiving this hook code, a CBT application must
		/// install a WH_JOURNALPLAYBACK hook procedure in response to the keyboard message.
		/// </summary>
		HCBT_KEYSKIPPED = 7,
	...
	/// <summary>Flags for low level keyboard hook event.</summary>
	[Flags]
	public enum LLKHF : uint
	{
		/// <summary>The key is an extended key, such as the right-hand ALT and CTRL keys that are on an enhanced 101- or 102-key keyboard.</summary>
		LLKHF_EXTENDED = 0x00000001,

		/// <summary>The event was injected. An application can use the <c>SendInput</c> function to synthesize input events.</summary>
		LLKHF_INJECTED = 0x00000010,

		/// <summary>The ALT key is pressed.</summary>
		LLKHF_ALTDOWN = 0x00000020,

		/// <summary>The key is being released. The value is 1 if the key is being released, or 0 if the key is being pressed.</summary>
		LLKHF_UP = 0x00000080,

		/// <summary>The event was injected from a process running at a lower integrity level.</summary>
		LLKHF_LOWER_IL_INJECTED = 0x00000002,
	}

	/// <summary>Flags for low level mouse hook event.</summary>
	[Flags]
	...
	/// objects within a window. Microsoft Active Accessibility servers and Microsoft UI Automation providers use the object identifiers to
	/// determine the object to which a <c>WM_GETOBJECT</c> message request refers.
	/// </para>
	/// <para>
	/// Clients receive these values in their WinEventProc callback function and use them to identify parts of a window. Servers use these
	/// values to identify the corresponding parts of a window when calling <c>NotifyWinEvent</c> or when responding to the
	/// <c>WM_GETOBJECT</c> message.
	/// </para>
	/// <para>
	/// Servers can define custom object IDs to identify other categories of objects within their applications. Custom object IDs must be
	/// assigned positive values because Microsoft Active Accessibility reserves zero and all negative values for the following standard
	/// object identifiers.
	/// </para>
	/// </summary>
	...
	public enum ObjectIdentifier : int
	{
		...
		OBJID_WINDOW = unchecked(0x00000000),
		...
		/// <summary>
		/// An alert that is associated with a window or an application. System provided message boxes are the only UI elements that send
		/// events with this object identifier. Server applications cannot use the AccessibleObjectFromX functions with this object
		...
		/// <summary>
		/// In response to this object identifier, third-party applications can expose their own object model. Third-party applications can
		/// return any COM interface in response to this object identifier.
		/// </summary>
	...
	/// <para>
	/// An application-defined code used by the hook procedure to determine how to process the message. The code must not have the same value
	/// as system-defined hook codes (MSGF_ and HC_) associated with the WH_SYSMSGFILTER and <c>WH_MSGFILTER</c> hooks.
	/// </para>
	/// </param>
	/// <returns>
	...
	/// <remarks>
	/// <para>
	/// The system calls <c>CallMsgFilter</c> to enable applications to examine and control the flow of messages during internal processing
	/// of dialog boxes, message boxes, menus, and scroll bars, or when the user activates a different window by pressing the ALT+TAB key combination.
	/// </para>
	...
	/// <para>
	/// The wParam value passed to the current hook procedure. The meaning of this parameter depends on the type of hook associated with the
	/// current hook chain.
	/// </para>
	/// </param>
	/// <param name="lParam">
	...
	/// <para>
	/// The lParam value passed to the current hook procedure. The meaning of this parameter depends on the type of hook associated with the
	/// current hook chain.
	/// </para>
	/// </param>
	/// <returns>
	...
	/// <para>
	/// This value is returned by the next hook procedure in the chain. The current hook procedure must also return this value. The meaning
	/// of the return value depends on the hook type. For more information, see the descriptions of the individual hook procedures.
	/// </para>
	/// </returns>
	/// <remarks>
	...
	/// <para>
	/// Calling <c>CallNextHookEx</c> is optional, but it is highly recommended; otherwise, other applications that have installed hooks will
	/// not receive hook notifications and may behave incorrectly as a result. You should call <c>CallNextHookEx</c> unless you absolutely
	/// need to prevent the notification from being seen by other applications.
	/// </para>
	/// </remarks>
	// https://docs.microsoft.com/en-us/windows/desktop/api/winuser/nf-winuser-callnexthookex LRESULT CallNextHookEx( HHOOK hhk, int nCode,
	// WPARAM wParam, LPARAM lParam );
	[DllImport(Lib.User32, SetLastError = false, ExactSpelling = true)]
	[PInvokeData("winuser.h", MSDNShortId = "callnexthookex")]
	public static extern IntPtr CallNextHookEx([In, AddAsMember] HHOOK hhk, int nCode, IntPtr wParam, IntPtr lParam);

	/// <summary>Determines whether there is an installed WinEvent hook that might be notified of a specified event.</summary>
	/// <param name="winEvent">
	/// <para>Type: <c>DWORD</c></para>
	/// <para>The event constant that hooks might be notified of. The function checks whether there is an installed hook for this event constant.</para>
	/// </param>
	/// <returns>
	...
	/// <remarks>
	/// <para>
	/// This method is guaranteed to never return a false negative. If this method returns <c>FALSE</c>, it means that no hooks in the system
	/// would be notified of the event. However, this method may return a false positive. In other words, it may return <c>TRUE</c> even
	/// though there are no hooks that would be notified. Thus, it is safe for components to circumvent some work if this method returns <c>FALSE</c>.
	/// </para>
	...
	/// </remarks>
	// https://docs.microsoft.com/en-us/windows/desktop/api/winuser/nf-winuser-iswineventhookinstalled BOOL IsWinEventHookInstalled( DWORD
	// event );
	[DllImport(Lib.User32, SetLastError = false, ExactSpelling = true)]
	[PInvokeData("winuser.h", MSDNShortId = "bc1e97ad-748d-420a-8c9a-72a555b685e1")]
	[return: MarshalAs(UnmanagedType.Bool)]
	public static extern bool IsWinEventHookInstalled(EventConstant winEvent);
	...
	/// for performing low priority tasks during idle time. For more information, see the ForegroundIdleProc hook procedure.
	/// </term>
	/// </item>
	/// <item>
	/// <term>WH_GETMESSAGE 3</term>
	/// <term>Installs a hook procedure that monitors messages posted to a message queue. For more information, see the GetMsgProc hook procedure.</term>
	/// </item>
	/// <item>
	...
	/// <para>
	/// A pointer to the hook procedure. If the dwThreadId parameter is zero or specifies the identifier of a thread created by a different
	/// process, the lpfn parameter must point to a hook procedure in a DLL. Otherwise, lpfn can point to a hook procedure in the code
	...
	/// <para>
	/// A handle to the DLL containing the hook procedure pointed to by the lpfn parameter. The hMod parameter must be set to <c>NULL</c> if
	/// the dwThreadId parameter specifies a thread created by the current process and if the hook procedure is within the code associated
	...
	/// <para>Type: <c>DWORD</c></para>
	/// <para>
	/// The identifier of the thread with which the hook procedure is to be associated. For desktop apps, if this parameter is zero, the hook
	/// procedure is associated with all existing threads running in the same desktop as the calling thread. For Windows Store apps, see the
	/// Remarks section.
	/// </para>
	/// </param>
	/// <returns>
	...
	/// <remarks>
	/// <para>
	/// <c>SetWindowsHookEx</c> can be used to inject a DLL into another process. A 32-bit DLL cannot be injected into a 64-bit process, and
	/// a 64-bit DLL cannot be injected into a 32-bit process. If an application requires the use of hooks in other processes, it is required
	/// that a 32-bit application call <c>SetWindowsHookEx</c> to inject a 32-bit DLL into 32-bit processes, and a 64-bit application call
	/// <c>SetWindowsHookEx</c> to inject a 64-bit DLL into 64-bit processes. The 32-bit and 64-bit DLLs must have different names.
	/// </para>
	/// <para>
	...
	/// installs a global hook on 64-bit Windows, the 32-bit hook is injected into each 32-bit process (the usual security boundaries apply).
	/// In a 64-bit process, the threads are still marked as "hooked." However, because a 32-bit application must run the hook code, the
	/// system executes the hook in the hooking app's context; specifically, on the thread that called <c>SetWindowsHookEx</c>. This means
	/// that the hooking application must continue to pump messages or it might block the normal functioning of the 64-bit processes.
	/// </para>
	/// <para>
	...
	/// </para>
	/// <para>
	/// To hook all applications on the desktop of a 64-bit Windows installation, install a 32-bit global hook and a 64-bit global hook, each
	/// from appropriate processes, and be sure to keep pumping messages in the hooking application to avoid blocking normal functioning. If
	/// you already have a 32-bit global hooking application and it doesn't need to run in each application's context, you may not need to
	/// create a 64-bit version.
	/// </para>
	/// <para>
	/// An error may occur if the hMod parameter is <c>NULL</c> and the dwThreadId parameter is zero or specifies the identifier of a thread
	...
	/// applications that have installed hooks will not receive hook notifications and may behave incorrectly as a result. You should call
	/// <c>CallNextHookEx</c> unless you absolutely need to prevent the notification from being seen by other applications.
	/// </para>
	/// <para>
	...
	/// </list>
	/// <para>
	/// For a specified hook type, thread hooks are called first, then global hooks. Be aware that the WH_MOUSE, WH_KEYBOARD, WH_JOURNAL*,
	/// WH_SHELL, and low-level hooks can be called on the thread that installed the hook rather than the thread processing the hook. For
	/// these hooks, it is possible that both the 32-bit and 64-bit hooks will be called if a 32-bit hook is ahead of a 64-bit hook in the
	/// hook chain.
	/// </para>
	/// <para>
	/// The global hooks are a shared resource, and installing one affects all applications in the same desktop as the calling thread. All
	/// global hook functions must be in libraries. Global hooks should be restricted to special-purpose applications or to use as a
	/// development aid during application debugging. Libraries that no longer need a hook should remove its hook procedure.
	/// </para>
	/// <para>
	/// <c>Windows Store app development</c> If dwThreadId is zero, then window hook DLLs are not loaded in-process for the Windows Store app
	/// processes and the Windows Runtime broker process unless they are installed by either UIAccess processes (accessibility tools). The
	/// notification is delivered on the installer's thread for these hooks:
	/// </para>
	/// <list type="bullet">
	/// <item>
	/// <term>WH_JOURNALPLAYBACK</term>
	...
	/// <para>
	/// Specifies the event constant for the lowest event value in the range of events that are handled by the hook function. This parameter
	/// can be set to <c>EVENT_MIN</c> to indicate the lowest possible event value. See <see cref="EventConstant"/> for defined values.
	/// </para>
	/// </param>
	/// <param name="eventMax">
	/// <para>Type: <c>UINT</c></para>
	/// <para>
	/// Specifies the event constant for the highest event value in the range of events that are handled by the hook function. This parameter
	/// can be set to EVENT_MAX to indicate the highest possible event value. See <see cref="EventConstant"/> for defined values.
	/// </para>
	/// </param>
	...
	/// <para>Type: <c>DWORD</c></para>
	/// <para>
	/// Specifies the ID of the process from which the hook function receives events. Specify zero (0) to receive events from all processes
	...
	/// <term>Prevents this instance of the hook from receiving the events that are generated by the thread that is registering this hook.</term>
	/// </item>
	/// </list>
	...
	/// <para>Type: <c>HWINEVENTHOOK</c></para>
	/// <para>
	/// If successful, returns an HWINEVENTHOOK value that identifies this event hook instance. Applications save this return value to use it
	/// with the UnhookWinEvent function.
	/// </para>
	...
	/// <para>
	/// If the idProcess parameter is nonzero and idThread is zero, the hook function receives the specified events from all threads in that
	/// process. If the idProcess parameter is zero and idThread is nonzero, the hook function receives the specified events only from the
	/// thread specified by idThread. If both are zero, the hook function receives the specified events from all threads and processes.
	/// </para>
	/// <para>
	/// Clients can call <c>SetWinEventHook</c> multiple times if they want to register additional hook functions or listen for additional events.
	/// </para>
	...
	/// </para>
	/// <para>
	/// For out-of-context events, the event is delivered on the same thread that called <c>SetWinEventHook</c>. In some situations, even if
	/// you request WINEVENT_INCONTEXT events, the events will still be delivered out-of-context. These scenarios include events from console
	/// windows and events from processes that have a different bit-depth (64 bit versus 32 bits) than the caller.
	/// </para>
	/// <para>
	...
	/// processing for the original event is finished. The problem with reentrancy in hook functions is that events are completed out of
	/// sequence unless the hook function handles this situation. For more information, see Guarding Against Reentrancy.
	/// </para>
	/// <para>
	/// <c>Windows Store app development</c> If dwFlags is WINEVENT_INCONTEXT AND (idProcess = 0 | idThread = 0), then window hook DLLs are
	/// not loaded in-process for the Windows Store app processes and the Windows Runtime broker process unless they are installed by
	...
	/// <para>Examples</para>
	/// <para>
	/// The following example code shows how a client application might listen for menu-start and menu-end events. For simplicity, the event
	/// handler just sends some information to the standard output.
	/// </para>
	/// </remarks>
	// https://docs.microsoft.com/en-us/windows/desktop/api/winuser/nf-winuser-setwineventhook HWINEVENTHOOK SetWinEventHook( DWORD eventMin,
	// DWORD eventMax, HMODULE hmodWinEventProc, WINEVENTPROC pfnWinEventProc, DWORD idProcess, DWORD idThread, DWORD dwFlags );
	[DllImport(Lib.User32, SetLastError = false, ExactSpelling = true)]
	[PInvokeData("winuser.h", MSDNShortId = "090bda1b-0635-4aa3-ae33-3987b36e30b8")]
	public static extern SafeHWINEVENTHOOK SetWinEventHook(EventConstant eventMin, EventConstant eventMax, [Optional] HINSTANCE hmodWinEventProc, WinEventProc pfnWinEventProc, uint idProcess, [Optional] uint idThread, WINEVENT dwFlags);
	...
	/// <remarks>
	/// <para>
	/// The hook procedure can be in the state of being called by another thread even after <c>UnhookWindowsHookEx</c> returns. If the hook
	/// procedure is not being called concurrently, the hook procedure is removed immediately before <c>UnhookWindowsHookEx</c> returns.
	/// </para>
	/// <para>Examples</para>
	...
	/// </para>
	/// <para>
	/// Call this function from the same thread that installed the event hook. <c>UnhookWinEvent</c> fails if called from a thread different
	/// from the call that corresponds to SetWinEventHook.
	/// </para>
	/// <para>
	...
	/// processes that loaded it. Although unloading does not occur immediately, the hook function is not called after <c>UnhookWinEvent</c>
	...
	/// </summary>
	// https://docs.microsoft.com/en-us/windows/desktop/api/winuser/ns-winuser-tagcbt_createwnda typedef struct tagCBT_CREATEWNDA { struct
	// tagCREATESTRUCTA *lpcs; HWND hwndInsertAfter; } CBT_CREATEWNDA, *LPCBT_CREATEWNDA;
	...
	public struct CBT_CREATEWND
	{
		...
		/// </summary>
		public ManagedStructPointer<CREATESTRUCT> lpcs;
	...
	/// <summary>
	/// <para>
	/// Contains information about a hardware message sent to the system message queue. This structure is used to store message information
	/// for the JournalPlaybackProc callback function.
	/// </para>
	/// </summary>
	// https://docs.microsoft.com/en-us/windows/desktop/api/winuser/ns-winuser-tageventmsg typedef struct tagEVENTMSG { UINT message; UINT
	// paramL; UINT paramH; DWORD time; HWND hwnd; } EVENTMSG, *PEVENTMSGMSG, *NPEVENTMSGMSG, *LPEVENTMSGMSG, *PEVENTMSG,
	...
	/// </summary>
	// https://docs.microsoft.com/en-us/windows/desktop/api/winuser/ns-winuser-tagkbdllhookstruct typedef struct tagKBDLLHOOKSTRUCT { DWORD
	// vkCode; DWORD scanCode; DWORD flags; DWORD time; ULONG_PTR dwExtraInfo; } KBDLLHOOKSTRUCT, *LPKBDLLHOOKSTRUCT, *PKBDLLHOOKSTRUCT;
	...
	public struct KBDLLHOOKSTRUCT
	{
		...
		/// </summary>
		public VK vkCode { readonly get => (VK)_vkCode; set => _vkCode = (uint)value; }
		...
		/// <term>0</term>
		/// <term>
		/// Specifies whether the key is an extended key, such as a function key or a key on the numeric keypad. The value is 1 if the key is
	...
	/// </summary>
	// https://docs.microsoft.com/en-us/windows/desktop/api/winuser/ns-winuser-tagmousehookstruct typedef struct tagMOUSEHOOKSTRUCT { POINT
	// pt; HWND hwnd; UINT wHitTestCode; ULONG_PTR dwExtraInfo; } MOUSEHOOKSTRUCT, *LPMOUSEHOOKSTRUCT, *PMOUSEHOOKSTRUCT;
	...
	public struct MOUSEHOOKSTRUCT
	{
		...
		/// </summary>
		public HitTestValues wHitTestCode;
	...
	/// <summary>
	/// <para>Contains information about a mouse event passed to a <c>WH_MOUSE</c> hook procedure, MouseProc.</para>
	/// <para>This is an extension of the MOUSEHOOKSTRUCT structure that includes information about wheel movement or the use of the X button.</para>
	/// </summary>
	...
	public struct MOUSEHOOKSTRUCTEX
	{
		...
		/// <para>Type: <c>DWORD</c></para>
		/// <para>
		/// If the message is WM_MOUSEWHEEL, the HIWORD of this member is the wheel delta. The LOWORD is undefined and reserved. A positive
		...
		/// </para>
		/// <para>
		/// If the message is WM_XBUTTONDOWN, WM_XBUTTONUP, WM_XBUTTONDBLCLK, WM_NCXBUTTONDOWN, WM_NCXBUTTONUP, or WM_NCXBUTTONDBLCLK, the
		/// HIWORD of <c>mouseData</c> specifies which X button was pressed or released, and the LOWORD is undefined and reserved. This
		/// member can be one or more of the following values. Otherwise, <c>mouseData</c> is not used.
		/// </para>
		/// <list type="table">
		/// <listheader>
		/// <term>Value</term>
	...
	/// </summary>
	// https://docs.microsoft.com/en-us/windows/desktop/api/winuser/ns-winuser-tagmsllhookstruct typedef struct tagMSLLHOOKSTRUCT { POINT pt;
	// DWORD mouseData; DWORD flags; DWORD time; ULONG_PTR dwExtraInfo; } MSLLHOOKSTRUCT, *LPMSLLHOOKSTRUCT, *PMSLLHOOKSTRUCT;
	...
	public struct MSLLHOOKSTRUCT
	{
		...
		/// positive value indicates that the wheel was rotated forward, away from the user; a negative value indicates that the wheel was
		/// rotated backward, toward the user. One wheel click is defined as <c>WHEEL_DELTA</c>, which is 120.
		/// </para>
		/// <para>
		/// If the message is WM_XBUTTONDOWN, WM_XBUTTONUP, WM_XBUTTONDBLCLK, WM_NCXBUTTONDOWN, WM_NCXBUTTONUP, or WM_NCXBUTTONDBLCLK, the
		/// high-order word specifies which X button was pressed or released, and the low-order word is reserved. This value can be one or
		/// more of the following values. Otherwise, <c>mouseData</c> is not used.
		/// </para>
		/// <list type="table">
		/// <listheader>
		/// <term>Value</term>
		...
		/// <para>Type: <c>DWORD</c></para>
		/// <para>
		/// The event-injected flags. An application can use the following values to test the flags. Testing LLMHF_INJECTED (bit 0) will tell
		/// you whether the event was injected. If it was, then testing LLMHF_LOWER_IL_INJECTED (bit 1) will tell you whether or not the
