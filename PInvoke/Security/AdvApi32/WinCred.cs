...
namespace Vanara.PInvoke
{
	public static partial class AdvApi32
	{
		...
		/// <summary>
		/// <para>
		/// The <c>CredGetSessionTypes</c> function returns the maximum persistence supported by the current logon session. A separate
		...
		/// <para>
		/// Pointer to an array to return the persistence values in. The passed in array should be MaximumPersistCount elements long. On
		/// return, each element specifies the maximum persistence supported by the corresponding credential type.
		/// </para>
		/// <para>The caller should use one of the following defines to index into the array:</para>
		/// <list type="bullet">
		/// <item>
		/// <term>CRED_TYPE_GENERIC</term>
		/// </item>
		/// <item>
		...
		/// </listheader>
		/// <item>
		/// <term>CRED_PERSIST_NONE</term>
		/// <term>
		/// No credential can be stored. This value will be returned if the credential type is not supported or has been disabled by policy.
		/// </term>
		/// </item>
		/// <item>
		/// <term>CRED_PERSIST_SESSION</term>
		/// <term>Only a session-specific credential can be stored.</term>
		/// </item>
		/// <item>
		/// <term>CRED_PERSIST_LOCAL_MACHINE</term>
		/// <term>
		/// Session-specific and computer-specific credentials can be stored. Windows XP: This credential cannot be stored for sessions in
		/// which the profile is not loaded.
		/// </term>
		/// </item>
		/// <item>
		/// <term>CRED_PERSIST_ENTERPRISE</term>
		/// <term>Any credential can be stored. Windows XP: This credential cannot be stored for sessions in which the profile is not loaded.</term>
		/// </item>
		/// </list>
		/// </param>
		/// <returns>
		...
		/// </para>
		/// </returns>
		// https://docs.microsoft.com/en-us/windows/desktop/api/wincred/nf-wincred-credgetsessiontypes BOOL CredGetSessionTypes( DWORD
		// MaximumPersistCount, LPDWORD MaximumPersist );
		...
		public struct CREDENTIAL_TARGET_INFORMATION
		{
			...
			public CRED_TYPE[] GetCredTypes() => CredTypes.ToArray<CRED_TYPE>((int)CredTypeCount);
		}
