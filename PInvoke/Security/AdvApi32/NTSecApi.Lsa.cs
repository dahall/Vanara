...
public static partial class AdvApi32
{
	...
	/// information, see Opening a Policy Object Handle.
	/// </param>
	/// <returns>A list of TRUSTED_DOMAIN_INFORMATION_EX structures that contain information about the enumerated trusted domains.</returns>
	// https://learn.microsoft.com/en-us/windows/win32/api/ntsecapi/nf-ntsecapi-lsaenumeratetrusteddomainsex NTSTATUS
	...
	public static IEnumerable<TRUSTED_DOMAIN_INFORMATION_EX> LsaEnumerateTrustedDomainsEx(LSA_HANDLE PolicyHandle)
	{
		...
		var hEnum = LSA_ENUMERATION_HANDLE.NULL;
		NTStatus ret;
		while ((ret = LsaEnumerateTrustedDomainsEx(PolicyHandle, ref hEnum, out var buf, maxBuf, out var count)) == NTStatus.STATUS_MORE_ENTRIES)
		{
			foreach (var i in buf.ToArray<TRUSTED_DOMAIN_INFORMATION_EX>((int)count))
				yield return i;
		}
		ret.ThrowIfFailed();
	}
	...
	public class LsaForestTrustInformation : IDisposable
	{
		...
		public static LsaForestTrustInformation FromBuffer(IntPtr ptr)
		{
			...
			foreach (var e in info.Entries.ToIEnum<LSA_FOREST_TRUST_RECORD>((int)info.RecordCount))
			{
				...
				{
					LSA_FOREST_TRUST_RECORD_TYPE.ForestTrustTopLevelName => new LsaForestTrustTopLevelName(e.ForestTrustData.TopLevelName, false),
					LSA_FOREST_TRUST_RECORD_TYPE.ForestTrustTopLevelNameEx => new LsaForestTrustTopLevelName(e.ForestTrustData.TopLevelName, true),
					LSA_FOREST_TRUST_RECORD_TYPE.ForestTrustDomainInfo => new LsaForestTrustDomainInfo { DnsName = e.ForestTrustData.DomainInfo.DnsName, NetbiosName = e.ForestTrustData.DomainInfo.NetbiosName, Sid = e.ForestTrustData.DomainInfo.Sid },
					_ => throw new ArgumentException("Unrecognized record type.", nameof(ptr)),
				};
				rec.Flags = e.Flags;
		...
		{
			private SafeLSA_UNICODE_STRING dnsName = new(null);
			private SafeLSA_UNICODE_STRING netbiosName = new(null);
			...
			protected internal override LSA_FOREST_TRUST_RECORD Convert()
			{
				...
				ret.ForestTrustData.DomainInfo.Sid = Sid ?? PSID.NULL;
				return ret;
			}
		}
		...
		public abstract class LsaForestTrustRecord
		{
			...
			protected internal virtual LSA_FOREST_TRUST_RECORD Convert() =>
				new() { Flags = Flags, Time = Time.ToFileTimeStruct() };
		}
		...
		{
			private SafeLSA_UNICODE_STRING topLevelName = new(null);
	...
	public class SafeLSA_UNICODE_STRING : SafeHANDLE
	{
		...
		/// <returns>The result of the conversion.</returns>
		public static implicit operator SafeLSA_UNICODE_STRING(string? val) => new(val);
