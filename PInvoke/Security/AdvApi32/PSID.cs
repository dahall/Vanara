...
public static partial class AdvApi32
{
	...
	[AdjustAutoMethodNamePattern("Sid", ""), DeferAutoMethodFrom(typeof(PSID))]
	public partial class SafePSID : SafeMemoryHandle<LocalMemoryMethods>, IEquatable<SafePSID>, IEquatable<PSID>, IEquatable<IntPtr>, ICloneable, ISecurityObject
	{
		...
		public static SafePSID CreateCapability(KnownSIDCapability capability) =>
			Init(KnownSIDAuthority.SECURITY_APP_PACKAGE_AUTHORITY, KnownSIDRelativeID.SECURITY_CAPABILITY_BASE_RID, (uint)capability);
		...
		/// </exception>
		public static SafePSID Init(PSID_IDENTIFIER_AUTHORITY sidAuthority, uint subAuth0, params uint[] subAuthorities1to7)
		{
...
public static class PSIDExtensions
{
	extension(PSID pSid)
	{
		...
		public bool Equals(PSID psid2) => EqualSid(pSid, psid2);

		/// <summary>Returns <see langword="true" /> if SID matches any well-known SID.</summary>
		/// <value><see langword="true" /> if this instance is well known; otherwise, <see langword="false" />.</value>
		public bool IsWellKnown => Enum.GetValues(typeof(WELL_KNOWN_SID_TYPE)).Cast<WELL_KNOWN_SID_TYPE>().Any(t => IsWellKnownSid(pSid, t));

		/// <summary>The <c>IsWellKnownType</c> function compares a SID to a well-known SID and returns <see langword="true"/> if they match.</summary>
		/// <param name="type">Member of the WELL_KNOWN_SID_TYPE enumeration to compare with the SID at pSid.</param>
		/// <returns>
		/// <para>Returns <see langword="true"/> if the SID at pSid matches the well-known SID indicated by <paramref name="type"/>.</para>
		/// <para>Otherwise, returns <see langword="false"/>.</para>
		/// </returns>
		public bool IsWellKnownType(WELL_KNOWN_SID_TYPE type) => IsWellKnownSid(pSid, type);
		...
		/// </remarks>
		public PSID_IDENTIFIER_AUTHORITY GetAuthority() => GetSidIdentifierAuthority(pSid);

		/// <summary>Gets the binary form of the SID structure.</summary>
		/// <returns>The binary form (byte array) of the SID structure.</returns>
		public byte[] GetBinaryForm() => (pSid.IsValidSid() ? ((IntPtr)pSid).ToByteArray(pSid.Length()) : null) ?? [];
		...
		/// <summary>Enumerates the subauthorities in a security identifier (SID). The subauthority values are relative identifiers (RID).</summary>
		/// <returns>A sequence of subauthorities.</returns>
		public IEnumerable<uint> GetSubAuthorities()
		{
		...
		/// </returns>
		public bool IsValidSid() => AdvApi32.IsValidSid(pSid);
		...
		/// </returns>
		public int Length() => AdvApi32.IsValidSid(pSid) ? AdvApi32.GetLengthSid(pSid) : 0;
