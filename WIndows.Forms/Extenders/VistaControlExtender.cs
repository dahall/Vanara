...
namespace Vanara.Windows.Forms
{
	...
	public sealed class VistaControlExtender : Component, IExtenderProvider
	{
		...
		public int GetMinVisibleItems(ComboBox comboBox) => GetValue<int>(comboBox, MinVisibleItems, SendMessage(new HandleRef(comboBox, comboBox.Handle), (int)ComboBoxMessage.CB_SETMINVISIBLE, 0, 0).ToInt32());
		...
		public void SetMinVisibleItems(ComboBox comboBox, int value)
		{
			if (SetValue(comboBox, MinVisibleItems, value) && IsMinVista && comboBox.IsHandleCreated)
				SendMessage(new HandleRef(comboBox, comboBox.Handle), (int)ComboBoxMessage.CB_SETMINVISIBLE, value, 0);
		}
