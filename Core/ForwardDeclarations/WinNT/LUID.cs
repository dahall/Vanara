...
/// </summary>
[TypeDef(typeof(long), Excludes = ExcludeOptions.Numerics | ExcludeOptions.Value | ExcludeOptions.ToString)]
public partial struct LUID
{
	/// <summary>Low order bits.</summary>
	public uint LowPart { readonly get => value.LowPart(); set => Value = Macros.MAKELONG64(value, HighPart); }

	/// <summary>High order bits.</summary>
	public int HighPart { readonly get => value.HighPart(); set => Value = Macros.MAKELONG64(LowPart, value); }
	...
	{
		long val = value;
		Win32Error.ThrowLastErrorIfFalse(FunctionHelper.CallMethodWithStrBuf((StringBuilder? sb, ref uint sz) => LookupPrivilegeName(systemName, val, sb, ref sz), out string? name));
		return name!;

		[DllImport(Lib.AdvApi32, SetLastError = true, CharSet = CharSet.Auto)]
		[return: MarshalAs(UnmanagedType.Bool)]
		static extern bool LookupPrivilegeName([MarshalAs(UnmanagedType.LPTStr)] string? lpSystemName, in long lpLuid, [Out, MarshalAs(UnmanagedType.LPTStr)] StringBuilder? lpName, ref uint cchName);
	}
	...
	{
		Win32Error.ThrowLastErrorIfFalse(LookupPrivilegeValue(systemName, name, out var val));
		return val;

		[DllImport(Lib.AdvApi32, SetLastError = true, CharSet = CharSet.Auto)]
		[return: MarshalAs(UnmanagedType.Bool)]
		static extern bool LookupPrivilegeValue([MarshalAs(UnmanagedType.LPTStr)] string? lpSystemName, [MarshalAs(UnmanagedType.LPTStr)] string lpName, out LUID lpLuid);
	}
	...
	{
		Win32Error.ThrowLastErrorIfFalse(AllocateLocallyUniqueId(out var ret));
		return ret;
	...
	readonly string IConvertible.ToString(IFormatProvider? _) => ToString();
}
