...
public static class ComReleaserFactory
{
	...
	public static ComReleaser<TObj> Create<TObj>(TObj obj) where TObj : class => new(obj);
}
...
public class ComReleaser<T> : IDisposable where T : class
{
	...
	public T Item => item ?? throw new ObjectDisposedException(nameof(ComReleaser<T>));
	...
	public static explicit operator ComReleaser<T>(T obj) => new(obj);
